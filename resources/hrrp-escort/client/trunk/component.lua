...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Trunk", {
    ...
    exports["hrrp-base"]:CallbacksRegisterClient("Trunk:GetPutIn", function(data, cb)
      InTrunk(NetToVeh(data))
    end)

    exports["hrrp-base"]:CallbacksRegisterClient("Trunk:GetPulledOut", function(data, cb)
...
function InTrunk(veh)
  if not DoesVehicleHaveDoor(veh, 6) and DoesVehicleHaveDoor(veh, 5) and IsThisModelACar(GetEntityModel(veh)) then
    ...
    if not LocalPlayer.state.isCuffed and not LocalPlayer.state.isDead then
      exports['hrrp-hud']:ActionShow('{keybind}primary_action{/keybind} Exit Trunk | {keybind}secondary_action{/keybind} Open/Close Trunk')
    end
...
AddEventHandler("Trunk:Client:PutIn", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Trunk:PutIn", NetworkGetNetworkIdFromEntity(entity.entity), function(state) end)
end)

AddEventHandler("Trunk:Client:PullOut", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Trunk:PullOut", NetworkGetNetworkIdFromEntity(entity.entity), function(state) end)
end)
