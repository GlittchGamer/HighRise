...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['hrrp-base']:RequestDependencies('Doors', {
    'Callbacks',
    'Logger',
    'Utils',
    'Chat',
...
function RunStartup()
  ...
  for k, v in ipairs(_doorConfig) do
    if v.model and v.coords then
      ...
      exports['hrrp-base']:LoggerWarn('Doors', 'Door: ' .. (v.id and v.id or k), ' Missing Required Data')
    end
  end
  ...
  exports['hrrp-base']:LoggerTrace('Doors', 'Loaded ^2' .. #_doorConfig .. '^7 Doors & ^2' .. #_elevatorConfig .. '^7 Elevators')
end
...
function RegisterCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer('Doors:Fetch', function(source, data, cb)
    cb(DOORS_CACHE, ELEVATOR_CACHE)
  end)

  exports['hrrp-base']:CallbacksRegisterServer('Doors:ToggleLocks', function(source, doorId, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Doors:Lockpick', function(source, doorId, cb)
    if type(doorId) == 'string' then
      doorId = DOORS_IDS[doorId]
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Doors:Elevators:ToggleLocks', function(source, data, cb)
    ...
    if targetElevator and targetElevator.canLock and CheckPlayerAuth(source, targetElevator.canLock) then
      local newState = exports['hrrp-doors']:SetElevatorLock(data.elevator, data.floor)
      if newState == nil then
        cb(false, false)
      else
        cb(true, newState)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Doors:Elevator:Validate", function(source, data, cb)
    ...
    if doorData then
      ...
      if newState ~= isLocked then
        ...
        if doorData.double and not doneDouble then
          self:SetLock(doorData.double, newState, true)
        end
      end
      return newState
    end
...
AddEventHandler('Proxy:Shared:RegisterReady', function()
  exports['hrrp-base']:RegisterComponent('Doors', DOORS)
end)
...
function CheckPlayerAuth(source, doorPermissionData)
  ...
  if player then
    ...
    if char then
      ...
      if exports['hrrp-jobs']:JobsPermissionsHasJob(source, 'dgang', false, false, 99, true) then
        return true
      end
      ...
      for k, v in ipairs(doorPermissionData) do
        ...
        elseif v.type == 'job' then
          if v.job then
            if exports['hrrp-jobs']:JobsPermissionsHasJob(source, v.job, v.workplace, v.grade, v.gradeLevel, v.reqDuty, v.jobPermission) then
              return true
            end
          elseif v.jobPermission then
            if exports['hrrp-jobs']:JobsPermissionsHasPermission(source, v.jobPermission) then
              return true
            end
          end
...
function GetDoorOutput(data)
  ...
  printout = printout ..
      "\n\tcoords = vector3(" ..
      tostring(Utils:Round(data.coords.x, 2)) .. ", " .. tostring(Utils:Round(data.coords.y, 2)) .. ", " .. tostring(Utils:Round(data.coords.z, 2)) .. "),"
  printout = printout .. "\n}\n\n"
  return printout
end
