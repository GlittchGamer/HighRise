...
AddEventHandler("Doors:Client:LockElevator", function(data)
  if ELEVATOR_STATE[data.elevator] and LocalPlayer.state.loggedIn then
    exports["hrrp-base"]:CallbacksServer("Doors:Elevators:ToggleLocks", data, function(success, newState)
      if success then
        if newState then
          exports['hrrp-hud']:NotificationError("Elevator Locked")
        else
          exports['hrrp-hud']:NotificationSuccess("Elevator Unlocked")
        end
      end
    end)
  end
...
AddEventHandler("Doors:Client:UseElevator", function(data)
  ...
  if elevatorData and elevatorData.floors and LocalPlayer.state.loggedIn then
    ...
    if floorData and floorData.coords then
      exports["hrrp-base"]:CallbacksServer("Doors:Elevator:Validate", floorData, function()
