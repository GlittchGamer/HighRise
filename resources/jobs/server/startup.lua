...
function RunStartup()
  ...
  local function replaceExistingDefaultJob(_id, document)
    ...
    if deleteResult > 0 then
      ...
      if insertResult > 0 then
        ...
        exports['core']:LoggerError("Jobs", "Error Inserting Job on Default Job Update")
        return false
      end
    else
      exports['core']:LoggerError("Jobs", "Error Deleting Job on Default Job Update")
      return false
    end
  end
  ...
  RefreshAllJobData()
  exports['core']:LoggerTrace("Jobs", string.format("Loaded ^2%s^7 Jobs", JOB_COUNT))
