...
_JOBS = {
  ...
  GiveJob = function(self, stateId, jobId, workplaceId, gradeId, noOverride)
    ...
    local char = exports['core']:FetchSID(stateId)
    if char then
  ...
  RemoveJob = function(self, stateId, jobId)
    ...
    if char then
      ...
      if found then
        local source = char:GetData('Source')
        char:SetData('Jobs', charJobData)
        exports['jobs']:JobsDutyOff(source, jobId, true)
        ...
        if removedJobData.Workplace and removedJobData.Workplace.Name then
          exports['core']:ExecuteClient(source, 'Notification', 'Info', 'No Longer Employed at ' .. removedJobData.Workplace.Name)
        else
          exports['core']:ExecuteClient(source, 'Notification', 'Info', 'No Longer Employed at ' .. removedJobData.Name)
        end
  ...
  Duty = {
    On = function(self, source, jobId, hideNotify)
          ...
          if hasJob then
            ...
            if dutyData then
              if dutyData.Id == hasJob.Id then
                ...
                local success = exports['jobs']:JobsDutyOff(source, false, true)
                if not success then
                  return false
                end
              end
            ...
            exports['jobs']:JobsDutyRefreshDutyData(hasJob.Id)
      ...
      if not hideNotify then
        exports['core']:ExecuteClient(source, 'Notification', 'Error', 'Failed to Go On Duty')
      end
    ...
    Off = function(self, source, jobId, hideNotify)
      ...
      if player then
        ...
        if char then
          ...
          if dutyData and (not jobId or (dutyData.Id == jobId)) then
            ...
            exports['jobs']:JobsDutyRefreshDutyData(dutyId)
            ...
            if not hideNotify then
              exports['core']:ExecuteClient(source, 'Notification', 'Info', 'You\'re Now Off Duty')
            end
      ...
      if not hideNotify then
        exports['core']:ExecuteClient(source, 'Notification', 'Error', 'Failed to Go Off Duty')
      end
  ...
  Permissions = {
    ...
    HasJob = function(self, source, jobId, workplaceId, gradeId, gradeLevel, checkDuty, permissionKey)
      local jobs = exports['jobs']:JobsPermissionsGetJobs(source)
            ...
            if (not workplaceId or (v.Workplace and v.Workplace.Id == workplaceId)) then
              if (not gradeId or (v.Grade.Id == gradeId)) then
                if (not gradeLevel or (v.Grade.Level and v.Grade.Level >= gradeLevel)) then
                  if not checkDuty or (checkDuty and exports['jobs']:JobsDutyGet(source, jobId)) then
                    if not permissionKey or (permissionKey and exports['jobs']:JobsPermissionsHasPermissionInJob(source, jobId, permissionKey)) then
                      return v
                    end
                  end
                end
      ...
      elseif permissionKey then
        return exports['jobs']:JobsPermissionsHasPermission(source, permissionKey)
      end
      return false
    end,
    -- Gets the permissions the character has in a job they have
    GetPermissionsFromJob = function(self, source, jobId, workplaceId)
      local jobData = exports['jobs']:JobsPermissionsHasJob(source, jobId, workplaceId)
      if jobData then
    ...
    HasPermissionInJob = function(self, source, jobId, permissionKey)
      local permissionsInJob = exports['jobs']:JobsPermissionsGetPermissionsFromJob(source, jobId)
    ...
    HasPermission = function(self, source, permissionKey)
      local allPermissions = exports['jobs']:JobsPermissionsGetAllPermissions(source)
  ...
  Management = {
    ...
    Transfer = function(self, jobId, newOwner)
      -- TODO
      --exports['core']:MiddlewareTriggerEvent("Business:Transfer", jobId, source:GetData("SID"), target:GetData("SID"))
    end,
    Upgrades = {
      -- TODO
...
AddEventHandler('Proxy:Shared:RegisterReady', function()
  exports['core']:RegisterComponent('Jobs', _JOBS)
end)
