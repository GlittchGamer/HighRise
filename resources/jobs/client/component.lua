_JOBS = {
  Permissions = {
    ...
    HasJob = function(self, jobId, workplaceId, gradeId, gradeLevel, checkDuty, permissionKey)
      local jobs = exports['jobs']:JobsPermissionsGetJobs()
          ...
          if (not workplaceId or (v.Workplace and v.Workplace.Id == workplaceId)) then
            if (not gradeId or (v.Grade.Id == gradeId)) then
              if (not gradeLevel or (v.Grade.Level and v.Grade.Level >= gradeLevel)) then
                if not checkDuty or (checkDuty and exports['jobs']:JobsDutyGet(jobId)) then
                  if not permissionKey or (permissionKey and exports['jobs']:JobsPermissionsHasPermissionInJob(jobId, permissionKey)) then
                    return v
                  end
                end
              end
    ...
    GetPermissionsFromJob = function(self, jobId, workplaceId)
      local jobData = exports['jobs']:JobsPermissionsHasJob(jobId, workplaceId)
      if jobData then
    ...
    HasPermissionInJob = function(self, jobId, permissionKey)
      local permissionsInJob = exports['jobs']:JobsPermissionsGetPermissionsFromJob(jobId)
    ...
    HasPermission = function(self, permissionKey)
      ...
      if jobs then
        for k, v in ipairs(jobs) do
          if exports['jobs']:JobsPermissionsHasPermissionInJob(v.Id, permissionKey) then
            return true
          end
        end
      end
  ...
  Duty = {
    On = function(self, jobId, cb)
      if jobId then
        if exports['jobs']:JobsDutyGet(jobId) then
          exports['hud']:NotificationError('Already On Duty as that Job')
          if cb then cb(false) end
          return
        end
      end

      exports['core']:CallbacksServer('Jobs:OnDuty', jobId, function(success)
        if cb then cb(success) end
      end)
    end,
    Off = function(self, jobId, cb)
      exports['core']:CallbacksServer('Jobs:OffDuty', jobId, function(success)
        if cb then cb(success) end
      end)
    end,
...
--[[
  exports['jobs']:JobsPermissionsGetJobs()
]]
