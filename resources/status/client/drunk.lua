...
function IsDrinkingAlcohol()
  local doingAnim = exports["animations"]:EmotesGet()
...
function RegisterDrunkCallbacks()
  exports['core']:CallbacksRegisterClient('Status:DrinkAlcohol', function(data, cb)
    if IsDrinkingAlcohol() then
      exports['hud']:NotificationError('Already Drinking!')
      return cb(false)
    end

    local alcohol = _alcoholConfig[data]
    if alcohol then
      exports["animations"]:EmotesPlay(alcohol.anim, false, alcohol.initialTime, true)
      Progress:Progress({
        name = data,
        ...
        if not cancelled then
          ...
          exports["animations"]:EmotesPlay(alcohol.anim, false, false, false)
          ...
          Citizen.CreateThread(function()
            local tick = 3
            while LocalPlayer.state.loggedIn and exports["animations"]:EmotesGet() == alcohol.anim do
              if tick >= 5 then
