...
function CreateStressPolys()
  for k, v in ipairs(_pzDefs) do
    ...
    if v.type == "poly" then
      exports['polyzonehandler']:PolyZoneCreatePoly(pId, v.points, v.options or {}, v.data or {})
    elseif v.type == "box" then
      exports['polyzonehandler']:PolyZoneCreateBox(pId, v.coords, v.length, v.width, v.options or {}, v.data or {})
    else
      exports['polyzonehandler']:PolyZoneCreateCircle(pId, v.coords, v.radius, v.options or {}, v.data or {})
    end
...
function CreateStressBlips()
  for k, v in ipairs(_pzDefs) do
    ...
    if v.blip ~= nil then
      exports['blips']:Add(pId, v.blip.label or "Stress Reliever", v.blip.coords, v.blip.blip or 66, v.blip.color or 83, v.blip.scale or 0.55)
    end
  end
end
...
AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
  if _pzs[id] and Status.Get:Single("PLAYER_STRESS").value > 0 and not _delay then
    ...
    _inPoly = id
    exports['hud']:ActionShow(string.format("{keybind}primary_action{/keybind} To %s", _pzDefs[_pzs[id]].action))
  end
end)
    ...
    if _pzDefs[_pzs[_inPoly]].anim ~= nil and not _delay then
      ...
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled then
          ...
          exports['hud']:NotificationSuccess("Stress Relieved")
        else
          exports['hud']:NotificationInfo("Stress Partially Relieved")
        end

        SetTimeout(tickTime * 2, function()
          _delay = false
          if _inPoly ~= nil and Status.Get:Single("PLAYER_STRESS").value > 0 then
            exports['hud']:ActionShow(string.format("{keybind}primary_action{/keybind} To %s", _pzDefs[_pzs[_inPoly]].action))
          end
        end)
      end)
    end
