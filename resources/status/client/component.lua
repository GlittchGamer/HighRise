...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Status", {
    ...
    exports['core']:CallbacksRegisterClient("Status:Modify", function(data, cb)
  ...
  Modify = {
    Add = function(self, status, value, addCd, force)
      if _statuses[status] ~= nil then
        ...
        exports['core']:LoggerError("Status", "Attempt To Add To Non-Existent Status")
      end
    end,
    Remove = function(self, status, value, force)
      ...
      if _statuses[status] ~= nil then
        ...
        exports['core']:LoggerError("Status", "Attempt To Remove From Non-Existent Status")
      end
    end,
  },
...
RegisterNetEvent("Status:Client:Reset", function()
  exports['core']:CallbacksServer("Commands:ValidateAdmin", {}, function(isAdmin)
    if isAdmin then
      for k, v in pairs(_statuses) do
...
RegisterNetEvent("Characters:Client:Spawn", function()
  ...
  for k, v in pairs(Status:GetRegistered()) do
    exports['core']:CallbacksServer("Status:Get", { name = v.name, max = v.max }, function(val)
      LocalPlayer.state["status:" .. v.name] = val
      exports['hud']:RegisterStatus(v.name, val, v.max, v.icon, v.color, v.flash, false, v.options)
    end, v.name)
  end
