...
AddEventHandler("Characters:Client:Spawn", function()
  exports['hud']:RegisterStatus("oxygen", 100, 100, "lungs", "#457F88", true, false, {
    hideHigh = true,
  })
...
function RemoveScubaGear()
  if IsPedSwimming(LocalPlayer.state.ped) then
    exports['hud']:NotificationError("Can't Take Off Gear Whilst Swimming")
    return
  end
  ...
  Progress:Progress({
    ...
    if not cancelled then
      ...
      exports['hud']:RegisterStatus("oxygen", 100, 100, "lungs", "#457F88", true, true, {
        hideHigh = true,
      })
...
function RegisterOxygenCallbacks()
  exports['core']:CallbacksRegisterClient("Status:UseScubaGear", function(data, cb)
    if IsPedSwimming(LocalPlayer.state.ped) then
      exports['hud']:NotificationError("Can't Put On Scuba Gear Whilst Swimming")
      return
    end
    ...
    if oxygenDepletionRate >= 1 then
      Progress:Progress({
        ...
        if not cancelled then
          ...
          exports['hud']:RegisterStatus("oxygen", 100, 100, "mask-snorkel", "#457F88", true, true, {
            hideHigh = false,
          })
