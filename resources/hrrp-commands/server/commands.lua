...
function RegisterChatCommands()
  ...
  Chat:RegisterAdminCommand("screenshot", function(source, args, rawCommand)
    ...
    local plyr = exports['hrrp-base']:FetchSID(sid)
    if plyr ~= nil then
  ...
  Chat:RegisterAdminCommand("broadcast", function(source, args, rawCommand)
    local auth = exports['hrrp-base']:FetchSource(source)
  ...
  -- 	local data = exports["hrrp-base"]:FetchComponent("Punishment"):Kick(tonumber(args[1]), args[2], source)
  ...
  -- 	local t = exports['hrrp-base']:FetchSID(tonumber(args[1]))
  -- 	if t ~= nil then
  -- 		if t:GetData("Source") ~= source then
  -- 			exports["hrrp-base"]:FetchComponent("Punishment"):Kick(t:GetData("Source"), args[2], source)
  -- 		else
  ...
  Chat:RegisterAdminCommand("unban", function(source, args, rawCommand)
    exports["hrrp-base"]:FetchComponent("Punishment").Unban:BanID(args[1], source)
  end, {
    help = "Unban Player",
    params = {
      {
  ...
  Chat:RegisterAdminCommand("unbanid", function(source, args, rawCommand)
    ...
    if type == "identifier" then
      exports["hrrp-base"]:FetchComponent("Punishment").Unban:Identifier(args[2], source)
    elseif type == "account" then
      exports["hrrp-base"]:FetchComponent("Punishment").Unban:AccountID(tonumber(args[2]), source)
    end
  end, {
    help = "Unban Site ID",
    params = {
  ...
  -- 			exports["hrrp-base"]:FetchComponent("Punishment").Ban:Source(targetSource, days, args[3], source)
  -- 		else
  ...
  -- 		local t = exports['hrrp-base']:FetchSID(targetSID)
  -- 		if t ~= nil then
  -- 			if t:GetData("Source") == source then
  -- 				return Chat.Send.System:Single(source, "Cannot Ban Yourself")
  -- 			end
  ...
  -- 			if (days >= 1 and days <= 7) or (player.Permissions:IsAdmin() and days >= -1 and days <= 90) then
  -- 				exports["hrrp-base"]:FetchComponent("Punishment").Ban:Source(t:GetData("Source"), days, args[3], source)
  -- 			else
  -- 				Chat.Send.System:Single(source, "Invalid Time")
  -- 			end
  -- 		else
  ...
  Chat:RegisterAdminCommand("banid", function(source, args, rawCommand)
    ...
    if player then
      ...
      if days >= -1 and days <= 90 then
        ...
        elseif type == "account" then
          local res =
              exports["hrrp-base"]:FetchComponent("Punishment").Ban:AccountID(target, days, args[4], source)
          if res and res.success then
