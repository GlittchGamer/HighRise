...
AddEventHandler('Keybinds:Client:KeyUp:primary_action', function()
  if _inLogout then
    exports['core']:CallbacksServer("Jail:Validate", {
...
AddEventHandler("Jail:Client:RetreiveItems", function()
  exports['core']:CallbacksServer("Jail:RetreiveItems")
end)
...
AddEventHandler("Jail:Client:CheckSentence", function()
  ...
  if not jailed or GlobalState["OS:Time"] >= (jailed.Release or 0) then
    exports['hud']:NotificationInfo("Time Served")
  else
    if jailed.Duration >= 9999 then
      exports['hud']:NotificationInfo("You've Been Setenced To The 9's")
    else
...
AddEventHandler("Jail:Client:Released", function()
  if Jail:IsJailed() and Jail:IsReleaseEligible() then
    exports['core']:CallbacksServer("Jail:Release", {}, function(s)
      if s then
        DoScreenFadeOut(1000)
        while not IsScreenFadedOut() do
          Citizen.Wait(10)
...
AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
  ...
  elseif id == "prison-logout" then
    _inLogout = true
    exports['hud']:ActionShow("{keybind}primary_action{/keybind} Switch Characters")
  end
end)
...
AddEventHandler("Jail:Client:StartWork", function()
  exports['core']:CallbacksServer("Jail:StartWork")
end)

AddEventHandler("Jail:Client:QuitWork", function()
  exports['core']:CallbacksServer("Jail:QuitWork")
end)
...
AddEventHandler("Jail:Client:MakeFood", function()
  Progress:Progress({
    ...
    if not status then
      exports['core']:CallbacksServer("Jail:MakeItem", "food")
    end
  end)
end)
...
AddEventHandler("Jail:Client:MakeDrink", function()
  Progress:Progress({
    ...
    if not status then
      exports['core']:CallbacksServer("Jail:MakeItem", "drink")
    end
  end)
end)
