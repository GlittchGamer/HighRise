...
PHONE.LSUnderground.Chopping = {
  CreateBlips = function(self)
    if Reputation:HasLevel("Salvaging", 3) or hasValue(LocalPlayer.state.Character:GetData("States") or {}, "ACCESS_LSUNDERGROUND") then
      _blips.public = exports['hrrp-blips']:Add(
        "chopping_public",
      ...
      if LocalPlayer.state.Character:GetData("ChopLists") ~= nil
        ...
        _blips.private = exports['hrrp-blips']:Add(
          "chopping_personal",
      ...
      exports['hrrp-blips']:Remove("chopping_public")
      exports['hrrp-blips']:Remove("chopping_personal")
    end

    if hasValue(LocalPlayer.state.Character:GetData("States") or {}, "ACCESS_LSUNDERGROUND") then
      _blips.vip = exports['hrrp-blips']:Add(
        "chopping_private",
      ...
      exports['hrrp-blips']:Remove("chopping_private")
    end
  end,
      ...
      if _validBone?.type == "door" then
        if not IsVehicleDoorDamaged(_validVeh, _validBone?.index) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:ChopPart", {
                vNet = VehToNet(_validVeh),
                index = _validBone?.index,
              }, function(c) end)
            else
              exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:CancelChop")
      ...
      elseif _validBone?.type == "tire" then
        if not IsVehicleTyreBurst(_validVeh, _validBone?.index) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:ChopTire", {
                vNet = VehToNet(_validVeh),
                index = _validBone?.index,
              }, function(r)
                if r then
      ...
      elseif _validBone?.type == "wheel" then
        ...
        if not wheels[boneIndex] then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:ChopWheel", {
                vNet = VehToNet(_validVeh),
                index = _validBone?.index,
              }, function(r)
                if r then
      ...
      elseif _validBone?.type == "body" then
        if DoesEntityExist(_validVeh) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:ChopVehicle", {
                vNet = VehToNet(_validVeh),
              }, function(r)
                -- if r then
      ...
      if not IsPedInAnyVehicle(LocalPlayer.state.ped) and distance and distance <= 30.0 then
        ...
        if inDistance then
          if not LocalPlayer.state.doingAction and not _delay then
            exports['hrrp-hud']:ActionShow(string.format("{keybind}primary_action{/keybind} %s", text))
          else
...
RegisterNetEvent('Phone:Client:LSUnderground:Chopping:CancelCurrent', function()
  if LocalPlayer.state.inChopZone ~= nil and LocalPlayer.state.chopping ~= nil then
    if _delay then
      exports['hrrp-hud']:NotificationError("Choplist Has Refreshed")
      Progress:Cancel()
    end
    LocalPlayer.state:set("chopping", nil, true)
    _validBone = nil
...
AddEventHandler("Phone:Client:LSUnderground:Chopping:Pickup", function()
  exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:Pickup")
end)

AddEventHandler("Phone:Client:LSUnderground:Chopping:GetPublicList", function()
  exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:GetPublicList")
end)
...
AddEventHandler("Phone:Client:LSUnderground:Chopping:StartChop", function(entity, data)
  ...
  then
    ...
    exports["hrrp-base"]:CallbacksServer("Phone:LSUnderground:Chopping:CheckVehicle", { vNet = vNet }, function(res)
      if res then
