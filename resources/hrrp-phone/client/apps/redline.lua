...
local function SetGPS(checkpoint)
  ...
  if currentCheckpoint then
    AddPointToGpsCustomRoute(
      _activeRace.trackData.Checkpoints[currentCheckpoint].coords.x * 1.0,
      _activeRace.trackData.Checkpoints[currentCheckpoint].coords.y * 1.0,
      _activeRace.trackData.Checkpoints[currentCheckpoint].coords.z * 1.0
    )
    if currentCheckpoint + 1 > #_activeRace.trackData.Checkpoints then
      if sprintCheckpoint ~= -1 then
        AddPointToGpsCustomRoute(
      ...
      AddPointToGpsCustomRoute(
        _activeRace.trackData.Checkpoints[currentCheckpoint + 1].coords.x * 1.0,
        _activeRace.trackData.Checkpoints[currentCheckpoint + 1].coords.y * 1.0,
        _activeRace.trackData.Checkpoints[currentCheckpoint + 1].coords.z * 1.0
      )
    end
  else
    AddPointToGpsCustomRoute(
          ...
          if DoesEntityExist(ped) then
            ...
            if DoesEntityExist(veh) then
              if #(myPos - GetEntityCoords(veh)) <= 100.0 then
                exports['hrrp-hud']:NotificationInfo("Police Nearby, Race Phasing Disabled")
                ghostingEnded = true
                UnGhostPlayer()
              end
            end
...
RegisterNetEvent("Phone:Client:Redline:CreateRace", function(race)
  print(json.encode(race))
  _races[race._id] = race
  SendNUIMessage({
...
RegisterNetEvent("Phone:Client:Redline:StartRace", function(id, position, currentRacers)
  if _races[id] then
    _races[id].state = 1
    SendNUIMessage({
      type = "STATE_UPDATE",
...
RegisterNUICallback("CreateRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:CreateRace", data, function(res)
...
RegisterNUICallback("CancelRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:CancelRace", data, function(res)
    cb(res)
  end)
end)

RegisterNUICallback("PracticeTrack", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:GetTrack", data, function(res)
    cb(res ~= nil)
    if res ~= nil then
      SetupTrack(res)
      _activeRace = res
...
RegisterNUICallback("JoinRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:JoinRace", data, function(res)
    if res then
      _activeRace = res
...
RegisterNUICallback("LeaveRace", function(data, cb)
  UnGhostPlayer()
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:LeaveRace", data, function(res)
    if _activeRace ~= nil then
      _activeRace.dnf = true
      Cleanup()
      UISounds.Play:FrontEnd(-1, "CHECKPOINT_MISSED", "HUD_MINI_GAME_SOUNDSET")
...
RegisterNUICallback("CreateTrack", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Permissions", {
    redline = { "create" },
  }, function(res)
    cb(res)
    if res then
  ...
  exports["hrrp-base"]:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      if #_pendingTrack.Checkpoints > 2 then
        ...
        exports["hrrp-base"]:CallbacksServer("Phone:Redline:SaveTrack", _pendingTrack, function(res2)
          cb(res2)
        end)
      else
...
RegisterNUICallback("DeleteTrack", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      exports["hrrp-base"]:CallbacksServer("Phone:Redline:DeleteTrack", data, function(res2)
        cb(res2)
      end)
    else
      cb(false)
...
RegisterNUICallback("ResetTrackHistory", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      exports["hrrp-base"]:CallbacksServer("Phone:Redline:ResetTrackHistory", data, function(res2)
        cb(res2)
      end)
    else
      cb(false)
...
RegisterNUICallback("StartRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:StartRace", _activeRace._id, cb)
end)

RegisterNUICallback("EndRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:EndRace", data, cb)
end)

RegisterNUICallback("SendInvite", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:SendInvite", data, cb)
end)

RegisterNUICallback("AcceptInvite", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:AcceptInvite", data, function(res)
    if res then
      _activeRace = res
...
RegisterNUICallback("DeclineInvite", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:DeclineInvite", data, cb)
end)

RegisterNetEvent('Phone:Redline:UpdateLeaderboard', function(currentPosition, amountOfRacers)
  SendNUIMessage({
    type = "UPDATE_LEADERBOARD",
    data = {
...
RegisterNUICallback("RemoveFromRace", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:Redline:RemoveRacer", data, cb)
end)
...
function StartRace()
  ...
  while countdown < countdownMax and _activeRace ~= nil and _loggedIn do
    exports['hrrp-hud']:NotificationInfo(string.format("Race Starting In %s", countdownMax - countdown))
  ...
  Citizen.CreateThread(function()
    ...
    while _activeRace ~= nil and _loggedIn do
      ...
      local currentRaceCheckpoints = _activeRace.trackData.Checkpoints[currentCheckpoint]
      if currentRaceCheckpoints == nil then
        currentRaceCheckpoints = _activeRace.trackData.Checkpoints[1]
      end

      local dist = #(vector3(currentRaceCheckpoints.coords.x, currentRaceCheckpoints.coords.y, currentRaceCheckpoints.coords.z) - myPos)

      if (ghosting and (not myveh or _activeRace.phasing == "none")) or (currentRaceCheckpoints ~= nil and next(currentRaceCheckpoints) ~= nil and dist > 400.0 and ghosting and _activeRace.phasing) then
        UnGhostPlayer()
      ...
      if dist <= currentRaceCheckpoints.size or sprintCheckpoint == -1 then
        local blip = raceBlips[currentCheckpoint]
        if currentCheckpoint == 1 and #cCps == #_activeRace.trackData.Checkpoints and _activeRace.trackData.Type ~= "p2p" then -- this is lapped race?
          cLp = cLp + 1
          cCps = {}
          if cLp <= tonumber(_activeRace.laps) then
            exports['hrrp-hud']:NotificationInfo(string.format("Lap %s", cLp))
        ...
        if sprintCheckpoint ~= -1 then -- i think this is sprint?
          SetBlipColour(blip, 0)
          table.insert(cCps, currentCheckpoint)
          print("Passed checkpoint")
          UISounds.Play:FrontEnd(-1, "CHECKPOINT_NORMAL", "HUD_MINI_GAME_SOUNDSET")

          if _activeRace.phasing == "checkpoints" and currentCheckpoint >= (_activeRace.phasingAdv + 1) then
            ghostingEnded = true
            UnGhostPlayer()
          end
          ...
          if currentCheckpoint < #_activeRace.trackData.Checkpoints then
            ...
            TriggerServerEvent('Phone:Redline:passCheckpoint', _activeRace._id, currentCheckpoint)
        ...
        currentRaceCheckpoints = _activeRace.trackData.Checkpoints[currentCheckpoint]
        if currentCheckpoint > #_activeRace.trackData.Checkpoints then
          blip = raceBlips[1]
        else
        ...
        if currentCheckpoint > #_activeRace.trackData.Checkpoints then
          ...
          ftr = _activeRace.trackData.Checkpoints[currentCheckpoint]
        end

        if
            cLp > tonumber(_activeRace.laps)
            or (_activeRace.trackData.Type == "p2p" and currentCheckpoint == #_activeRace.trackData.Checkpoints)
        then
          ftr = currentRaceCheckpoints
        end
        ...
        sprintCheckpoint = currentCheckpoint
      end
