...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Phone", {
    ...
    RetrieveComponents()
    exports['hrrp-keybinds']:Add("phone_toggle", "F3", "keyboard", "Phone - Open/Close", function()
      TogglePhone()
    end)

    exports['hrrp-keybinds']:Add("phone_ansend", "", "keyboard", "Phone - Accept/End Call", function()
      if _call ~= nil then
    ...
    exports['hrrp-keybinds']:Add("phone_answer", "", "keyboard", "Phone - Accept Call", function()
      if _call ~= nil then
        if _call.state == 1 then
          Phone.Call:Accept()
        end
    ...
    exports['hrrp-keybinds']:Add("phone_end", "", "keyboard", "Phone - End Call", function()
      if _call ~= nil then
        Phone.Call:End()
      end
    end)

    exports['hrrp-keybinds']:Add("phone_mute", "", "keyboard", "Phone - Mute/Unmute Sound", function()
      ...
      exports["hrrp-base"]:CallbacksServer("Phone:Settings:Update", {
        type = "volume",
...
function TogglePhone()
  if not _openCd then
    if not exports['hrrp-hud']:IsDisabled() then
      if not Jail:IsJailed() and hasValue(LocalPlayer.state.Character:GetData("States"), "PHONE") then
        Phone:Open()
      else
        exports['hrrp-hud']:NotificationError("You Don't Have a Phone", 2000)
...
RegisterNUICallback("Home", function(data, cb)
  cb("OK")
  exports["hrrp-base"]:CallbacksServer("Phone:Apps:Home", data)
end)

RegisterNUICallback("Dock", function(data, cb)
  cb("OK")
  exports["hrrp-base"]:CallbacksServer("Phone:Apps:Dock", data)
end)

RegisterNUICallback("Reorder", function(data, cb)
  cb("OK")
  exports["hrrp-base"]:CallbacksServer("Phone:Apps:Reorder", data)
end)

RegisterNUICallback("UpdateAlias", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Phone:UpdateAlias", data, cb)
end)
...
RegisterNUICallback("SaveShare", function(data, cb)
  if data.type == "contacts" then
    exports["hrrp-base"]:CallbacksServer("Phone:Contacts:Create", data.data, function(nId)
      cb(nId)
      if nId then
  ...
  elseif data.type == "documents" then
    exports["hrrp-base"]:CallbacksServer("Phone:Documents:RecieveShare", data.data, function(success)
      cb(success)
      if success then
...
RegisterNUICallback("ShareMyContact", function(data, cb)
  cb(true)
  exports["hrrp-base"]:CallbacksServer("Phone:ShareMyContact", {})
end)
