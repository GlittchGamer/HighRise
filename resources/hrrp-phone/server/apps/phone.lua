...
PHONE.Call = {
  End = function(self, source)
    if _calls[source] ~= nil then
      ...
      if _calls[source].target ~= nil and _calls[_calls[source].target] ~= nil then
        exports['hrrp-base']:LoggerTrace("Phone", string.format("%s Ending Call With %s", source, _calls[source].target))
  ...
  CreateRecord = function(self, record)
    exports.oxmysql:insert(
      "INSERT INTO phone_calls (owner, number, time, duration, method, limited, anonymous, decryptable, unread) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      { record.owner, record.number, record.time, record.duration, record.method, record.limited, record.anonymous,
        record.decryptable, record.unread })
  end,
  Decrypt = function(self, owner, number)
    exports.oxmysql:execute("UPDATE phone_calls SET anonymous = 0 WHERE owner = ? AND number = ? AND decryptable = 1",
      { owner, number })
  end,
  Read = function(self, owner)
    exports.oxmysql:execute("UPDATE phone_calls SET unread = 0 WHERE owner = ?", { owner })
  end,
}
...
AddEventHandler("Phone:Server:RegisterMiddleware", function()
  Middleware:Add("Characters:Spawning", function(source)
    ...
    exports.oxmysql:query("SELECT * FROM phone_calls WHERE owner = ? AND deleted = 0 ORDER BY time DESC LIMIT 100",
      { char:GetData("Phone") },
      function(calls)
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Phone:CreateCall", function(src, data, cb)
      ...
      if tplyr ~= nil then
        ...
        if target ~= nil and hasValue(target:GetData("States"), "PHONE") then
          if _calls[target:GetData("Source")] == nil then
            ...
            exports['hrrp-base']:LoggerTrace("Phone", string.format("%s Starting Call With %s", src, tplyr:GetData("Source")))
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Phone:AcceptCall", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Phone:EndCall", function(src, data, cb)
    Phone.Call:End(src)
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:Phone:ReadCalls", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
