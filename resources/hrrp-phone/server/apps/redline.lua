...
function LoadTracks(updateClients)
  exports['hrrp-base']:LoggerTrace('Phone:Redline', 'Loading Race Tracks')
  local fetchRaceTracks = MySQL.query.await('SELECT * FROM `tracks`', {})
  if #fetchRaceTracks == 0 then
    exports['hrrp-base']:LoggerTrace('Phone:Redline', 'No have been loaded for PD Tracks.')
    fetchedTracks = true
    goto continue
  end
  ...
  exports['hrrp-base']:LoggerInfo("Phone:Red", string.format("Loading ^2%s^7 PD Racing Tracks", #_tracks))
...
function FinishRace(id)
  ...
  for k, v in pairs(Fetch:All()) do
    ...
    if c ~= nil then
      local dutyData = exports['hrrp-jobs']:JobsDutyGet(v:GetData("Source"))
...
function SyncLeaderboard(raceId)
  local raceData = _races[raceId]
  local leaderboard = CalculateLeaderboard(raceData)

  for position, v in ipairs(leaderboard) do
    local racer = raceData.racers[v.alias]
    if racer.source then
      TriggerClientEvent("Phone:Redline:UpdateLeaderboard", racer.source, position, #raceData.racers)
    end
  end
end

function CalculateLeaderboard(raceInfo)
  local leaderboard = {}
  for k, v in pairs(raceInfo.racers) do
    table.insert(leaderboard, {
      alias = k,
      passedPoints = v.passedPoints,
    })
  end

  table.sort(leaderboard, function(a, b)
    --sort by passedPoints
    if not raceInfo.racers[a.alias].passedPoints then
      raceInfo.racers[a.alias].passedPoints = 0
      a.passedPoints = 0
    end
    if not raceInfo.racers[b.alias].passedPoints then
      raceInfo.racers[b.alias].passedPoints = 0
      b.passedPoints = 0
    end



    return a.passedPoints > b.passedPoints
  end)

  return leaderboard
end

function PassedCheckpoint(src, raceId, checkpoint)
  local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  local alias = char:GetData("Alias").redline

  _races[raceId].racers[alias].passedPoints = _races[raceId].racers[alias].passedPoints + 1;
  SyncLeaderboard(raceId)
end

RegisterNetEvent('Phone:Redline:passCheckpoint', function(raceID, checkpoint)
  local src = source
  local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  local alias = char:GetData("Alias").redline

  if not _races[raceID] then
    return
  end

  if not _races[raceID].racers[alias] then
    return
  end

  if not _races[raceID].racers[alias].passedPoints then
    _races[raceID].racers[alias].passedPoints = 0;
  end
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:GetTrack", function(src, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:SaveTrack", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    local alias = char:GetData("Alias").redline
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:SaveLaptimes", function(src, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:DeleteTrack", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    local alias = char:GetData("Alias").redline
    if alias ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:ResetTrackHistory", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    local alias = char:GetData("Alias").redline
    if alias ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:CreateRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:CancelRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:StartRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    if
        _races[tonumber(data)].host_id == char:GetData("ID")
        and hasValue(char:GetData("States") or {}, "RACE_DONGLE")
    then
      ...
      if dist > 25 then
        ...
        cb(true)
        TriggerClientEvent("Phone:Client:Redline:StartRace", -1, tonumber(data), 1, #_races[tonumber(data)].racers)
      end
    else
      cb(false)
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:EndRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:JoinRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    local alias = char:GetData("Alias").redline
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Redline:LeaveRace", function(src, data, cb)
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    local alias = char:GetData("Alias").redline
