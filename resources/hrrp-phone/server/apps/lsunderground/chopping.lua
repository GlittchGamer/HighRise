...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  ...
  Chat:RegisterAdminCommand('choplists', function(source, args, rawCommand)
    if args[1] == "all" then
      exports['hrrp-base']:LoggerTrace("Chopping", "Generating New Public Chop List")
      _publicChoplist = {
      ...
      exports['hrrp-base']:LoggerTrace("Chopping", "Generating New VIP Chop List")
      _vipChopList = {
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:CheckVehicle", function(source, data, cb)
    ...
    if list ~= nil and not isInProg then
      ...
      if isInProg then
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Vehicle Already Being Chopped")
      end
      cb(false)
    end
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:ChopPart", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:ChopTire", function(source, data, cb)
    if data?.index ~= nil then
      local plyr = exports['hrrp-base']:FetchSource(source)
      local pState = Player(source).state
      local entState = Entity(NetworkGetEntityFromNetworkId(data.vNet)).state
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:ChopWheel", function(source, data, cb)
    if data?.index ~= nil then
      local plyr = exports['hrrp-base']:FetchSource(source)
      local pState = Player(source).state
      local entState = Entity(NetworkGetEntityFromNetworkId(data.vNet)).state
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:ChopVehicle", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    local pState = Player(source).state
    local entState = Entity(NetworkGetEntityFromNetworkId(data.vNet)).state
    if plyr ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:CancelChop", function(source, data, cb)
    if _pChopping[source] ~= nil then
      _inProgress[_pChopping[source]] = nil
      _pChopping[source] = nil
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:Pickup", function(source, data, cb)
    ...
    if #pickups > 0 then
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Have Nothing To Pickup")
    end
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:LSUnderground:Chopping:GetPublicList", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        if not _emailed[char:GetData("ID")] or os.time() > _emailed[char:GetData("ID")] then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Recently Requested Active Chop Lists")
        end
      end
    end
  end)
    ...
    if type == 1 or type == 2 then
      for k, v in pairs(_inProgress) do
        if v.type == type and v.model == model and v.source ~= source then
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Vehicle Type Is Already Being Chopped")
          return true
        end
      end
    elseif type == 3 then
        ...
        if ent ~= nil then
          ...
          if pState.inChopZone == "chopping_public" and Reputation:GetLevel(source, "Salvaging") >= 3 then
            ...
            if not chopEntry then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Vehicle Not On Chop List")
              return nil
          ...
          then
            ...
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not In A Valid Dropoff Location")
            return nil
          end
        else
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Entity")
          return nil
        end
      end
    end
