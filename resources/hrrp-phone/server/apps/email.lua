PHONE.Email = {
  ...
  Send = function(self, serverId, sender, time, subject, body, flags)
    local plyr = exports['hrrp-base']:FetchSource(serverId)
    if plyr ~= nil then
...
AddEventHandler("Phone:Server:RegisterMiddleware", function()
  ...
  Middleware:Add("Phone:CharacterCreated", function(source, cData)
    return {
      {
        app = "email",
        alias = string.format("%s_%s%s@highrise.net", cData.First, cData.Last, cData.SID),
      },
    }
  end)
end)
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Email:Read", function(source, data, cb)
    local src = source
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    cb(Phone.Email:Read(char:GetData("Phone"), data))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:Email:Delete", function(source, data, cb)
    local src = source
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    cb(Phone.Email:Delete(char:GetData("ID"), data))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:Email:DeleteExpired", function(source, data, cb)
    local src = source
