...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("Phone:Garage:GetCars", function(source, data, cb)
    local src = source
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
    Vehicles.Owned:GetAll(nil, 0, char:GetData("SID"), cb)
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Phone:Garage:TrackVehicle", function(source, data, cb)
    local src = source
    local char = exports['hrrp-base']:FetchSource(src):GetData("Character")
