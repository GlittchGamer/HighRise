		...
		while _threading do
			for k, v in pairs(FLEECA_LOCATIONS) do
				if _fcGlobalReset[k] ~= nil and os.time() > _fcGlobalReset[k] then
					exports['hrrp-base']:LoggerInfo("Robbery", string.format("Fleeca - %s Heist Has Been Reset", v.label))
					ResetFleeca(k)
				end
			end
			Citizen.Wait(30000)
	...
	Citizen.CreateThread(function()
		while _threading do
			for k, v in pairs(FLEECA_LOCATIONS) do
				...
				then
					exports['hrrp-base']:LoggerInfo("Robbery", string.format("Vault Door At %s Opening", v.label))
