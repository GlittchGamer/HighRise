...
function ResetFleeca(fleecaId)
  ...
  exports['hrrp-doors']:SetLock(string.format("%s_tills", fleecaId), true)
  exports['hrrp-doors']:SetLock(string.format("%s_gate", fleecaId), true)
...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Fleeca:Vault:StartLootTrolley", function(source, data, cb)
    ...
    if pState.fleeca ~= nil then
      if
          ...
          and (
            data.cart == 1
            or not exports['hrrp-doors']:IsLocked(string.format("%s_gate", pState.fleeca))
          )
      then
        _inProgress[pState.fleeca] = true
        exports['hrrp-base']:LoggerInfo("Robbery",
          string.format("%s %s (%s) Started Looting Trolley #%s At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.cart,
            pState.fleeca))
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Fleeca:Vault:FinishLootTrolley", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
    ...
    if pState.fleeca ~= nil then
      ...
      then
        ...
        exports['hrrp-base']:LoggerInfo("Robbery",
          string.format("%s %s (%s) Finished Looting Trolley #%s At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.cart,
            pState.fleeca))
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Fleeca:Vault:CancelLootTrolley", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
    ...
    if pState.fleeca ~= nil then
      ...
      then
        exports['hrrp-base']:LoggerInfo("Robbery",
          string.format("%s %s (%s) Cancelled Looting Trolley #%s At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.cart,
            pState.fleeca))
        _inUse.Vault[pState.fleeca][data.cart] = nil
      end
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Fleeca:SecureBank", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state

    if pState.fleeca ~= nil then
      if exports['hrrp-jobs']:JobsPermissionsHasJob(source, "police") then
        if pState.onDuty then
          ...
          exports['hrrp-base']:LoggerInfo("Robbery", string.format("%s %s (%s) Secured Fleeca %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), pState.fleeca))
          TriggerClientEvent("Robbery:Client:Fleeca:CloseVaultDoor", -1, pState.fleeca)
          exports['hrrp-doors']:SetLock(string.format("%s_tills", pState.fleeca), true)
          exports['hrrp-doors']:SetLock(string.format("%s_gate", pState.fleeca), true)
        else
        end
      else
      end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Fleeca:SequenceGate", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
    if pState.fleeca ~= nil then
      exports['hrrp-doors']:SetLock(string.format("%s_gate", pState.fleeca), false)
    else
      cb(false)
    end
  end)
        ...
        if #(bankData.points.vaultPC.coords - playerCoords) <= 1.5 then
          ...
          then
            if not _inUse.VaultDoor[pState.fleeca] then
              exports['hrrp-base']:LoggerInfo("Robbery",
                string.format("%s %s (%s) Started Hacking Vault Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), pState.fleeca))
              ...
              exports["hrrp-base"]:CallbacksClient(
                ...
                function(success, data)
                  if success then
                    ...
                    exports['hrrp-base']:LoggerInfo("Robbery",
                      string.format("%s %s (%s) Successfully Hacked Vault Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                        pState.fleeca))
                    GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] = {
                      state = 2,
                    ...
                    exports['hrrp-base']:LoggerInfo("Robbery",
                      string.format("%s %s (%s) Failed Hacking Vault Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), pState
                        .fleeca))
              ...
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Someone Else Is Doing A Thing", 6000)
            end
            return
          elseif
              GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] == nil
      ...
      if not GlobalState["AntiShitlord"] or os.time() >= GlobalState["AntiShitlord"] or _inProgress[pState.fleeca] then
        ...
        if #(bankData.points.vaultGate.coords - playerCoords) <= 1.5 then
          ...
          if
              GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] ~= nil
              and GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)].state == 3
              and exports['hrrp-doors']:IsLocked(string.format("%s_gate", pState.fleeca))
          then
            if _inUse.Vault[pState.fleeca] == nil or not _inUse.GateDoor[pState.fleeca] then
              exports['hrrp-base']:LoggerInfo("Robbery",
                string.format("%s %s (%s) Started Thermiting Vault Gate Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                  pState.fleeca))
              _inProgress[pState.fleeca] = true
              if not GlobalState["AntiShitlord"] or os.time() >= GlobalState["AntiShitlord"] then
                GlobalState["AntiShitlord"] = os.time() + (60 * math.random(20, 30))
              ...
              exports["hrrp-base"]:CallbacksClient(
                ...
                function(success, data)
                  if success then
                    exports['hrrp-base']:LoggerInfo("Robbery",
                      string.format("%s %s (%s) Successfully Thermited Vault Gate Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                        pState.fleeca))
                    ...
                    exports['hrrp-base']:LoggerInfo("Robbery",
                      string.format("%s %s (%s) Failed Thermiting Vault Gate Door At %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                        pState.fleeca))
                    Status.Modify:Add(source, "PLAYER_STRESS", 6)
                  end
  ...
  Inventory.Items:RegisterUse("fleeca_card", "FleecaRobbery", function(source, itemData)
    ...
    if pState.fleeca ~= nil then
      if pState.fleeca == itemData.MetaData.BankId then
        ...
        if not GlobalState["AntiShitlord"] or os.time() >= GlobalState["AntiShitlord"] or _inProgress[pState.fleeca] then
          ...
          then
            ...
            then
              if not _inUse.VaultDoor[pState.fleeca] then
                exports['hrrp-base']:LoggerInfo("Robbery",
                  string.format("%s %s (%s) Attempting To Open Vault Door At %s With Access Card", char:GetData("First"), char:GetData("Last"),
                ...
                exports["hrrp-base"]:CallbacksClient(
                  ...
                  function(success, data)
                    if success and data.entered == tostring(itemData.MetaData.VaultCode) then
                      exports['hrrp-base']:LoggerInfo("Robbery",
                        string.format("%s %s (%s) Open Vault Door At %s With Access Card", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                          pState.fleeca))
                      local timer = math.random(2, 4)
                      GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] = {
                        state = 2,
                      ...
                      exports['hrrp-base']:LoggerInfo("Robbery",
                        string.format("%s %s (%s) Failed Opening Vault Door At %s With Access Card", char:GetData("First"), char:GetData("Last"),
                          char:GetData("SID"), pState.fleeca))
                      GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] = {
                        state = 4,
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Access Card: Incorrect Location", 6000)
        return
      end
    end
  end)
  ...
  Inventory.Items:RegisterUse("moneybag", "FleecaRobbery", function(source, itemData)
    ...
    if os.time() >= itemData.MetaData.Finished then
      local amt = itemData.MetaData?.CustomAmt and (math.random(itemData.MetaData?.CustomAmt.Random) + itemData.MetaData?.CustomAmt.Min) or
          (math.random(5000) + 10000)
      exports['hrrp-base']:LoggerInfo("Robbery", string.format("%s %s (%s) Used A Money Bag, Received $%s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), amt))
      Inventory.Items:RemoveSlot(itemData.Owner, itemData.Name, 1, itemData.Slot, itemData.invType)
      Wallet:Modify(source, amt)
    else
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Ready Yet", 6000)
    end
  end)
end)
