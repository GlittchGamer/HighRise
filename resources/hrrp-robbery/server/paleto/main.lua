...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  for k, v in ipairs(_pbDoorIds) do
    exports['hrrp-doors']:SetLock(v, true)
  end

  Middleware:Add("playerDropped", PaletoClearSourceInUse)
  Middleware:Add("Characters:Logout", PaletoClearSourceInUse)

  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:SecureBank", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:DisableAlarm", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:TriggeredLaser", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:GetDoors", function(source, data, cb)
    local its = {}
    ...
    for k, v in ipairs(_pbDoorsGarbage) do
      table.insert(its, {
        label = v.label,
        data = v.data,
        disabled = not exports['hrrp-doors']:IsLocked(v.doorId),
        event = exports['hrrp-doors']:IsLocked(v.doorId) and "Robbery:Client:Paleto:Door" or nil,
      })
    end
    cb(its)
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:VaultTerminal", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:UnlockDoor", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, true) then
            ...
            if data.data.id ~= nil then
              ...
              then
                exports['hrrp-doors']:SetLock(data.data.door, false)
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Door Unlocked")

                if _pbDoorsGarbage[data.data.id].requireCode then
                  Inventory.Items:RemoveAll(char:GetData("SID"), 1, "paleto_access_codes")
                end
              else
                exports['hrrp-doors']:SetLock(data.data.door, true)
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Access Code")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:ElectricBox:Hack", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
            ...
            if not _pbInUse.powerBoxes[data.boxId] then
              ...
              if Inventory.Items:Has(char:GetData("SID"), 1, "adv_electronics_kit", 1) then
                ...
                if itemData ~= nil then
                  ...
                  exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Hack", {
                    config = {
                      countdown = 3,
                      timer = 5,
                      limit = 14000,
                    ...
                    if success then
                      ...
                      if IsPaletoPowerDisabled() then
                        ...
                        GlobalState["Fleeca:Disable:savings_paleto"] = true
                        exports['hrrp-doors']:SetLock("bank_savings_paleto_gate", false)
                      else
                        Sounds.Play:Location(
                          source,
                          data.ptFxPoint,
                        ...
                        exports['hrrp-doors']:SetLock("bank_savings_paleto_gate", true)
                      end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:PC:Hack", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, true) then
            ...
            then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Electric Box Already Disabled", 6000)
              return
            end
            ...
            if not _pbInUse.pcHacks[data.pcId] then
              ...
              if Inventory.Items:Has(char:GetData("SID"), 1, "adv_electronics_kit", 1) then
                ...
                if itemData ~= nil then
                  ...
                  exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Tracking", {
                    config = {
                      countdown = 3,
                      delay = 2000,
                      limit = 14000,
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:Workstation", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, true) then
            if not IsPaletoExploitInstalled() then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Network Firewalls Still Active", 6000)
              return
            elseif _bankStates.paleto.workstation and _bankStates.paleto.workstation > os.time() then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Workstation Has Already Been Hacked", 6000)
              return
            end
            ...
            if not _pbInUse.workstation then
              ...
              then
                ...
                if itemData ~= nil then
                  ...
                  exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:AimHack", {
                    config = {
                      countdown = 3,
                      limit = 15750,
                      timer = 500,
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:OfficeHack", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:Drill", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, false) then
            ...
            then
              ...
              return
            elseif exports['hrrp-doors']:IsLocked("bank_savings_paleto_vault") then
              return
            end
            if not _pbInUse.drillPoints[data] then
              ...
              if Inventory.Items:Has(char:GetData("SID"), 1, "drill", 1) then
                ...
                if slot ~= nil then
                  ...
                  exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Drill", {
                    passes = 1,
                    duration = 25000,
                    config = {},
                    data = {},
                ...
                _pbInUse.drillPoints[data] = false
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Need A Drill", 6000)
              end
            else
              exports['hrrp-base']:ExecuteClient(
                source,
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:Search", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, false) then
            ...
            then
              ...
              return
            elseif exports['hrrp-doors']:IsLocked(data.door) then
              return
            end
            if not _pbInUse.searchPoints[data.searchId] then
              ...
              exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Lockpick", {
                config = 0.75,
                data = {
                  stages = 3,
                },
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:StartSafe", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          if PaletoIsGloballyReady(source, true) then
            if _bankStates.paleto.officeSafe and _bankStates.paleto.officeSafe > os.time() then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Safe Has Already Been Looted", 6000)
              return
            end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Paleto:Safe", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
