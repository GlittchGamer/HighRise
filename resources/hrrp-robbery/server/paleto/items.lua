        ...
        then
          if PaletoIsGloballyReady(source, true) then
            ...
            elseif (_bankStates.paleto.substations[subStationId] or 0) > os.time() then
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "This Substation Is Already Disabled", 6000)
              return
            end
      ...
      then
        if PaletoIsGloballyReady(source, true) then
          ...
          elseif not IsPaletoPowerDisabled() then
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Regional Power Is Still Active", 6000)
            return
          end
          ...
          for k, v in ipairs(_pbDoorThermite) do
            if exports['hrrp-doors']:IsLocked(v.door) then
          ...
          if exports['hrrp-doors']:IsLocked("bank_savings_paleto_security") then
                      ...
                      exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Thermite", {
                        ...
                        if success then
                          ...
                          if IsSecurityAccessible() then
                            exports['hrrp-doors']:SetLock("bank_savings_paleto_security", false)
                          else
                            exports['hrrp-doors']:SetLock("bank_savings_paleto_security", true)
                          end
            ...
            if #(v.coords - myCoords) <= 1.5 then
              if AreRequirementsUnlocked(v.requiredDoors) then
                if not _pbInUse[k] then
                  ...
                  exports["hrrp-base"]:CallbacksClient(source, "Robbery:Games:Captcha", {
                    location = {
                      coords = v.coords,
                      heading = v.heading,
                    },
