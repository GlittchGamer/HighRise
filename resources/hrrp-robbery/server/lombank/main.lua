...
function AreRequirementsUnlocked(reqs)
  for k, v in ipairs(reqs or {}) do
    if exports['hrrp-doors']:IsLocked(v) then
      return false
    end
  end
  return true
...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:SecureBank", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:Vault:StartLootTrolley", function(source, data, cb)
    ...
    if pState.inLombank then
      if
          ...
          and not exports['hrrp-doors']:IsLocked(lbThermPoints[string.format("lowerVaultRoom%s", pState.lombankRoom)].door)
      then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:Vault:FinishLootTrolley", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:ElectricBox:Hack", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:ElectricBox:Thermite", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
          ...
          then
            if not _lbInUse.powerBoxes[data.boxId] then
              ...
              if Inventory.Items:Has(char:GetData("SID"), 1, "thermite", 1) then
                ...
                _lbInUse.powerBoxes[data.boxId] = false
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Need Thermite", 6000)
              end
            else
              exports['hrrp-base']:ExecuteClient(
                source,
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Lombank:Drill", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
