    ...
    for k, v in ipairs(_queues) do
      ...
      Tasks:Register(v.id, v.wait, function(data)
        ...
        while plyr == nil and char == nil and #sources > 0 do
          ...
          plyr = exports['hrrp-base']:FetchSource(src)
          if plyr ~= nil then
        ...
        if plyr ~= nil and char ~= nil then
          exports['hrrp-base']:LoggerInfo(
            "Robbery Queue",
            string.format(
              "%s %s (%s) Has Been Chosen For ^3%s^7",
              char:GetData("First"),
              char:GetData("Last"),
              char:GetData("SID"),
              v.name
          ...
          Phone.Email:Send(
            plyr:GetData("Source"),
            "unknown@highnet.anon",
            os.time() * 1000,
            string.format("You Have Been Chosen For A %s", v.name),
            string.format(
              [[
							Hello %s<br /><br />
							Congratulations, you have been chosen to receive a %s. Please head to Digital Den in the Boring Shopping Mall before tsunami to pick up your item.
						]],
              char:GetData("First"),
              v.name
            )
          ...
          exports['hrrp-base']:LoggerInfo("Robbery Queue", string.format("No Eligible Player Found For ^3%s^7", v.name))
        end
