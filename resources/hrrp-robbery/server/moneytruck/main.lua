...
AddEventHandler("Robbery:Server:Setup", function()
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:MoneyTruck:EnteredTruck", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:MoneyTruck:CheckLoot", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:MoneyTruck:CancelLoot", function(source, data, cb)
    local pState = Player(source).state
    local ent = NetworkGetEntityFromNetworkId(data)
    if ent ~= 0 then
      local entState = Entity(ent).state
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:MoneyTruck:Loot", function(source, data, cb)
    local pState = Player(source).state
    local ent = NetworkGetEntityFromNetworkId(data)
    if ent ~= 0 then
      local entState = Entity(ent).state
          ...
          if plyr ~= nil then
            ...
            if char ~= nil then
              ...
              exports['hrrp-base']:LoggerInfo("Robbery",
                string.format("%s %s (%s) Looted A Money Truck (VIN: %s)", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                  Entity(ent).state.VIN))
