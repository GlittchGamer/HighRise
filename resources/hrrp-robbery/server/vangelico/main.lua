...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Vangelico:BreakCase", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Vangelico:SecureStore", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
    if _alerted ~= nil and _alerted >= os.time() then
      PutShittyThingsOnCD()
      GlobalState["Vangelico:State"] = 2
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Store Has Been Secure", 6000)
    else
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Unable To Secure Store, No Recent Crime Reported", 6000)
      exports['hrrp-base']:LoggerInfo(
        "Robbery",
        string.format(
