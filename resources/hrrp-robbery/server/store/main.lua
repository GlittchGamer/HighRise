...
local _safes = {
  {
    id = "safe1",
    coords = vector3(1959.0, 3749.39, 32.34),
    width = 0.6,
    length = 0.6,
    options = {
      heading = 28,
...
function Threads()
  ...
  Citizen.CreateThread(function()
    while true do
      exports['hrrp-base']:LoggerTrace("Robbery", "Resetting Store Alert States With Expired Emergency Alerts")
...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:Register", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:StartSafeCrack", function(source, data, cb)
      ...
      if plyr ~= nil then
        ...
        if char ~= nil then
          ...
          then
            ...
            if not _storeInUse[pState.storePoly] then
              ...
              if slot ~= nil then
                ...
                exports['hrrp-base']:LoggerInfo("Robbery",
                  string.format("%s %s (%s) Started Store Robbery (Safe) At Store %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
                    pState.storePoly))

                exports["hrrp-base"]:CallbacksClient(source, "Robbery:Store:DoSafeCrack", {
                  passes = 1,
                  config = {
                    countdown = 3,
                    preview = 2500,
            ...
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Unable To Crack Safe", 6000)
          end
        end
      end
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:StartSafeSequence", function(source, data, cb)
    if GetGameTimer() < STORE_SERVER_START_WAIT then
      exports['hrrp-base']:ExecuteClient(
        source,
        "Notification",
    ...
    elseif (GlobalState["Duty:police"] or 0) < STORE_REQUIRED_POLICE then
      ...
      return cb(false)
    elseif GlobalState["RobberiesDisabled"] then
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Temporarily Disabled, Please See City Announcements", 6000)
      return cb(false)
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:StartLockpick", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        ...
        then
          ...
          exports['hrrp-base']:LoggerInfo("Robbery",
            string.format("%s %s (%s) Started Store Robbery (Register) At Store %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"),
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:Safe", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if GlobalState[string.format("Safe:%s", data.id)] == nil and not GlobalState["RestartLockdown"] then
      ...
      if data.results then
        state = 1
        cb(true)
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Lock Disengage Initiated, Please Stand By", 6000)
      else
        -- Do something?
        cb(true)
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You've Damaged The Electronics On The Lock", 6000)
      end
      if _storeAlerts[data.store] == nil or _storeAlerts[data.store] < os.time() then
        exports['hrrp-base']:LoggerInfo("Robbery",
          string.format("%s %s (%s) Started Store Robbery (Safe) At Store %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.store))
        _storeAlerts[data.store] = (os.time() + (60 * 5))
      ...
      exports['hrrp-base']:LoggerTrace("Robbery", string.format("Safe %s Will Unlock At %s", data.id, obj.expires))
      _robbedSafes[data.id] = obj
      GlobalState[string.format("Safe:%s", data.id)] = obj
      GlobalState["StoreAntiShitlord"] = os.time() + (60 * math.random(5, 10))
    else
      exports['hrrp-base']:LoggerError("Robbery", string.format("Safe %s Was Already Cracked", data.id))
    end
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:LootSafe", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if _robbedSafes[data.id] ~= nil and _robbedSafes[data.id].state == 2 then
      ...
      exports['hrrp-base']:LoggerInfo("Robbery", string.format("%s %s (%s) Looted %s Safe", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.id))
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Robbery:Store:SecureSafe", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if myDuty and myDuty == "police" then
      if _robbedSafes[data.id] ~= nil and _robbedSafes[data.id].state ~= 4 then
        ...
        _robbedSafes[data.id].expires = (os.time() + 60 * math.random(30, 60))
        exports['hrrp-base']:LoggerInfo("Robbery", string.format("%s %s (%s) Secured %s Safe", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), data.id))
        GlobalState[string.format("Safe:%s", data.id)] = _robbedSafes[data.id]
        Sounds.Stop:Location(_robbedSafes[data.id].source, _robbedSafes[data.id].coords, "alarm")
      end
    end
  ...
  while true do
    for k, v in pairs(_robbedSafes) do
      if v.expires < os.time() then
        if v.state == 1 then
          exports['hrrp-base']:LoggerTrace("Robbery", string.format("Safe %s Expired While State 1, Updating To State 2", k))
          ...
          exports['hrrp-base']:LoggerTrace("Robbery", string.format("Safe %s Expired While State 2, Resetting", k))
          _robbedSafes[k] = nil
