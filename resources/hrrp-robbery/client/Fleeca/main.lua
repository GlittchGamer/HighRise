...
AddEventHandler("Robbery:Client:Setup", function()
  ...
  for k, v in ipairs(GlobalState["FleecaRobberies"]) do
    ...
    exports['hrrp-polyzone']:PolyZoneCreateBox(bankData.id, bankData.coords, bankData.width, bankData.length, bankData.options)
        ...
        {
          {
            ...
            isEnabled = function(s, s2)
              return (
                ...
                or (not exports['hrrp-doors']:IsLocked(string.format("%s_gate", LocalPlayer.state.fleeca)))
              )
            end,
          },
        },
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Robbery:Fleeca:Keypad:Vault", function(data, cb)
...
AddEventHandler("Robbery:Client:Fleeca:StartSecuring", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports["hrrp-base"]:CallbacksServer("Robbery:Fleeca:SecureBank", {})
    end
  end)
end)

AddEventHandler("Robbery:Client:Fleeca:SequenceGate", function()
  exports["hrrp-base"]:CallbacksServer("Robbery:Fleeca:SequenceGate", {}, function(r) end)
end)

AddEventHandler("Robbery:Client:Fleeca:LootTrolley", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Robbery:Fleeca:Vault:StartLootTrolley", { cart = data }, function(proc)
    if proc then
      if
      ...
      then
        ...
        if #(myCoords - cartData.coords) <= 3.0 then
          ...
          exports["hrrp-base"]:CallbacksServer("Robbery:Fleeca:Vault:FinishLootTrolley", {
            cart = data,
          })
          Citizen.Wait(1800)
  ...
  if LocalPlayer.state.fleeca ~= nil then
    ...
    then
      for k, v in ipairs(GlobalState[string.format("FleecaRobberies:%s", LocalPlayer.state.fleeca)].carts) do
        ...
        exports['ox_target']:TargetingAddEntity(obj, "coins", {
          {
            icon = "coins",
            text = "Take Loot",
            ...
            isEnabled = function()
              return GlobalState[string.format("Fleeca:%s:VaultDoor", LocalPlayer.state.fleeca)] ~= nil
                  ...
                  and (k == 1 or not exports['hrrp-doors']:IsLocked(string.format("%s_gate", LocalPlayer.state.fleeca)))
            end,
          },
        }, 3.0)
      end
    ...
    for k, v in ipairs(GlobalState[string.format("FleecaRobberies:%s", LocalPlayer.state.fleeca)].carts) do
      for k2, v2 in ipairs(TROLLY_TYPES) do
        ...
        if obj ~= 0 then
          ...
          exports['ox_target']:TargetingRemoveEntity(obj)
        end
        ...
        if obj2 ~= 0 then
          ...
          exports['ox_target']:TargetingRemoveEntity(obj2)
        end
      end
    end
  end
