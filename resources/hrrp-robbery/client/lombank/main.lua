function LBNeedsReset()
  for k, v in pairs(lbThermPoints) do
    if not exports['hrrp-doors']:IsLocked(v.door) then
      return true
    end
  end
...
AddEventHandler("Robbery:Client:Setup", function()
  exports['hrrp-polyzone']:PolyZoneCreatePoly("dumbcunt", {
  ...
  exports['hrrp-polyzone']:PolyZoneCreatePoly("bank_lombank", {
  ...
  exports['hrrp-polyzone']:PolyZoneCreatePoly("lombank_power", {
  ...
  exports['ox_target']:TargetingZonesAddBox("lombank_secure", "shield-keyhole", vector3(7.69, -923.1, 29.9), 2.8, 1.6, {
    heading = 340,
    --debugPoly=true,
    minZ = 28.9,
    maxZ = 30.7,
  ...
  exports['hrrp-polyzone']:PolyZoneCreateBox("lombank_death", vector3(24.86, -921.78, 25.74), 7.4, 7.8, {
    heading = 340,
    --debugPoly=true,
    minZ = 24.74,
    maxZ = 28.74,
  ...
  for k, v in ipairs(_lombankRooms) do
    exports['hrrp-polyzone']:PolyZoneCreateBox(string.format("lombank_room_%s", v.roomId), v.coords, v.length, v.width, v.options, {
      isLombankRoom = true,
      roomId = v.roomId,
    })
  end
...
AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
  if type(data) == "table" then
    if data.isDeath then
      if not data.door or exports['hrrp-doors']:IsLocked(data.door) then
  ...
  elseif data.isLombankRoom then
    ...
    for k, v in ipairs(_lbCarts) do
      exports['ox_target']:TargetingAddObject(v, "treasure-chest", {
        {
          text = "Grab Loot",
          icon = "hand",
          ...
          isEnabled = function(d, entity)
            ...
            return not exports['hrrp-doors']:IsLocked("lombank_lower_gate")
                and not exports['hrrp-doors']:IsLocked("lombank_lower_vault")
                and not exports['hrrp-doors']:IsLocked(string.format("lombank_lower_room_%s", data.roomId))
...
AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZones, data)
  ...
  elseif data.isLombankRoom then
    ...
    for k, v in ipairs(_lbCarts) do
      exports['ox_target']:TargetingRemoveObject(v)
    end
  end
end)
...
AddEventHandler("Robbery:Client:Lombank:StartSecuring", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports["hrrp-base"]:CallbacksServer("Robbery:Lombank:SecureBank", {})
    end
  end)
end)

AddEventHandler("Robbery:Client:Lombank:ElectricBox:Hack", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Robbery:Lombank:ElectricBox:Hack", data, function() end)
end)

AddEventHandler("Robbery:Client:Lombank:ElectricBox:Thermite", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Robbery:Lombank:ElectricBox:Thermite", data, function() end)
end)

AddEventHandler("Robbery:Client:Lombank:Drill", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Robbery:Lombank:Drill", data.id, function() end)
end)
