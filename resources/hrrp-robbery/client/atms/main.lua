...
AddEventHandler("Robbery:Client:Setup", function()
  exports["hrrp-base"]:CallbacksRegisterClient('Robbery:ATM:StartRobbing', function(data, callback)
    ...
    if IsATMModel(Target.entity) then
      if not data.Cooldown then
        exports["hrrp-base"]:CallbacksServer('Robbery:ATM:CanRobATM', Target.entity, function(cb)
          if cb then
            exports["hrrp-base"]:CallbacksServer('Robbery:ATM:SetATMRobbed', Target.entity, function(cb) end)
            ...
            callback(false)
            exports['hrrp-hud']:NotificationError('This ATM has already been robbed.', 2500)
          end
        end)
      else
        exports['hrrp-hud']:NotificationError('You cannot rob an ATM right now.', 2500)
        callback(false)
      end
    else
      callback(false)
...
StartDrilling = function(entity, itemData)
  ...
  Drilling.Start(function(status)
    if status then
      ...
      exports["hrrp-base"]:CallbacksServer('Robbery:ATM:GetReward', {}, function(HRRP) end)
    else
      StopParticleFxLooped(blowPtfx, false)
