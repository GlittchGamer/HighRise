...
AddEventHandler("Robbery:Client:Setup", function()
  exports['hrrp-polyzone']:PolyZoneCreatePoly("vangelico", {
  ...
  exports['ox_target']:TargetingZonesAddBox("vangelico-pd", "calculator", vector3(-620.09, -223.81, 38.06), 1.0, 1.0, {
    name = "vangelico-pd",
    heading = 35,
    --debugPoly = true,
    minZ = 37.91,
  ...
  for k, v in ipairs(GlobalState["VangelicoCases"]) do
    ...
    exports['ox_target']:TargetingZonesAddBox(pId, "container-storage", v.coords, v.length, v.width, v.options, {
      {
        icon = "hammer",
        text = "Smash Case",
...
AddEventHandler("Robbery:Client:Vangelico:SecureStore", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports["hrrp-base"]:CallbacksServer("Robbery:Vangelico:SecureStore")
    end
  end)
end)
...
AddEventHandler("Polyzone:Enter", function(id, point, insideZones, data)
  if id == "vangelico" then
    exports['ox_target']:TargetingAddObject(GetHashKey("hei_prop_hei_keypad_03"), "calculator", {
      {
        icon = "bell-on",
        text = "Disable Alarm",
...
AddEventHandler("Polyzone:Exit", function(id, point, insideZones, data)
  if id == "vangelico" then
    exports['ox_target']:TargetingRemoveObject(GetHashKey("hei_prop_hei_keypad_03"))
  end
end)
  ...
  then
    ...
    Progress:ProgressWithTickEvent({
      ...
      if not cancelled then
        exports["hrrp-base"]:CallbacksServer("Robbery:Vangelico:BreakCase", data.index)
      end
    end)
  end
end)
