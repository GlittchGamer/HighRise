...
AddEventHandler("Doors:Client:LockElevator", function(data)
  if ELEVATOR_STATE[data.elevator] and LocalPlayer.state.loggedIn then
    exports['core']:CallbacksServer("Doors:Elevators:ToggleLocks", data, function(success, newState)
      if success then
        if newState then
          exports['hud']:NotificationError("Elevator Locked")
        else
          exports['hud']:NotificationSuccess("Elevator Unlocked")
        end
      end
    end)
  end
...
AddEventHandler("Doors:Client:UseElevator", function(data)
  ...
  if elevatorData and elevatorData.floors and LocalPlayer.state.loggedIn then
    ...
    if floorData and floorData.coords then
      exports['core']:CallbacksServer("Doors:Elevator:Validate", floorData, function()
