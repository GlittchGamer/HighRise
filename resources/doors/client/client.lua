...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Doors", {
    ...
    exports['keybinds']:Add('doors_garage_fob', 'f10', 'keyboard', 'Doors - Use Garage Keyfob', function()
...
function InitDoors()
  ...
  DOORS_STATE = {}
  ELEVATOR_STATE = {}
  exports['core']:CallbacksServer('Doors:Fetch', {}, function(fetchedDoors, fetchedElevators)
...
DOORS = {
  ...
  CheckRestriction = function(self, doorId)
    ...
    if doorData and LocalPlayer.state.Character then
      ...
      if exports['jobs']:JobsPermissionsHasJob('dgang', false, false, 99, true) then
        return true
      end
      ...
      for k, v in ipairs(doorData.restricted) do
        ...
        elseif v.type == 'job' then
          if v.job then
            if exports['jobs']:JobsPermissionsHasJob(v.job, v.workplace, v.grade, v.gradeLevel, v.reqDuty, v.jobPermission) then
              return true
            end
          elseif v.jobPermission then
            if exports['jobs']:JobsPermissionsHasPermission(v.jobPermission) then
              return true
            end
          end
...
function StartShowingDoorInfo(doorId)
  ...
  exports['hud']:ActionShow(actionMsg)
end
...
AddEventHandler('Keybinds:Client:KeyUp:primary_action', function()
  if _lookingAtDoor and _showingDoorInfo then
    StopShowingDoorInfo()
    DoorAnim()
    exports['core']:CallbacksServer('Doors:ToggleLocks', _lookingAtDoor, function(success, newState)
      if success then
