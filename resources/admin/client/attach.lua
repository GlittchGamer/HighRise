...
RegisterNetEvent('Admin:Client:Invisible', function()
  if _isInvis then
    ...
    exports['hud']:NotificationInfo("Invisibility: Off")
    _isInvis = false
  else
    SetEntityVisible(LocalPlayer.state.ped, false)
    exports['hud']:NotificationInfo("Invisibility: On")
    _isInvis = true
  end
end)
...
local function toggleSpectate(targetPed, targetPlayerId)
  ...
  if isSpectateEnabled then
    ...
    if not lastSpectateLocation then
      exports['hud']:NotificationError('Last location previous to spectate was not stored properly')
    end

    if not storedTargetPed then
      exports['hud']:NotificationError("Target ped was not stored to unspectate")
    end
...
local function cleanupFailedResolve()
  ...
  exports['hud']:NotificationError("Failed to Spectate")
end
