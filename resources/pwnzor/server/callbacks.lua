...
function RegisterCallbacks()
  ...
  Chat:RegisterAdminCommand("pwnzorsource", function(source, args, rawCommand)
    local player = exports['core']:FetchSource(tonumber(args[1]))
    if player ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Pwnzor:GetEvents", function(source, data, cb)
    if not exports['pwnzor']:PwnzorPlayersGet(source, "GetEvents") then
      exports['pwnzor']:PwnzorPlayersSet(source, "GetEvents")
      cb(_blacklistedClientEvents)
    else
      if not exports['core']:FetchSource(source).Permissions:IsAdmin() then
        Punishment.Ban:Source(source, -1, "Attempt To Recall GetEvents", "Pwnzor")
      end
    end
  end)

  exports['core']:CallbacksRegisterServer("Pwnzor:GetCommands", function(source, data, cb)
    if not exports['pwnzor']:PwnzorPlayersGet(source, "GetCommands") then
      exports['pwnzor']:PwnzorPlayersSet(source, "GetCommands")
  ...
  exports['core']:CallbacksRegisterServer("Pwnzor:AFK", function(source, data, cb)
    if Config.Components.AFK.Enabled then
      Punishment:Kick(source, "You Were Kicked For Being AFK", "Pwnzor")
    end
  end)

  exports['core']:CallbacksRegisterServer("Pwnzor:Trigger", function(source, data, cb)
    cb("💙 From Pwnzor 🙂")
    if not exports['core']:FetchSource(source).Permissions:IsAdmin() then
