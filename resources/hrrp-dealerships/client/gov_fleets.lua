function CreateGovermentFleetShops()
  for k, v in ipairs(_fleetConfig) do
    if v.interactionPed then
      exports['hrrp-pedinteraction']:Add('gov_fleet_shop_' .. v.job, v.interactionPed.model, v.interactionPed.coords, v.interactionPed.heading,
...
AddEventHandler('FleetDealers:Client:ConfirmPurchase', function(data)
  Confirm:Show(
    ...
    string.format('Please Confirm the Purchase of a %s %s (%s Livery) for $%s', data.vehData.make, data.vehData.model, data.liveryName,
      formatNumberToCurrency(data.vehData.price)),
    data
  )
end)
