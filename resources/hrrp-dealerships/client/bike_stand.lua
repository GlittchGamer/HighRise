...
function CreateBikeStands()
  for k, v in ipairs(_bikeStands) do
    if v.interactionPed then
      exports['hrrp-pedinteraction']:Add('bike_stand_' .. k, v.interactionPed.model, v.coords, v.interactionPed.heading, v.interactionPed.range, {
        {
          icon = 'bicycle',
          text = 'Bicycle Stand',
...
function CreateBikeStandBlips()
  for k, v in ipairs(_bikeStands) do
    if v.blip then
      exports['hrrp-blips']:Add('bike_stand_' .. k, 'Bicycle Stand', v.coords, v.blip.sprite, v.blip.color, v.blip.scale)
    end
  end
end
...
AddEventHandler('BikeStands:Client:Open', function(entityData, data)
  if _justBoughtFuckingBike[data.location] then
    return exports['hrrp-hud']:NotificationError('You Just Bought a Bike off Me! Weirdo!')
  end
  ...
  if data and data.location and data.bike then
    ...
    Callbacks:ServerCallback('BikeStand:Purchase', {
      ...
      if success then
        ...
        exports['hrrp-hud']:NotificationSuccess(string.format('Purchased %s, It Has Been Brought out for You.', bikeData.name))
      else
        exports['hrrp-hud']:NotificationError('Purchase Failed')
      end
    end)
  end
end)
