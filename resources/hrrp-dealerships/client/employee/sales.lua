...
function OpenDealershipSales(dealerId)
  ...
  if dealerData then
    Callbacks:ServerCallback('Dealerships:Sales:FetchData', dealerId, function(authed, stocks, serverTime, defaultInterestRate, dealerMData)
      if not authed then
        exports['hrrp-hud']:NotificationError('You\'re Not Authorized to Make Sales', 3500, 'car-building')
        return
      end
      ...
      salesMenu = exports['hrrp-menu']:Create('salesMenu', string.format('View %s Stock', _dealerships[dealerId].abbreviation), function()
      ...
      salesMenu.Add:Text(string.format('There are %s different models of vehicle, totalling %s vehicles', stockData.total, stockData.totalQuantity),
        { 'pad', 'center', 'code' })
      ...
      for _, category in ipairs(orderedCategories) do
        if stockData.sorted[category] and #stockData.sorted[category] > 0 then
          salesMenuSub[category] = exports['hrrp-menu']:Create('salesMenuCat-' .. category, _catalogCategories[category])
          ...
          for k, v in ipairs(stockData.sorted[category]) do
            if v.quantity > 0 then
              ...
              salesMenuSub[vehMenuIdentifier] = exports['hrrp-menu']:Create('salesMenu-' .. vehMenuIdentifier, vehName)
              ...
              -- Cash Sales Menu
              salesMenuSub[cashSaleIdentifier] = exports['hrrp-menu']:Create('salesMenu-' .. cashSaleIdentifier, vehName .. ' - New Sale')
              ...
              -- Loan Sales Menu
              salesMenuSub[loanSaleIdentifier] = exports['hrrp-menu']:Create('salesMenu-' .. loanSaleIdentifier, vehName .. ' - New Sale')

              local saleTextElem = salesMenuSub[loanSaleIdentifier].Add:Text(VehicleSalesGetLoanText(dealerMData, dealerData, v, loanData, defaultInterestRate),
                { 'code', 'pad' })
              ...
              salesMenuSub[loanSaleIdentifier].Add:Slider('Down Payment %', {
                ...
                salesMenuSub[loanSaleIdentifier].Update:Item(saleTextElem, VehicleSalesGetLoanText(dealerMData, dealerData, v, loanData, defaultInterestRate),
                  { 'code', 'pad' })
              end)
