...
function OpenDealerManagementMenu(dealer)
  Callbacks:ServerCallback('Dealerships:GetDealershipData', { dealerId = dealer }, function(data)
    ...
    manageMenu = exports['hrrp-menu']:Create('dmanageMenu', string.format('Manage %s', dealerData.abbreviation), function()
    ...
    manageMenu.Add:Button('Save Changes', { success = true }, function()
      ...
      Callbacks:ServerCallback('Dealerships:UpdateDealershipData', {
        ...
        if success then
          exports['hrrp-hud']:NotificationSuccess('Changes Saved Successfully', 2500, 'car-building')
        else
          exports['hrrp-hud']:NotificationError('Failed Saving Changes', 2500, 'car-building')
        end
      end)
    end)
