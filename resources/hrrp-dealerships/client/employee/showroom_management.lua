...
function OpenShowroomManagement(dealerId)
  if _dealerships[dealerId] and #_dealerships[dealerId].showroom > 0 then
    Callbacks:ServerCallback('Dealerships:ShowroomManagement:FetchData', dealerId, function(authed, stocks)
      ...
      if not authed then
        exports['hrrp-hud']:NotificationError('You\'re Not Authorized to Open Showroom Management')
        return
      end

      showroomManagementSub = {}
      showroomManagement = exports['hrrp-menu']:Create('showroomManagement', string.format('Manage %s Showroom', _dealerships[dealerId].abbreviation), function()
      ...
      for pos = 1, #_dealerships[dealerId].showroom do
        ...
        showroomManagement.Add:AdvButton(
          posVehicle and string.format('%s %s', posVehicle.make and posVehicle.make or 'Unknown', posVehicle.model and posVehicle.model or 'Unknown') or
          'No Vehicle', { secondaryLabel = '#' .. pos }, function()
...
function StartSettingShowroomPosition(stockData, dealerId, position, positionVehicle)
  showroomManagement = exports['hrrp-menu']:Create('showroomManagement',
  ...
  if positionVehicle then
    showroomManagement.Add:SubMenuBack('Clear Current Vehicle', {}, function()
      ...
      Callbacks:ServerCallback('Dealerships:ShowroomManagement:SetPosition', {
        ...
        if success then
          exports['hrrp-hud']:NotificationSuccess('Successfully Cleared Vehicle in Position #' .. position)
        else
          exports['hrrp-hud']:NotificationError('Failed to Clear Vehicle in Position #' .. position)
        end
  ...
  for _, category in ipairs(orderedCategories) do
    if stockData.sorted[category] and #stockData.sorted[category] > 0 then
      showroomManagementSub[category] = exports['hrrp-menu']:Create('showroomManagementCat-' .. category, _catalogCategories[category])
...
function SetDealerShowroomVehicleAtPosition(dealerId, position, vehData)
  ...
  Game.Vehicles:SpawnLocal(vector3(playerCoords.x, playerCoords.y, playerCoords.z - 20.0), vehData.vehicle, 100, function(vehicle)
    ...
    Callbacks:ServerCallback('Dealerships:ShowroomManagement:SetPosition', {
      ...
      if success then
        exports['hrrp-hud']:NotificationSuccess('Successfully Updated Vehicle in Position #' .. position)
      else
        exports['hrrp-hud']:NotificationError('Failed to Update Vehicle in Position #' .. position)
      end
      OpenShowroomManagement(dealerId)
    end)
  end)
