...
function RegisterVehicleRentalCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer('Rentals:Purchase', function(source, data, cb)
    ...
    if type(rentalSpot) == "number" and type(rentalVehicle) == "number" and spaceCoords and spaceHeading and _vehicleRentals[rentalSpot] then
      ...
      if char and rentalVehicleData then
        ...
        if Wallet:Modify(source, -rentalCost) then
          ...
          exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Not Enough Money to Rent', 5000)
          cb(false)
        end
      else
        cb(false)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Rentals:GetPending', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData('Character')
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Rentals:Return', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData('Character')
