    ...
    Chat:RegisterAdminCommand("setstock", function(source, args, rawCommand)
      ...
      params = {
        ...
        },
        {
          name = "Data - Category",
          help =
          "Category e.g. import, compact, suv, sedans, muscle, sport, sportclassic, super, motorcycles, offroad, rally, van, utility, misc",
        },
      },
    }, 8)
...
--! The below needs testing, since we append data to the DEALERSHIPS table
for k, v in pairs(DEALERSHIPS) do
  if type(v) == "function" then
    exportHandler("Dealerships" .. k, v)
  elseif type(v) == "table" then
    createExportForObject(v, "Dealerships" .. k)
  end
end

AddEventHandler('Proxy:Shared:RegisterReady', function() --!Needs testing
  exports['hrrp-base']:RegisterComponent('Dealerships', DEALERSHIPS)
end)

function RegisterCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:GetDealerStock', function(source, dealerId, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:ShowroomManagement:FetchData', function(source, dealerId, cb)
    if _dealerships[dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_showroom') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:StockViewing:FetchData', function(source, dealerId, cb)
    if _dealerships[dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_stock') or exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_sell') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:Sales:FetchData', function(source, dealerId, cb)
    if _dealerships[dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_stock') or exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_sell') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:ShowroomManagement:SetPosition', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if data and data.dealerId and type(data.position) == 'number' and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_showroom') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('BikeStand:Purchase', function(source, data, cb)
    if type(data.vehicleHash) == 'number' and type(data.price) == 'number' and data.price > 100 then
      ...
      if char and char:GetData('SID') then
        ...
        if Wallet:Modify(source, -data.price) then
          Vehicles.Owned:AddToCharacter(char:GetData('SID'), data.vehicleHash, 0,
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:CheckPersonsCredit', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")

    local stateId = math.tointeger(data.SID)

    if data and data.dealerId and stateId and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_sell') then
      local target = exports['hrrp-base']:FetchSID(stateId)
      if target then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:GetDealershipData', function(source, data, cb)
    if data and data.dealerId and _dealerships[data.dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_manage') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:UpdateDealershipData', function(source, data, cb)
    if data and data.dealerId and _dealerships[data.dealerId] and data.updating and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_manage') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:FetchHistory', function(source, dealerId, cb)
    if dealerId and _dealerships[dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerId, 'dealership_manage') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:FetchCurrentOwner', function(source, data, cb)
    if data and data.dealerId and _dealerships[data.dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_manage') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:BuyBackStart', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if data and data.dealerId and data.netId and _dealerships[data.dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_buyback') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:BuyBack', function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if data and data.dealerId and data.netId and _dealerships[data.dealerId] and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, data.dealerId, 'dealership_buyback') then
      local veh = NetworkGetEntityFromNetworkId(data.netId)
          ...
          if vehModel then
            ...
            if stockInfo and stockInfo.data and vehStrikes and not remainingLoan then
              ...
              if vehicle and vehicle:GetData('Owner')?.Type == 0 then
                local owner = exports['hrrp-base']:FetchSID(vehicle:GetData('Owner').Id)
                if owner then
                    ...
                    if #(GetEntityCoords(ownerPed) - GetEntityCoords(myPed)) <= 5.0 then
                      ...
                      if d and d.Account and p and p.Account then
                        local success = Banking.Balance:Charge(d.Account, buybackPrice, {
                          ...
                          description = string.format('Vehicle Buyback of a %s %s (%s) From %s %s (%s)',
                            vehEnt.state.Make, vehEnt.state.Model, vehEnt.state.VIN, owner:GetData("First"),
                        ...
                        if success then
                          ...
                          Banking.Balance:Deposit(p.Account, buybackPrice, {
                            ...
                            description = string.format('%s Vehicle Buyback of a %s %s (%s)',
                              _dealerships[data.dealerId].abbreviation, vehEnt.state.Make, vehEnt.state.Model,
                              vehEnt.state.VIN),
                            data = {
                              buyer = {
                                ID = char:GetData('ID'),
