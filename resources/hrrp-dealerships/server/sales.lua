...
function RegisterVehicleSaleCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer('Dealerships:Sales:StartSale', function(source, saleData, cb)
    ...
    if dealership and type and data then
      ...
      if char and dealerData and customerStateId and vehicle and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, dealerData.id, 'dealership_sell') then
        local targetPlayer = exports['hrrp-base']:FetchSID(customerStateId)
                  ...
                  Billing:Create(targetSrc, dealerData.abbreviation .. ' - Sales', salePrice, '',
                    function(wasPayed, withAccount)
                      if wasPayed then
                          ...
                          Vehicles.Owned:AddToCharacter(targetCharacter:GetData('SID'),
                            GetHashKey(saleVehicleData.vehicle), 0, {
                              ...
                              if success and vehicleData then
                                ...
                                exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Success',
                                  'Completed Sales Process - The Customer Received their Vehicle', 7500, 'car-building')
                                SendCompletedCashSaleEmail({
                                    SID = targetCharacter:GetData('SID'),
                                    First = targetCharacter:GetData('First'),
                                    Last = targetCharacter:GetData('Last'),
                                    Source = targetSrc,
                                  }, dealerData, saleVehicleData.data, salePrice, vehicleData.VIN,
                                  vehicleData.RegisteredPlate)

                                SendDealerProfits(dealerData, dealerRecieves, char:GetData('BankAccount'),
                                  playerCommission,
                                ...
                                exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Error Completing Vehicle Sale', 5000,
                                  'car-building')
                                exports['hrrp-base']:ExecuteClient(targetSrc, 'Notification', 'Error', 'Error Completing Vehicle Sale', 5000,
                                  'car-building')
                              end
                        ...
                        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Payment Failed', 5000, 'car-building')
                      end
                    end)
                      ...
                      if downPaymentPercent and loanWeeks and defaultInterestRate then
                        ...
                        if loanData.maxBorrowable >= salePriceAfterDown then
                          SendPendingLoanEmail({
                              ...
                              Last = targetCharacter:GetData('Last'),
                              Source = targetSrc,
                            }, dealerData, saleVehicleData.data, downPaymentPercent, downPayment, loanWeeks, perWeek,
                            salePriceAfterDown, function()
                              exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Info',
                                'The Loan Terms Were Accepted by the Customer',
                                5000, 'car-building')
                              Billing:Create(
                                targetSrc,
                                dealerData.name,
                                downPayment,
                                string.format('Vehicle Loan Downpayment, %s %s', saleVehicleData.data.make,
                                  saleVehicleData.data.model),
                                function(wasPayed, withAccount)
                                  if wasPayed then
                                    local preGenerateVIN = Vehicles.Identification.VIN:GenerateOwned()
                                    local loanSuccess = Loans:CreateVehicleLoan(targetSrc, preGenerateVIN, salePrice,
                                      downPayment, loanWeeks)
                                    if not loanSuccess then
                                      exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error',
                                        'The Customers Loan was not successful',
                                        5000, 'car-building')
                                      return
                                    end

                                    local removeSuccess = Dealerships.Stock:Remove(dealerData.id, saleVehicleData.vehicle, 1)
                                    if removeSuccess == false then --? We have to do this because we either return a number or boolean and since lua types are the same, yeah...
                                      exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error',
                                        'Error Removing Vehicle from Stock',
                                        5000, 'car-building')
                                      return
                                    end
                                      ...
                                      Vehicles.Owned:AddToCharacter(targetCharacter:GetData('SID'),
                                        GetHashKey(saleVehicleData.vehicle), 0, {
                                          ...
                                          if success and vehicleData then
                                            ...
                                            exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Success',
                                              'Completed Sales Process - The Customer Received their Vehicle', 7500,
                                              'car-building')
                                            SendCompletedLoanSaleEmail({
                                              ...
                                              loanWeeks,
                                              perWeek, salePriceAfterDown, vehicleData.VIN, vehicleData.RegisteredPlate)

                                            SendDealerProfits(dealerData, dealerRecieves, char:GetData('BankAccount'),
                                              playerCommission, saleVehicleData.data, {
                                                SID = targetCharacter:GetData('SID'),
                                                First = targetCharacter:GetData('First'),
                                                Last = targetCharacter:GetData('Last'),
                                                Source = targetSrc,
                                            ...
                                            exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error',
                                              ...
                                              5000, 'car-building')
                                            exports['hrrp-base']:ExecuteClient(targetSrc, 'Notification', 'Error',
                                              'Error Completing Vehicle Sale', 5000, 'car-building')
                                          end
                                        end, false, dealerData.storage, preGenerateVIN)
                                    end
                                  else
                                    exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Loan Downpayment Failed', 5000,
                                      'car-building')
                                  end
                                end
                              )
