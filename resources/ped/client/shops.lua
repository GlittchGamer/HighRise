...
function CreateSpecificPolyzoneType(type, id, data)
  if data.type == 'poly' then
    exports['polyzonehandler']:PolyZoneCreatePoly(id, data.points, {
      minZ = data.minZ,
      maxZ = data.maxZ,
    }, {
      pedShop = type,
  ...
  elseif data.type == 'box' then
    exports['polyzonehandler']:PolyZoneCreateBox(id, data.center, data.length, data.width, {
...
function CreateShopsBlips()
  for k, v in ipairs(_clothingStores) do
    if v.blip then
      exports['blips']:Add('clothing_store_' .. k, "Clothing Store", v.blip, 73, 44)
    end
  end

  for k, v in ipairs(_barberShops) do
    exports['blips']:Add('barber_shop_' .. k, "Barbers", v.center, 71, 42)
  end

  for k, v in ipairs(_tattooShops) do
    exports['blips']:Add('tattoo_shop_' .. k, "Tattoo Parlor", v.center, 75, 48)
  end

  exports['blips']:Add('plastic_surgery', "Plastic Surgeon", _plasticSurgery.center, 362, 7)
end
...
AddEventHandler('Polyzone:Enter', function(id, point, insideZone, data)
  if data.pedShop then
    ...
    exports['hud']:ActionShow(string.format(action, GetPedShopCost(withinPedShop)))
  end
end)
