...
function FetchEvidence()
  local p = promise.new()
  exports['core']:CallbacksServer('Evidence:Fetch', {}, function(evidence)
    p:resolve(evidence)
  end)
    ...
    if hitting and endCoords then
      ...
      if closest and lastDist <= 1.75 then
        if LocalPlayer.state.onDuty == 'police' then
          exports["animations"]:EmotesPlay('pickup', false, false, true, true)
          TriggerServerEvent('Evidence:Server:PickupEvidence', closest)
          -- else
          --     print('Destroy Evidence')
        end
      else
        exports['hud']:NotificationError('Not Close Enough to Any Evidence')
      end
    end
  end
end
