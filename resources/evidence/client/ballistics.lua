...
AddEventHandler('Polyzone:Enter', function(id, point, insideZone, data)
  if data and data.ballistics and LocalPlayer.state.onDuty == 'police' then
    ...
    exports['hud']:ActionShow('{keybind}primary_action{/keybind} Test & File Gun Ballistics | {key}Use Projectile Evidence{/key} File Projectile & Compare')
  end
end)
...
AddEventHandler('Keybinds:Client:KeyUp:primary_action', function()
  if _withinBallistics and LocalPlayer.state.loggedIn then
    if _holdingGun ~= nil then
      ...
      exports['core']:CallbacksServer('Evidence:Ballistics:FileGun', data, function(success, alreadyFiled, matchingEvidence, policeWeaponId)
        exports["animations"]:EmotesPlay('type3', false, 8000, true, true)
        Progress:Progress({
          ...
          if not status then
            if success then
              if alreadyFiled then
                exports['hud']:NotificationSuccess('Ballistics Filed Successfully - Gun Was Already Filed')
              else
                exports['hud']:NotificationSuccess('Ballistics Filed Successfully - Gun Now Filed')
              end
              ...
              exports['hud']:NotificationError('Ballistics Testing Failed')
            end
          end
        end)
      end)
    else
      exports['hud']:NotificationError('Hold the Gun You Want to Test')
    end
  end
end)
...
RegisterNetEvent('Evidence:Client:FiledProjectile', function(tooDegraded, success, alreadyFiled, filedEvidenceData, matchingWeaponData, evidenceId)
  if tooDegraded then
    return exports['hud']:NotificationError('Projectile too Degraded to Run Ballistics')
  end
  ...
  Progress:Progress({
    ...
    if not status then
      if success then
        if alreadyFiled then
          exports['hud']:NotificationSuccess('Projectile Was Already Filed', 7500)
        else
          exports['hud']:NotificationSuccess('Projectile Filed Successfully', 7500)
        end
