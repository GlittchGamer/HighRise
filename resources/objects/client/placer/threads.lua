...
function PlaceCast()
  ...
  local a, hit, hitcoords, _, material, entity = GetShapeTestResultIncludingMaterial(StartExpensiveSynchronousShapeTestLosProbe(cameraCoord.x, cameraCoord.y,
    cameraCoord.z, destination.x, destination.y, destination.z, -1, LocalPlayer.state.ped, 0))
...
function RunPlacementThread(model, canPlaceInside, showFurnitureButtons, stupidFlag)
  ...
  Citizen.CreateThread(function()
    while _placeData ~= nil and _placing do
      ...
      StopEntityFire(obj)
      isValid = hit and (not IsEntityAVehicle(entity) and not IsEntityAPed(entity)) and #(coords - myPos) <= 5.0 and
      (canPlaceInside or (GetInteriorFromEntity(myPed) == 0 and GetInteriorFromEntity(obj) == 0))
