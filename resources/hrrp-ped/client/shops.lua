...
function CreateSpecificPolyzoneType(type, id, data)
  if data.type == 'poly' then
    exports['hrrp-polyzone']:PolyZoneCreatePoly(id, data.points, {
      minZ = data.minZ,
      maxZ = data.maxZ,
    }, {
      pedShop = type,
  ...
  elseif data.type == 'box' then
    exports['hrrp-polyzone']:PolyZoneCreateBox(id, data.center, data.length, data.width, {
...
function CreateShopsBlips()
  for k, v in ipairs(_clothingStores) do
    if v.blip then
      exports['hrrp-blips']:Add('clothing_store_' .. k, "Clothing Store", v.blip, 73, 44)
    end
  end

  for k, v in ipairs(_barberShops) do
    exports['hrrp-blips']:Add('barber_shop_' .. k, "Barbers", v.center, 71, 42)
  end

  for k, v in ipairs(_tattooShops) do
    exports['hrrp-blips']:Add('tattoo_shop_' .. k, "Tattoo Parlor", v.center, 75, 48)
  end

  exports['hrrp-blips']:Add('plastic_surgery', "Plastic Surgeon", _plasticSurgery.center, 362, 7)
end
...
AddEventHandler('Polyzone:Enter', function(id, point, insideZone, data)
  if data.pedShop then
    ...
    exports['hrrp-hud']:ActionShow(string.format(action, GetPedShopCost(withinPedShop)))
  end
end)
