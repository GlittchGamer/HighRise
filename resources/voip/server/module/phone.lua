_fuckingVOIPPhone = {
	Phone = {
		AddToCall = function(self, source, callChannel)
            ...
            exports['core']:LoggerTrace('VOIP', ('Adding %s to Call %s'):format(source, callChannel))
        ...
        RemoveFromCall = function(self, source, callChannel)
            if not voiceData[source] then return end
            exports['core']:LoggerTrace('VOIP', ('Removing %s from Call %s'):format(source, callChannel))
...
AddEventHandler('Proxy:Shared:ExtendReady', function(component)
    if component == 'VOIP' then
        exports['core']:ExtendComponent(component, _fuckingVOIPPhone)
    end
end)
