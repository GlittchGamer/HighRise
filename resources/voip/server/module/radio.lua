_fuckingVOIPRadio = {
	Radio = {
		AddToChannel = function(self, source, radioChannel)
			...
			exports['core']:LoggerTrace('VOIP', ('Adding %s to Radio Channel %s'):format(source, radioChannel))
		...
		RemoveFromChannel = function(self, source, radioChannel)
			exports['core']:LoggerTrace('VOIP', ('Removing %s from Radio Channel %s'):format(source, radioChannel))
...
AddEventHandler('Proxy:Shared:ExtendReady', function(component)
    if component == 'VOIP' then
        exports['core']:ExtendComponent(component, _fuckingVOIPRadio)
    end
end)
