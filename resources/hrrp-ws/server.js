...
const httpsServer = https.createServer({
  key: fs.readFileSync(path.join(GetResourcePath(GetCurrentResourceName()), 'privkey.pem')),
  cert: fs.readFileSync(path.join(GetResourcePath(GetCurrentResourceName()), 'cert.pem')),
});
...
function generateSocketToken(namespace, data) {
  ...
  if (GlobalState.IsProduction) {
    ...
    const token = jwt.sign(
      ...
      'dev-testing-qi5%z%#dku4$bm**i9ie312312312312312',
    );
...
function verifySocketToken(namespace, token) {
  if (GlobalState.IsProduction) {
    ...
    try {
      const res = jwt.verify(token, 'dev-testing-qi5%z%#dku4$bm**i9ie312312312312312', {});
...
on('Core:Shared:Ready', () => {
  ...
  setTimeout(() => {
    ...
    console.log(`WebSocket Server Listening On: 4002 (${conv.includes('https') ? 'HTTPS' : 'HTTP'}) Sending URL: ${conv}`);
    hasRan = true;
  }, 10000);
});
