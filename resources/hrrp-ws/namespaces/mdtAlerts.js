...
AddEventHandler('Core:Shared:Ready', () => {
  if (hasRan2) return;
  exports['hrrp-base'].RequestDependencies('WebSockets', ['MDT', 'EmergencyAlerts'], (error) => {
...
mdtAlerts.on('connection', (socket) => {
  ...
  if (token) {
    ...
    if (tData && tData.source && tData.job && tData.callsign) {
      if (units[tData.job] && units[tData.job].find((u) => u.source === tData.source)) {
        units[tData.job] = units[tData.job].filter((unit) => unit.source !== tData.source);
      ...
      const playerData = EmergencyAlerts.GetUnitData(EmergencyAlerts, tData.source, tData.job);
      ...
      socket.emit(
        ...
        alerts.filter((alert) => {
          return (
            tData.development ||
            (typeof alert.type === 'string' && playerData.alerts.includes(alert.type)) ||
            (typeof alert.type === 'object' && alert.type.some((t) => playerData.alerts.includes(t)) && (alert.time >= Date.now() - 900000 || alert.attached.length > 0))
          );
        }),
        units,
        newUnit,
      ...
      socket.on('disconnect', () => {
        units[socket.data.job] = units[socket.data.job].filter((unit) => unit.source !== socket.data.source);
      ...
      socket.on('changeUnit', (job, primary, type) => {
        try {
          ...
          if (unitIndex > -1) {
            ...
            addDispatchLog('unitChange', null, `${primary} Transitioning to ${typeNames[type]} Unit`, logColors[job]);
          }
        } catch (e) {}
      });
        ...
        try {
          ...
          if (unitIndex > -1) {
            ...
            if (units[job][unitIndex].available) {
              addDispatchLog('availabilityChange', null, `${primary} is now 10-8 (Available)`, logColors[job]);
            } else {
              addDispatchLog('availabilityChange', null, `${primary} is now 10-6 (Unavailable)`, logColors[job]);
            }
          }
        } catch (e) {}
      });
      ...
      socket.on('operateUnderUnit', (job, primary, unit) => {
        try {
          ...
          if (unitIndex > -1) {
            ...
            addDispatchLog('subUnitChanges', null, `${unit} Now Operating Under ${primary}`, logColors[job]);
          }
        } catch (e) {}
      });
      ...
      socket.on('breakOffUnit', (job, primary, unit) => {
        try {
          ...
          if (unitIndex > -1) {
            ...
            addDispatchLog('subUnitChanges', null, `${unit} Breaking Off From ${primary}`, logColors[job]);
          }
        } catch (e) {}
      });
      ...
      socket.on('changeRadioChannel', (channel) => {
        try {
          const unitIndex = units[socket.data.job].findIndex((u) => u.primary === socket.data.callsign);
          if (unitIndex > -1) {
      ...
      socket.on('updateAlertUnits', (alertId, units) => {
        try {
          ...
          if (alertIndex > -1 && units.length >= 0) {
            ...
            mdtAlerts.emit('alertUpdateUnits', alertId, alerts[alertIndex].attached);

            alerts[alertIndex].attached.forEach((att) => {
              if (!alertShit[alertId][att]) {
                alertShit[alertId][att] = true;
                addDispatchLog('attaching', null, `${att} Attached to ${alerts[alertIndex].code} | ${alerts[alertIndex].title}`, '#5D348B');
              }
            });

            Object.keys(alertShit[alertId]).forEach((att) => {
              if (alertShit[alertId][att] && !alerts[alertIndex].attached.includes(att)) {
                alertShit[alertId][att] = false;
                addDispatchLog('attaching', null, `${att} Detached From ${alerts[alertIndex].code} | ${alerts[alertIndex].title}`, '#5D348B');
              }
            });
          }
        } catch (e) {}
      ...
      socket.on('addRadioInfo', (data) => {
        ...
        addDispatchLog('radioUpdate', null, `Radio Added: ${data.radio} | ${data.text}`, '#6e6e6e');
      });
      ...
      socket.on('removeRadioInfo', (id, data) => {
        ...
        addDispatchLog('radioUpdate', null, `Radio Removed: ${data.radio}`, '#6e6e6e');
      });

      socket.on('logMessage', (message) => {
        addDispatchLog('message', `${jobNames[socket.data.job]} Message | [${socket.data.callsign}] ${socket.data.name}`, message, logColors[socket.data.job]);
      });
