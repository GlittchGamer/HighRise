...
AddEventHandler("Core:Server:ForceSave", function()
  ...
  if #docs > 0 then
    exports['hrrp-base']:LoggerInfo("Weed", string.format("Saving ^2%s^7 Plants", #docs))
...
function RegisterTasks()
  ...
  Citizen.CreateThread(function()
    while true do
      Citizen.Wait((1000 * 60) * 10)
      exports['hrrp-base']:LoggerTrace("Weed", "Growing Plants")
      local updatingStuff = {}

      for k, v in pairs(_plants) do
        if (os.time() - v.plant.planted) >= Config.Lifetime then
          exports['hrrp-base']:LoggerTrace("Weed", "Deleting Weed Plant Because Some Dumb Cunt Didn't Harvest It")
  ...
  Citizen.CreateThread(function()
    while true do
      ...
      exports['hrrp-base']:LoggerTrace("Weed", "Increasing Plant Outputs")
      for k, v in pairs(_plants) do
        if v.plant.growth < 100 then
          local mat = Materials[v.plant.material]
          if mat ~= nil then
  ...
  Citizen.CreateThread(function()
    while true do
      Citizen.Wait((1000 * 60) * 10)
      exports['hrrp-base']:LoggerTrace("Weed", "Degrading Water")
      for k, v in pairs(_plants) do
        if v.plant.water > -25 then
          ...
          exports['hrrp-base']:LoggerTrace("Weed", "Deleting Weed Plant Because Some Dumb Cunt Didn't Water It")
          Weed.Planting:Delete(k)
        end
      end
    end
  ...
  Citizen.CreateThread(function()
    while true do
      ...
      exports['hrrp-base']:LoggerTrace("Weed", "Ticking Down Fertilizer")
      for k, v in pairs(_plants) do
