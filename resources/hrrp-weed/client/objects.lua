...
RegisterNetEvent('Weed:Client:Objects:Init', function(plants)
  if plants and type(plants) == 'table' then
    for k, v in pairs(plants) do
      v.plant.location = v.plant.location
      _activePlants[k] = v
    end
    ...
    exports['hrrp-base']:LoggerError('Weed', 'Failed to Load Weed Objects')
  end
end)
...
function CreateWeedPlant(id, data)
  ...
  local obj = CreateObject(Plants[stage].model, data.plant.location.x + 0.0, data.plant.location.y + 0.0, data.plant.location.z + Plants[stage].offset, false,
    true)
