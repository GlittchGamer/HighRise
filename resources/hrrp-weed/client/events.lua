...
RegisterNetEvent("Weed:Client:Login", function(l)
  exports['hrrp-pedinteraction']:Add("weed-dealer", `s_m_y_dealer_01`, vector3(l.coords.x, l.coords.y, l.coords.z), l.heading, 50.0, {
    {
      icon = "cannabis",
      text = "Buy Package",
...
AddEventHandler("Weed:Client:Check", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Weed:CheckPlant", GetWeedPlant(entity.entity), function(data)
    if data ~= nil then
  ...
  if Inventory.Items:Has(string.format("fertilizer_%s", data.type), 1) then
    ...
    Progress:Progress({
      ...
      if not cancelled then
        exports["hrrp-base"]:CallbacksServer("Weed:FertilizePlant", data, function(status) end)
      end
    end)
  else
    exports['hrrp-hud']:NotificationError("You Don't Have Fertilizer")
  end
end)
...
AddEventHandler("Weed:Client:Water", function(pId)
  if Inventory.Items:Has("water", 1) then
    ...
    Progress:Progress({
      ...
      if not cancelled then
        exports["hrrp-base"]:CallbacksServer("Weed:WaterPlant", pId, function(status) end)
      end
    end)
  end
end)
...
AddEventHandler("Weed:Client:Harvest", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Weed:HarvestPlant", GetWeedPlant(entity.entity), function(status) end)
    end
  end)
end)
...
AddEventHandler("Weed:Client:Harvest2", function(nId)
  ...
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Weed:HarvestPlant", nId, function(status) end)
    end
  end)
end)

AddEventHandler("Weed:Client:Package", function()
  exports["hrrp-base"]:CallbacksServer("Weed:BuyPackage", {}, function(status) end)
end)

-- AddEventHandler("Weed:Client:Brick", function()
-- 	exports["hrrp-base"]:CallbacksServer("Weed:SellBrick", {}, function(status) end)
-- end)
...
AddEventHandler("Weed:Client:Destroy", function(nId)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Weed:DestroyPlant", nId, function(status) end)
    end
  end)
end)
...
AddEventHandler("Weed:Client:PDDestroy", function(entity)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Weed:PDDestroyPlant", GetWeedPlant(entity.entity), function(status) end)
    end
  end)
end)
