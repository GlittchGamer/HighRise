...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("minigame", {
    "Hud",
    "Callbacks",
    "Action",
    "Progress",
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Minigame", MINIGAME)
end)
...
function _playGame(params, action)
  ...
  if not IsEntityDead(player) or action.useWhileDead then
    if not isPlayingGame then
      ...
      exports['hrrp-hud']:NotificationError("Already Doing An Action", 5000)
    end
  else
    exports['hrrp-hud']:NotificationError("Already Doing An Action", 5000)
  end
        ...
        if not isAnim then
          if action.animation then
            ...
            elseif action.animation.anim ~= nil then
              exports["hrrp-animations"]:EmotesPlay(action.animation.anim, false, action.duration or -1, true)
            else
              if GetVehiclePedIsIn(LocalPlayer.state.ped) == 0 then
