...
RegisterNUICallback('GetData', function(data, cb)
  ...
  exports['core']:CallbacksServer('Characters:GetServerData', {}, function(serverData)
    SendNUIMessage({
      type = 'LOADING_SHOW',
      data = { message = 'Getting Character Data' }
    })

    exports['core']:CallbacksServer('Characters:GetCharacters', {}, function(characters)
...
RegisterNUICallback('CreateCharacter', function(data, cb)
  cb("ok")
  exports['core']:CallbacksServer('Characters:CreateCharacter', data, function(character)
...
RegisterNUICallback('DeleteCharacter', function(data, cb)
  cb("ok")
  exports['core']:CallbacksServer('Characters:DeleteCharacter', data.id, function(status)
    if status then
      SendNUIMessage({
...
RegisterNUICallback('SelectCharacter', function(data, cb)
  cb("ok")
  exports['core']:CallbacksServer('Characters:GetSpawnPoints', data.id, function(spawns)
    if spawns then
      SendNUIMessage({
        type = 'SET_SPAWNS',
        data = { spawns = spawns }
...
RegisterNUICallback('PlayCharacter', function(data, cb)
  cb("ok")
  exports['core']:CallbacksServer('Characters:GetCharacterData', data.character.ID, function(cData)
    cData.spawn = data.spawn
    deletePeds()
    TriggerEvent('Characters:Client:SetData', -1, cData, function()
      local pedSpawn = exports['ped']:SpawnToWorld(cData)
      exports['characters']:SpawnToWorld(cData, function()
        LocalPlayer.state.canUsePhone = true
        if data.spawn.event ~= nil then
          exports['core']:CallbacksServer(data.spawn.event, data.spawn, function()
