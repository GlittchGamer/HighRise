RegisterNetEvent('FleetDealers:Server:Purchase', function(shop, vehicle, livery)
  ...
  if char and shopData and exports['jobs']:JobsPermissionsHasPermissionInJob(src, shopData.job, shopData.requiredPermission) then
          ...
          Vehicles.Owned:AddToFleet(
            ...
            function(success, vehicle)
              if success and vehicle then
                exports['core']:ExecuteClient(src, 'Notification', 'Success',
                ...
                exports['core']:LoggerError('Dealerships',
                ...
                exports['core']:ExecuteClient(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed', 5000, 'cars')
              end
            end,
            properties
          )
        else
          exports['core']:ExecuteClient(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed - Not Enough Money in the Bank', 5000, 'cars')
        end
      else
        exports['core']:ExecuteClient(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed', 5000, 'cars')
      end
    else
      exports['core']:ExecuteClient(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed - Invalid Vehicle', 5000, 'cars')
    end
  else
    exports['core']:ExecuteClient(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed', 5000, 'cars')
  end
