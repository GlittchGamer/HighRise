function CreateRentalSpots()
  for k, v in ipairs(_vehicleRentals) do
    if v.interactionPed then
      exports['pedinteraction']:Add('veh_rental_' .. k, v.interactionPed.model, v.coords, v.interactionPed.heading, v.interactionPed.range, {
        {
          icon = 'car-side',
...
function CreateRentalSpotsBlips()
  for k, v in ipairs(_vehicleRentals) do
    exports['blips']:Add('veh_rental_' .. k, v.name, v.coords, v.blip.sprite, v.blip.color, v.blip.scale)
  end
end
...
AddEventHandler('VehicleRentals:Client:ConfirmRental', function(data)
  ...
  if availableSpace then
    Callbacks:ServerCallback('Rentals:Purchase', {
      ...
      if success then
        exports['hud']:NotificationSuccess('Rental Purchased, It is Parked Nearby. Rental Vehicle Plate: ' .. plate)
      else
        exports['hud']:NotificationError('Rental Purchase Failed')
      end
    end)
  end
end)
  ...
  Callbacks:ServerCallback('Rentals:GetPending', { rental = data.rental }, function(pending)
    if pending then
      ...
      if #menu.main.items > 0 then
        ...
        exports['hud']:NotificationError('You Have no Vehicle Rentals to Return')
      end
    else
      exports['hud']:NotificationError('You Have no Vehicle Rentals to Return')
    end
...
AddEventHandler('VehicleRentals:Client:ConfirmReturnRental', function(data)
  if data and data.vehicle and data.VIN and IsAbleToReturnVehicle(data.vehicle, data.rental) then
    Callbacks:ServerCallback('Rentals:Return', {
      ...
      if success then
        exports['hud']:NotificationSuccess('Rental Returned & Deposit Returned')
      end
    end)
  end
end)
...
function IsAbleToReturnVehicle(vehicle, rentalSpot)
  if DoesEntityExist(vehicle) then
    ...
    local withinReturnZone = exports['polyzonehandler']:PolyZoneIsCoordsInZone(vehicleCoords, false, 'vehicleRental', rentalSpot)
