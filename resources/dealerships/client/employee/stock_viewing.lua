...
function ViewDealershipStocks(dealerId)
  ...
  if dealerData then
    Callbacks:ServerCallback('Dealerships:StockViewing:FetchData', dealerId, function(authed, stocks, serverTime, dealerProfitPercent)
      if not authed then
        exports['hud']:NotificationError('You\'re Not Authorized to View Stock', 3500, 'car-building')
        return
      end
      ...
      stockViewing = exports['menu']:Create('stockViewing', string.format('View %s Stock', dealerData.abbreviation), function()
      ...
      for _, category in ipairs(orderedCategories) do
        if stockData.sorted[category] and #stockData.sorted[category] > 0 then
          stockViewingSub[category] = exports['menu']:Create('stockViewingCat-' .. category, _catalogCategories[category])
