...
function OpenDealershipSales(dealerId)
  ...
  if dealerData then
    Callbacks:ServerCallback('Dealerships:Sales:FetchData', dealerId, function(authed, stocks, serverTime, defaultInterestRate, dealerMData)
      if not authed then
        exports['hud']:NotificationError('You\'re Not Authorized to Make Sales', 3500, 'car-building')
        return
      end
      ...
      salesMenu = exports['menu']:Create('salesMenu', string.format('View %s Stock', _dealerships[dealerId].abbreviation), function()
      ...
      for _, category in ipairs(orderedCategories) do
        if stockData.sorted[category] and #stockData.sorted[category] > 0 then
          salesMenuSub[category] = exports['menu']:Create('salesMenuCat-' .. category, _catalogCategories[category])
          ...
          for k, v in ipairs(stockData.sorted[category]) do
            if v.quantity > 0 then
              ...
              salesMenuSub[vehMenuIdentifier] = exports['menu']:Create('salesMenu-' .. vehMenuIdentifier, vehName)
              ...
              -- Cash Sales Menu
              salesMenuSub[cashSaleIdentifier] = exports['menu']:Create('salesMenu-' .. cashSaleIdentifier, vehName .. ' - New Sale')
              ...
              -- Loan Sales Menu
              salesMenuSub[loanSaleIdentifier] = exports['menu']:Create('salesMenu-' .. loanSaleIdentifier, vehName .. ' - New Sale')
