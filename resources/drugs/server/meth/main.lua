...
AddEventHandler("Drugs:Server:Startup", function()
  ...
  exports['core']:LoggerTrace("Drugs:Meth", string.format("Restored ^2%s^7 Meth Tables", #tables))
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:FinishTablePlacement", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:PickupTable", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:CheckTable", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:StartCooking", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        if data and _placedTables[data.tableId] ~= nil then
          if _placedTables[data.tableId].cooldown == nil or os.time() > _placedTables[data.tableId].cooldown then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Success", string.format("Batch Started, Will Be Ready In %s Minutes", data.cookTime))
            cb(true)
          else
            cb(false)
          end
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:PickupCook", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:GetTableDetails", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:GetItems", function(source, data, cb)
    local itms = {}
  ...
  exports['core']:CallbacksRegisterServer("Drugs:Meth:BuyItem", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local hasVpn = hasValue(char:GetData("States"), "PHONE_VPN")
    ...
    for k, v in ipairs(_toolsForSale) do
      if v.id == data then
        ...
        if Crypto.Exchange:Remove(v.coin, char:GetData("CryptoWallet"), v.price) then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", string.format("Not Enough %s", coinData.Name), 6000)
        end
      end
    end
  end)
