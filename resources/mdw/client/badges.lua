...
function RegisterBadgeCallbacks()
  exports['core']:CallbacksRegisterClient("MDT:Client:CanShowBadge", function(data, cb)
    if
        not inBadgeAnim
        and not _mdtOpen
        and not LocalPlayer.state.doingAction
        and not LocalPlayer.state.isDead
        and not exports["animations"]:EmotesGet()
  ...
  exports['core']:CallbacksRegisterClient("MDT:Client:CanShowLicense", function(data, cb)
    if
        not inBadgeAnim
        and not _mdtOpen
        and not LocalPlayer.state.doingAction
