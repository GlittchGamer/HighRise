...
RegisterNetEvent("Job:Client:DutyChanged", function(state)
  if state and _trackedJobs[state] then
    Citizen.CreateThread(function()
      ...
      exports['core']:LoggerTrace("Tracking", "Start Emergency Tracking")
      while
      ...
      exports['core']:LoggerTrace("Tracking", "Clear Emergency Tracking")
...
AddEventHandler("MDT:Client:DisableTracker", function(entity, data)
  ...
  then
    Progress:ProgressWithTickEvent({
      ...
      if not cancelled then
        if
            ...
            and not playerState.trackerDisabled
        then
          exports['core']:CallbacksServer("EmergencyAlerts:DisablePDTracker", entity.serverId, function(success)
            if success then
              exports['hud']:NotificationSuccess("Disabled Their Tracker")
            end
          end)
        end
      end
    ...
    exports['hud']:NotificationError("Unable to Disable Tracker")
  end
end)
