AddEventHandler("MDT:Server:RegisterCallbacks", function()
  exports['core']:CallbacksRegisterServer("MDT:Hire", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("MDT:Fire", function(source, data, cb)
    ...
    if char and data and data.SID and (hasPerms or isSystemAdmin) then
      ...
      if charData then
        ...
        if isSystemAdmin then
          ...
          local plyrJob = exports['jobs']:JobsPermissionsHasJob(source, loggedInJob)
  ...
  exports['core']:CallbacksRegisterServer("MDT:ManageEmployment", function(source, data, cb)
    local player = exports['core']:FetchSource(source)
    local char = player:GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("MDT:Update:jobPermissions", function(source, data, cb)
    local player = exports['core']:FetchSource(source)
    local char = player:GetData("Character")
    local isSystemAdmin = char:GetData('MDTSystemAdmin')
    local hasPerms, loggedInJob = CheckMDTPermissions(source, {
  ...
  exports['core']:CallbacksRegisterServer("MDT:Suspend", function(source, data, cb)
    local player = exports['core']:FetchSource(source)
    local char = player:GetData("Character")
      ...
      if charData then
        ...
        if canRemove and data.Length and type(data.Length) == "number" and data.Length > 0 and data.Length < 99 then
          ...
          if updateUserDB then
            local char = exports['core']:FetchSID(data.SID)
            if char then
              ...
              exports['jobs']:JobsDutyOff(char:GetData("Source"), data.JobId)
            end
          end
        else
          cb(false)
  ...
  exports['core']:CallbacksRegisterServer("MDT:Unsuspend", function(source, data, cb)
    local player = exports['core']:FetchSource(source)
    local char = player:GetData("Character")
