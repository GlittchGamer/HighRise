...
function OpenVehicleCustoms(canInstallPerformance, costMultiplier, settings)
  ...
  customsMenu = exports['hrrp-menu']:Create('vehicle_customs', 'Vehicle Customs', function()
    CUSTOMS_OPEN = true
  end, function()
    Citizen.Wait(100)
    customsMenu = false
    ...
    if not isSaving then
      exports['hrrp-base']:LoggerTrace('VehicleCustoms', 'Not Saving - Reset Mods')
      Vehicles.Properties:Set(CUST_VEH, originalData)
      exports['hrrp-hud']:NotificationError('Changes Discarded - You Weren\'t Charged')
    end
    CUSTOMS_OPEN = false
  end, true)
  ...
  exports['hrrp-hud']:NotificationPersistentStandard('veh_customs', 'Current Total: $' .. 0)
  ...
  local function updateVehiclePropertyState()
    ...
    exports['hrrp-hud']:NotificationPersistentStandard('veh_customs', 'Current Total: $' .. newCost)
    customsMenu.Update:Item(costElementId, 'Current Total: $' .. newCost, {}, true)
  end

  -- PAINT STUFF

  customsMenuSubs['paint'] = exports['hrrp-menu']:Create('vehicle_customs_paint', 'Paint')

  if not settings.GOVERNMENT_PAINT then
    customsMenuSubs['paint'].Add:Text('Primary Paint Color', { 'heading' })
    customsMenuSubs['paint'].Add:Text('Changing Either Primary Paint Option Costs $' .. math.ceil(_customsConfig.paintCost * costMultiplier),
    ...
    customsMenuSubs['paint'].Add:Text('Changing Either Secondary Paint Option Costs $' .. math.ceil(_customsConfig.paintCost * costMultiplier),
      { 'pad', 'code', 'center', 'textLarge' })
  ...
  customsMenuSubs['livery'] = exports['hrrp-menu']:Create('vehicle_customs_livery', 'Liveries')
  ...
  -- EXTRAS
  customsMenuSubs['extras'] = exports['hrrp-menu']:Create('vehicle_customs_extras', 'Extras')
  local hasExtras = false

  local extraItemIds = {}

  customsMenuSubs['extras'].Add:Text('All Changes to Extras Cost $' .. math.ceil(_customsConfig.cost.extras * costMultiplier),
    { 'pad', 'code', 'center', 'textLarge' })
  ...
  -- Lighting
  customsMenuSubs['lighting'] = exports['hrrp-menu']:Create('vehicle_customs_lighting', 'Lighting')
  ...
  customsMenuSubs['wheels'] = exports['hrrp-menu']:Create('vehicle_customs_wheels', 'Wheels')
  ...
  customsMenuSubs['wheels'].Add:Ticker(
    'Wheels - $' .. math.ceil(_customsConfig.cost.mods.frontWheels and (_customsConfig.cost.mods.frontWheels * costMultiplier) or 0), {
      disabled = false,
  ...
  if VEHICLE_CLASS == 8 then
    customsMenuSubs['wheels'].Add:Ticker(
      'Rear Wheels - $' .. math.ceil(_customsConfig.cost.mods.backWheels and (_customsConfig.cost.mods.backWheels * costMultiplier) or 0), {
        disabled = false,
  ...
  if not settings.BLOCK_TIRESMOKE then
    ...
    customsMenuSubs['wheels'].Add:Text('Tire Smoke Color - $' .. math.ceil(_customsConfig.cost.tyreSmokeColor * costMultiplier),
      { 'pad', 'code', 'center', 'textLarge' })
  ...
  customsMenuSubs['body_parts'] = exports['hrrp-menu']:Create('vehicle_customs_body_parts', 'Body Parts')

  for k, v in ipairs(_customsConfig.bodyParts) do
    if v.mod == 'horns' then
      customsMenuSubs['body_parts'].Add:Select(
        v.name .. ' - $' .. math.ceil(_customsConfig.cost.mods[v.mod] and (_customsConfig.cost.mods[v.mod] * costMultiplier) or 0), {
          disabled = false,
  ...
  customsMenuSubs['body_parts'].Add:Select(
    'Window Tints - $' .. math.ceil(_customsConfig.cost.windowTint and (_customsConfig.cost.windowTint * costMultiplier) or 0), {
      disabled = false,
  ...
  customsMenuSubs['body_parts'].Add:Select(
    'Plate Type - $' .. math.ceil(_customsConfig.cost.plateIndex and (_customsConfig.cost.plateIndex * costMultiplier) or 0), {
  ...
  if canInstallPerformance then
    customsMenuSubs['performance'] = exports['hrrp-menu']:Create('vehicle_customs_performance', 'Performance')
  ...
  customsMenu.Add:Button('Pay and Save Changes', { success = true }, function()
    ...
    if LocalPlayer.state.Character:GetData('Cash') >= currentCost then
      isSaving = true
      exports['hrrp-base']:LoggerTrace('VehicleCustoms', 'Attept Mods Save')
      customsMenu:Close()

      exports["hrrp-base"]:CallbacksServer('Vehicles:CompleteCustoms', {
        ...
        if success then
          ...
          exports['hrrp-hud']:NotificationSuccess('New Modifications Saved & Paid For')
          if newNewData then
            Vehicles.Properties:Set(CUST_VEH, newNewData)
          end
        else
          exports['hrrp-hud']:NotificationError('Error Saving Vehicle Modifications - Not Enough Money?')
          Vehicles.Properties:Set(CUST_VEH, originalData)
        end
      end)
    else
      exports['hrrp-hud']:NotificationError('Not Enough Cash')
    end
  end)
