function RegisterChatCommands()
  Chat:RegisterStaffCommand("heal", function(source, args, rawCommand)
    if args[1] ~= nil then
      local admin = exports['hrrp-base']:FetchSource(source)
      ...
      if player ~= nil then
        ...
        if char ~= nil and ((char:GetData("Source") ~= admin:GetData("Source")) or admin.Permissions:IsAdmin()) then
          exports["hrrp-base"]:CallbacksClient(char:GetData("Source"), "Damage:Heal", true)
      ...
      if player ~= nil then
        local char = player:GetData("Character")
        if char ~= nil then
          exports["hrrp-base"]:CallbacksClient(source, "Damage:Heal", true)
          Status:Set(source, "PLAYER_STRESS", 0)
        end
      end
    end
      ...
      if v:GetData("Character") ~= nil then
        ...
        if Player(src).state.isDead then
          ...
          if #(GetEntityCoords(ped) - GetEntityCoords(myPed)) <= radius then
            exports["hrrp-base"]:CallbacksClient(src, "Damage:Heal", true)
          end
        end
      end
    end
  ...
  Chat:RegisterAdminCommand("god", function(source, args, rawCommand)
    if Player(source).state.isGodmode then
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Info", "God Mode Disabled")
      exports["hrrp-base"]:CallbacksClient(source, "Damage:Admin:Godmode", false)
    else
      SetPlayerInvincible(source, true)
      Player(source).state.isGodmode = true
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Info", "God Mode Enabled")
      exports["hrrp-base"]:CallbacksClient(source, "Damage:Admin:Godmode", true)
    end
  end, {
  ...
  Chat:RegisterAdminCommand("die", function(source, args, rawCommand)
    exports["hrrp-base"]:CallbacksClient(source, "Damage:Kill")
  end, {
    help = "Kill Yourself",
  })
end
