...
function Init()
  exports['hrrp-pedinteraction']:Add("hospital-check-in", `u_f_m_miranda_02`, vector3(-437.484, -323.269, 33.911), 162.630, 25.0, hospitalCheckin,
    "notes-medical",
    "WORLD_HUMAN_CLIPBOARD")

  for k, v in ipairs(Config.BedModels) do
    exports['ox_target']:TargetingAddObject(v, "stretcher", {
      {
        icon = "stretcher",
        text = "Lay in Bed",
  ...
  for k, v in ipairs(Config.BedPolys) do
    exports['ox_target']:TargetingZonesAddBox(string.format("hospitalbed-%s", k),
      "stretcher",
      v.center,
      v.length,
      v.width,
...
AddEventHandler("Hospital:Client:LaydownAnimation", function(hitting, coords)
  ...
  exports["hrrp-animations"]:EmotesPlay("passout3", true)
  LocalPlayer.state:set("isHospitalized", true, true)

  while exports["hrrp-animations"]:EmotesGet() == "passout3" do
    Citizen.Wait(250)
  end
...
AddEventHandler("Hospital:Client:RetreiveItems", function()
  exports["hrrp-base"]:CallbacksServer("Hospital:RetreiveItems")
end)
...
AddEventHandler("Hospital:Client:HiddenRevive", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports["hrrp-base"]:CallbacksServer("Hospital:HiddenRevive", {}, function(s)
        if s then
          Escort:StopEscort()
        end
      end)
...
RegisterNetEvent("Hospital:Client:GetOut", function()
  exports["hrrp-base"]:CallbacksServer('Hospital:LeaveBed', {}, function()
    _doing = false
    LeaveBed()
  end)
end)
