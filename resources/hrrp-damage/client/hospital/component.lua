...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Hospital", {
    ...
    exports['hrrp-pedinteraction']:Add("HiddenHospital", `s_m_m_doctor_01`, GlobalState["HiddenHospital"].coords, GlobalState["HiddenHospital"].heading, 25.0, {
      {
        icon = "heart-pulse",
    ...
    exports['hrrp-polyzone']:PolyZoneCreateBox('hospital-check-in-zone', vector3(-436.09, -326.23, 34.91), 2.0, 3.0, {
      heading = 338,
      --debugPoly=true,
      minZ = 33.91,
      maxZ = 36.31
...
HOSPITAL = {
  CheckIn = function(self)
    exports["hrrp-base"]:CallbacksServer('Hospital:Treat', {}, function(bed)
      if bed ~= nil then
        _countdown = Config.HealTimer
        LocalPlayer.state:set("isHospitalized", true, true)
        self:SendToBed(Config.Beds[bed], false, bed)
      else
        exports['hrrp-hud']:NotificationError('No Beds Available')
      end
    end)
  end,
  SendToBed = function(self, bed, isRp, bedId)
    ...
    if bedId then
      local p = promise.new()
      exports["hrrp-base"]:CallbacksServer('Hospital:OccupyBed', bedId, function(s)
        p:resolve(s)
      end)
    ...
    if bed ~= nil and fuck then
      ...
      exports['hrrp-hud']:NotificationError('Invalid Bed or Bed Occupied')
    end
  end,
  FindBed = function(self, object)
    local coords = GetEntityCoords(object)
    exports["hrrp-base"]:CallbacksServer('Hospital:FindBed', coords, function(bed)
      if bed ~= nil then
  ...
  LeaveBed = function(self)
    exports["hrrp-base"]:CallbacksServer('Hospital:LeaveBed', _bedId, function()
      _bedId = nil
    end)
  end,
}
  ...
  if id == 'hospital-check-in-zone' then
    ...
    if not LocalPlayer.state.isEscorted and (GlobalState["ems:pmc:doctor"] == nil or GlobalState["ems:pmc:doctor"] == 0) then
      if not GlobalState["Duty:ems"] or GlobalState["Duty:ems"] == 0 then
        exports['hrrp-hud']:ActionShow('{keybind}primary_action{/keybind} Check In {key}$150{/key}')
      else
        exports['hrrp-hud']:ActionShow('{keybind}primary_action{/keybind} Check In {key}$5000{/key}')
      end
    end
  end
end)
