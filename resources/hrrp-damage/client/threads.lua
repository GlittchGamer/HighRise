  ...
  if (LocalPlayer.state.isDead and not LocalPlayer.state.isHospitalized) and GetCloudTimeAsInt() > LocalPlayer.state.releaseTime and not _respawning then
    ...
    if not LocalPlayer.state.deadData?.isMinor then
      Progress:Progress({
        ...
        if not status then
          exports["hrrp-base"]:CallbacksServer("Hospital:Respawn", {}, function(bedId)
            if bedId ~= nil then
              _sendToHosp = bedId
              ...
              exports['hrrp-hud']:NotificationError("Unable To Respawn Yet, Please Wait")
            end
          end)
        else
