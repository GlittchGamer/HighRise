...
AddEventHandler('Businesses:Client:Startup', function()
  ...
  exports['keybinds']:Add('bowling_aim_right', 'RIGHT', 'keyboard', 'Bowling - Aim Right', function()
  ...
  exports['keybinds']:Add('bowling_aim_left', 'LEFT', 'keyboard', 'Bowling - Aim Left', function()
    if awaitingBowl then
      local heading = GetEntityHeading(LocalPlayer.state.ped) + 0.7
  ...
  exports['core']:CallbacksRegisterClient('Bowling:DoBowl', function(data, cb)
...
AddEventHandler('Bowling:Client:StartGame', function(entity, alleyId)
  ...
  if nickName and string.len(nickName) >= 3 then
    exports['core']:CallbacksServer('Bowling:StartGame', {
      alleyId = alleyId,
      nickName = nickName,
    }, function(success)
      if success then
        ...
        exports['hud']:NotificationError('Couldn\'t Start Game', 5000, 'bowling-ball-pin')
      end
    end)
  else
    exports['hud']:NotificationError('Please provide a name longer than 2 letters', 5000, 'bowling-ball-pin')
  end
end)
...
AddEventHandler('Bowling:Client:JoinGame', function(entity, alleyId)
  local nickName = GetBowlingNickName()
  if nickName and string.len(nickName) >= 3 then
    exports['core']:CallbacksServer('Bowling:JoinGame', {
      alleyId = alleyId,
      nickName = nickName,
    }, function(success)
      if success then
        ...
        exports['hud']:NotificationError('Couldn\'t Join Game', 5000, 'bowling-ball-pin')
      end
    end)
  else
    exports['hud']:NotificationError('Please provide a name longer than 2 letters', 5000, 'bowling-ball-pin')
...
AddEventHandler('Polyzone:Enter', function(id, testedPoint, insideZones, data)
  if data?.bowling_start then
    ...
    exports['hud']:ActionShow('{keybind}primary_action{/keybind} Start Bowling')
...
AddEventHandler('Keybinds:Client:KeyUp:primary_action', function()
  if insideBowlingStart and not awaitingBowl then
    ...
    if alley and not alley.finished and alley.currentPlayer == LocalPlayer.state.Character:GetData('SID') then
      ...
      exports['hud']:NotificationError('Not Your Turn', 5000, 'bowling-ball-pin')
    end
  end
end)

AddEventHandler('Bowling:Client:EndGame', function(entity, alleyId)
  exports['core']:CallbacksServer('Bowling:EndGame', {
    alleyId = alleyId,
  }, function(success)
    if success then
      SendBowlingNotification('Ended a Game of Bowling')
    else
      exports['hud']:NotificationError('Couldn\'t End Game', 5000, 'bowling-ball-pin')
    end
  end)
end)

AddEventHandler('Bowling:Client:ResetAll', function()
  exports['core']:CallbacksServer('Bowling:ResetAll', {}, function(success)
    if success then
      SendBowlingNotification('Reset All')
    else
      exports['hud']:NotificationError('Couldn\'t Reset', 5000, 'bowling-ball-pin')
    end
  end)
end)

AddEventHandler('Bowling:Client:ClearPins', function()
  exports['core']:CallbacksServer('Bowling:ClearPins', {}, function(success)
    if success then
      SendBowlingNotification('Cleared Pins')
    else
      exports['hud']:NotificationError('Couldn\'t Clear Pins', 5000, 'bowling-ball-pin')
    end
  end)
end)
