...
local url = 'nui://businesses/dui/bowling/index.html'
local scale = 0.08
...
AddEventHandler('Bowling:Client:SetTV', function()
  ...
  exports['core']:CallbacksServer('Bowling:UpdateTVLink', tvLink, function(success)
    if success then
      SendBowlingNotification('Updated Link!')
    else
      exports['hud']:NotificationError('Error', 5000, 'bowling-ball-pin')
    end
  end)
end)
