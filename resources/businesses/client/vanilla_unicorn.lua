...
AddEventHandler('Businesses:Client:Startup', function()
  exports['polyzonehandler']:PolyZoneCreatePoly('vu_dancers', {
  ...
  exports['polyzonehandler']:PolyZoneCreatePoly('vu_makeitrain', {
  ...
  Interaction:RegisterMenu("vu_stripper_pole", "Vanilla Unicorn Dancers", "party-horn", function()
    if exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn' and exports['jobs']:JobsPermissionsHasPermissionInJob('unicorn', 'STRIP_POLE') then
      local subMenu = {}
      ...
      exports['hud']:NotificationError('Invalid Permissions')
    end
  end, function()
    return exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn'
  end)

  Interaction:RegisterMenu("vu_makeitrain", 'Make It Rain', "money-bill-1-wave", function()
    if not _makingItRain and exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') then
      local makeItRain = {
        {
          type = 'cash',
          text = '$100 Cash',
    ...
    return exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') and GetNearbyFuckingStripper()
  end)
end)
    ...
    if #(v - pedCoords) <= 1.5 then
      ...
      if dist == -1 or dist > 1.5 then
        ...
        if poleDance then
          ...
          exports["animations"]:EmotesPlay(poleDance.anim, false, false, false)
        end
      else
        exports['hud']:NotificationError('Pole Taken')
      end
      return
    end
  end
...
function MakeItRainBitch(targetSource, cashType, time)
  ...
  Citizen.CreateThread(function()
    _makingItRain = true
    exports["animations"]:EmotesPlay('makeitrain', false, false, false)
    ...
    while
      ...
      and exports["animations"]:EmotesGet() == 'makeitrain'
    ...
    do
      local p = promise.new()
      exports['core']:CallbacksServer('VU:MakeItRain', {
        target = targetSource,
        type = cashType,
      }, function(success, cd)
        if not success then
          exports['hud']:NotificationError(cd and 'Reached Cooldown' or 'Error - Ran Out of Money')
          _makingItRain = false
        end
