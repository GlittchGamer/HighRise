AddEventHandler("Businesses:Client:Startup", function()
      ...
      },
      {
        ...
        action = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            exports['core']:CallbacksServer("StorageUnits:PoliceRaid", {
      ...
      },
      {
        ...
        shouldShow = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            return (unit.owner and unit.owner.SID == LocalPlayer.state.Character:GetData("SID")) or exports['jobs']:JobsPermissionsHasJob(unit.managedBy)
          end
        end,
      },
      {
        ...
        shouldShow = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            return exports['jobs']:JobsPermissionsHasJob(unit.managedBy)
          end
        end,
      },
    })
  ...
  exports['core']:CallbacksRegisterClient("StorageUnits:Passcode", function(code, cb)
...
_STORAGEUNITS = {
  Access = function(self)
    local nearUnit = StorageUnits:GetNearUnit()
    if nearUnit and nearUnit?.unitId then
      exports['core']:CallbacksServer("StorageUnits:Access", nearUnit?.unitId)
    end
  end,
  Manage = function(self, specificUnit)
    ...
    if nearUnit and nearUnit?.unitId then
      ...
      if unit then
        ...
        if exports['jobs']:JobsPermissionsHasJob(unit.managedBy) then
          if unit.owner then
            table.insert(menu.main.items, {
        ...
        if exports['jobs']:JobsPermissionsHasJob(unit.managedBy, false, false, false, false, "UNIT_SELL") then
          table.insert(menu.main.items, {
...
AddEventHandler("StorageUnits:Client:NewPasscode", function(values, data)
  if values and values.passcode and #values.passcode >= 4 then
    exports['core']:CallbacksServer("StorageUnits:ChangePasscode", {
      unit = data.unit,
      passcode = values.passcode,
    }, function(success)
      if success then
        exports['hud']:NotificationSuccess("Updated Passcode")
      else
        exports['hud']:NotificationError("Failed to Update Passcode")
      end
    end)
  end
end)
...
AddEventHandler("StorageUnits:Client:SellUnit", function(values, data)
  if values and values.SID then
    ...
    if stateId and stateId > 0 then
      exports['core']:CallbacksServer("StorageUnits:SellUnit", {
        unit = data.unit,
        SID = stateId,
      }, function(success)
        if success then
          exports['hud']:NotificationSuccess("Storage Unit Sold")
        else
          exports['hud']:NotificationError("Failed to Sell Storage Unit")
        end
      end)
    else
      exports['hud']:NotificationError("Invalid State ID")
    end
  end
end)
