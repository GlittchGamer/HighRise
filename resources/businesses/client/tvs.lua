...
local createdDUI
local url = 'nui://businesses/dui/tvs/index.html'
local scale = 0.08
...
AddEventHandler('TVs:Client:SetLink', function(e, data)
  ...
  exports['core']:CallbacksServer('TVs:UpdateTVLink', {
    tv = data,
    link = tvLink
  }, function(success)
    if success then
      exports['hud']:NotificationSuccess('Updated Link!', 5000, 'tv')
    else
      exports['hud']:NotificationError('Error', 5000, 'tv')
    end
  end)
end)
