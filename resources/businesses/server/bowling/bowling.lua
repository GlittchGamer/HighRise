...
AddEventHandler('Businesses:Server:Startup', function()
    ...
    exports['core']:CallbacksRegisterServer('Bowling:StartGame', function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Bowling:JoinGame', function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Bowling:EndGame', function(source, data, cb)
        local char = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Bowling:UpdateTVLink', function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Bowling:ResetAll', function(source, data, cb)
        if Player(source).state.onDuty == 'bowling' then
    ...
    exports['core']:CallbacksRegisterServer('Bowling:ClearPins', function(source, data, cb)
        if Player(source).state.onDuty == 'bowling' then
            for k, v in ipairs(GetAllObjects()) do
                local model = GetEntityModel(v)
                if model == -1501785249 or model == -563331074 and #(GetEntityCoords(v) - vector3(738.970, -775.074, 26.446)) <= 100.0 then
...
RegisterServerEvent('Bowling:Server:StartBowling', function(alleyId)
    ...
    if char and alleyId then
        ...
        if alley and alley.active and mySID == alley.currentPlayer then
            exports['core']:CallbacksClient(source, 'Bowling:DoBowl', alleyId, function(res)
