...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Trunk", {
    ...
    exports['core']:CallbacksRegisterClient("Trunk:GetPutIn", function(data, cb)
      InTrunk(NetToVeh(data))
    end)

    exports['core']:CallbacksRegisterClient("Trunk:GetPulledOut", function(data, cb)
...
function InTrunk(veh)
  if not DoesVehicleHaveDoor(veh, 6) and DoesVehicleHaveDoor(veh, 5) and IsThisModelACar(GetEntityModel(veh)) then
    ...
    if not LocalPlayer.state.isCuffed and not LocalPlayer.state.isDead then
      exports['hud']:ActionShow('{keybind}primary_action{/keybind} Exit Trunk | {keybind}secondary_action{/keybind} Open/Close Trunk')
    end
...
AddEventHandler("Trunk:Client:PutIn", function(entity, data)
  exports['core']:CallbacksServer("Trunk:PutIn", NetworkGetNetworkIdFromEntity(entity.entity), function(state) end)
end)

AddEventHandler("Trunk:Client:PullOut", function(entity, data)
  exports['core']:CallbacksServer("Trunk:PullOut", NetworkGetNetworkIdFromEntity(entity.entity), function(state) end)
end)
