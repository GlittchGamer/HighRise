...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("DJ", {
    "Menu",
    "Notification",
  }, function(error)
    if #error > 0 then
...
RegisterNetEvent("dj:client:playMusic", function(data)
  ...
  local p = promise.new()
  exports['core']:CallbacksServer("dj:server:songInfo", {}, function(cb)
    p:resolve(cb)
  end)
  ...
  djMenu.Add:Input("Youtube URL", {
    ...
    if song == "" or song == nil then
      exports['hud']:NotificationError("Empty text cannot be played dummy.", 2500, "fas fa-volume-off")
      return
    end
    if not string.find(song, "youtu") then
      song = "https://www.youtube.com/watch?v=" .. song
    end
    exports['core']:CallbacksServer("dj:server:playMusic", {
      song = song,
      zoneNum = boothId,
    }, function(success, zoneNum)
      if success then
        TriggerEvent("dj:client:playMusic", { zone = zoneNum })
        exports['hud']:NotificationSuccess("Loading link: " .. song, 2500, "fas fa-pause")
      else
        exports['hud']:NotificationError("Failed to Load Song", 2500, "fas fa-volume-off")
      end
    end)
  end)
  ...
  if xSound:soundExists(booth) then
    if xSound:isPlaying(booth) then
      djMenu.Add:Button("Pause Music", { success = true }, function()
        exports['core']:CallbacksServer("dj:server:pauseMusic", {
          zoneName = booth,
          zoneNum = boothId,
        }, function(success)
          if success then
            exports['hud']:NotificationSuccess("Paused Music", 2500, "fas fa-pause")
          else
            exports['hud']:NotificationError("Failed to Pause Music", 2500, "fas fa-pause")
          end
        end)
      end)
    elseif xSound:isPaused(booth) then
      djMenu.Add:Button("Resume Music", { success = true }, function()
        exports['core']:CallbacksServer("dj:server:resumeMusic", {
          zoneName = booth,
          zoneNum = boothId,
        }, function(success)
          if success then
            exports['hud']:NotificationSuccess("Resume Music", 2500, "fas fa-play")
          else
            exports['hud']:NotificationError("Failed to Resume Music", 2500, "fas fa-play")
          end
        end)
      end)
    end
    ...
    djMenuSub["changevolume"].Add:Slider("Change Volume", {
      ...
      exports['core']:CallbacksServer("dj:server:changeVolume", {
        volume = volume,
        zoneName = booth,
        zoneNum = boothId,
      }, function(success, zoneNum)
        if success then
          ...
          TriggerEvent("dj:client:playMusic", { zone = zoneNum })
          exports['hud']:NotificationSuccess(
            string.format("Changed Volume to %s/100", tostring(math.ceil(volume * 100))),
            2500,
            "fas fa-volume-off"
          )
        else
          exports['hud']:NotificationError("Failed to Change Volume", 2500, "fas fa-volume-off")
        end
      end)
    end)
    ...
    djMenu.Add:Button("Stop Music", { success = true }, function()
      exports['core']:CallbacksServer("dj:server:stopMusic", {
        zoneName = booth,
        zoneNum = boothId,
      }, function(success)
        if success then
          exports['hud']:NotificationSuccess("Stop Music", 2500, "fas fa-stop")
        else
          exports['hud']:NotificationError("Failed to Stop Music", 2500, "fas fa-stop")
        end
      end)
    end)
  end
