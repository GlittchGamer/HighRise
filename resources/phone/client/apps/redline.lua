          ...
          if DoesEntityExist(ped) then
            ...
            if DoesEntityExist(veh) then
              if #(myPos - GetEntityCoords(veh)) <= 100.0 then
                exports['hud']:NotificationInfo("Police Nearby, Race Phasing Disabled")
                ghostingEnded = true
                UnGhostPlayer()
              end
            end
...
RegisterNUICallback("CreateRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:CreateRace", data, function(res)
...
RegisterNUICallback("CancelRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:CancelRace", data, function(res)
    cb(res)
  end)
end)

RegisterNUICallback("PracticeTrack", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:GetTrack", data, function(res)
    cb(res ~= nil)
    if res ~= nil then
      SetupTrack(res)
      _activeRace = res
...
RegisterNUICallback("JoinRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:JoinRace", data, function(res)
    if res then
      _activeRace = res
...
RegisterNUICallback("LeaveRace", function(data, cb)
  UnGhostPlayer()
  exports['core']:CallbacksServer("Phone:Redline:LeaveRace", data, function(res)
    if _activeRace ~= nil then
      _activeRace.dnf = true
      Cleanup()
      UISounds.Play:FrontEnd(-1, "CHECKPOINT_MISSED", "HUD_MINI_GAME_SOUNDSET")
...
RegisterNUICallback("CreateTrack", function(data, cb)
  exports['core']:CallbacksServer("Phone:Permissions", {
    redline = { "create" },
  }, function(res)
    cb(res)
    if res then
  ...
  exports['core']:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      if #_pendingTrack.Checkpoints > 2 then
        ...
        exports['core']:CallbacksServer("Phone:Redline:SaveTrack", _pendingTrack, function(res2)
          cb(res2)
        end)
      else
        exports['hud']:NotificationError("Not Enough Checkpoints")
        cb(false)
      end
    else
      cb(false)
...
RegisterNUICallback("DeleteTrack", function(data, cb)
  exports['core']:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      exports['core']:CallbacksServer("Phone:Redline:DeleteTrack", data, function(res2)
        cb(res2)
      end)
    else
      cb(false)
...
RegisterNUICallback("ResetTrackHistory", function(data, cb)
  exports['core']:CallbacksServer("Phone:Permissions", {
    ...
    if res then
      exports['core']:CallbacksServer("Phone:Redline:ResetTrackHistory", data, function(res2)
        cb(res2)
      end)
    else
      cb(false)
...
RegisterNUICallback("StartRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:StartRace", _activeRace._id, cb)
end)

RegisterNUICallback("EndRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:EndRace", data, cb)
end)

RegisterNUICallback("SendInvite", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:SendInvite", data, cb)
end)

RegisterNUICallback("AcceptInvite", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:AcceptInvite", data, function(res)
    if res then
      _activeRace = res
...
RegisterNUICallback("DeclineInvite", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:DeclineInvite", data, cb)
end)
...
RegisterNUICallback("RemoveFromRace", function(data, cb)
  exports['core']:CallbacksServer("Phone:Redline:RemoveRacer", data, cb)
end)
      ...
      if dist <= currentRaceCheckpoints.size or sprintCheckpoint == -1 then
        ...
        if currentCheckpoint == 1 and #cCps == #_activeRace.trackData.Checkpoints and _activeRace.trackData.Type ~= "p2p" then -- this is lapped race?
          ...
          if cLp <= tonumber(_activeRace.laps) then
            exports['hud']:NotificationInfo(string.format("Lap %s", cLp))
...
function CreateCheckpoint()
  ...
  if lcp == nil or dist > 5 then
    ...
    exports['hud']:NotificationError("Point Too Close To Last Point")
  end
end
