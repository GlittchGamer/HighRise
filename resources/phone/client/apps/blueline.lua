...
RegisterNUICallback("CreateRacePD", function(data, cb)
  exports['core']:CallbacksServer("Racing:Server:CreateRace", data, function(res)
...
RegisterNUICallback("CancelRacePD", function(data, cb)
  exports['core']:CallbacksServer("Phone:Blueline:CancelRace", data, function(res)
    cb(res)
  end)
end)

RegisterNUICallback("PracticeTrackPD", function(data, cb)
  exports['core']:CallbacksServer("Phone:Blueline:GetTrack", data, function(res)
    cb(res ~= nil)
    if res ~= nil then
      SetupTrackPD(res)
      _activeRace = res
...
RegisterNUICallback("JoinRacePD", function(data, cb)
  exports['core']:CallbacksServer("Phone:Blueline:JoinRace", data, function(res)
    if res then
      _activeRace = res
      AddRaceBlipPD(_activeRace.trackData.Checkpoints[1])
      SetNewWaypoint(
...
RegisterNUICallback("LeaveRacePD", function(data, cb)
  exports['core']:CallbacksServer("Phone:Blueline:LeaveRace", data, function(res)
...
RegisterNUICallback("CreateTrackPD", function(data, cb)
  if exports['jobs']:JobsPermissionsHasPermissionInJob("police", "PD_MANAGE_TRIALS") then
    local isCreating = exports['racing']:CreatorStart('blueline')
    cb(true)
  else
    cb(false)
  end
...
RegisterNUICallback("FinishCreatorPD", function(data, cb)
  if exports['jobs']:JobsPermissionsHasPermissionInJob("police", "PD_MANAGE_TRIALS") then
    local createTrack = exports['racing']:CreatorStop(true)
    ...
    exports['core']:CallbacksServer("Racing:Server:SaveRace", { track = createTrack.cachedTrack, raceType = data.type, raceVariant = 'blueline' },
      function(res2)
        cb(res2)
      end)
  else
...
RegisterNUICallback("DeleteTrackPD", function(data, cb)
  if exports['jobs']:JobsPermissionsHasPermissionInJob("police", "PD_MANAGE_TRIALS") then
    exports['core']:CallbacksServer("Phone:Blueline:DeleteTrack", data, function(res2)
      cb(res2)
    end)
  else
    cb(false)
...
RegisterNUICallback("ResetTrackHistoryPD", function(data, cb)
  if exports['jobs']:JobsPermissionsHasPermissionInJob("police", "PD_MANAGE_TRIALS") then
    exports['core']:CallbacksServer("Phone:Blueline:ResetTrackHistory", data, function(res2)
      cb(res2)
    end)
  else
    cb(false)
...
RegisterNUICallback("StopCreatorPD", function(data, cb)
  cb("OK")
  exports['racing']:CreatorStop()
end)

RegisterNUICallback("StartRacePD", function(data, cb)
  exports['core']:CallbacksServer("Racing:Server:StartRace", data, cb)
end)

RegisterNUICallback("EndRacePD", function(data, cb)
  exports['core']:CallbacksServer("Phone:Blueline:EndRace", data, cb)
end)
...
function LapDetailsPD(data)
  exports['core']:CallbacksServer("Phone:Blueline:SaveLaptimes", data)
      ...
      if dist <= cp.size or safeCheckpoint == -1 then
        ...
        if currentCheckpoint == 1 and #completedCheckpoints == #_activeRace.trackData.Checkpoints and _activeRace.trackData.Type ~= "p2p" then
          ...
          if currentLap <= tonumber(_activeRace.laps) then
            exports['hud']:NotificationInfo(string.format("Lap %s", currentLap))
