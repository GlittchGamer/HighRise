...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
  exports['core']:CallbacksRegisterServer("Phone:Garage:GetCars", function(source, data, cb)
    local src = source
    local char = exports['core']:FetchSource(src):GetData("Character")
    Vehicles.Owned:GetAll(nil, 0, char:GetData("SID"), cb)
  end)

  exports['core']:CallbacksRegisterServer("Phone:Garage:TrackVehicle", function(source, data, cb)
    local src = source
    local char = exports['core']:FetchSource(src):GetData("Character")
