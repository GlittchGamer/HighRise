...
function Startup()
	for k, v in ipairs(Config.Restaurants) do
		exports['hrrp-base']:LoggerTrace("Restaurant", string.format("Registering Restaurant ^3%s^7", v.Name))
		if v.Benches then
			for benchId, bench in pairs(v.Benches) do
				exports['hrrp-base']:LoggerTrace("Restaurant", string.format("Registering Crafting Bench ^2%s^7 For ^3%s^7", bench.label, v.Name))
		...
		if v.Storage then
			for _, storage in pairs(v.Storage) do
				exports['hrrp-base']:LoggerTrace("Restaurant", string.format("Registering Poly Inventory ^2%s^7 For ^3%s^7", storage.id, v.Name))
