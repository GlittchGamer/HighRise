function GetSpawnLocations()
  local spawns = MySQL.query.await("SELECT * FROM `locations` WHERE `Type` = ?", { 'spawn' })
  if not spawns or spawns[1] == nil then
    return false
  end
  for k, v in pairs(spawns) do
    spawns[k].Coords = json.decode(v.Coords)
  end
  return spawns
end

function RegisterCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer('Admin:GetPlayerList', function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:GetDisconnectedPlayerList', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    if player and player.Permissions:IsStaff() then
      local rDs = exports['hrrp-base']:FetchComponent('RecentDisconnects')
      cb(rDs)
    else
      cb(false)
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:GetPlayer', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    if player and player.Permissions:IsStaff() then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:BanPlayer', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:KickPlayer', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:ActionPlayer', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    if player and data.action and data.targetSource and player.Permissions:IsStaff() then
      local target = exports['hrrp-base']:FetchSource(data.targetSource)
      if target then
        ...
        if targetChar then
          ...
          if data.action == 'bring' and canFuckWith and notMe then
            ...
            exports['hrrp-pwnzor']:PwnzorPlayersTempPosIgnore(target:GetData("Source"))
          ...
          elseif data.action == 'heal' then
            if (notMe or player.Permissions:IsAdmin()) then
              exports["hrrp-base"]:CallbacksClient(targetChar:GetData("Source"), "Damage:Heal", true)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:CurrentVehicleAction', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:NoClip', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:UpdatePhonePerms', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    if player.Permissions:IsAdmin() then
      local target = exports['hrrp-base']:FetchSource(data.target)
      if target ~= nil then
        local char = target:GetData("Character")
        if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Admin:ToggleInvisible', function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    if player and player.Permissions:IsAdmin() then
      exports['hrrp-base']:LoggerWarn(
        "Admin",
