...
RegisterNetEvent('Admin:Client:Invisible', function()
  if _isInvis then
    ...
    exports['hrrp-hud']:NotificationInfo("Invisibility: Off")
    _isInvis = false
  else
    SetEntityVisible(LocalPlayer.state.ped, false)
    exports['hrrp-hud']:NotificationInfo("Invisibility: On")
    _isInvis = true
  end
end)
...
local function toggleSpectate(targetPed, targetPlayerId)
  ...
  if isSpectateEnabled then
    ...
    if not lastSpectateLocation then
      exports['hrrp-hud']:NotificationError('Last location previous to spectate was not stored properly')
    end

    if not storedTargetPed then
      exports['hrrp-hud']:NotificationError("Target ped was not stored to unspectate")
    end
...
local function cleanupFailedResolve()
  ...
  exports['hrrp-hud']:NotificationError("Failed to Spectate")
end
