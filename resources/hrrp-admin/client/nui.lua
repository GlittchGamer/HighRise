...
RegisterNUICallback('GetPlayerList', function(data, cb)
    if data and data.disconnected then
        exports["hrrp-base"]:CallbacksServer('Admin:GetDisconnectedPlayerList', data, cb)
    else
        exports["hrrp-base"]:CallbacksServer('Admin:GetPlayerList', data, cb)
    end
end)

RegisterNUICallback('GetPlayer', function(data, cb)
    exports["hrrp-base"]:CallbacksServer('Admin:GetPlayer', data, cb)
end)
...
RegisterNUICallback('ActionPlayer', function(data, cb)
    exports["hrrp-base"]:CallbacksServer('Admin:ActionPlayer', data, cb)
end)

RegisterNUICallback('KickPlayer', function(data, cb)
    exports["hrrp-base"]:CallbacksServer('Admin:KickPlayer', data, cb)
end)

RegisterNUICallback('BanPlayer', function(data, cb)
    exports["hrrp-base"]:CallbacksServer('Admin:BanPlayer', data, cb)
end)

RegisterNUICallback('CurrentVehicleAction', function(data, cb)
    local insideVehicle = GetVehiclePedIsIn(LocalPlayer.state.ped, true)
    if insideVehicle and insideVehicle > 0 and DoesEntityExist(insideVehicle) then
        exports["hrrp-base"]:CallbacksServer('Admin:CurrentVehicleAction', data, function(canDo)
            if canDo then
...
RegisterNUICallback('ToggleInvisible', function(data, cb)
    exports["hrrp-base"]:CallbacksServer('Admin:ToggleInvisible', data, cb)
end)
