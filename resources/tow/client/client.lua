...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies('Tow', {
    ...
    exports['pedinteraction']:Add('veh_tow_jerry', `a_m_m_eastsa_01`, vector3(-247.645, -1183.099, 22.090), 312.942, 50.0, {
      {
        icon = "truck-tow",
    ...
    exports['polyzonehandler']:PolyZoneCreateBox('tow_impound_zone', vector3(-236.96, -1173.44, 23.04), 19.4, 24.4, {
      heading = 270,
      minZ = 22.04,
      maxZ = 26.04
    })
...
AddEventHandler('Proxy:Shared:RegisterReady', function()
  exports['core']:RegisterComponent('Tow', _TOW)
end)
  ...
  if not _towingAction and _towTrucks[truckModel] and truckState and not truckState.towingVehicle then
    ...
    if canTow then
      ...
      Progress:ProgressWithStartAndTick({
        ...
        if not canTow then
          Progress:Cancel()
          exports['hud']:NotificationError(errorMessage, 5000, 'truck-tow')
        end
        ...
        if not wasCancelled then
          ...
          if success then
            ...
            exports['hud']:NotificationSuccess('Vehicle Now on Tow Truck', 5000, 'truck-tow')

            if Entity(success).state.towObjective then
              exports['blips']:Remove("towjob-pickup")
              Phone.Notification:Update('TOW_OBJ', 'Yard Manager', 'Great, bring it back to the yard')
            end
          else
            truckState:set('towingVehicle', false, true)
            exports['hud']:NotificationError('Failed to Tow Vehicle', 5000, 'truck-tow')
          end
        end
      end)
    else
...
AddEventHandler('Vehicles:Client:ReleaseTow', function(entityData)
  ...
  if _towTrucks[truckModel] and truckState then
    if truckState.towingVehicle then
      ...
      if success then
        exports['hud']:NotificationSuccess('Vehicle Released from Truck', 5000, 'truck-tow')
        truckState:set('towingVehicle', false, true)
      end
    else
      exports['hud']:NotificationError('No Vehicle Being Towed', 5000, 'truck-tow')
    end
  end
end)

RegisterNetEvent('Tow:Client:MarkPickup', function(coords, vehNet)
  exports['blips']:Add("towjob-pickup", "Vehicle Pickup", coords, 326, 65, 0.8, 2, false, true)
