...
_JOBS = {
  ...
  GiveJob = function(self, stateId, jobId, workplaceId, gradeId, noOverride)
    ...
    local char = exports['hrrp-base']:FetchSID(stateId)
    if char then
  ...
  RemoveJob = function(self, stateId, jobId)
    ...
    if char then
      ...
      if found then
        local source = char:GetData('Source')
        char:SetData('Jobs', charJobData)
        exports['hrrp-jobs']:JobsDutyOff(source, jobId, true)
        ...
        if removedJobData.Workplace and removedJobData.Workplace.Name then
          exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Info', 'No Longer Employed at ' .. removedJobData.Workplace.Name)
        else
          exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Info', 'No Longer Employed at ' .. removedJobData.Name)
        end
  ...
  Duty = {
    On = function(self, source, jobId, hideNotify)
          ...
          if hasJob then
            ...
            if dutyData then
              if dutyData.Id == hasJob.Id then
                ...
                local success = exports['hrrp-jobs']:JobsDutyOff(source, false, true)
                if not success then
                  return false
                end
              end
            ...
            exports['hrrp-jobs']:JobsDutyRefreshDutyData(hasJob.Id)
      ...
      if not hideNotify then
        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Failed to Go On Duty')
      end
    ...
    Off = function(self, source, jobId, hideNotify)
      ...
      if player then
        ...
        if char then
          ...
          if dutyData and (not jobId or (dutyData.Id == jobId)) then
            ...
            exports['hrrp-base']:LoggerInfo("Jobs",
            ...
            exports['hrrp-jobs']:JobsDutyRefreshDutyData(dutyId)
            ...
            if not hideNotify then
              exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Info', 'You\'re Now Off Duty')
            end
      ...
      if not hideNotify then
        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Failed to Go Off Duty')
      end
  ...
  Permissions = {
    ...
    HasJob = function(self, source, jobId, workplaceId, gradeId, gradeLevel, checkDuty, permissionKey)
      local jobs = exports['hrrp-jobs']:JobsPermissionsGetJobs(source)
            ...
            if (not workplaceId or (v.Workplace and v.Workplace.Id == workplaceId)) then
              if (not gradeId or (v.Grade.Id == gradeId)) then
                if (not gradeLevel or (v.Grade.Level and v.Grade.Level >= gradeLevel)) then
                  if not checkDuty or (checkDuty and exports['hrrp-jobs']:JobsDutyGet(source, jobId)) then
                    if not permissionKey or (permissionKey and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, jobId, permissionKey)) then
                      return v
                    end
                  end
                end
      ...
      elseif permissionKey then
        return exports['hrrp-jobs']:JobsPermissionsHasPermission(source, permissionKey)
      end
      return false
    end,
    -- Gets the permissions the character has in a job they have
    GetPermissionsFromJob = function(self, source, jobId, workplaceId)
      local jobData = exports['hrrp-jobs']:JobsPermissionsHasJob(source, jobId, workplaceId)
      if jobData then
    ...
    HasPermissionInJob = function(self, source, jobId, permissionKey)
      local permissionsInJob = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, jobId)
    ...
    HasPermission = function(self, source, permissionKey)
      local allPermissions = exports['hrrp-jobs']:JobsPermissionsGetAllPermissions(source)
  ...
  Management = {
    ...
    Transfer = function(self, jobId, newOwner)
      -- TODO
      --exports['hrrp-base']:MiddlewareTriggerEvent("Business:Transfer", jobId, source:GetData("SID"), target:GetData("SID"))
    end,
    Upgrades = {
      -- TODO
