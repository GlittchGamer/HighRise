...
function RegisterJobChatCommands()
        ...
        if success then
          if jobExists.Workplace then
            Chat.Send.System:Single(source,
              string.format('Gave State ID: %s Job: %s - %s - %s', target, jobExists.Name, jobExists.Workplace.Name, jobExists.Grade.Name))
          else
  ...
  Chat:RegisterAdminCommand('onduty', function(source, args, rawCommand)
    exports['hrrp-jobs']:JobsDutyOn(source, args[1])
  end, {
    help = 'Go On Duty',
    params = {
  ...
  Chat:RegisterAdminCommand('offduty', function(source, args, rawCommand)
    exports['hrrp-jobs']:JobsDutyOff(source)
  end, {
    help = 'Go Off Duty'
  })
  ...
  Chat:RegisterAdminCommand('checkjobs', function(source, args, rawCommand)
    ...
    local player
    if not args[1] then
      player = exports['hrrp-base']:FetchSource(source)
    else
      player = exports['hrrp-base']:FetchSID(target)
    end
  ...
  Chat:RegisterAdminCommand('dutycount', function(source, args, rawCommand)
    ...
    local jobExists = Jobs:Get(jobId)
    if jobExists then
      local dutyData = exports['hrrp-jobs']:JobsDutyGetDutyData(jobId)
  ...
  Chat:RegisterAdminCommand('dutytest', function(source, args, rawCommand)
    ...
    if jobExists then
      ...
      exports['hrrp-jobs']:JobsDutyRefreshDutyData(jobId)
