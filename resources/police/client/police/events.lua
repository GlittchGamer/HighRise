  ...
  if veh then
    Progress:Progress({
      ...
      if not cancelled then
        ...
        if timeout <= 0 then
          exports['hud']:NotificationError("Unable To Retrieve Vehicle Information")
          return
        end
...
AddEventHandler("Police:Client:GSR", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports['core']:CallbacksServer("Police:GSRTest", entity.serverId, function() end)
    end
  end)
end)
...
AddEventHandler("Police:Client:BAC", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports['core']:CallbacksServer("Police:BACTest", entity.serverId, function() end)
    end
  end)
end)
...
AddEventHandler("Police:Client:DNASwab", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports['core']:CallbacksServer("Police:DNASwab", entity.serverId, function() end)
    end
  end)
end)
...
AddEventHandler("Police:Client:OpenLocker", function()
  exports['core']:CallbacksServer("MDT:OpenPersonalLocker", {}, function(success)
    if not success then
      exports['hud']:NotificationError("Callsign Not Set, Unable To Open Locker")
    end
  end)
end)
...
AddEventHandler('Police:Client:RemoveMask', function(entity, data)
  exports['core']:CallbacksServer("Police:RemoveMask", entity.serverId)
end)
