...
AddEventHandler("Businesses:Server:Startup", function()
  ...
  Chat:RegisterAdminCommand("unitcopy", function(source, args, rawCommand)
    ...
    if near?.unitId then
      exports['hrrp-base']:ExecuteClient(source, "Admin", "CopyClipboard", near?.unitId)
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Copied Storage Unit ID")
    end
  end, {
  ...
  exports['hrrp-base']:CallbacksRegisterServer("StorageUnits:Access", function(source, data, cb)
    local unit = GlobalState[string.format("StorageUnit:%s", data)]
    if unit and unitPasscodes[unit.id] and unit.owner then
      exports["hrrp-base"]:CallbacksClient(source, "StorageUnits:Passcode", unitPasscodes[unit.id], function(success, data)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("StorageUnits:SellUnit", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char and data.unit and data.SID then
      local unit = GlobalState[string.format("StorageUnit:%s", data.unit)]
      if unit and exports['hrrp-jobs']:JobsPermissionsHasJob(source, unit.managedBy, false, false, false, false, "UNIT_SELL") then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("StorageUnits:ChangePasscode", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("StorageUnits:PoliceRaid", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
    if player and char and data and data.unit then
      local unit = GlobalState[string.format("StorageUnit:%s", data.unit)]
      if unit then
        if Player(source).state.onDuty == "police" and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, "police", "PD_RAID") then
...
function StorageUnitStartup()
  if not _ran then
    ...
    exports['hrrp-base']:LoggerTrace("StorageUnits", "Loaded ^2" .. #storageUnits .. "^7 Storage Units", { console = true })
    local unitIds = {}
