...
AddEventHandler("Businesses:Server:Startup", function()
  exports['hrrp-base']:CallbacksRegisterServer("Businesses:SAGMA:OpenTable", function(source, data, cb)
    Inventory:OpenSecondary(source, 132, data)
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Businesses:SAGMA:Sell", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if exports['hrrp-jobs']:JobsPermissionsHasJob(source, _jobName, false, false, false, false, "JOB_SELL_GEMS") then
...
AddEventHandler("Businesses:Server:SAGMA:ViewGem", function(source, data)
  ...
  if plyr ~= nil then
    ...
    if char ~= nil then
      if exports['hrrp-jobs']:JobsPermissionsHasJob(source, _jobName, false, false, false, true, "JOB_USE_GEM_TABLE") then
