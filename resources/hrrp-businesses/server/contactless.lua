...
AddEventHandler("Businesses:Server:Startup", function()
  exports['hrrp-base']:CallbacksRegisterServer("Contactless:Create", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Contactless:Clear", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Contactless:Pay", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char and data and data.terminalId and _pendingContactless[data.terminalId] and GlobalState[string.format("PendingContactless:%s", data.terminalId)] then
      ...
      if wSuccess then
        ...
        Phone.Notification:Add(pData.billerSource, "Contactless Payment Received", string.format("Received $%s for %s.", math.floor(pData.amount), pData.jobName),
          os.time() * 1000, 8000, "bank", {})
        exports['hrrp-base']:ExecuteClient(pData.billerSource, "Notification", "Success", "Contactless Payment Received")

        Phone.Notification:Add(source, "Contactless Payment Accepted", string.format("Paid $%s to %s", math.floor(pData.amount), pData.jobName), os.time() * 1000,
          8000, "bank", {})
        ...
        Phone.Notification:Add(pData.billerSource, "Contactless Payment Failed",
          string.format("Payment of $%s for %s failed.", math.floor(pData.amount), pData.jobName), os.time() * 1000, 8000, "bank", {})
        exports['hrrp-base']:ExecuteClient(pData.billerSource, "Notification", "Error", "Contactless Payment Failed")

        Phone.Notification:Add(source, "Contactless Payment Failed",
          string.format("Payment of $%s to %s just failed. Is your balance sufficient?", math.floor(pData.amount), pData.jobName), os.time() * 1000, 8000, "bank",
          {})
