...
AddEventHandler("Businesses:Client:Startup", function()
  for k, v in ipairs(_sellers) do
    exports['hrrp-pedinteraction']:Add(string.format('SAGMASeller%s', k), v.model, v.coords, v.heading, 25.0, {
      {
        icon = 'ring',
        text = 'Sell Gems',
  ...
  for k, v in pairs(_sagmaPaintings) do
    exports['ox_target']:TargetingZonesAddBox("sagma-art-" .. k, "palette", v.coords, v.length, v.width, v.options, {
      {
        icon = "palette",
  ...
  for k, v in ipairs(_appraisalTables) do
    exports['ox_target']:TargetingZonesAddBox("sagma-table-" .. k, "table-picnic", v.coords, v.length, v.width, v.options, {
      {
        icon = "gem",
        text = "View Gem Table",
...
AddEventHandler("SAGMA:Client:Sell", function()
  exports["hrrp-base"]:CallbacksServer("Businesses:SAGMA:Sell", {})
end)
...
function ActivateTable(tableId, color, quality, item)
  ...
  exports['hrrp-hud']:GemTableOpen(quality)
...
function CleanupTable()
  ...
  DeleteEntity(_gemObj)
  exports['hrrp-hud']:GemTableClose()
