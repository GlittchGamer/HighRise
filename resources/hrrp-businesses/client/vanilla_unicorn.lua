...
AddEventHandler('Businesses:Client:Startup', function()
  exports['hrrp-polyzone']:PolyZoneCreatePoly('vu_dancers', {
  ...
  exports['hrrp-polyzone']:PolyZoneCreatePoly('vu_makeitrain', {
  ...
  Interaction:RegisterMenu("vu_stripper_pole", "Vanilla Unicorn Dancers", "party-horn", function()
    if exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn' and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob('unicorn', 'STRIP_POLE') then
      local subMenu = {}
      ...
      exports['hrrp-hud']:NotificationError('Invalid Permissions')
    end
  end, function()
    return exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn'
  end)

  Interaction:RegisterMenu("vu_makeitrain", 'Make It Rain', "money-bill-1-wave", function()
    if not _makingItRain and exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') then
      local makeItRain = {
        {
          type = 'cash',
          text = '$100 Cash',
    ...
    return exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') and GetNearbyFuckingStripper()
  end)
end)
    ...
    if #(v - pedCoords) <= 1.5 then
      ...
      if dist == -1 or dist > 1.5 then
        ...
        if poleDance then
          ...
          exports["hrrp-animations"]:EmotesPlay(poleDance.anim, false, false, false)
        end
      else
        exports['hrrp-hud']:NotificationError('Pole Taken')
      end
      return
    end
  end
...
function MakeItRainBitch(targetSource, cashType, time)
  ...
  Citizen.CreateThread(function()
    _makingItRain = true
    exports["hrrp-animations"]:EmotesPlay('makeitrain', false, false, false)
    ...
    while
      ...
      and exports["hrrp-animations"]:EmotesGet() == 'makeitrain'
    ...
    do
      local p = promise.new()
      exports["hrrp-base"]:CallbacksServer('VU:MakeItRain', {
        target = targetSource,
        type = cashType,
      }, function(success, cd)
        if not success then
          exports['hrrp-hud']:NotificationError(cd and 'Reached Cooldown' or 'Error - Ran Out of Money')
          _makingItRain = false
        end
