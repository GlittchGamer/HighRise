...
AddEventHandler("Businesses:Client:Startup", function()
  ...
  exports['hrrp-polyzone']:PolyZoneCreateBox("rockford-stage-area", vector3(-1002.26, -257.48, 39.04), 12.2, 16.8, {
    heading = 323,
    --debugPoly=true,
    minZ = 38.04,
    maxZ = 42.44
...
AddEventHandler("Businesses:Client:RockfordStage", function()
  exports["hrrp-base"]:CallbacksServer("Businesses:ToggleRockfordStage", {}, function(state)
    if state then
      exports['hrrp-hud']:NotificationSuccess("DJ Stand Enabled", 5000, "speakers")
    else
      exports['hrrp-hud']:NotificationError("DJ Stand Disabled", 5000, "speakers")
    end
  end)
end)
...
AddStateBagChangeHandler("rockford_dj", nil, function(bagName, key, value, _unused, replicated)
  if exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), "rockford-stage-area") then
    if value then
      CreateDJStuff()
    else
      DeleteDJStuff()
