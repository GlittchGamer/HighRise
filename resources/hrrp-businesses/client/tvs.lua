...
local createdDUI
local url = 'nui://hrrp-businesses/dui/tvs/index.html'
local scale = 0.08
...
AddEventHandler('TVs:Client:SetLink', function(e, data)
  ...
  exports["hrrp-base"]:CallbacksServer('TVs:UpdateTVLink', {
    tv = data,
    link = tvLink
  }, function(success)
    if success then
      exports['hrrp-hud']:NotificationSuccess('Updated Link!', 5000, 'tv')
    else
      exports['hrrp-hud']:NotificationError('Error', 5000, 'tv')
    end
  end)
end)
