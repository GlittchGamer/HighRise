...
local url = 'nui://hrrp-businesses/dui/bowling/index.html'
local scale = 0.08
...
AddEventHandler('Bowling:Client:SetTV', function()
  ...
  exports["hrrp-base"]:CallbacksServer('Bowling:UpdateTVLink', tvLink, function(success)
    if success then
      SendBowlingNotification('Updated Link!')
    else
      exports['hrrp-hud']:NotificationError('Error', 5000, 'bowling-ball-pin')
    end
  end)
end)
