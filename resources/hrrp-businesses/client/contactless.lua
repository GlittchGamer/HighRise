...
AddEventHandler("Businesses:Client:PayContactlessPayment", function(_, data)
  if data and data.id then
    exports["hrrp-base"]:CallbacksServer("Contactless:Pay", {
      terminalId = data.id,
    }, function(success)
      if success then
  ...
  if data and data.job and data.id then
    ...
    if input?.amount and input?.description then
      ...
      if amount and amount > 0 and amount <= 10000 then
        exports["hrrp-base"]:CallbacksServer("Contactless:Create", {
          job = data.job,
          terminalId = data.id,
          payment = amount,
          ...
          if success then
            exports['hrrp-hud']:NotificationSuccess("Contactless Terminal Set, Now Get Them to Pay!")
          else
            exports['hrrp-hud']:NotificationError(res or "Error")
          end
        end)
      else
        exports['hrrp-hud']:NotificationError("Maximum Contactless Amount is $10,000")
      end
    end
  end
end)
...
AddEventHandler("Businesses:Client:ClearContactlessPayment", function(_, data)
  if data and data.job and data.id then
    exports["hrrp-base"]:CallbacksServer("Contactless:Clear", {
      job = data.job,
      terminalId = data.id,
    }, function(success, res)
      if success then
        exports['hrrp-hud']:NotificationSuccess("Contactless Terminal Cleared")
      else
        exports['hrrp-hud']:NotificationError(res or "Error")
      end
    end)
