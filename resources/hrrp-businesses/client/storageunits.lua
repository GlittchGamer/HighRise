AddEventHandler("Businesses:Client:Startup", function()
      ...
      },
      {
        ...
        action = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            exports["hrrp-base"]:CallbacksServer("StorageUnits:PoliceRaid", {
      ...
      },
      {
        ...
        shouldShow = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            return (unit.owner and unit.owner.SID == LocalPlayer.state.Character:GetData("SID")) or exports['hrrp-jobs']:JobsPermissionsHasJob(unit.managedBy)
          end
        end,
      },
      {
        ...
        shouldShow = function()
          ...
          if nearUnit and nearUnit?.unitId then
            ...
            return exports['hrrp-jobs']:JobsPermissionsHasJob(unit.managedBy)
          end
        end,
      },
    })
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("StorageUnits:Passcode", function(code, cb)
...
_STORAGEUNITS = {
  Access = function(self)
    local nearUnit = StorageUnits:GetNearUnit()
    if nearUnit and nearUnit?.unitId then
      exports["hrrp-base"]:CallbacksServer("StorageUnits:Access", nearUnit?.unitId)
    end
  end,
  Manage = function(self, specificUnit)
    ...
    if nearUnit and nearUnit?.unitId then
      ...
      if unit then
            ...
            items = {
              {
                ...
                description = unit.lastAccessed and
                    string.format("Unit Last Accessed %s ago.", GetFormattedTimeFromSeconds(GlobalState["OS:Time"] - unit.lastAccessed)) or "Never",
              },
            }
          },
        }
        ...
        if exports['hrrp-jobs']:JobsPermissionsHasJob(unit.managedBy) then
          if unit.owner then
            table.insert(menu.main.items, {
            ...
            table.insert(menu.main.items, {
              label = "Unit Sold By",
              description = string.format("Sold By %s %s (State ID: %s) %s ago.", unit.soldBy.First, unit.soldBy.Last, unit.soldBy.SID,
                GetFormattedTimeFromSeconds(GlobalState["OS:Time"] - unit.soldAt)),
            })
          end
        end

        if exports['hrrp-jobs']:JobsPermissionsHasJob(unit.managedBy, false, false, false, false, "UNIT_SELL") then
          table.insert(menu.main.items, {
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("StorageUnits", _STORAGEUNITS)
end)
...
AddEventHandler("StorageUnits:Client:NewPasscode", function(values, data)
  if values and values.passcode and #values.passcode >= 4 then
    exports["hrrp-base"]:CallbacksServer("StorageUnits:ChangePasscode", {
      unit = data.unit,
      passcode = values.passcode,
    }, function(success)
      if success then
        exports['hrrp-hud']:NotificationSuccess("Updated Passcode")
      else
        exports['hrrp-hud']:NotificationError("Failed to Update Passcode")
      end
    end)
  end
end)
...
AddEventHandler("StorageUnits:Client:SellUnit", function(values, data)
  if values and values.SID then
    ...
    if stateId and stateId > 0 then
      exports["hrrp-base"]:CallbacksServer("StorageUnits:SellUnit", {
        unit = data.unit,
        SID = stateId,
      }, function(success)
        if success then
          exports['hrrp-hud']:NotificationSuccess("Storage Unit Sold")
        else
          exports['hrrp-hud']:NotificationError("Failed to Sell Storage Unit")
        end
      end)
    else
      exports['hrrp-hud']:NotificationError("Invalid State ID")
    end
  end
end)
