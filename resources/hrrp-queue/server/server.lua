...
QUEUE.Connect = function(self, source, playerName, setKickReason, deferrals)
  ...
  Citizen.CreateThread(function()
    ...
    if GlobalState.IsProduction then
      print("Definitely not in here")
      while GetGameTimer() < (_dbReadyTime + (Config.Settings.QueueDelay * (1000 * 60))) do
        local min = math.floor(((math.floor(_dbReadyTime / 1000) + (Config.Settings.QueueDelay * 60)) - math.floor(GetGameTimer() / 1000)) /
          60)
        local secs = math.floor(((math.floor(_dbReadyTime / 1000) + (Config.Settings.QueueDelay * 60)) - math.floor(GetGameTimer() / 1000)) -
          (min * 60))

        if min <= 0 then
          deferrals.update(string.format(Config.Strings.WaitingSeconds, secs, secs > 1 and "Seconds" or "Second"))
        else
          deferrals.update(string.format(Config.Strings.Waiting, min, min > 1 and "Minutes" or "Minute", secs,
    ...
    local roles = exports['hrrp-discord']:getRoles(source)
    if not roles then
      deferrals.done('⚠️ You must have Discord open to join this server.')
      return
    end
    ...
    for name, role in pairs(Config.Roles) do
      local hasRole = false
      ...
      if hasRole then
        table.insert(options, DeferralCards.Input:Choice({
          title = role.name,
          value = name
        }))
      end
        ...
        DeferralCards.Container:Create({
          items = {
            DeferralCards.CardElement:TextBlock({
              text = 'Which queue would you like to join?',
              weight = 'Light',
              size = 'large',
              horizontalAlignment = 'center',
            }),
            DeferralCards.CardElement:TextBlock({
              text = 'The below queue options are determined by your Discord roles. Contact support if this information is incorrect.',
              weight = 'normal',
              size = 'small',
              horizontalAlignment = 'center',
            }),
            DeferralCards.CardElement:TextBlock({
              text = 'Please note: If you join the Whitelist Job Queue you will only be able to select a character for that job.',
              weight = 'normal',
              size = 'small',
              horizontalAlignment = 'center',
            }),
    ...
    local joined_queue = false
    local priority_points = 0
    deferrals.presentCard(card, function(pData, pRawData)
      if pData.submitId == 'submit_join' then
        -- exports['hrrp-base']:UtilsPrint(pRawData)
        if Config.Roles[pRawData.role] then
          priority_points = Config.Roles[pRawData.role].points
          if Config.Roles[pRawData.role].whitelist_job then
          end
          --we only allow them to select characters with that job
        end
        answered = true
        joined_queue = true
      elseif pData.submitId == 'submit_cancel' then
        deferrals.done('Cancelling the queue...')
      end
    end)

    while not answered do Wait(1500) end
    ...
    while plyr.State == States.QUEUED and GetPlayerEndpoint(source) do
      ...
      if priority_points > 0 then
        if plyr.TimeBoost > 0 then
          ...
          msg = "\n\nTotal Priority of " .. priority_points .. ": " .. plyr.Message
        end
      end
...
AddEventHandler("onResourceStart", function(resource)
  if resource == "hrrp-base" then
    while GetResourceState(resource) ~= "started" do
      Citizen.Wait(0)
    end
    for plySource, ply in pairs(Data.Session.Players) do
      TriggerClientEvent("Queue:Client:SessionActive", plySource)
      TriggerEvent("Queue:Server:SessionActive", plySource, {
        Groups = ply.Groups,
        Name = ply.Name,
        ID = ply.ID,
...
local setupEventHuh = nil
setupEventHuh = AddEventHandler("Core:Shared:Ready", function()
  _dbReadyTime = GetGameTimer()
  _dbReady = true

  RemoveEventHandler(setupEventHuh)
end)
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Queue", QUEUE)
end)

AddEventHandler("Queue:Shared:DependencyUpdate", RetrieveComponents)
function RetrieveComponents()
  c = exports["hrrp-base"]:FetchComponent("Config")

  WebAPI = exports["hrrp-base"]:FetchComponent("WebAPI")
  Punishment = exports["hrrp-base"]:FetchComponent("Punishment")
  Queue = exports["hrrp-base"]:FetchComponent("Queue")
  Convar = exports["hrrp-base"]:FetchComponent("Convar")
...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Queue", {
    "Config",
    ...
    Chat:RegisterAdminCommand("tempwl", function(source, args, rawCommand)
      ...
      if accountID then
        if not _tempWhitelist[accountID] then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Temp Whitelist Added For ID: " .. accountID)
        else
          _tempWhitelist[accountID] = false
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Temp Whitelist Revoked For ID: " .. accountID)
        end
      end
    end, {
    ...
    Chat:RegisterAdminCommand("tempwlgroup", function(source, args, rawCommand)
      ...
      if group then
        if not _tempWhitelistGroup[group] then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Temp Whitelist Added For Group: " .. group)
        else
          _tempWhitelistGroup[group] = false
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Temp Whitelist Revoked For Group: " .. group)
        end
      end
    end, {
...
Citizen.CreateThread(function()
  ...
  if not exports["hrrp-base"]:FetchComponent("WebAPI").Enabled then
