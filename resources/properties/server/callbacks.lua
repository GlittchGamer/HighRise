...
function RegisterCallbacks()
  exports['core']:CallbacksRegisterServer("Properties:RingDoorbell", function(source, data, cb)
    TriggerClientEvent("Properties:Client:Doorbell", -1, data)
    cb()
  end)

  exports['core']:CallbacksRegisterServer("Properties:RequestAgent", function(source, data, cb)
    ...
    if property ~= nil then
      for k, v in pairs(Fetch:All()) do
        ...
        if c ~= nil then
          if exports['jobs']:JobsPermissionsHasPermissionInJob(v:GetData("Source"), "realestate", "JOB_SELL") then
            Phone.Email:Send(
              v:GetData("Source"),
  ...
  exports['core']:CallbacksRegisterServer("Properties:EditProperty", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Properties:SpawnInside", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    ...
    if property ~= nil and char then
      ...
      exports['core']:MiddlewareTriggerEvent("Properties:Enter", source, data.id)
  ...
  exports['core']:CallbacksRegisterServer("Properties:EnterProperty", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local property = _properties[data]

    if
        (property.keys ~= nil and property.keys[char:GetData("ID")])
        or (not property.sold and exports['jobs']:JobsPermissionsHasPermissionInJob(source, "realestate", "JOB_DOORS"))
    ...
    then
      ...
      exports['core']:MiddlewareTriggerEvent("Properties:Enter", source, data)
  ...
  exports['core']:CallbacksRegisterServer("Properties:ExitProperty", function(source, data, cb)
    local property = GlobalState[string.format("%s:Property", source)]

    exports['pwnzor']:PwnzorPlayersTempPosIgnore(source)
    exports['core']:MiddlewareTriggerEvent("Properties:Exit", source, property)
  ...
  exports['core']:CallbacksRegisterServer("Properties:ChangeLock", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
  ...
  exports['core']:CallbacksRegisterServer("Properties:Validate", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
  ...
  exports['core']:CallbacksRegisterServer("Properties:Upgrade", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
          ...
          if nextLevel and nextLevel.price and p and p.Account then
            ...
            if success then
              ...
              if not upgraded then
                exports['core']:LoggerError("Properties",
  ...
  exports['core']:CallbacksRegisterServer("Properties:ChangeInterior", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
  ...
  exports['core']:CallbacksRegisterServer("Properties:Dyn8:Search", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char then
  ...
  exports['core']:CallbacksRegisterServer("Properties:Dyn8:Sell", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData('Character')
    local prop = _properties[data.property]
    if Player(source).state.onDuty == 'realestate' then
      if prop ~= nil and not prop.sold and char then
        if _selling[data.property] == nil then
          local target = exports['core']:FetchSID(tonumber(data.target))
          if target ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Properties:Dyn8:CheckCredit", function(source, data, cb)
    local target = exports['core']:FetchSID(tonumber(data?.target))
    if target ~= nil then
      local targetChar = target:GetData('Character')
      if targetChar then
  ...
  exports['core']:CallbacksRegisterServer("Properties:Dyn8:Transfer", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData('Character')
    local prop = _properties[data.property]
    if Player(source).state.onDuty == 'realestate' then
      if prop ~= nil and prop.sold and char then
        local owner = exports['core']:FetchSID(prop.owner.SID)
        local newOwner = exports['core']:FetchSID(tonumber(data.target))
