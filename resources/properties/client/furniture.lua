    ...
    if LoadModel(model) then
        ...
        if furnData then
            if _specCategories[furnData.cat] then
                ...
                exports['ox_target']:TargetingAddEntity(obj, icon, menu)
            end
        end
        ...
        if state then
            for k, v in ipairs(_spawnedFurniture) do
                if not v.targeting then
                    exports['ox_target']:TargetingAddEntity(v.entity, "draw-square", {
                        {
                            icon = "arrows-up-down-left-right",
                            text = "Move",
                            event = "Furniture:Client:OnMove",
...
function CycleFurniture(direction)
    ...
    exports['objects']:Start(GetHashKey(fKey), "Furniture:Client:Place", {}, true, "Furniture:Client:Cancel", true, fData.placeGround)
end

AddEventHandler("Furniture:Client:Place", function(data, placement)
    if _placingFurniture then

        exports['core']:CallbacksServer("Properties:PlaceFurniture", {
            ...
            if success then
                exports['hud']:NotificationSuccess("Placed Item")
            else
...
AddEventHandler("Furniture:Client:Move", function(data, placement)
    if _placingFurniture and data.id then
        ...
        exports['core']:CallbacksServer("Properties:MoveFurniture", {
            id = data.id,
            coords = {
                x = placement.coords.x,
                y = placement.coords.y,
            ...
            if success then
                exports['hud']:NotificationSuccess("Moved Item")
            else
                exports['hud']:NotificationError("Error")
            end
...
AddEventHandler("Furniture:Client:CancelMove", function(data)
    if _placingFurniture and data.id then
        ...
        exports['hud']:NotificationError("Move Cancelled")
        _placingFurniture = false
        LocalPlayer.state.placingFurniture = false
        if not _skipPhone then
            Phone:Open()
