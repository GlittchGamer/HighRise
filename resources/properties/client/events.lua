...
RegisterNetEvent("Properties:Client:InnerStuff", function(propertyData, int, furniture)
  ...
  -- 		exports["animations"]:EmotesWakeUp(intr.locations.wakeup)
  -- 	end)
  -- end
...
AddEventHandler("Properties:Client:Duty", function(t, data)
  ...
  if property?.data?.jobDuty then
    if LocalPlayer.state.onDuty == property?.data?.jobDuty then
      exports['jobs']:JobsDutyOff(property?.data?.jobDuty)
    else
      exports['jobs']:JobsDutyOn(property?.data?.jobDuty)
    end
  end
end)
  ...
  if ownedProps then
    for k, v in ipairs(ownedProps) do
      if v.type == 'house' then
        exports['blips']:Add('property-' .. v.id, 'House: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 40, 53, 0.6, 2)
      elseif v.type == 'office' then
        exports['blips']:Add('property-' .. v.id, 'Office: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 475, 53, 0.6,
          2)
      elseif v.type == 'warehouse' then
        exports['blips']:Add('property-' .. v.id, 'Warehouse: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 473, 53,
          0.6, 2)
      end
    end
  end
