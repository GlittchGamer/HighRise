...
AddEventHandler("Drugs:Client:Startup", function()
  for k, v in ipairs(_tableModels) do
    exports['ox_target']:TargetingAddObject(v, "table-picnic", {
      {
        text = "Pickup Table",
        icon = "hand",
      ...
      },
      {
        ...
        isEnabled = function(data, entity)
          local entState = Entity(entity.entity).state
          return entState?.isMethTable and (not _methTables[entState.methTable]?.cooldown or GetCloudTimeAsInt() > _methTables[entState.methTable]?.cooldown) and
      ...
      },
      {
        ...
        isEnabled = function(data, entity)
          local entState = Entity(entity.entity).state
          return entState?.isMethTable and _methTables[entState?.methTable]?.activeCook and _methTables[entState?.methTable]?.pickupReady and
              (_methTables[entState.methTable].owner == nil or _methTables[entState.methTable].owner == LocalPlayer.state.Character:GetData("SID"))
        end,
      },
    }, 3.0)
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Drugs:Meth:PlaceTable", function(data, cb)
    exports['hrrp-objects']:Start(`bkr_prop_meth_table01a`, "Drugs:Client:Meth:FinishPlacement", data, false)
    cb()
  end)

  exports["hrrp-base"]:CallbacksRegisterClient("Drugs:Meth:Use", function(data, cb)
    Citizen.Wait(400)
...
AddEventHandler("Drugs:Client:Meth:FinishPlacement", function(data, endCoords)
  ...
  Progress:Progress({
    ...
    if not status then
      exports["hrrp-base"]:CallbacksServer("Drugs:Meth:FinishTablePlacement", {
        data = data,
        endCoords = endCoords
      }, function(s)
        -- if s then
...
AddEventHandler("Drugs:Client:Meth:PickupTable", function(entity, data)
  if Entity(entity.entity).state?.isMethTable then
    Progress:Progress({
      ...
      if not status then
        exports["hrrp-base"]:CallbacksServer("Drugs:Meth:PickupTable", Entity(entity.entity).state.methTable, function(s)
          -- if s then
...
AddEventHandler("Drugs:Client:Meth:StartCook", function(entity, data)
  ...
  if entState.isMethTable and entState.methTable then
    exports["hrrp-base"]:CallbacksServer("Drugs:Meth:CheckTable", entState.methTable, function(s)
      if s then
        Progress:Progress({
          name = "meth_pickup",
          duration = 5 * 1000,
        ...
        exports['hrrp-hud']:NotificationError("Table Is Not Ready")
      end
    end)
  end
end)
        ...
        Progress:Progress({
          ...
          if not status then
            Progress:Progress({
              ...
              if not status then
                exports["hrrp-base"]:CallbacksServer("Drugs:Meth:StartCooking", data, function(s)
...
AddEventHandler("Drugs:Client:Meth:PickupCook", function(entity, data)
  ...
  if entState.isMethTable and entState.methTable then
    Progress:Progress({
      ...
      if not status then
        exports["hrrp-base"]:CallbacksServer("Drugs:Meth:PickupCook", entState.methTable, function(s)
          if s then
          else
            exports['hrrp-hud']:NotificationError("Table Is Not Ready")
          end
...
AddEventHandler("Drugs:Client:Meth:TableDetails", function(entity, data)
  local entState = Entity(entity.entity).state
  if entState.isMethTable and entState.methTable then
    exports["hrrp-base"]:CallbacksServer("Drugs:Meth:GetTableDetails", entState.methTable, function(s)
...
AddEventHandler("Drugs:Client:Meth:ViewItems", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Drugs:Meth:GetItems", {}, function(items)
    local itemList = {}
...
AddEventHandler("Drugs:Client:Meth:BuyItem", function(data)
  exports["hrrp-base"]:CallbacksServer("Drugs:Meth:BuyItem", data)
end)
