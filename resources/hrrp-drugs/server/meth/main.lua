...
AddEventHandler("Drugs:Server:Startup", function()
  ...
  exports['hrrp-base']:LoggerTrace("Drugs:Meth", string.format("Restored ^2%s^7 Meth Tables", #tables))
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:FinishTablePlacement", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:PickupTable", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:CheckTable", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:StartCooking", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        if data and _placedTables[data.tableId] ~= nil then
          if _placedTables[data.tableId].cooldown == nil or os.time() > _placedTables[data.tableId].cooldown then
            ...
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", string.format("Batch Started, Will Be Ready In %s Minutes", data.cookTime))
            cb(true)
          else
            cb(false)
          end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:PickupCook", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:GetTableDetails", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
        if data and _placedTables[data] ~= nil then
          ...
          if tableData.cooldown ~= nil then
            ...
            if timeUntil > 0 then
              str = str ..
                  string.format("<li><b>Cooldown:</b> Available %s (in about %s)</li>", os.date("%m/%d/%Y %I:%M %p", tableData.cooldown),
          ...
          if tableData.active_cook ~= nil then
            ...
            if timeUntil > 0 then
              str = str ..
                  string.format("<li><b>Cook Status:</b> Finished At %s (in about %s)</li>", os.date("%m/%d/%Y %I:%M %p", cook.end_time),
                    GetFormattedTimeFromSeconds(timeUntil))
            else
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:GetItems", function(source, data, cb)
    local itms = {}
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Drugs:Meth:BuyItem", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local hasVpn = hasValue(char:GetData("States"), "PHONE_VPN")
    ...
    for k, v in ipairs(_toolsForSale) do
      if v.id == data then
        ...
        if Crypto.Exchange:Remove(v.coin, char:GetData("CryptoWallet"), v.price) then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", string.format("Not Enough %s", coinData.Name), 6000)
        end
      end
    end
  end)
