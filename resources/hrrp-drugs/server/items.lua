...
function RegisterItemUse()
  Inventory.Items:RegisterUse("meth_table", "DrugShit", function(source, slot, itemData)
    exports["hrrp-base"]:CallbacksClient(source, "Drugs:Meth:PlaceTable", slot.id, function() end)
  end)
        ...
        if _effectCds.meth[char:GetData("SID")] == nil or os.time() > _effectCds.meth[char:GetData("SID")] then
          ...
          if methItem?.id ~= nil then
            ...
            if Inventory.Items:RemoveId(char:GetData("SID"), 1, methItem) then
              exports["hrrp-base"]:CallbacksClient(source, "Drugs:Meth:Use", methItem.Quality, function(s)
                if s then
            ...
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Need Meth To Smoke")
          end
        else
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Cannot Use That Yet")
        end
      end
    end
  end)
  ...
  -- 				_effectCds.meth[char:GetData("SID")] = os.time() + (60 * 1)
  -- 				if Inventory.Items:RemoveSlot(slot.Owner, slot.Name, 1, slot.Slot, slot.invType) then
  -- 					exports["hrrp-base"]:CallbacksClient(source, "Drugs:Meth:Use", slot.Quality, function(s)
  -- 						if s then
  -- 							Drugs.Addiction:Add(source, "Meth", 0.25)
    ...
    if plyr ~= nil then
      ...
      if char ~= nil then
        if _effectCds.coke[char:GetData("SID")] == nil or os.time() > _effectCds.coke[char:GetData("SID")] then
          ...
          if Inventory.Items:RemoveSlot(slot.Owner, slot.Name, 1, slot.Slot, slot.invType) then
            exports["hrrp-base"]:CallbacksClient(source, "Drugs:Coke:Use", slot.Quality, function(s)
              if s then
