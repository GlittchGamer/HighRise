COMPONENTS.Punishment = {
  ...
  Kick = function(self, source, reason, issuer, afk)
    local tPlayer = COMPONENTS.exports['hrrp-base']:FetchSource(source)
    if not tPlayer then
    ...
    if issuer ~= "Pwnzor" then
      ...
      local iPlayer = COMPONENTS.exports['hrrp-base']:FetchSource(issuer)
      if not iPlayer then
        return { success = false }
      end
...
COMPONENTS.Punishment.Ban = {
  Source = function(self, source, expires, reason, issuer)
    ...
    if issuer ~= "Pwnzor" then
      ...
      iPlayer = COMPONENTS.exports['hrrp-base']:FetchSource(issuer)
      if not iPlayer then
        return { success = false }
      end
      if iPlayer.Permissions:GetLevel() < tPlayer.Permissions:GetLevel() then
