      ...
      if type(t) ~= 'table' then
        ...
        for k, v in pairs(t) do
          ...
          if type(v) == 'table' then
            ...
            COMPONENTS.Logger:Trace("Utils", " ^1[^3debug^1] [" .. type(t) .. "]^7 " .. (s or '') .. kfmt .. ' = ' ..
              vfmt, { console = true })
          end
        end
      end
    else
      COMPONENTS.Logger:Error("Utils", "^1Error Printing Request - The Passed through variable seems to be nil^7",
        { console = true })
    end
  end,
...
for k, v in pairs(COMPONENTS.Utils) do
  if type(v) == "function" then
    exportHandler("Utils" .. k, v)
  elseif type(v) == "table" then
    createExportForObject(v, "Utils" .. k)
  end
end
