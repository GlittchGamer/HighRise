...
COMPONENTS.WaitList = {
  PrintQueue = function(self, id)
    if _queues[id] ~= nil then
      for k, v in ipairs(_queues[id].queue) do
        local plyr = COMPONENTS.exports['hrrp-base']:FetchSource(v.source)
        if plyr ~= nil then
...
local function exportHandler(exportName, func)
  AddEventHandler(('__cfx_export_%s_%s'):format(GetCurrentResourceName(), exportName), function(setCB)
    setCB(function(...)
      return func(COMPONENTS.WaitList, ...)
    end)
  end)
end
...
for k, v in pairs(COMPONENTS.WaitList) do
  if type(v) == "function" then
    exportHandler("WaitList" .. k, v)
  elseif type(v) == "table" then
    createExportForObject(v, "WaitList" .. k)
  end
end
