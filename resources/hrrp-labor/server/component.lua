...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Labor", {
    ...
    if #error > 0 then
      exports['hrrp-base']:LoggerCritical("Labor", "Failed To Load All Dependencies")
      return
    end
    RetrieveComponents()
    RegisterCallbacks()
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Labor", _LABOR)
end)
          ...
          if v.Joiner == joiner then
            if v.Group then
              for k, v in ipairs(_Groups) do
                if v.Creator.ID == joiner then
                  for k2, v2 in ipairs(v.Members) do
                    local plyr = exports['hrrp-base']:FetchSource(v2.ID)
                    if plyr ~= nil then
