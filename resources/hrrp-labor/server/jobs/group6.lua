...
_G6 = {
  ...
  Inventory = {
    ...
    DepositBag = function(self, playerSrc, VIN)
      local char = exports['hrrp-base']:FetchSource(playerSrc):GetData("Character")
...
AddEventHandler("Labor:Server:Startup", function()
  exports['hrrp-base']:CallbacksRegisterServer("Group6:StartJob", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Group6:VehicleSpawn", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Group6:DepositBag', function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Group6:OpenSafe', function(source, data, cb)
    if not data then
  ...
  exports['hrrp-base']:CallbacksRegisterServer('Group6:GrabBag', function(source, data, cb)
    if _joiners[source] and _group6[_joiners[source]].truck and _group6[_joiners[source]].state == 2 then
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You are not on the job")
      cb(false)
    end
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Group6:DespawnVehicle", function(source, data, cb)
  ...
  RegisterNetEvent('Group6:Server:CrackSafe', function(netID)
    ...
    if count and count > 0 then
      if Inventory.Items:Remove(char:GetData("SID"), 1, itemNeeded, 1) then
        ...
        exports["hrrp-base"]:CallbacksClient(source, "Group6:Client:CrackSafe", netID, function(success)
          if success then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Failed to remove the item")
        return
      end
    else
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You do not have the item")
      return
    end
  end)
end)
