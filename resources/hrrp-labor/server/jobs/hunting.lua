...
AddEventHandler("Labor:Server:Startup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Hunting:StartJob", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Hunting:FinishJob", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Hunting:HarvestAnimal", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Hunting:GenerateAnimal", function(source, data, cb)
    exports["hrrp-base"]:CallbacksClient(source, "exports['hrrp-polyzone']:PolyZoneGetZonePlayerIn", {}, function(zone)
      if zone and string.sub(zone.id, 1, string.len("hunting")) == "hunting" then
        ...
        if HuntingConfig.Zones[zoneId] ~= nil then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Allowed To Hunt Here")
        end
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Allowed To Hunt Here")
      end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Hunting:Sell", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if _saleData[data] ~= nil then
      if repLvl >= _saleData[data].rep then
        ...
        if (count) > 0 then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Have No Hides Of That Tier")
        end
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Must Prove Yourself First")
      end
    end
  end)
end)
      ...
      then
        ...
        exports["hrrp-base"]:CallbacksClient(source, "exports['hrrp-polyzone']:PolyZoneGetZonePlayerIn", {}, function(zone)
          if zone and string.sub(zone.id, 1, string.len("hunting")) == "hunting" then
            local zoneId = tonumber(string.sub(zone.id, string.len("hunting") + 1))
            if HuntingConfig.Zones[zoneId] ~= nil then
              exports["hrrp-base"]:CallbacksClient(source, "Hunting:PlaceTrap", item.Name, function(successful)
                if successful then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Cannot Use Bait Yet")
      end
    end)
  end
end
