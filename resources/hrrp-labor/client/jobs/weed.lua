...
RegisterNetEvent("WeedRun:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["receive"] = RegisterNetEvent(string.format("WeedRun:Client:%s:Receive", joiner), function(location, pedModel)
    _state = 1
    _blip = exports['hrrp-blips']:Add("WeedRun", "Buyer", location, 514, 11, 0.9, 2, false, true)
    SetNewWaypoint(location.x, location.y)
    exports['hrrp-pedinteraction']:Add("WeedDelivery", pedModel, vector3(location[1], location[2], location[3]), location[4], 50.0, {
      {
...
AddEventHandler("WeedRun:Client:Deliver", function()
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("WeedRun:StartDropoff", {}, function(r)
        Progress:Progress({
          name = 'weed-sale-2',
          ...
          if not cancelled then
            exports["hrrp-base"]:CallbacksServer("WeedRun:DoDropoff", {}, function(r2)
              if r2 then
                exports['hrrp-blips']:Remove("WeedRun")
                _state = 2
              end
            end)
          end
...
AddEventHandler("WeedRun:Client:Enable", function()
  exports["hrrp-base"]:CallbacksServer('WeedRun:Enable', {})
end)

AddEventHandler("WeedRun:Client:Disable", function()
  exports["hrrp-base"]:CallbacksServer('WeedRun:Disable', {})
end)

AddEventHandler("WeedRun:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer('WeedRun:StartJob', _joiner, function(state)
    if not state then
...
RegisterNetEvent("WeedRun:Client:OffDuty", function(time)
  ...
  exports['hrrp-pedinteraction']:Remove("WeedDelivery")
  exports['hrrp-blips']:Remove("WeedDelivery")
end)
