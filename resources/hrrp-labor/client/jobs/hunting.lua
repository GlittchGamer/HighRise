...
function StartBait(item, loc)
  ...
  Citizen.CreateThread(function()
    ...
    if _baited and LocalPlayer.state.loggedIn then
      ...
      Citizen.Wait(t)
      exports["hrrp-base"]:CallbacksServer("Hunting:GenerateAnimal", item, function(r)
        if r ~= nil then
...
RegisterNetEvent("Hunting:Client:Polys", function(c)
  ...
  for k, v in ipairs(_localConfig.Zones) do
    exports['hrrp-polyzone']:PolyZoneCreateCircle(string.format("hunting%s", k), v.coords, v.radius, v.options)
  end

  for k, v in pairs(_localConfig.Animals) do
    exports['ox_target']:TargetingAddPedModel(v.Model, "knife-kitchen", {
      {
        icon = "rabbit-running",
...
AddEventHandler("Labor:Client:Setup", function()
  for k, v in ipairs(_sellers) do
    exports['hrrp-pedinteraction']:Add(string.format("HideSeller%s", k), v.model, v.coords, v.heading, 25.0, {
      {
        icon = "sack-dollar",
  ...
  exports['hrrp-pedinteraction']:Add("HuntingJob", `cs_hunter`, vector3(-773.955, 5604.745, 32.741), 167.427, 25.0, {
    {
      icon = "cart-shopping",
      text = "Shop",
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Hunting:PlaceTrap", function(item, cb)
    if _baited then
      return cb(false)
    end
    ...
    if hit then
      if gms[materialHash] then
        ...
        exports['hrrp-hud']:NotificationError("Cannot Place Trap Here")
        cb(false)
      end
    else
      exports['hrrp-hud']:NotificationError("Cannot Place Trap Here")
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Hunting:SpawnAnimal", function(item, cb)
    if _baited then
      return cb(false)
    end
...
RegisterNetEvent("Hunting:Client:OnDuty", function(joiner, time)
  ...
  _blip = exports['hrrp-blips']:Add("HuntingStart", "Shop Owner", { x = -773.955, y = 5604.745, z = 32.741 }, 480, 2, 1.4)
  ...
  eventHandlers["startup"] = RegisterNetEvent(string.format("Hunting:Client:%s:Startup", joiner), function()
    ...
    if _blip ~= nil then
      exports['hrrp-blips']:Remove("HuntingStart")
      RemoveBlip(_blip)
    end
  end)
...
AddEventHandler("Hunting:Client:Sell", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Hunting:Sell", data.tier)
end)

AddEventHandler("Hunting:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer("Hunting:StartJob", _joiner, function(state)
    if not state then
...
AddEventHandler("Hunting:Client:FinishJob", function()
  exports["hrrp-base"]:CallbacksServer("Hunting:FinishJob", _joiner, function(state)
...
AddEventHandler("Hunting:Client:Harvest", function(entity, data)
  ...
  Progress:ProgressWithTickEvent({
    ...
    if not cancelled and not DecorGetBool(entity.entity, "HuntingHarvested") then
      ...
      exports["hrrp-base"]:CallbacksServer("Hunting:HarvestAnimal", data)
    end
...
AddEventHandler("Hunting:Client:StartJob", function()
  -- exports["hrrp-base"]:CallbacksServer("Mining:StartJob", _joiner, function(state)
  -- 	if not state then
  -- 		exports['hrrp-hud']:NotificationError("Unable To Start Job")
  -- 	end
  -- end)
