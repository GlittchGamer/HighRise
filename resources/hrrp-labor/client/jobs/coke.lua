...
function SpawnPeds()
  ...
  loadModel(_drop.ped)
  for i = 1, 25 do
    local coords = vector3(_drop.coords.x + math.random(-math.abs(math.ceil(_drop.size / 2)), math.ceil(_drop.size / 2)),
      _drop.coords.y + math.random(-math.abs(math.ceil(_drop.size / 2)), math.ceil(_drop.size / 2)), _drop.coords.z)
...
AddEventHandler("Coke:Client:StartWork", function()
  exports["hrrp-base"]:CallbacksServer("Coke:StartWork")
end)

AddEventHandler("Coke:Client:Abort", function()
  exports["hrrp-base"]:CallbacksServer("Coke:Abort")
end)
...
RegisterNetEvent("Coke:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["receive"] = RegisterNetEvent(string.format("Coke:Client:%s:Receive", joiner), function()
    _state = 1
    _blip = exports['hrrp-blips']:Add("CokeDrop", "Unknown Contact", { x = 4495.498, y = -4514.340, z = 3.021 }, 306, 2, 1.4)
  end)
  ...
  eventHandlers["poly-enter"] = AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZone, data)
    if id == "cayo_perico" and _state == 1 then
      exports["hrrp-base"]:CallbacksServer("Coke:ArriveAtCayo", {}, function() end)
    elseif id == "CokeDrop" and _state == 3 and not _spawned and _joiner == GetPlayerServerId(LocalPlayer.state.PlayerID) then
      exports["hrrp-base"]:CallbacksServer("Coke:ArrivedAtPoint", {}, function()
        SpawnPeds(data)
  ...
  eventHandlers["poly-exit"] = AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZone, data)
    if id == "cayo_perico" then
      if _state == 5 then
        exports["hrrp-base"]:CallbacksServer("Coke:LeftCayo", {}, function() end)
      else
  ...
  eventHandlers["cayo-start"] = AddEventHandler("Coke:Client:StartHeist", function()
    exports["hrrp-base"]:CallbacksServer("Coke:StartHeist", {}, function() end)
  end)

  eventHandlers["cayo-setup"] = RegisterNetEvent(string.format("Coke:Client:%s:SetupHeist", joiner), function(drop)
    _state = 3
    _drop = drop
    exports['hrrp-blips']:Remove("CokeDrop")
    _blip = exports['hrrp-blips']:Add("CokeDrop", "Unknown Objective", { x = _drop.coords.x, y = _drop.coords.y, z = _drop.coords.z }, 306, 2, 1.4)
    ...
    exports['hrrp-polyzone']:PolyZoneCreateCircle("CokeDrop", _drop.coords, _drop.size * 2.0, {
      --debugPoly = true
    }, _drop)
  end)
  ...
  eventHandlers["go-back"] = RegisterNetEvent(string.format("Coke:Client:%s:GoBack", joiner), function()
    ...
    exports['hrrp-polyzone']:PolyZoneRemove("CokeDrop")
    _blip = exports['hrrp-blips']:Add("CokeDrop", "Unknown Contact", { x = 1292.455, y = -3170.885, z = 4.906 }, 306, 2, 1.4)
  end)
  ...
  eventHandlers["finish"] = AddEventHandler("Coke:Client:Finish", function()
    exports["hrrp-base"]:CallbacksServer("Coke:Finish", {}, function() end)
  end)
end)

AddEventHandler("Coke:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer("Coke:StartJob", _joiner, function(state)
    if not state then
...
RegisterNetEvent("Coke:Client:OffDuty", function(time)
  ...
  exports['hrrp-pedinteraction']:Remove("CokeDrop")
  exports['hrrp-polyzone']:PolyZoneRemove("CokeDrop")
  exports['hrrp-blips']:Remove("CokeDrop")
  RemoveBlip(_blipArea)
