...
AddEventHandler("Labor:Client:Setup", function()
  ...
  exports['hrrp-pedinteraction']:Add("OxyRunner", `s_m_m_movspace_01`, _queueLoc.coords, _queueLoc.heading, 25.0, {
    {
      icon = "tablets",
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("OxyRun:GetSpawn", function(data, cb)
    ...
    local success, spawn, heading = GetNthClosestVehicleNodeFavourDirection(offset.x, offset.y, offset.z, _l.coords.x, _l.coords.y, _l.coords.z, 20, 0,
      0x40400000, 0)
        ...
        Citizen.CreateThread(function()
          ...
          if forceEnd or _psychoShit ~= nil and _psychoShit.veh == VehToNet(vehicle) and not _fuckedOff then
            ...
            Citizen.SetTimeout(30000, function()
              exports["hrrp-base"]:CallbacksServer("OxyRun:DeleteShit", _psychoShit)
            end)
          end
        end)
...
--         exports["hrrp-base"]:CallbacksServer("OxyRun:DestroyVehicle")
--       end
-- 	end
-- end)
...
RegisterNetEvent("OxyRun:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["primary_action"] = AddEventHandler("Keybinds:Client:KeyUp:primary_action", function()
    if _working and _state == 3 and LocalPlayer.state.inOxyPickup and not LocalPlayer.state.doingAction then
      ...
      if veh == _v.ent and GetPedInVehicleSeat(_v.ent, -1) == LocalPlayer.state.ped then
        ...
        exports["hrrp-base"]:CallbacksServer("OxyRun:CheckPickup", {}, function(s)
          if s then
            Progress:ProgressWithTickEvent({
              ...
              if _state ~= 3 or veh ~= _v.ent or GetPedInVehicleSeat(_v.ent, -1) ~= LocalPlayer.state.ped then
                Progress:Cancel()
                exports["hrrp-base"]:CallbacksServer("OxyRun:CancelPickup")
                return
              end

              exports["hrrp-base"]:CallbacksServer("OxyRun:PickupProduct")
            end, function(cancelled)

            end)
          else
            exports['hrrp-hud']:NotificationError("Not Enough Room In Your Trunk")
          end
        end)
      end
    end
  ...
  eventHandlers["poly-enter"] = AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
    if _working and id == "OxyPickup" then
      ...
      if _state == 2 then
        exports["hrrp-base"]:CallbacksServer("OxyRun:EnteredPickup")
      else
        local veh = GetVehiclePedIsIn(LocalPlayer.state.ped)
        if _state == 3 and veh == _v.ent and GetPedInVehicleSeat(_v.ent, -1) == LocalPlayer.state.ped then
          exports['hrrp-hud']:ActionShow("{keybind}primary_action{/keybind} Load Product")
        end
      end
  ...
  eventHandlers["poly-exit"] = AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZones, data)
    ...
    elseif _working and id == "OxySale" and LocalPlayer.state.loggedIn then
      ...
      exports["hrrp-base"]:CallbacksServer("OxyRun:LeftZone")
    end
  end)

  eventHandlers["entered-car"] = RegisterNetEvent('Vehicles:Client:EnterVehicle', function(veh)
    exports["hrrp-base"]:CallbacksServer("OxyRun:EnteredCar", {
  ...
  eventHandlers["start-pickup"] = RegisterNetEvent(string.format("OxyRun:Client:%s:StartPickup", joiner), function(pu, veh)
    ...
    _blip = exports['hrrp-blips']:Add("OxyRun", "Oxy Pickup", { x = pu.coords.x, y = pu.coords.y, z = pu.coords.z }, 51, 64, 0.9)
    exports['hrrp-polyzone']:PolyZoneCreateBox("OxyPickup", pu.coords, pu.length, pu.width, pu.options)
        ...
        if not ending then
          if _v?.ent ~= nil and DoesEntityExist(_v.ent) then
            if GetEntityHealth(_v.ent) == 0 or not IsVehicleDriveable(_v.ent) then
              exports['hrrp-base']:LoggerTrace("OxyRun", "Vehicle Health 0 or Not Drivable")
              ending = true
              exports["hrrp-base"]:CallbacksServer("OxyRun:DestroyVehicle")
            end
          end
  ...
  eventHandlers["start-sale"] = RegisterNetEvent(string.format("OxyRun:Client:%s:StartSale", joiner), function(location)
    ...
    if _blip then
      exports['hrrp-blips']:Remove("OxyRun")
    end
    _blip = exports['hrrp-blips']:Add("OxyRun", "Oxy Sale", { x = _l.coords.x, y = _l.coords.y, z = _l.coords.z }, 51, 64, 0.9)
    ...
    exports['hrrp-polyzone']:PolyZoneCreateCircle("OxySale", _l.coords, _l.radius, _l.options)
    ...
    Citizen.CreateThread(function()
      while _working and _state == 4 do
        ...
        if dist <= 10.0 then
          exports["hrrp-base"]:CallbacksServer("OxyRun:EnteredArea")
        end
  ...
  eventHandlers["target"] = RegisterNetEvent("OxyRun:Client:MakeSale", function(data)
    ...
    exports["hrrp-base"]:CallbacksServer("OxyRun:SellProduct", _psychoShit.veh, function(s)
      if (s) then
  ...
  eventHandlers["veh-poofed"] = RegisterNetEvent(string.format("OxyRun:Client:%s:VehiclePoofed", joiner), function()
    exports["hrrp-base"]:CallbacksServer("OxyRun:VehiclePoofed")
  end)
end)

AddEventHandler("OxyRun:Client:Enable", function()
  exports["hrrp-base"]:CallbacksServer('OxyRun:Enable', {})
end)

AddEventHandler("OxyRun:Client:TurnIn", function()
  exports["hrrp-base"]:CallbacksServer('OxyRun:TurnIn', _joiner)
end)

AddEventHandler("OxyRun:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer('OxyRun:StartJob', _joiner, function(state)
    if not state then
...
RegisterNetEvent("OxyRun:Client:OffDuty", function(time)
  ...
  exports['hrrp-polyzone']:PolyZoneRemove("OxyPickup")
  exports['hrrp-polyzone']:PolyZoneRemove("OxySale")
  if _blip then
    exports['hrrp-blips']:Remove("OxyRun")
  end
