...
AddEventHandler("Labor:Client:Setup", function()
  exports['hrrp-polyzone']:PolyZoneCreatePoly(_POLYID, {
  ...
  exports['hrrp-pedinteraction']:Add("SalvagingJob", `s_m_y_construct_02`, vector3(2369.771, 3157.132, 47.209), 10.633, 25.0, {
    {
      icon = "helmet-safety",
      text = "Start Work",
...
RegisterNetEvent("Salvaging:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["startup"] = RegisterNetEvent(string.format("Salvaging:Client:%s:Startup", joiner), function()
    ...
    eventHandlers["enter-poly"] = AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
      ...
      for k, v in ipairs(_models) do
        ...
        exports['ox_target']:TargetingAddObject(v, "car", {
          {
            icon = "engine",
            text = "Scrap",
    ...
    if not _inPoly and exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped, false, _POLYID, true)) and _count < 15 then
      for k, v in ipairs(_models) do
        exports['ox_target']:TargetingRemoveObject(v)
        exports['ox_target']:TargetingAddObject(v, "car", {
          {
  ...
  eventHandlers["delivery"] = RegisterNetEvent(string.format("Salvaging:Client:%s:StartDelivery", joiner), function(point)
    ...
    _blip = exports['hrrp-blips']:Add("SalvDelivery", "Deliver Goods", point.coords, 478, 2, 1.4)

    exports['hrrp-pedinteraction']:Add("SalvagingDelivery", `mp_m_waremech_01`, point.coords, point.heading, 25.0, {
      {
  ...
  eventHandlers["actions"] = RegisterNetEvent(string.format("Salvaging:Client:%s:Action", joiner), function(netid)
    ...
    if _count >= 15 then
      ...
      if _blip ~= nil then
        RemoveBlip(_blip)
        exports['hrrp-blips']:Remove("SalvDelivery")
      end
    end
  end)
end)
...
AddEventHandler("Salvaging:Client:ScrapCar", function(s, s2)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer('Salvaging:SalvageCar', NetworkGetNetworkIdFromEntity(s.entity))
    end
  end)
end)

AddEventHandler("Salvaging:Client:TriggerDelivery", function()
  exports["hrrp-base"]:CallbacksServer('Salvaging:TriggerDelivery', _joiner)
end)

AddEventHandler("Salvaging:Client:EndDelivery", function()
  exports["hrrp-base"]:CallbacksServer('Salvaging:EndDelivery', _joiner)
  exports['hrrp-pedinteraction']:Remove("SalvagingDelivery")
end)

AddEventHandler("Salvaging:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer('Salvaging:StartJob', _joiner, function(state)
    if not state then
