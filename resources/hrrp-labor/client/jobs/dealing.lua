...
function SendPedToPlayer(ped, coords)
  ...
  SetTimeout((1000 * 60) * 2, function()
    if not Entity(ped).state.boughtDrugs and _working then
      exports["hrrp-base"]:CallbacksServer("CornerDealing:PedTimeout", {})
...
function AddTargetingShit(ped)
  exports['ox_target']:TargetingAddPed(ped, "hands-holding-dollar", {
    {
      icon = "list-timeline",
      text = "Sell Product",
...
AddEventHandler("Labor:Client:Setup", function()
  ...
  exports['hrrp-pedinteraction']:Add("CornerDealing", GetHashKey("csb_grove_str_dlr"), _queueLoc.coords, _queueLoc.heading, 25.0, {
    {
      icon = "clock-nine",
      text = "Sign Up",
...
RegisterNetEvent("CornerDealing:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["start-cornering"] = AddEventHandler("CornerDealing:Client:StartCornering", function(entity, data)
    ...
    if _working and _state == 0 then
      exports["hrrp-base"]:CallbacksServer("CornerDealing:CheckCorner", {
        ...
        if s then
          ...
          exports["hrrp-base"]:CallbacksServer("CornerDealing:StartCornering", {
  ...
  eventHandlers["stop-cornering"] = AddEventHandler("CornerDealing:Client:StopCornering", function(data, entity)
    if _working then
      exports["hrrp-base"]:CallbacksServer("CornerDealing:StopCornering")
    end
  end)
              ...
              if not ending then
                if DoesEntityExist(_SellingVeh) then
                  if GetEntityHealth(_SellingVeh) == 0 or not IsVehicleDriveable(_SellingVeh) then
                    exports['hrrp-base']:LoggerTrace("CornerDealing", "Vehicle Health 0 or Not Drivable")
                    ending = true
                    exports["hrrp-base"]:CallbacksServer("CornerDealing:DestroyVehicle")
                  end
                end
              end
              Citizen.Wait(10)
            ...
            while _working and _state == 1 and _startTime == start do
              if not ending and _SellingVeh ~= 0 and _SellingVeh ~= nil then
                ...
                if #(vehPos - _SellingCorner) > 50.0 then
                  exports['hrrp-base']:LoggerTrace("CornerDealing", "Vehicle Too Far From Corner")
                  ending = true
                  exports["hrrp-base"]:CallbacksServer("CornerDealing:LeaveArea")
                end
              end
              Citizen.Wait(10)
            end
            ...
            while _working and _state == 1 and LocalPlayer.state.loggedIn and notFoundCount < 10 and _startTime == start do
              if _SellingPed == nil then
                ...
                if not _SellingPed then
                  ...
                  if not Entity(_SellingPed).state.sentToPed then
                    if NetworkGetEntityIsNetworked(_SellingPed) then
                      exports["hrrp-base"]:CallbacksServer(
                      ...
                      if NetworkHasControlOfEntity(_SellingPed) then
                        ...
                        exports["hrrp-base"]:CallbacksServer("CornderDealing:SyncEvent", {
                          event = "goto",
                ...
                if notFoundCount >= 10 then
                  exports["hrrp-base"]:CallbacksServer("CornerDealing:NoPeds", {})
                  return
                end
              end
              Citizen.Wait(10000)
  ...
  eventHandlers["remove-targeting"] = RegisterNetEvent(
    ...
    function()
      if _SellingPed ~= nil then
        exports['ox_target']:TargetingRemovePed(_SellingPed)
      end
    end
  )
...
AddEventHandler("CornerDealing:Client:ShowMenu", function(entity, data)
  ...
  if _working and _state == 1 then
    exports["hrrp-base"]:CallbacksServer("CornerDealing:GetSaleMenu", {
      netId = NetworkGetNetworkIdFromEntity(entity.entity),
    }, function(data)
      if data ~= nil and #data > 0 then
        ...
        exports['hrrp-hud']:NotificationError("You Have Nothing To Sell")
      end
    end)
  end
end)
...
AddEventHandler("CornerDealing:Client:Sell", function(data)
  ...
  exports["hrrp-base"]:CallbacksServer("CornerDealing:DoSale", {
    item = data,
    netId = NetworkGetNetworkIdFromEntity(_SellingPed),
  }, function(r)
    if r then
      ...
      exports["hrrp-animations"]:EmotesPlay("handoff", false, 3000, true, true)
    end
...
AddEventHandler("CornerDealing:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer("CornerDealing:StartJob", _joiner, function(state)
    if not state then
...
-- 						exports["hrrp-base"]:CallbacksServer("CornerDealing:PedDied", {})
-- 					end
-- 				end
-- 			end
-- 		end
...
AddEventHandler("CornerDealing:Client:Enable", function()
  exports["hrrp-base"]:CallbacksServer("CornerDealing:Enable", {})
end)

AddEventHandler("CornerDealing:Client:Disable", function()
  exports["hrrp-base"]:CallbacksServer("CornerDealing:Disable", {})
end)
