...
AddEventHandler("Labor:Client:Setup", function()
  for k, v in ipairs(_sellers) do
    exports['hrrp-pedinteraction']:Add(string.format("GemSeller%s", k), v.model, v.coords, v.heading, 25.0, {
      {
        icon = "sack-dollar",
        text = "Sell Diamonds",
  ...
  exports['hrrp-pedinteraction']:Add("MiningJob", `s_m_y_construct_02`, vector3(2741.874, 2791.691, 34.214), 155.045, 25.0, {
    {
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Mining:DoTheThingBrother", function(item, cb)
...
RegisterNetEvent("Mining:Client:MineOre", function(data, oreData)
  if Inventory.Check.Player:HasItem("pickaxe", 1) then
    exports["hrrp-base"]:CallbacksServer('Mining:Server:MineOre', oreData, function(finishedMining)
    ...
    exports['hrrp-hud']:NotificationError("You Need A Pickaxe To Mine This.")
  end
end)
...
RegisterNetEvent("Mining:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["startup"] = RegisterNetEvent(string.format("Mining:Client:%s:Startup", joiner), function(nodes)
    ...
    for k, v in pairs(_nodes) do
      ...
      exports['hrrp-blips']:Add(string.format("MiningNode-%s", v.id), "Mining Node", v.coords, 679, 18, 0.8)
  ...
  eventHandlers["actions"] = RegisterNetEvent(string.format("Mining:Client:%s:Action", joiner), function(data)
    for k, v in pairs(_nodes) do
      if v.id == data then
        exports['hrrp-blips']:Remove(string.format("MiningNode-%s", v.id))
        _nodes[k] = nil
...
AddEventHandler("Mining:Client:SellStone", function()
  exports["hrrp-base"]:CallbacksServer('Mining:SellStone', {})
end)

AddEventHandler("Mining:Client:PuchaseAxe", function()
  exports["hrrp-base"]:CallbacksServer('Mining:PurchasePickaxe', {})
end)

AddEventHandler("Mining:Client:TurnIn", function()
  exports["hrrp-base"]:CallbacksServer('Mining:TurnIn', _joiner)
end)

AddEventHandler("Mining:Client:SellGem", function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Mining:SellGem", data)
end)
