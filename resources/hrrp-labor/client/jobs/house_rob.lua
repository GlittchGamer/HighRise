...
AddEventHandler("Labor:Client:Setup", function()
  ...
  exports['hrrp-pedinteraction']:Add("HouseRobbery", `csb_grove_str_dlr`, _queueLoc.coords, _queueLoc.heading, 25.0, {
    {
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("HouseRobbery:Lockpick", function(data, cb)
    _lpStage = 0
    DoLockpick({ timer = 1.0, base = 5 }, data, cb)
    EmergencyAlerts:CreateIfReported(40.0, "bane", true)
  end)

  exports["hrrp-base"]:CallbacksRegisterClient("HouseRobbery:AdvLockpick", function(data, cb)
    _lpStage = 0
  ...
  Interaction:RegisterMenu("house-robbery", "Enter House", "window-frame-open", function(data)
    ...
    if _working and _p ~= nil and _nodes ~= nil then
      ...
      if robberyHouse then
        return #(vector3(LocalPlayer.state.myPos.x, LocalPlayer.state.myPos.y, LocalPlayer.state.myPos.z) - vector3(robberyHouse.x, robberyHouse.y, robberyHouse.z)) <=
            3.0
      end
    end
...
function ExitHouse()
  exports["hrrp-base"]:CallbacksServer("HouseRobbery:Exit", {}, function(propId, intr)
    LocalPlayer.state.inRobbedHouse = false
    ...
    if intr then
      for k, v in ipairs(intr.robberies.locations) do
        exports['ox_target']:TargetingZonesRemoveZone(string.format("house-robbery-%s", k))
      end
    end

    if _stuff ~= nil then
      for k, v in ipairs(_stuff.pois or {}) do
        exports['ox_target']:TargetingRemoveEntity(v)
        DeleteObject(v)
      end
    end
    ...
    exports['hrrp-polyzone']:PolyZoneRemove("property-house-rob-zone")
  end)
end
...
function EnterHouse()
  if _p ~= nil and _working then
    exports["hrrp-base"]:CallbacksServer("HouseRobbery:BreakIn", _p, function(r, f, exit)
      if r then
...
RegisterNetEvent("HouseRobbery:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["receive"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Receive", joiner), function(propertyId, data)
    ...
    _blip = exports['hrrp-blips']:Add("HouseRobbery", "Target House", { x = data.coords.x, y = data.coords.y, z = data.coords.z }, 40, 23, 0.9)
    ...
    Citizen.CreateThread(function()
      ...
      exports["hrrp-base"]:CallbacksServer("HouseRobbery:ArrivedNear", {})
    end)
  end)
  ...
  eventHandlers["lockpicked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Lockpicked", joiner), function(data)
    _nodes = data
    _state = 4
    exports['hrrp-hud']:NotificationSuccess("House Has Been Lockpicked")
  end)
    ...
    if data.id and not _nodes.searched[data.id] then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled then
          ...
          exports["hrrp-base"]:CallbacksServer("HouseRobbery:Search", data.id)
        end
      end)
    end
  end)
  ...
  eventHandlers["inner"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:InnerStuff", joiner), function(intr)
    ...
    if _nodes.chances.alarm then
      ...
      exports['ox_target']:TargetingAddEntity(o, "sensor-triangle-exclamation", {
        {
          ...
          isEnabled = function(data, entity)
            return _working and _nodes ~= nil and _state == 4 and _nodes.chances.alarm and not _nodes.states.alarm.disabled and not _nodes.states.alarm
                .triggered and (_scPass <= 3 and _scFails <= 2)
          end,
        },
      })
    ...
    if intr.zone then
      exports['hrrp-polyzone']:PolyZoneCreateBox("property-house-rob-zone", intr.zone.center, intr.zone.length, intr.zone.width, intr.zone.options, {})
    end
...
AddEventHandler("HouseRobbery:Client:Enable", function()
  exports["hrrp-base"]:CallbacksServer('HouseRobbery:Enable', {})
end)

AddEventHandler("HouseRobbery:Client:StartJob", function()
  exports["hrrp-base"]:CallbacksServer('HouseRobbery:StartJob', _joiner, function(state)
    if not state then
...
AddEventHandler("HouseRobbery:Client:HackSuccess", function(data)
  ...
  if _scPass > 3 then
    _scPass = 1
    exports["hrrp-base"]:CallbacksServer("HouseRobbery:HackAlarm", {
      propertyId = data.pId,
      state = true
    }, function()
      exports['hrrp-hud']:NotificationSuccess("Alarm Bypassed")
    end)
  else
    Citizen.Wait(1500)
    DoHack(data.pId)
