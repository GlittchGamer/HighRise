  ...
  switch (event.data.action) {
    case "playSound":
      if (
        ...
        AudioPlayers[event.data.source][event.data.file].volume(
          event.data.volume
        );
      ...
      if (AudioPlayers[event.data.source] == null)
        AudioPlayers[event.data.source] = Array();
    ...
    case "stopSound":
      if (
        Boolean(AudioPlayers[event.data.source]) &&
        Boolean(AudioPlayers[event.data.source][event.data.file])
      ) {
        AudioPlayers[event.data.source][event.data.file].stop();
        delete AudioPlayers[event.data.source][event.data.file];
        // $.post('https://hrrp-sounds/SoundEnd', JSON.stringify({
        //     source: event.data.source,
        //     file: event.data.file
        // }))
      }
    ...
    case "fadeSound":
      if (
        ...
        AudioPlayers[event.data.source][event.data.file].fade(
          AudioPlayers[event.data.source][event.data.file].volume(),
          0,
          1000
        );
