...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("DJ", {
    "Menu",
    "Notification",
  }, function(error)
    if #error > 0 then
...
function RegisterDjZones()
  ...
  for k, v in ipairs(Config.Locations) do
    if v.enableBooth then
      exports['ox_target']:TargetingZonesAddBox("dj-booth" .. k, "compact-disc", v.coords, 1.0, 1.0, {
        name = "djbooth" .. k,
        heading = 0,
        --debugPoly=true,
        minZ = v.coords.z - 1.5,
      ...
      }, {
        {
          icon = "compact-disc",
          text = "Open Stereo",
          event = "hrrp-dj:client:playMusic",
          data = { zone = k },
        },
      })
...
RegisterNetEvent("hrrp-dj:client:playMusic", function(data)
  local booth = ""
  local boothId = 0
  ...
  local p = promise.new()
  exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:songInfo", {}, function(cb)
    p:resolve(cb)
  end)
  ...
  djMenu.Add:Input("Youtube URL", {
    ...
    if song == "" or song == nil then
      exports['hrrp-hud']:NotificationError("Empty text cannot be played dummy.", 2500, "fas fa-volume-off")
      return
    end
    if not string.find(song, "youtu") then
      song = "https://www.youtube.com/watch?v=" .. song
    end
    exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:playMusic", {
      song = song,
      zoneNum = boothId,
    }, function(success, zoneNum)
      if success then
        TriggerEvent("hrrp-dj:client:playMusic", { zone = zoneNum })
        exports['hrrp-hud']:NotificationSuccess("Loading link: " .. song, 2500, "fas fa-pause")
      else
        exports['hrrp-hud']:NotificationError("Failed to Load Song", 2500, "fas fa-volume-off")
      end
    end)
  end)
  ...
  if xSound:soundExists(booth) then
    if xSound:isPlaying(booth) then
      djMenu.Add:Button("Pause Music", { success = true }, function()
        exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:pauseMusic", {
          zoneName = booth,
          zoneNum = boothId,
        }, function(success)
          if success then
            exports['hrrp-hud']:NotificationSuccess("Paused Music", 2500, "fas fa-pause")
          else
            exports['hrrp-hud']:NotificationError("Failed to Pause Music", 2500, "fas fa-pause")
          end
        end)
      end)
    elseif xSound:isPaused(booth) then
      djMenu.Add:Button("Resume Music", { success = true }, function()
        exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:resumeMusic", {
          zoneName = booth,
          zoneNum = boothId,
        }, function(success)
          if success then
            exports['hrrp-hud']:NotificationSuccess("Resume Music", 2500, "fas fa-play")
          else
            exports['hrrp-hud']:NotificationError("Failed to Resume Music", 2500, "fas fa-play")
          end
        end)
      end)
    end
    ...
    djMenuSub["changevolume"].Add:Slider("Change Volume", {
      ...
      exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:changeVolume", {
        volume = volume,
        zoneName = booth,
        zoneNum = boothId,
      }, function(success, zoneNum)
        if success then
          ...
          TriggerEvent("hrrp-dj:client:playMusic", { zone = zoneNum })
          exports['hrrp-hud']:NotificationSuccess(
            string.format("Changed Volume to %s/100", tostring(math.ceil(volume * 100))),
            2500,
            "fas fa-volume-off"
          )
        else
          exports['hrrp-hud']:NotificationError("Failed to Change Volume", 2500, "fas fa-volume-off")
        end
      end)
    end)
    ...
    djMenu.Add:Button("Stop Music", { success = true }, function()
      exports["hrrp-base"]:CallbacksServer("hrrp-dj:server:stopMusic", {
        zoneName = booth,
        zoneNum = boothId,
      }, function(success)
        if success then
          exports['hrrp-hud']:NotificationSuccess("Stop Music", 2500, "fas fa-stop")
        else
          exports['hrrp-hud']:NotificationError("Failed to Stop Music", 2500, "fas fa-stop")
        end
      end)
    end)
  end
