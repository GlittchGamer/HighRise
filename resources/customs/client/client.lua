...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies('VehicleCustoms', {
    'Callbacks',
    'Notification',
    'Game',
    'Action',
...
AddEventHandler("Characters:Client:Spawn", function()
  ...
  for k, v in ipairs(_customsLocations) do
    if v.blip then
      if v.aircraft then
        exports['blips']:Add('veh_customs_' .. k, 'Aircraft Vehicle Customs', v.blip, 643, 34, 0.6)
      else
        exports['blips']:Add('veh_customs_' .. k, 'Vehicle Customs', v.blip, 643, 12, 0.6)
      end
    end
  end
end)
...
function CreateCustomsZones()
  for k, v in ipairs(_customsLocations) do
    ...
    if v.zone and v.zone.type == 'poly' and v.zone.points then
      exports['polyzonehandler']:PolyZoneCreatePoly('veh_customs_' .. k, v.zone.points, {
        minZ = v.zone.minZ,
        maxZ = v.zone.maxZ,
      }, data)
    elseif v.zone and v.zone.type == 'box' and v.zone.center and v.zone.length and v.zone.width then
      exports['polyzonehandler']:PolyZoneCreateBox('veh_customs_' .. k, v.zone.center, v.zone.length, v.zone.width, {
      ...
      if locationData then
        if CheckClassRestriction(DRIVING_VEHICLE_CLASS, locationData.restrictClass) then
          ...
          if withinCustomsType == 0 then
            exports['hud']:ActionShow('{keybind}primary_action{/keybind} Customize Vehicle')
          elseif withinCustomsType == 1 then
            exports['hud']:ActionShow('{keybind}primary_action{/keybind} Repair Vehicle')
          end
        end
      end
    end
  ...
  if withinCustoms and DRIVING_VEHICLE and not LocalPlayer.state.doingAction then
    ...
    if locationData then
      ...
      if not CheckJobRestriction(locationData.restrictJobs) then
        exports['hud']:NotificationError('Authorized Vehicles & Personnel Only')
        return
      end
    ...
    elseif withinCustomsType == 1 then
      if bodyHealth < 1000 or engineHealth < 1000 then
        ...
        exports['hud']:NotificationError('Vehicle Doesn\'t Need Repairs')
      end
    end
  end
end)
...
AddEventHandler('Vehicles:Client:OpenVehicleCustoms', function(data)
  ...
  DoSlowVehicleNormalRepair(data.cost, function(s)
    if s then
      exports['hud']:NotificationSuccess('Repaired Vehicle For $' .. data.cost)
  ...
  if withinCustoms and DRIVING_VEHICLE then
    ...
    if vehEnt and vehEnt.state.VIN then
      if Vehicles.Keys:Has(vehEnt.state.VIN, vehEnt.state.GroupKeys) then
        ...
        exports['hud']:NotificationError('Cannot Modify a Vehicle That You Don\'t Have Keys For')
      end
    else
...
function DoSlowVehicleNormalRepair(cost, cb)
  ...
  Progress:Progress({
    ...
    if not cancelled and DRIVING_VEHICLE then
      exports['core']:CallbacksServer('Vehicles:CompleteRepair', {
        cost = cost,
      }, function(success)
        if success then
