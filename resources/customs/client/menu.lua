...
function OpenVehicleCustoms(canInstallPerformance, costMultiplier, settings)
  ...
  customsMenu = exports['menu']:Create('vehicle_customs', 'Vehicle Customs', function()
    CUSTOMS_OPEN = true
  end, function()
    Citizen.Wait(100)
    customsMenu = false
    ...
    if not isSaving then
      exports['core']:LoggerTrace('VehicleCustoms', 'Not Saving - Reset Mods')
      Vehicles.Properties:Set(CUST_VEH, originalData)
      exports['hud']:NotificationError('Changes Discarded - You Weren\'t Charged')
    end
    CUSTOMS_OPEN = false
  end, true)
  ...
  exports['hud']:NotificationPersistentStandard('veh_customs', 'Current Total: $' .. 0)
  ...
  local function updateVehiclePropertyState()
    ...
    exports['hud']:NotificationPersistentStandard('veh_customs', 'Current Total: $' .. newCost)
    customsMenu.Update:Item(costElementId, 'Current Total: $' .. newCost, {}, true)
  end

  -- PAINT STUFF

  customsMenuSubs['paint'] = exports['menu']:Create('vehicle_customs_paint', 'Paint')
  ...
  customsMenuSubs['livery'] = exports['menu']:Create('vehicle_customs_livery', 'Liveries')
  ...
  -- EXTRAS
  customsMenuSubs['extras'] = exports['menu']:Create('vehicle_customs_extras', 'Extras')
  ...
  -- Lighting
  customsMenuSubs['lighting'] = exports['menu']:Create('vehicle_customs_lighting', 'Lighting')
  ...
  customsMenuSubs['wheels'] = exports['menu']:Create('vehicle_customs_wheels', 'Wheels')
  ...
  customsMenuSubs['body_parts'] = exports['menu']:Create('vehicle_customs_body_parts', 'Body Parts')
  ...
  if canInstallPerformance then
    customsMenuSubs['performance'] = exports['menu']:Create('vehicle_customs_performance', 'Performance')
  ...
  customsMenu.Add:Button('Pay and Save Changes', { success = true }, function()
    ...
    if LocalPlayer.state.Character:GetData('Cash') >= currentCost then
      isSaving = true
      exports['core']:LoggerTrace('VehicleCustoms', 'Attept Mods Save')
      customsMenu:Close()

      exports['core']:CallbacksServer('Vehicles:CompleteCustoms', {
        ...
        if success then
          ...
          exports['hud']:NotificationSuccess('New Modifications Saved & Paid For')
          if newNewData then
            Vehicles.Properties:Set(CUST_VEH, newNewData)
          end
        else
          exports['hud']:NotificationError('Error Saving Vehicle Modifications - Not Enough Money?')
          Vehicles.Properties:Set(CUST_VEH, originalData)
        end
      end)
    else
      exports['hud']:NotificationError('Not Enough Cash')
    end
  end)
