...
ANIMATIONS.PedFeatures = {
  ToggleCrouch = function(self, toggle)
    ...
    if toggle then
      ...
      if not LocalPlayer.state.drunkMovement then
        exports["animations"]:PedFeaturesRequestFeaturesUpdate()
      end
      _isCrouched = false
    end
  end,
  SetWalk = function(self, walk, label)
    if LocalPlayer.state.isLimping then
      ...
      if walk == 'reset' then
        walkStyle = walk
        exports['core']:CallbacksServer('Animations:UpdatePedFeatures', { type = 'walk', data = 'default' }, function(success)
          if success then
            exports['hud']:NotificationInfo('Reset Walking Style', 5000)
          end
        end)
      else
        walkStyle = walk
        exports['core']:CallbacksServer('Animations:UpdatePedFeatures', { type = 'walk', data = walk }, function(success)
          if success then
            exports['hud']:NotificationSuccess('Saved Walking Style: ' .. label, 5000)
          end
        end)
      end
    else
  ...
  SetExpression = function(self, expression, label)
    if expression == 'reset' then
      ...
      exports['core']:CallbacksServer('Animations:UpdatePedFeatures', { type = 'expression', data = 'default' }, function(success)
        if success then
          exports['hud']:NotificationInfo('Expression Reset', 5000)
        end
      end)
    else
      SetFacialIdleAnimOverride(LocalPlayer.state.ped, expression, 0)
      facialExpression = expression
      exports['core']:CallbacksServer('Animations:UpdatePedFeatures', { type = 'expression', data = expression }, function(success)
        if success then
          exports['hud']:NotificationSuccess('Saved Expression: ' .. label, 5000)
        end
      end)
    end
  end,
