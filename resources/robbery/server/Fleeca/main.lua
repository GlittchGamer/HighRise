...
function ResetFleeca(fleecaId)
  ...
  exports['doors']:SetLock(string.format("%s_tills", fleecaId), true)
  exports['doors']:SetLock(string.format("%s_gate", fleecaId), true)
...
AddEventHandler("Robbery:Server:Setup", function()
  ...
  exports['core']:CallbacksRegisterServer("Robbery:Fleeca:Vault:StartLootTrolley", function(source, data, cb)
    ...
    if pState.fleeca ~= nil then
      if
          ...
          and (
            data.cart == 1
            or not exports['doors']:IsLocked(string.format("%s_gate", pState.fleeca))
          )
      then
  ...
  exports['core']:CallbacksRegisterServer("Robbery:Fleeca:Vault:FinishLootTrolley", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
  ...
  exports['core']:CallbacksRegisterServer("Robbery:Fleeca:Vault:CancelLootTrolley", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
  ...
  exports['core']:CallbacksRegisterServer("Robbery:Fleeca:SecureBank", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local pState = Player(source).state

    if pState.fleeca ~= nil then
      if exports['jobs']:JobsPermissionsHasJob(source, "police") then
        if pState.onDuty then
          ...
          exports['core']:LoggerInfo("Robbery", string.format("%s %s (%s) Secured Fleeca %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), pState.fleeca))
          TriggerClientEvent("Robbery:Client:Fleeca:CloseVaultDoor", -1, pState.fleeca)
          exports['doors']:SetLock(string.format("%s_tills", pState.fleeca), true)
          exports['doors']:SetLock(string.format("%s_gate", pState.fleeca), true)
        else
        end
      else
      end
  ...
  exports['core']:CallbacksRegisterServer("Robbery:Fleeca:SequenceGate", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local pState = Player(source).state
    if pState.fleeca ~= nil then
      exports['doors']:SetLock(string.format("%s_gate", pState.fleeca), false)
    else
      cb(false)
    end
  end)
        ...
        if #(bankData.points.vaultPC.coords - playerCoords) <= 1.5 then
          ...
          then
            if not _inUse.VaultDoor[pState.fleeca] then
              ...
              exports['core']:ExecuteClient(source, "Notification", "Error", "Someone Else Is Doing A Thing", 6000)
            end
            return
          elseif
              GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] == nil
      ...
      if not GlobalState["AntiShitlord"] or os.time() >= GlobalState["AntiShitlord"] or _inProgress[pState.fleeca] then
        ...
        if #(bankData.points.vaultGate.coords - playerCoords) <= 1.5 then
          ...
          if
              GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)] ~= nil
              and GlobalState[string.format("Fleeca:%s:VaultDoor", pState.fleeca)].state == 3
              and exports['doors']:IsLocked(string.format("%s_gate", pState.fleeca))
          then
  ...
  Inventory.Items:RegisterUse("fleeca_card", "FleecaRobbery", function(source, itemData)
    ...
    if pState.fleeca ~= nil then
      if pState.fleeca == itemData.MetaData.BankId then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "Invalid Access Card: Incorrect Location", 6000)
        return
      end
    end
  end)
  ...
  Inventory.Items:RegisterUse("moneybag", "FleecaRobbery", function(source, itemData)
    ...
    if os.time() >= itemData.MetaData.Finished then
      ...
      exports['core']:LoggerInfo("Robbery", string.format("%s %s (%s) Used A Money Bag, Received $%s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), amt))
      Inventory.Items:RemoveSlot(itemData.Owner, itemData.Name, 1, itemData.Slot, itemData.invType)
      Wallet:Modify(source, amt)
    else
      exports['core']:ExecuteClient(source, "Notification", "Error", "Not Ready Yet", 6000)
    end
  end)
end)
