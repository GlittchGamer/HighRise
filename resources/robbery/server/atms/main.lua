...
AddEventHandler("Robbery:Server:Setup", function()
	exports['core']:CallbacksRegisterServer("Robbery:ATM:CanRobATM", function(source, data, cb)
	...
	exports['core']:CallbacksRegisterServer('Robbery:ATM:SetATMRobbed', function(src, state, cb)
		exports['core']:LoggerInfo('ATM Robbery', state .. ' ATM entity is on cooldown.')
	...
	Inventory.Items:RegisterUse('drill', 'RandomItems', function(source, ItemData)
		...
		exports['core']:CallbacksClient(source, 'Robbery:ATM:StartRobbing', {
			atm = 139778,
			item = ItemData,
			...
			if not _ATM_COOLDOWNS[Char:GetData('SID')] and state then
				...
				exports['core']:LoggerInfo('ATM Robbery', Char:GetData('First') .. ' ' .. Char:GetData('Last') .. ' is on ATM Cooldown.')
	...
	BeginATMCooldown = function(source)
		...
		exports['core']:LoggerInfo('ATM Robbery', Char:GetData('First') .. ' ' .. Char:GetData('Last') .. ' got off ATM Cooldown.')
	...
	BeginEntityCooldown = function(source, _Entity)
		Wait(20 * 60000)
		exports['core']:LoggerInfo('ATM Robbery', _Entity .. ' ATM entity is off cooldown.')

		_ROBBED_ATMS[_Entity] = false
	end

	exports['core']:CallbacksRegisterServer('Robbery:ATM:GetReward', function(source, data, cb)
		local Player = exports['core']:FetchSource(source)
		local Char = Player:GetData('Character')
