...
AddEventHandler("Robbery:Client:Setup", function()
  ...
  for k, v in ipairs(GlobalState["FleecaRobberies"]) do
    ...
    exports['polyzonehandler']:PolyZoneCreateBox(bankData.id, bankData.coords, bankData.width, bankData.length, bankData.options)
        ...
        {
          {
            ...
            isEnabled = function(s, s2)
              return (
                ...
                or (not exports['doors']:IsLocked(string.format("%s_gate", LocalPlayer.state.fleeca)))
              )
            end,
          },
        },
  ...
  exports['core']:CallbacksRegisterClient("Robbery:Fleeca:Keypad:Vault", function(data, cb)
...
AddEventHandler("Robbery:Client:Fleeca:StartSecuring", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports['core']:CallbacksServer("Robbery:Fleeca:SecureBank", {})
    end
  end)
end)

AddEventHandler("Robbery:Client:Fleeca:SequenceGate", function()
  exports['core']:CallbacksServer("Robbery:Fleeca:SequenceGate", {}, function(r) end)
end)

AddEventHandler("Robbery:Client:Fleeca:LootTrolley", function(entity, data)
  exports['core']:CallbacksServer("Robbery:Fleeca:Vault:StartLootTrolley", { cart = data }, function(proc)
    if proc then
      if
      ...
      then
        ...
        if #(myCoords - cartData.coords) <= 3.0 then
          ...
          exports['core']:CallbacksServer("Robbery:Fleeca:Vault:FinishLootTrolley", {
            cart = data,
          })
          Citizen.Wait(1800)
        ...
        exports['ox_target']:TargetingAddEntity(obj, "coins", {
          {
            ...
            isEnabled = function()
              return GlobalState[string.format("Fleeca:%s:VaultDoor", LocalPlayer.state.fleeca)] ~= nil
                  ...
                  and (k == 1 or not exports['doors']:IsLocked(string.format("%s_gate", LocalPlayer.state.fleeca)))
            end,
          },
        }, 3.0)
      end
