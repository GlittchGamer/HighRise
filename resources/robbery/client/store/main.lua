...
AddEventHandler("Robbery:Client:Setup", function()
  ...
  for k, v in pairs(GlobalState["StoreRobberies"]) do
    exports['polyzonehandler']:PolyZoneCreateBox(v.id, v.coords, v.width, v.length, v.options)
    _polys[v.id] = true
  end
  ...
  exports['core']:CallbacksRegisterClient("Robbery:Store:DoSafeCrack", function(data, cb)
    _memPass = 1
...
AddEventHandler("Robbery:Client:Store:LockpickRegister", function(entity, data)
  ...
  then
    exports['core']:CallbacksServer("Robbery:Store:StartLockpick", coords, function(s)
      if s then
...
AddEventHandler("Robbery:Client:Store:LockpickSuccess", function(data)
  ...
  if _lpPass then
    if _lpPass >= 4 then
      _lpPass = false
      exports['core']:CallbacksServer("Robbery:Store:Register", {
        results = true,
        coords = data,
        store = _inPoly,
      }, function(s) end)
...
AddEventHandler("Robbery:Client:Store:ActualCrackSafe", function(entity, data)
  ...
  if GlobalState[string.format("Safe:%s", data.id)] == nil then
    exports['core']:CallbacksServer("Robbery:Store:StartSafeCrack", data, function(s) end)
  end
end)
...
AddEventHandler("Robbery:Client:Store:SequenceSafe", function(entity, data)
  ...
  local coords = GetEntityCoords(entity.entity)
  if GlobalState[string.format("Safe:%s", data.id)] == nil then
    exports['core']:CallbacksServer("Robbery:Store:StartSafeSequence", {}, function(r)
      if r then
...
AddEventHandler("Robbery:Client:Store:OpenSafe", function(entity, data)
  ...
  data.store = _inPoly
  exports['core']:CallbacksServer("Robbery:Store:LootSafe", data, function(s) end)
end)
...
AddEventHandler("Robbery:Client:Store:SecureSafe", function(entity, data)
  ...
  Progress:Progress({
    ...
    if not status then
      data.store = _inPoly
      exports['core']:CallbacksServer("Robbery:Store:SecureSafe", data, function(s) end)
    end
  end)
end)
...
AddEventHandler("Robbery:Client:Store:SafeCrackSuccess", function(data)
  ...
  if _scPass >= 3 then
    ...
    data.results = true
    data.store = _inPoly
    exports['core']:CallbacksServer("Robbery:Store:Safe", data, function(s) end)
  else
    _scPass = _scPass + 1
    Citizen.Wait(1500)
    SCSeq(data)
