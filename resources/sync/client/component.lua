...
SYNC = {
  Start = function(self)
    exports['core']:LoggerTrace("Sync", "Starting Time and Weather Sync")
    _isStopped = false
  ...
  Stop = function(self, hour)
    exports['core']:LoggerTrace("Sync", "Stopping Time and Weather Sync")
    _isStopped = true
        ...
        if _inCayo or _inCayoStorm then
          ...
          if _weatherState ~= GlobalState["Sync:Weather"] then
            ...
            if not isTransionHappening then
              ...
              _weatherState = GlobalState["Sync:Weather"]
              exports['core']:LoggerTrace("Sync", "Transitioning to Weather: " .. _weatherState)
              ClearOverrideWeather()
              ClearWeatherTypePersist()
              SetWeatherTypeOvertimePersist(_weatherState, 15.0)
              Citizen.Wait(15000)
              exports['core']:LoggerTrace("Sync", "Finished Transitioning to Weather: " .. _weatherState)
