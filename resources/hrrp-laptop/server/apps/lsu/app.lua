...
AddEventHandler("Laptop:Server:RegisterCallbacks", function()
  Citizen.CreateThread(function()
    ...
    while true do
      ...
      exports['hrrp-base']:LoggerInfo("Laptop - LSU", "Market Place Items Restocked")
    end
  end)

  GlobalState.LSUPickupLocation = locations[math.random(#locations)]

  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:GetDetails", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Market:Checkout", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Market:Collect", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char ~= nil then
      ...
      if pendingPickup then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "fack off, not got nufink' for u m8")
      end
    ...
    if char ~= nil then
      if not pState.onDuty or not _blacklistedJobs[pState.onDuty] then
        if not hasValue(char:GetData("States") or {}, "ACCESS_LSUNDERGROUND") then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Already A Member Of LS Underground")
        end
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Can't Use This Item")
      end
    end
  end)
end)
