...
AddEventHandler("Characters:Server:PlayerLoggedOut", HandleCharacterLogout)
AddEventHandler("Characters:Server:PlayerDropped", HandleCharacterLogout)

AddEventHandler("Laptop:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Admin:CreateContract", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Admin:GetBans", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local perm = char:GetData("LaptopPermissions")
      if perm["lsunderground"] and perm["lsunderground"]["admin"] then
        local fetchBannedChars = MySQL.query.await(
          'SELECT `SID`, `First`, `Last`, `Alias`, `LSUNDGBan`  FROM `characters` WHERE `LSUNDGBan` IS NOT NULL',
          { true })
        if not fetchBannedChars or #fetchBannedChars == 0 then
          return cb(false)
        end

        for k, v in ipairs(fetchBannedChars) do
          local Alias = json.decode(v.Alias)
          v.RacingAlias = Alias?.redline?.name or 'N/A'
          table.insert(fetchBannedChars, v)
        end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Admin:Ban", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char and data?.SID then
      local perm = char:GetData("LaptopPermissions")

      if perm["lsunderground"] and perm["lsunderground"]["admin"] then
        local fetchChar = MySQL.single.await(
          'SELECT `SID`, `First`, `Last`, `Alias`, `LSUNDGBan`  FROM `characters` WHERE `SID` = ?',
          { data.SID })

        if not fetchChar then
          return cb(false)
        end

        local LSUNDGBan = json.decode(fetchChar.LSUNDGBan) or {}
        local isBanned = false
        for k, v in ipairs(LSUNDGBan) do
          if v == "Boosting" then
            isBanned = true
            break
          end
        ...
        if not isBanned then
          table.insert(LSUNDGBan, "Boosting")
        end

        local updateChar = MySQL.Sync.execute(
          'UPDATE `characters` SET `LSUNDGBan` = ? WHERE `SID` = ?',
          { json.encode(LSUNDGBan), data.SID })

        local targetChar = exports['hrrp-base']:FetchSID(data.SID)
        if targetChar then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Admin:Unban", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char and data?.SID then
      local perm = char:GetData("LaptopPermissions")
      ...
      if perm["lsunderground"] and perm["lsunderground"]["admin"] then
        ...
        local LSUNDGBan = json.decode(fetchCharacter.LSUNDGBan) or {}
        local updated = false
        for k, v in ipairs(LSUNDGBan) do
          if v == "Boosting" then
            table.remove(LSUNDGBan, k)
            updated = true
            break
          end
        end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:EnterQueue", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if char and team and team.ID and not char:GetData("LSUNDGBan") then
      local perm = char:GetData("LaptopPermissions")
      local alias = char:GetData("Profiles")?.redline
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:ExitQueue", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      for k, v in ipairs(_boostingQueue) do
        if v.source == source then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:DeclineContract", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:TransferContract", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
        ...
        if v.id == data.id then
          ...
          if found then
            ...
            Laptop.Notification:Add(
              ...
              string.format("A New %s Class Contract Was Just Transferred to You. Buy In: %s $%s", v.vehicle.class,
                v.prices.standard.price, v.prices.standard.coin),
              os.time() * 1000,
              10000,
              "lsunderground",
              {
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:AcceptContract", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local boostingContracts = char:GetData("BoostingContracts") or {}
      local perm = char:GetData("LaptopPermissions")
          ...
          if team?.ID and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
            if Crypto:Has(source, v.prices.standard.coin, v.prices.standard.price) then
              ...
              Laptop.Notification:Add(team.ID, "New Request for a Boosting Contract",
                string.format("%s %s Requested a Boosting Contract (%s)", char:GetData("First"), char:GetData("Last"),
                  v.vehicle.class), os.time() * 1000, 15000, "lsunderground", {
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Exterior", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      ...
      if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 0 then
        ...
        local noAlert = false
        if _boosting[team.ID].vehicleData?.classLevel then
          if _boosting[team.ID].vehicleData.classLevel <= 2 then     -- D and C
            noAlert = true
          elseif _boosting[team.ID].vehicleData.classLevel == 3 then -- B
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:Ignition", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local team = Laptop.Teams:GetByMemberSource(source)
      ...
      if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 1 then
        if _boosting[team.ID].trackerTotal and _boosting[team.ID].trackerTotal > 0 then
          ...
          Laptop.Teams.Members:NotificationAddWithId(
            ...
            "BOOSTING_CONTRACT",
            "Current Contract",
            string.format("Disable tracking devices inside the vehicle. %s/%s", _boosting[team.ID].trackerCount,
          ...
          Citizen.CreateThread(function()
            ...
            while _boosting[team] and _boosting[team].state == 2 do
              if DoesEntityExist(_boosting[team].vehicle) then
                ...
                TriggerClientEvent("EmergencyAlerts:Client:TrackerBlip", -1, "police", string.format("boosting-%s", team),
                  string.format("[Police]: Class %s Boost (From %s)", _boosting[team].vehicleData.class, location),
                  coords, 523, 6, 1.0, true, 155)
              end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:DropOff", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local team = Laptop.Teams:GetByMemberSource(source)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Laptop:LSUnderground:Boosting:LeftArea", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local team = Laptop.Teams:GetByMemberSource(source)
  ...
  Inventory.Items:RegisterUse("boosting_tracking_disabler", "Boosting", function(source, slot, itemData)
    ...
    then
      Citizen.SetTimeout(500, function()
        exports["hrrp-base"]:CallbacksClient(source, "Laptop:LSUnderground:Boosting:TrackerHacker", {}, function(using, success)
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Can't Use Right Now...")
    end
  end)
        ...
        if team?.ID and team.State == 0 and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
          ...
          for k, v in ipairs(team.Members) do
            ...
            if char then
              ...
              if not profiles?.redline or not hasVpn or not isLSU or not hasDongle or isPolice then
                fail = true
                break
              end
            else
            ...
            if Crypto.Exchange:Remove(v.prices.standard.coin, owner:GetData("CryptoWallet"), v.prices.standard.price) then
              ...
              for _, m in ipairs(team.Members) do
                Laptop.Notification:Add(m.Source, "Failed to Start Boosting Contract",
                  string.format("%s %s doesn't have enough crypto to start the contract.", owner:GetData("First"),
                    owner:GetData("Last")), os.time() * 1000, 10000, "lsunderground", {}, {})
              end
            end
          else
            Laptop.Teams.Members:Notification(
...
LAPTOP.LSUnderground.Boosting = {
  ...
  GiveContract = function(self, source, vehicle, prices, timeLength, settings)
    ...
    if char then
      ...
      exports['hrrp-base']:LoggerInfo("Boosting",
    ...
    if team and contract and contract?.vehicle?.vehicle then
      ...
      for k, v in ipairs(team.Members) do
        ...
        if v.SID == contract.owner?.SID then
          ...
          if cChar then
            exports['hrrp-base']:LoggerInfo("Boosting",
              string.format("%s [%s %s (%s)] Started Class %s Contract (%s)", cChar:GetData("Profiles")?.redline?.name,
                cChar:GetData("First"), cChar:GetData("Last"), cChar:GetData("SID"), contract.vehicle.class,
      ...
      Vehicles:SpawnTemp(source, GetHashKey(contract.vehicle.vehicle), 'automobile', pickUpLocation.coords.xyz,
        pickUpLocation.coords.w, function(spawnedVehicle, VIN, plate)
          if spawnedVehicle then
            ...
            exports['hrrp-base']:LoggerError("Boosting",
              string.format("Failed to Spawn Vehicle For Boost. Contract Owner SID: %s. Buy In: %s $%s", contract.owner
                .SID, contract.prices.standard.price, contract.prices.standard.coin))
          end
        end, {
        ...
        for k, v in ipairs(_boosting[teamId].members) do
          ...
          if v.SID == _boosting[teamId].contractOwner?.SID then
            ...
            if cChar then
              ...
              exports['hrrp-base']:LoggerInfo("Boosting",
                string.format("%s [%s %s (%s)] Completed Class %s Contract (%s)%s. Rep Gained: %s Crypto Gained: %s",
                  cChar:GetData("Profiles")?.redline?.name, cChar:GetData("First"), cChar:GetData("Last"),
                  cChar:GetData("SID"), _boosting[teamId].vehicleData.class, _boosting[teamId].vehicleData.label,
                  _boosting[teamId].vehicleData.rewarded and " (Manually Created)" or "", earnedRep, earnedCrypto))
            end
          end
        end
...
function SetupBoostingQueue()
  ...
  Citizen.CreateThread(function()
    ...
    exports['hrrp-base']:LoggerInfo("Boosting", "Boosting Contracts Can Now Be Rewarded")
    ...
    while true do
      ...
      if #_boostingQueue > 0 then
        ...
        if not chosen then
          exports['hrrp-base']:LoggerInfo("Boosting", string.format("Nobody Chosen for Boosting Contract (%s | %s)", index, #_boostingQueue))
        end
      end
