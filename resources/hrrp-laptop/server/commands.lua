function RegisterChatCommands()
  ...
  Chat:RegisterAdminCommand("boostingevent2", function(source, args, rawCommand)
    local char = exports['hrrp-base']:FetchSID(tonumber(args[1]))
    if char then
      local profiles = char:GetData("Profiles")
      if profiles?.redline then
        Chat.Send.System:Single(source,
          string.format("%s %s (%s) - Alias %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), profiles.redline.name))
      end
    end
  end, {
