...
function CreateFuelStationPolyzones()
  for k, v in ipairs(Config.FuelStations) do
    exports['polyzonehandler']:PolyZoneCreateBox("fuel_" .. k, v.center, v.length, v.width, {
      heading = v.heading,
      minZ = v.minZ,
      maxZ = v.maxZ,
    }, {
...
AddEventHandler("Characters:Client:Spawn", function()
  ...
  -- 		exports['blips']:Add('fuel-station-'.. k, 'Fuel Station', v.center, 361, 64, 0.4)
  -- 	end
  -- end
end)
...
AddEventHandler("Fuel:Client:FillCan", function()
  ...
  Progress:Progress({
    ...
    if not cancelled then
      exports['core']:CallbacksServer("Fuel:FillCan", {
        current = current,
        pct = pct,
      }, function(s)
        if s then
...
AddEventHandler("Vehicles:Client:StartFueling", function(entityData)
  ...
  if not fuelData.needsFuel then
    exports['hud']:NotificationError("Vehicle Does Not Need Refueling")
    return
  end

  if LocalPlayer.state.Character:GetData("Cash") < fuelData.cost then
    exports['hud']:NotificationError("Not Enough Cash to Refuel")
    return
  end
  ...
  Progress:ProgressWithStartAndTick({
    ...
    if GetIsVehicleEngineRunning(entityData.entity) then
      ...
      if chance == 69 then
        ...
        exports['hud']:NotificationInfo("Nice One Champ")
    ...
    exports['core']:CallbacksServer("Fuel:CompleteFueling", {
      ...
      if success and amount then
        exports['hud']:NotificationSuccess(string.format("Refueled Vehicle for $%d", amount))
        SetVehicleFuelLevel(entityData.entity, GetVehicleFuelLevel(entityData.entity) + fuelAmount)
      else
        exports['hud']:NotificationSuccess("Error Refueling")
      end
    end)
  end)
end)
    ...
    if vehState.VIN and vehState.Fuel ~= nil then
      ...
      if requiredFuel and requiredFuel > 1 then
        ...
        if fuelAmount <= 0 then
          return exports['hud']:NotificationError("The Petrol Can Is Empty")
        end
        ...
        Progress:ProgressWithStartAndTick({
          ...
          exports['core']:CallbacksServer("Fuel:CompleteJerryFueling", {
            vehNet = VehToNet(entityData.entity),
            newAmount = math.floor(vehState.Fuel + fuelAmount + 0.0),
          }, function(success)
            if success then
              exports['hud']:NotificationSuccess("Refueled Vehicle")
            else
              exports['hud']:NotificationError("Error Refueling")
            end
          end)
        end)
      else
