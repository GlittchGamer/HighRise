...
function OpenWheelMenu()
  ...
  wheelMenu = exports['menu']:Create('vehicle_wheels', 'Vehicle Wheels', function()
    wheelMenuOpen = true
  ...
  wheelMenu.Add:Button('Save', { success = true }, function()
    exports['core']:LoggerTrace('Fitment', 'Attept Save')

    if editedFrontTrack or editedRearTrack or editedWidth then
      exports['core']:CallbacksServer('Vehicles:WheelFitment', {
        ...
        if success then
          exports['hud']:NotificationSuccess('Wheel Fitment Saved')
        else
          exports['hud']:NotificationError('Wheel Fitment Saving Failed')
        end
      end)

      wheelMenu:Close()
    else
      exports['hud']:NotificationError('There Was Nothing to Save')
    end
  end)
  ...
  wheelMenu.Add:Button('Reset', { error = true }, function()
    exports['core']:LoggerTrace('Fitment', 'Attept Reset')
    ...
    exports['core']:CallbacksServer('Vehicles:WheelFitment', {
      ...
      if success then
        exports['hud']:NotificationSuccess('Wheel Fitment Reset')
      else
        exports['hud']:NotificationError('Wheel Fitment Reset Failed')
      end
    end)
