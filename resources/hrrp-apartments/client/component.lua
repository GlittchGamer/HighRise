...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Apartment", {
    ...
    for k, v in ipairs(GlobalState["Apartments"]) do
      ...
      exports['hrrp-polyzone']:PolyZoneCreateBox(aptId, apt.coords, apt.length, apt.width, apt.options, {
        tier = k
      })

      exports['hrrp-blips']:Add(aptId, apt.name, apt.coords, 475, 25)
      _pzs[aptId] = {
        name = apt.name,
        id = apt.id,
      }
...
-- 	exports["hrrp-base"]:CallbacksServer("Apartment:GetRequests", {}, function(requets)
-- 		if #reqeusts > 0 then
-- 			local menu = {
...
-- 			ListMenu:Show(menu)
-- 		else
-- 			exports['hrrp-hud']:NotificationError("You Have No Requesting Visitors")
-- 		end
-- 	end)
-- end
...
RegisterNetEvent("Apartment:Client:InnerStuff", function(aptId, unit, wakeUp)
  ...
  if wakeUp then
    Citizen.SetTimeout(250, function()
      exports["hrrp-animations"]:EmotesWakeUp(p.interior.wakeup)
    end)
  end
...
AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
  if _pzs[id] and string.format("apt-%s", LocalPlayer.state.Character:GetData("Apartment") or 1) == id then
    ...
    exports['hrrp-hud']:ActionShow(str)
  end
end)
...
AddEventHandler("Apartment:Client:DoRequestEntry", function(values, data)
  exports["hrrp-base"]:CallbacksServer("Apartment:RequestEntry", {
    inZone = data,
    target = values.unit,
  })
end)
...
_APTS = {
  Enter = function(self, tier, id)
    exports["hrrp-base"]:CallbacksServer("Apartment:Enter", {
      id = id or -1,
      tier = tier,
    }, function(s)
      if s then
  ...
  Exit = function(self)
    ...
    exports["hrrp-base"]:CallbacksServer("Apartment:Exit", {}, function()
      DoScreenFadeOut(1000)
      while not IsScreenFadedOut() do
        Citizen.Wait(10)
      end
  ...
  Extras = {
    Stash = function(self)
      exports["hrrp-base"]:CallbacksServer("Apartment:Validate", {
