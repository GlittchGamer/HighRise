...
function GetPZoneAtCoords(endCoords, distanceOverride)
  local isInside, insideZone = interactionComboZone:isPointInside(endCoords)
  if isInside and insideZone then
    local zoneData  = interactionZones[insideZone.name]
    local proximity = distanceOverride and distanceOverride or zoneData.proximity
    if zoneData and zoneData.enabled and (#(GetEntityCoords(GLOBAL_PED) - endCoords) <= proximity) then
      return zoneData
    end
  end
  return false
...
function GetZones(point)
  if not interactionComboZone then
    return false
  end
  return interactionComboZone:getZones(point)
end
