...
AddEventHandler('Vehicles:Client:StartUp', function()
  exports['pedinteraction']:Add('veh_impound', `ig_floyd`, vector3(-193.282, -1162.433, 22.7), 270.4, 10.0, {
    {
      icon = "truck-tow",
    ...
    },
    {
      ...
      isEnabled = function(data, entityData)
        if not exports['jobs']:JobsPermissionsHasJob('tow') then
          return true
        end
        return false
      end,
...
AddEventHandler('Vehicles:Client:CharacterLogin', function()
  if _vehicleImpound then
    exports['blips']:Add('veh_impound', _vehicleImpound.name, _vehicleImpound.coords, 317, 16, 0.6)
  end
end)
...
AddEventHandler('Vehicles:Client:RequestTow', function(entityData)
  if DoesEntityExist(entityData.entity) and IsVehicleEmpty(entityData.entity) then
    ...
    if vState and vState.VIN then
      ...
      local canRegularImpound = exports['jobs']:JobsPermissionsHasPermission(_impoundConfig.RequiredPermission)
      local canPoliceImpound = exports['jobs']:JobsPermissionsHasPermission(_impoundConfig.Police.RequiredPermission)
    ...
    if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
          exports['core']:CallbacksServer("Vehicles:Impound:TagVehicle", {
            ...
            exports['hud']:NotificationSuccess('Tow Requested')
          end)
        else
          exports['hud']:NotificationError('Tow Request Failed')
        end
      end)
    else
      exports['hud']:NotificationError('Cannot Request Tow On That Vehicle Anymore...')
    end
  end
end)
...
AddEventHandler('Vehicles:Client:Impound', function(data)
  if attemptingImpoundOnVehicle and DoesEntityExist(attemptingImpoundOnVehicle) then
    if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
          exports['core']:CallbacksServer('Vehicles:Impound', {
            ...
            if success then
              exports['hud']:NotificationSuccess('Vehicle Impounded')
            else
      ...
      exports['hud']:NotificationError('Cannot Impound That Vehicle Anymore...')
    end
  end
end)

AddEventHandler('Vehicles:Client:TowReleaseMenu', function()
  exports['core']:CallbacksServer('Vehicles:GetVehiclesInImpound', {}, function(vehicleData, time)
    if vehicleData and #vehicleData > 0 then
      ...
      exports['hud']:NotificationInfo('None of Your Vehicles Are Impounded')
    end
  end)
end)
...
AddEventHandler('Vehicles:Client:TowRequestRelease', function(data)
  if data and data.VIN then
    ...
    if freeImpoundSpace and #(pedCoords - freeImpoundSpace.xyz) <= 50.0 then
      exports['core']:CallbacksServer('Vehicles:RetrieveVehicleFromImpound', {
        VIN = data.VIN,
        coords = freeImpoundSpace.xyz,
        heading = freeImpoundSpace.w,
      }, function(success)
        if success then
          exports['hud']:NotificationSuccess('Vehicle Retrieved From Impound. It is Parked in the Lot Outside', 10000)
        else
          exports['hud']:NotificationError('Error Retrieving From Impound')
        end
      end)
    else
      exports['hud']:NotificationError('Impound Parking Lot Full')
    end
  end
end)
