AddEventHandler('Vehicles:Client:StartUp', function()
  exports['core']:CallbacksRegisterClient('Vehicles:GetFakePlateAddingVehicle', function(data, cb)
  ...
  if entityData and DoesEntityExist(entityData.entity) and CanModelHaveFakePlate(GetEntityModel(entityData.entity)) then
    Progress:Progress({
      ...
      if not cancelled and Vehicles:HasAccess(entityData.entity) and (Vehicles.Utils:IsCloseToRearOfVehicle(entityData.entity) or Vehicles.Utils:IsCloseToFrontOfVehicle(entityData.entity)) then
        exports['core']:CallbacksServer('Vehicles:RemoveFakePlate', VehToNet(entityData.entity), function(success)
          if success then
            exports['hud']:NotificationSuccess('Removed Plate Successfully')
          else
            exports['hud']:NotificationError('Could not Remove Plate')
          end
        end)
      else
        exports['hud']:NotificationError('Could not Remove Plate')
  ...
  if entityData and DoesEntityExist(entityData.entity) then
    Progress:Progress({
      ...
      if not cancelled and Vehicles:HasAccess(entityData.entity, true) then
        exports['core']:CallbacksServer('Vehicles:RemoveHarness', VehToNet(entityData.entity), function(success)
          if success then
            exports['hud']:NotificationSuccess('Removed Harness Successfully')
          else
            exports['hud']:NotificationError('Could not Remove Harness')
          end
        end)
      else
        exports['hud']:NotificationError('Could not Remove Harness')
