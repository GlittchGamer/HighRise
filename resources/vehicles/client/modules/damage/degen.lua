...
function RunVehiclePartsDamage(veh, isRandom, damageAmount, destroyed)
  if DoesEntityExist(veh) then
    ...
    if isRandom then
      ...
      exports['core']:LoggerTrace('Vehicles', 'Running Regular Damage Degen - Data: ' .. json.encode(currentDamage, { indent = true }))
    else
      if damageAmount then
        ...
        exports['core']:LoggerTrace('Vehicles', 'Running Collision Damage Degen - Data: ' .. json.encode(currentDamage))
      elseif destroyed then
        ...
        exports['core']:LoggerTrace('Vehicles', 'Running Destroyed Damage Degen - Data: ' .. json.encode(currentDamage))
      end
    end
  ...
  if DoesEntityExist(veh) and INSIDE_HAS_DEGEN then
    ...
    if type(currentDamage) == 'table' then
      ...
      if currentDamage.Axle and currentDamage.Axle <= 30.0 then
        ...
        if amount then
          exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Axle')
      ...
      if currentDamage.Electronics and currentDamage.Electronics <= 22.5 then
        if (currentDamage.Electronics >= 15.0 and funChance >= 80) or (currentDamage.Electronics < 15.0 and currentDamage.Electronics >= 5.0 and funChance >= 65) or (currentDamage.Electronics < 5.0 and funChance >= 50) then
          exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Electronics')
      ...
      if (currentDamage.Brakes and currentDamage.Brakes <= 25.0) and funChance >= 50 then
        exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Brakes')
        Citizen.CreateThread(function()
          WEAK_BRAKES = true
      ...
      if (currentDamage.FuelInjectors and currentDamage.FuelInjectors <= 30.0) then
        ...
        if amount then
          exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Fuel Injectors')
      ...
      if (currentDamage.Radiator and currentDamage.Radiator <= 30.0) then
        ...
        if damageHit then
          ...
          if newHealth <= -2000 then
            exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Radiator')
      ...
      if (currentDamage.Transmission and currentDamage.Transmission <= 30.0) then
        ...
        if amount then
          exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Transmission')
          Citizen.CreateThread(function()
            for i = 1, amount do
      ...
      if (currentDamage.Clutch and currentDamage.Clutch <= 30.0) then
        ...
        if wait then
          exports['core']:LoggerTrace('Vehicles', 'Running Damage Effects - Clutch')
...
function SetVehicleWeakBrakesState(veh, state)
  if state then
    exports['core']:LoggerTrace('Vehicles', 'Weak Brakes: On')
    SetVehicleHandlingOverrideMultiplier(veh, 'fBrakeForce', 'Float', 0.5)
  else
    exports['core']:LoggerTrace('Vehicles', 'Weak Brakes: Off')
