...
function RegisterCallbacks()
    exports['core']:CallbacksRegisterServer('Vehicles:GetKeys', function(source, VIN, cb)
        Vehicles.Keys:Add(source, VIN)
        cb(true)
    end)

    exports['core']:CallbacksRegisterServer('Vehicles:ToggleLocks', function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:BreakOpenLock', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data.netId)
        local vehState = Entity(veh).state
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:GetVehiclesInStorage', function(source, storageId, cb)
            ...
            if type(storageData.fleet) == 'table' and myDuty then
                for k, v in ipairs(storageData.fleet) do
                    if v.JobId == myDuty then
                        local jobData = exports['jobs']:JobsPermissionsHasJob(source, v.JobId, v.WorkplaceId)
                        if jobData then
                            local jobPermissions = exports['jobs']:JobsPermissionsGetPermissionsFromJob(source, jobData.Id)
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:GetVehiclesInPropertyStorage', function(source, storageId, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:GetVehiclesInStorageSelect', function(source, data, cb)
        if data.VIN then
            Vehicles.Owned:GetVIN(data.VIN, cb)
        end
    end)

    exports['core']:CallbacksRegisterServer('Vehicles:RetrieveVehicleFromStorage', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
            ...
            if vehicle and vehicle.VIN then
                ...
                elseif vehicle.Owner.Type == 1 then
                    ...
                    if onDuty and onDuty == vehicle.Owner.Id then
                        local jobPermissions = exports['jobs']:JobsPermissionsGetPermissionsFromJob(source, vehicle.Owner.Id, vehicle.Owner.Workplace)
                        if jobPermissions then
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:PutVehicleInStorage', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
        ...
        if storageData.retrievalOnly then
            exports['core']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Store Vehicles Here')
            cb(false)
            return
        end
        ...
        elseif vehicleOwner.Type == 1 then
            if storageData.fleet and DoesVehiclePassFleetRestrictions(vehicleOwner, storageData.fleet) then
                ...
                if onDuty and onDuty == vehicleOwner.Id then
                    local jobPermissions = exports['jobs']:JobsPermissionsGetPermissionsFromJob(source, vehicleOwner.Id, vehicleOwner.Workplace)
                    if jobPermissions then
                        local allowedLevel = GetAllowedFleetVehicleLevelFromJobPermissions(jobPermissions)
                ...
                exports['core']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Store This Vehicle Here')
            end
        end
    ...
    exports['core']:CallbacksRegisterServer("Vehicles:Impound:TagVehicle", function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:PutVehicleInPropertyStorage', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:Impound', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
        ...
        if DoesEntityExist(veh) and myDuty and exports['jobs']:JobsPermissionsHasPermission(source, _impoundConfig.RequiredPermission) or exports['jobs']:JobsPermissionsHasPermission(source, _impoundConfig.Police.RequiredPermission) then
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:GetVehiclesInImpound', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
        if not character then
            cb(false)
            return
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:RetrieveVehicleFromImpound', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:CompleteCustoms', function(source, data, cb)
      local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:WheelFitment', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:CompleteRepair', function(source, data, cb)
        local character = exports['core']:FetchSource(source):GetData('Character')
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:CleanVehicle', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data.vNet)
        local vehState = Entity(veh)
        if DoesEntityExist(veh) and vehState and vehState.state.VIN then
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:RemoveFakePlate', function(source, data, cb)
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:RemoveHarness', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data)
        if veh and DoesEntityExist(veh) then
            local vehState = Entity(veh).state
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:Tranfers:CompleteTransfer', function(source, data, cb)
        local SID, VIN in data
        local player = exports['core']:FetchSource(source)

        if SID and VIN and player then
            local char = player:GetData('Character')
            local target = exports['core']:FetchSID(SID)
            ...
            if char and target and vehicle and vehicle:GetData('Owner')?.Type == 0 and vehicle:GetData('Owner')?.Id == char:GetData('SID') then
                ...
                if targetChar and source ~= targetChar:GetData('Source') then
                    ...
                    if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) <= 10.0 then
                        ...
                        exports['core']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Transfer to Someone That Isn\'t Nearby')
                    end
                end
            end
        end
    ...
    exports['core']:CallbacksRegisterServer('Vehicles:RemoveNitrous', function(source, data, cb)
        local char = exports['core']:FetchSource(source):GetData("Character")
        local veh = NetworkGetEntityFromNetworkId(data)
