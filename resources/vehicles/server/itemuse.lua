function RegisterItemUses()
  Inventory.Items:RegisterUse("lockpick", "Vehicles", function(source, slot, itemData)
    Citizen.SetTimeout(500, function()
      exports['core']:CallbacksClient(source, "Vehicles:Lockpick", true, function(using, success)
        if using then
  ...
  Inventory.Items:RegisterUse("adv_lockpick", "Vehicles", function(source, slot, itemData)
    Citizen.SetTimeout(500, function()
      exports['core']:CallbacksClient(source, "Vehicles:AdvLockpick", true, function(using, success)
        if using then
          local newValue = slot.CreateDate - (60 * 60 * 24)
          if success then
            newValue = slot.CreateDate - (60 * 60 * 12)
  ...
  Inventory.Items:RegisterUse("electronics_kit", "Vehicles", function(source, slot, itemData)
    Citizen.SetTimeout(500, function()
      exports['core']:CallbacksClient(source, "Vehicles:Hack", true, function(using, success)
        if using then
          local newValue = slot.CreateDate - (60 * 60 * 24)
          if success then
            newValue = slot.CreateDate - (60 * 60 * 12)
  ...
  Inventory.Items:RegisterUse("adv_electronics_kit", "Vehicles", function(source, slot, itemData)
    Citizen.SetTimeout(500, function()
      exports['core']:CallbacksClient(source, "Vehicles:AdvHack", true, function(using, success)
        if using then
          local newValue = slot.CreateDate - (60 * 60 * 24)
          if success then
            newValue = slot.CreateDate - (60 * 60 * 12)
  ...
  Inventory.Items:RegisterUse("screwdriver", "Vehicles", function(source, slot, itemData)
    Citizen.SetTimeout(1500, function()
      exports['core']:CallbacksClient(source, "Vehicles:Lockpick", {
        {
          base = 4000,
          mod = 900,
        },
  ...
  Inventory.Items:RegisterUse("repairkit", "Vehicles", function(source, itemData)
    exports['core']:CallbacksClient(source, "Vehicles:RepairKit", false, function(success)
      if success then
  ...
  Inventory.Items:RegisterUse("repairkitadv", "Vehicles", function(source, itemData)
    exports['core']:CallbacksClient(source, "Vehicles:RepairKit", true, function(success)
      if success then
        Inventory.Items:RemoveSlot(itemData.Owner, itemData.Name, 1, itemData.Slot, itemData.invType)
      end
    end)
  ...
  Inventory.Items:RegisterUse("fakeplates", "Vehicles", function(source, itemData)
    ...
    if currentMeta then
      exports['core']:CallbacksClient(source, "Vehicles:GetFakePlateAddingVehicle", {}, function(veh)
        if not veh then
          return
        end
        ...
        if veh and DoesEntityExist(veh) then
          ...
          if not vehicle:GetData("FakePlate") then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Success", "Fake Plate Installed")
          else
            exports['core']:ExecuteClient(source, "Notification", "Error", "A Fake Plate is Already Installed")
          end
        end
      end)
    end
  ...
  Inventory.Items:RegisterUse("car_bomb", "Vehicles", function(source, itemData)
    exports['core']:CallbacksClient(source, "Vehicles:UseCarBomb", {}, function(veh, reason, config)
      if not veh then
        if reason then
          exports['core']:ExecuteClient(source, "Notification", "Error", reason)
        end
        return
      end
      veh = NetworkGetEntityFromNetworkId(veh)
      if veh and DoesEntityExist(veh) then
        ...
        if char then
          ...
          if not vehState.CarBomb then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Success", "Car Bomb Installed")
          else
            exports['core']:ExecuteClient(source, "Notification", "Error", "Vehicle Already Has Car Bomb")
          end
        else
          exports['core']:ExecuteClient(source, "Notification", "Error", "Error Installing Car Bomb")
        end
      end
    end)
  end)
  ...
  Inventory.Items:RegisterUse("harness", "Vehicles", function(source, itemData)
    exports['core']:CallbacksClient(source, "Vehicles:InstallHarness", {}, function(veh)
      if not veh then
        return
      end
      veh = NetworkGetEntityFromNetworkId(veh)
      if veh and DoesEntityExist(veh) then
        ...
        if Inventory.Items:RemoveSlot(itemData.Owner, itemData.Name, 1, itemData.Slot, itemData.invType) then
          vehState.Harness = 10
          exports['core']:ExecuteClient(source, "Notification", "Success", "Harness Installed")
        end
      end
    end)
  end)
  ...
  Inventory.Items:RegisterUse("nitrous", "Vehicles", function(source, itemData)
    if itemData?.MetaData?.Nitrous and itemData?.MetaData?.Nitrous > 0 then
      exports['core']:CallbacksClient(source, "Vehicles:InstallNitrous", {}, function(veh)
        if not veh then
          return
        end
        veh = NetworkGetEntityFromNetworkId(veh)
        if veh and DoesEntityExist(veh) then
          ...
          if Inventory.Items:RemoveSlot(itemData.Owner, itemData.Name, 1, itemData.Slot, itemData.invType) then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Success", "Nitrous Oxide Installed")
          end
        end
      end)
    else
      exports['core']:ExecuteClient(source, "Notification", "Error", "The Bottle is Empty!")
    end
  end)
end
...
function UseCarPolish(source, itemData, type)
  ...
  exports['core']:CallbacksClient(source, "Vehicles:UseCarPolish", {}, function(veh)
    if not veh then
      return
    end
    veh = NetworkGetEntityFromNetworkId(veh)
    if veh and DoesEntityExist(veh) then
      ...
      if (not vehState.Polish) or (vehState.Polish?.Type ~= type) or (vehState.Polish?.Time and (os.time() - vehState.Polish?.Time) >= (60 * 60 * 24)) then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Success", "Polish Applied")
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "Vehicle Already Has That Polish and It Was Recently Installed")
      end
    end
  end)
end
