...
function RegisterPersonalPlateCallbacks()
  Inventory.Items:RegisterUse("personal_plates", "Vehicles", function(source, itemData)
    ...
    if not char or (Player(source).state.onDuty ~= "government" and Player(source).state.onDuty ~= "dgang") then
      exports['core']:ExecuteClient(source, "Notification", "Error", "Error")
      return
    end

    exports['core']:CallbacksClient(source, "Vehicles:GetPersonalPlate", {}, function(veh, plate)
      ...
      if veh and DoesEntityExist(veh) then
        ...
        if not vehicle then
          exports['core']:ExecuteClient(source, "Notification", "Error", "Can't Do It on This Vehicle")
          return
        end
        ...
        if not newPlate then
          exports['core']:ExecuteClient(source, "Notification", "Error", "Invalid Plate Formatting")
          return
        end

        if IsPersonalPlateTaken(newPlate) then
          exports['core']:ExecuteClient(source, "Notification", "Error", "That Plate is Taken")
          return
        end
        ...
        exports['core']:ExecuteClient(source, "Notification", "Success", "Personal Plate Setup")
        exports['core']:LoggerInfo('Vehicles', string.format("Personal Plate Change For Vehicle: %s. %s -> %s", vehState.VIN, originalPlate, newPlate))
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "Error")
      end
    end)
