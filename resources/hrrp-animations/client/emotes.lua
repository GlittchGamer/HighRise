...
ANIMATIONS.Emotes = {
  Play = function(self, emote, fromUserInput, time, notCancellable, skipDisarm)
    ...
    if fromUserInput and (IsPedBeingStunned(LocalPlayer.state.ped) or IsPedRagdoll(LocalPlayer.state.ped) or IsPedFalling(LocalPlayer.state.ped)) then
      return exports['hrrp-hud']:NotificationError('Cannot Do Animation Now')
    end
    ...
    if emote ~= nil and type(emote) == 'string' then
      ...
      elseif AnimData.PropEmotes[name] ~= nil then
        ...
        exports['hrrp-hud']:NotificationError('Invalid Emote')
      end
...
local function exportHandler(exportName, func)
  AddEventHandler(('__cfx_export_%s_%s'):format(GetCurrentResourceName(), exportName), function(setCB)
    setCB(function(...)
      return func(ANIMATIONS.Emotes, ...)
    end)
  end)
end

for k, v in pairs(ANIMATIONS.Emotes) do
  if type(v) == "function" then
    exportHandler('Emotes' .. k, v)
  end
end
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
  ...
  AttachEntityToEntity(prop, LocalPlayer.state.ped, GetPedBoneIndex(LocalPlayer.state.ped, bone), off1, off2, off3, rot1, rot2, rot3, true, true, false, true, 1,
    true)
  ...
  if (fromUserInput and emoteData.AdditionalOptions.AvailableToChar) or (not fromUserInput) then
    ...
    if not GLOBAL_VEH then
      ...
      elseif ChosenDict == "ScenarioObject" then
        ...
        ClearPedTasks(LocalPlayer.state.ped)
        TaskStartScenarioAtPosition(LocalPlayer.state.ped, ChosenAnimation, BehindPlayer['x'], BehindPlayer['y'], BehindPlayer['z'],
          GetEntityHeading(LocalPlayer.state.ped), 0, 1, false)
        IsInAnimation = true
        IsInEmoteName = emoteName
        return
    ...
    if emoteData.AdditionalOptions then
      ...
      if emoteData.AdditionalOptions.PtfxAsset then
        if emoteData.AdditionalOptions.PtfxAlways then
          ...
          PtfxPrompt = true
          exports['hrrp-hud']:NotificationInfo(emoteData.AdditionalOptions.PtfxInfo, 5000)
          Citizen.CreateThread(function()
            while PtfxPrompt do
    ...
    if emoteData.AdditionalOptions then
      if emoteData.AdditionalOptions.Prop then
        ...
        if emoteData.AdditionalOptions.SecondProp then
          ...
          SecondPropPl1, SecondPropPl2, SecondPropPl3, SecondPropPl4, SecondPropPl5, SecondPropPl6 = table.unpack(emoteData.AdditionalOptions
            .SecondPropPlacement)
        ...
        if SecondPropEmote then
          local stupidProp2 = AddPropToPlayer(SecondPropName, SecondPropBone, SecondPropPl1, SecondPropPl2, SecondPropPl3, SecondPropPl4, SecondPropPl5,
            SecondPropPl6)
        end
      end
    end
...
function PtfxStart()
  ...
  Ptfx = StartNetworkedParticleFxLoopedOnEntityBone(PtfxName, PtfxAt, Ptfx1, Ptfx2, Ptfx3, Ptfx4, Ptfx5, Ptfx6, GetEntityBoneIndexByName(PtfxName, "VFX"),
