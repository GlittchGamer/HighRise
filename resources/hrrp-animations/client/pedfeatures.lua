...
ANIMATIONS.PedFeatures = {
  ToggleCrouch = function(self, toggle)
    ...
    if toggle then
      ...
      if not LocalPlayer.state.drunkMovement then
        exports["hrrp-animations"]:PedFeaturesRequestFeaturesUpdate()
      end
      _isCrouched = false
    end
  end,
  SetWalk = function(self, walk, label)
    if LocalPlayer.state.isLimping then
      ...
      if walk == 'reset' then
        walkStyle = walk
        exports["hrrp-base"]:CallbacksServer('Animations:UpdatePedFeatures', { type = 'walk', data = 'default' }, function(success)
          if success then
            exports['hrrp-hud']:NotificationInfo('Reset Walking Style', 5000)
          end
        end)
      else
        walkStyle = walk
        exports["hrrp-base"]:CallbacksServer('Animations:UpdatePedFeatures', { type = 'walk', data = walk }, function(success)
          if success then
            exports['hrrp-hud']:NotificationSuccess('Saved Walking Style: ' .. label, 5000)
          end
        end)
      end
    else
  ...
  SetExpression = function(self, expression, label)
    if expression == 'reset' then
      ...
      exports["hrrp-base"]:CallbacksServer('Animations:UpdatePedFeatures', { type = 'expression', data = 'default' }, function(success)
        if success then
          exports['hrrp-hud']:NotificationInfo('Expression Reset', 5000)
        end
      end)
    else
      SetFacialIdleAnimOverride(LocalPlayer.state.ped, expression, 0)
      facialExpression = expression
      exports["hrrp-base"]:CallbacksServer('Animations:UpdatePedFeatures', { type = 'expression', data = expression }, function(success)
        if success then
          exports['hrrp-hud']:NotificationSuccess('Saved Expression: ' .. label, 5000)
        end
      end)
    end
  end,
...
local function exportHandler(exportName, func)
  AddEventHandler(('__cfx_export_%s_%s'):format(GetCurrentResourceName(), exportName), function(setCB)
    setCB(function(...)
      return func(ANIMATIONS.PedFeatures, ...)
    end)
  end)
end

for k, v in pairs(ANIMATIONS.PedFeatures) do
  if type(v) == "function" then
    exportHandler('PedFeatures' .. k, v)
  end
end
