RegisterNetEvent("Animations:Client:CampChair", function()
  if LocalPlayer.state.loggedIn then
    local inAnim = exports["hrrp-animations"]:EmotesGet()
    if inAnim and inAnim == "campchair" then
      exports["hrrp-animations"]:EmotesCancel()
    elseif not inAnim then
      exports["hrrp-animations"]:EmotesPlay("campchair", false, false, false)
    end
  end
end)
...
RegisterNetEvent("Animations:Client:BeanBag", function()
  if LocalPlayer.state.loggedIn then
    ...
    if inAnim and inAnim == "beanbag" then
      exports["hrrp-animations"]:EmotesCancel()
    elseif not inAnim then
      exports["hrrp-animations"]:EmotesPlay("beanbag", false, false, false)
    end
  end
end)
...
RegisterNetEvent('Animations:Client:Binoculars', function()
  if LocalPlayer.state.loggedIn then
    ...
    elseif not inAnim and not IsPedInAnyVehicle(LocalPlayer.state.ped, true) then
      exports["hrrp-animations"]:EmotesPlay('binoculars', false, false, false)
      exports['hrrp-hud']:Hide()
      ...
      Citizen.CreateThread(function()
        ...
        while exports["hrrp-animations"]:EmotesGet() == "binoculars" and LocalPlayer.state.loggedIn do
...
RegisterNetEvent('Animations:Client:Camera', function()
  if LocalPlayer.state.loggedIn then
    ...
    if inAnim and inAnim == "camera_item" then
      exports["hrrp-animations"]:EmotesCancel()
    elseif not inAnim and not IsPedInAnyVehicle(LocalPlayer.state.ped, true) then
      exports["hrrp-animations"]:EmotesPlay('camera_item', false, false, false)
      exports['hrrp-hud']:Hide()
      ...
      Citizen.CreateThread(function()
        ...
        while exports["hrrp-animations"]:EmotesGet() == "camera_item" and LocalPlayer.state.loggedIn do
...
function EnsureCharacterAnimStates()
  ...
  if character then
    ...
    if #animStates > 0 then
      ...
      exports["hrrp-animations"]:EmotesPlay(animStates[1], false, false, true)
      _doingAnimState = animStates[1]
    elseif _doingAnimState and _doingAnimState == exports["hrrp-animations"]:EmotesGet() then
...
RegisterNetEvent('Animations:Client:DoPDCallEmote', function(emote)
  if LocalPlayer.state.loggedIn then
    exports["hrrp-animations"]:EmotesPlay('phonecall', false, 10000, true)
  end
end)

RegisterNetEvent('Animations:Client:DiceRoll', function()
  exports["hrrp-animations"]:EmotesPlay('dice', true)
