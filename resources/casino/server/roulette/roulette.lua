...
function CheckRouletteWinners(tableId, bets, winningIndex)
  ...
  for k, v in pairs(totalsWon) do
    if v > 0 then
      local plyr = exports['core']:FetchSource(k)
      if plyr then
...
AddEventHandler("Casino:Server:Startup", function()
  ...
  exports['core']:CallbacksRegisterServer("Casino:JoinRoulette", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Casino:LeaveRoulette", function(source, data, cb)
    --local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  -- Bets
  exports['core']:CallbacksRegisterServer("Casino:BetRoulette", function(source, data, cb)
    --local char = exports['core']:FetchSource(source):GetData("Character")
    ...
    if (amount >= 100) and _roulette[roulettePlayer.Table] and not _roulette[roulettePlayer.Table].Started and _roulette[roulettePlayer.Table].Seats[roulettePlayer.LocalChair] then
      if (_roulette[roulettePlayer.Table].Seats[roulettePlayer.LocalChair].TotalBet + amount) <= _rouletteTables[roulettePlayer.Table].maxBet then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "Over Table Bet Limit")
        cb(false)
      end
    else
      cb(false)
