...
AddEventHandler("Casino:Server:Startup", function()
  ...
  exports['core']:CallbacksRegisterServer("Casino:JoinBlackjack", function(source, chairId, cb)
  ...
  exports['core']:CallbacksRegisterServer("Casino:LeaveBlackjack", function(source, data, cb)
    --local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Casino:StartBlackjack", function(source, data, cb)
    local blackjackPlayer = _blackjackPlayers[source]
    if not blackjackPlayer then
      return cb(false)
    end
  ...
  -- Bet Confirmation
  exports['core']:CallbacksRegisterServer("Casino:BetBlackjack", function(source, data, cb)
    --local char = exports['core']:FetchSource(source):GetData("Character")
    local blackjackPlayer = _blackjackPlayers[source]
    if not blackjackPlayer then
      return cb(false)
    ...
    if _blackjack[blackjackPlayer.Table] and _blackjack[blackjackPlayer.Table].Seats[blackjackPlayer.LocalChair] then
      if Casino.Chips:Modify(source, -data) then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "Not Enough Chips")
        cb(false)
      end
    else
      cb(false)
...
function GetHitStandResponse(src, currentHand, canDouble, currentBet)
  local p = promise.new()
  exports['core']:CallbacksClient(src, "Casino:Client:RequestHitStand", { currentHand = currentHand, canDouble = canDouble, currentBet = currentBet },
