...
AddEventHandler("Casino:Client:Startup", function()
  exports['ox_target']:TargetingZonesAddBox("casino-wheelspin", "circle-dashed", vector3(989.45, 42.64, 71.27), 1.0, 1.2, {
    heading = 330,
    --debugPoly=true,
    minZ = 70.47,
    maxZ = 73.27
  }, {
    {
      ...
      isEnabled = function()
        return GlobalState["CasinoOpen"] and not GlobalState["Casino:WheelStarted"] and not GlobalState["Casino:WheelSpinning"] and
            not GlobalState["Casino:WheelLocked"]
      end,
    },
    {
      icon = "gift-card",
...
AddEventHandler("Casino:Client:UnlockWheel", function()
  exports['core']:CallbacksServer("Casino:UnlockWheel", {}, function(success)
    if success then
      exports['hud']:NotificationSuccess("Wheel Unlocked")
    else
...
AddEventHandler("Casino:Client:StartSpin", function(_, data)
  exports['core']:CallbacksServer("Casino:WheelStart", data, function(success, tooPoor)
    if success then
      ...
      exports['core']:CallbacksServer("Casino:WheelSpin", {})
      ...
      if tooPoor then
        if data.turbo then
          exports['hud']:NotificationError("Not Enough Cash or No VIP Card")
        else
