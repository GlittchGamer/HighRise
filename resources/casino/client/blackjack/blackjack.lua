...
AddEventHandler("Casino:Client:Startup", function()
  ...
  exports['core']:CallbacksRegisterClient("Casino:Client:RequestHitStand", function(data, cb)
    local itemList = {
      {
        label = "Stand",
    ...
    elseif res?.timeout then
      cb(false)
      exports['hud']:NotificationError("Ran Out of Time...")
    else
      cb(false)
    end
  end)
...
AddEventHandler("Casino:Client:JoinBlackjack", function(_, data)
  ...
  if tableId == data.table then
    exports['core']:CallbacksServer("Casino:JoinBlackjack", _BJclosestChair, function(success, table, chair)
      if success then
        ...
        if table == "vip" then
          exports['hud']:NotificationError("You're Not a VIP Loser")
        else
          exports['hud']:NotificationError("Someone Is Sat There")
        end
      end
    end)
  end
...
AddEventHandler("Casino:Client:StartBlackjack", function(_, data)
  if _BJsatAtTable and data?.table == _BJsatAtTable then
    exports['core']:CallbacksServer("Casino:StartBlackjack", {}, function(success)
      if success then
...
AddEventHandler("Casino:Client:LeaveBlackjack", function(_, data)
  if _BJsatAtTable and data?.table == _BJsatAtTable then
    exports['core']:CallbacksServer("Casino:LeaveBlackjack", {}, function(success)
      if success then
        _BJsatAtTable = false
        _BJsatAtLocalChair = false
        leaveBlackjackSeat()
...
RegisterNetEvent("Casino:Client:BlackjackConfirmBet", function(betAmounts, tableId)
  ...
  if res?.success and res?.data?.confirmBet and res.data.confirmBet >= 100 then
    exports['core']:CallbacksServer("Casino:BetBlackjack", res.data.confirmBet, function(success, gameData)
      if success then
