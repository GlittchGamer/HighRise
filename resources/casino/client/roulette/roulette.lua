...
AddEventHandler("Casino:Client:Startup", function()
  ...
  exports['keybinds']:Add("casino_camera", "C", "keyboard", "Casino - Change Roulette Camera", function()
...
AddEventHandler("Casino:Client:JoinRoulette", function(_, data)
  ...
  if chairData then
    exports['core']:CallbacksServer("Casino:JoinRoulette", { table = data.table, chair = chairData.chairId }, function(success, table, chair)
      if success then
    ...
    exports['hud']:NotificationError("Too Far From Chair")
  end
end)

function LeaveRoulette(skipAnim)
  exports['core']:CallbacksServer("Casino:LeaveRoulette", {}, function(success)
    if success then
...
function RoulettePlaceChips(betType)
  ...
  if betAmount then
    exports['core']:CallbacksServer("Casino:BetRoulette", {
      amount = betAmount,
      betId = betType,
    }, function(success)
      if success then
