...
local startupEventProxy = nil
startupEventProxy = AddEventHandler("Proxy:Shared:RegisterReady", function()
  -- if middlewaresAdded then return end
  ...
  RemoveEventHandler(startupEventProxy)
end)
...
AddEventHandler("Queue:Server:SessionActive", function(source, data)
  Citizen.CreateThread(function()
    ...
    if data == nil then
      ...
      COMPONENTS.Logger:Info("Base",
        string.format("%s (%s) Connected With Source %s", COMPONENTS.Players[source]:GetData("Name"),
          COMPONENTS.Players[source]:GetData("AccountID"), source), {
          console = true,
          discord = {
            embed = true,
            type = 'info',
...
COMPONENTS.Player = {
  ...
  CheckTokens = function(self, source, accountId, existing)
    ...
    if existing ~= nil then
      ...
      MySQL.update.await("UPDATE `users` SET `tokens` = @tokens WHERE `_id` = @id", {
        ['@tokens'] = json.encode(existing),
        ['@id'] = accountId
      })
      MeTokens = existing
    else
...
for k, v in pairs(COMPONENTS.Player) do
  if type(v) == "function" then
    exportHandler("Player" .. k, v)
  elseif type(v) == "table" then
    createExportForObject(v, "Player" .. k)
  end
end
