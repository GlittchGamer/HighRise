        ...
        for k, v in ipairs(COMPONENTS[component]._required) do
          if data[v] == nil then
            COMPONENTS.Logger:Warn('Proxy',
              string.format("[%s] Attempt To Extend Component While Missing Required Attribute: ^2%s^7", GetInvokingResource(), tostring(v)), { console = true })
            return
          end
        end
      end
...
exports('FetchComponent', function(component)
  if not COMPONENTS[component] then
    COMPONENTS.Logger:Warn('Proxy',
      string.format("^1Attempt To Fetch Non-Existent Component^7: ^2%s^7 by script: %s", tostring(component), GetInvokingResource()), { console = true })
    return nil
  end
      ...
      for k, v in pairs(d) do
        if COMPONENTS[v] ~= nil then
          ...
          elseif _attempts[v] > 50 then
            ...
            COMPONENTS.Logger:Error('Proxy', ('[^2%s^7] Failed To Load For [^3%s^7] Invoking Resource %s'):format(v, c, callingResource), { console = true })
            d[k] = nil
          else
