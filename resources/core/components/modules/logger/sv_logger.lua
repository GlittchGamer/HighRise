...
local function exportHandler(exportName, func)
  AddEventHandler(('__cfx_export_%s_%s'):format(GetCurrentResourceName(), exportName), function(setCB)
    setCB(function(...)
      return func(COMPONENTS.Logger, ...)
    end)
  end)
end

for k, v in pairs(COMPONENTS.Logger) do
  if type(v) == "function" then
    exportHandler('Logger' .. k, v)
  end
end
...
function doLog(level, component, log, flags, data)
  Citizen.CreateThread(function()
    ...
    if GlobalState.IsProduction and flags.database then
      MySQL.insert.await(
        "INSERT INTO logs (date, level, component, log, data) VALUES (@date, @level, @component, @log, @data)", {
          ["@date"] = os.time(),
          ["@level"] = level,
          ["@component"] = component,
          ["@log"] = log,
