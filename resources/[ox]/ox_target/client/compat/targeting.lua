...
local _globalPeds = {
...

	{
		...
		isEnabled = function(_, entity)
      local isScriptedNPC = exports['flags']:PedsHasFlag(entity.entity, 'isNPC')
...
local _globalVehicle = {
  {
	...
	},
	{
		...
		if entityData.entity and DoesEntityExist(entityData.entity) then
			if exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(entityData.entity), 'tow_impound_zone') then
			return true
			end
		end
		return false
	...
	-- Mechanic
	{
		...
		if DoesEntityExist(entityData.entity) and (
				Vehicles.Utils:IsCloseToRearOfVehicle(entityData.entity)
				or Vehicles.Utils:IsCloseToFrontOfVehicle(entityData.entity)
			) and exports['mechanic']:CanAccessVehicleAsMechanic(entityData.entity) then
	...
	},
	{
		icon = "car-garage",
		isEnabled = function(data, entityData)
		local inZone = exports['polyzonehandler']:PolyZoneIsCoordsInZone(GetEntityCoords(entityData.entity), false, 'dealerBuyback')
		if inZone then
...
setupEvent = AddEventHandler('Proxy:Shared:RegisterReady', function()
    api.removeTargetsByProperty({ key = "target", value = true })
    api.removeGlobalsByProperty({ key = "target", value = true })
    api.removeZonesByProperty({ key = "target", value = true })
