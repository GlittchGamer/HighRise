AddEventHandler("MDT:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Hire", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Fire", function(source, data, cb)
    ...
    if char and data and data.SID and (hasPerms or isSystemAdmin) then
      ...
      if charData then
        ...
        if isSystemAdmin then
          ...
          local plyrJob = exports['hrrp-jobs']:JobsPermissionsHasJob(source, loggedInJob)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:ManageEmployment", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Update:jobPermissions", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
    local isSystemAdmin = char:GetData('MDTSystemAdmin')
    local hasPerms, loggedInJob = CheckMDTPermissions(source, {
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Suspend", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
      ...
      if charData then
        ...
        if canRemove and data.Length and type(data.Length) == "number" and data.Length > 0 and data.Length < 99 then
          ...
          if updateUserDB then
            local char = exports['hrrp-base']:FetchSID(data.SID)
            if char then
              ...
              exports['hrrp-jobs']:JobsDutyOff(char:GetData("Source"), data.JobId)
            end
          end
        else
          cb(false)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Unsuspend", function(source, data, cb)
    local player = exports['hrrp-base']:FetchSource(source)
    local char = player:GetData("Character")
