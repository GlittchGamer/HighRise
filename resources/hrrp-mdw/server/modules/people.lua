...
_MDT.People = {
  ...
  WHERE EXISTS (
    SELECT 1
    FROM JSON_TABLE(Jobs, '$[*]' COLUMNS (Id INT PATH '$.Id')) AS job
    WHERE job.Id = ?
  )
]]
    ...
    FROM characters
    WHERE (
      CONCAT(First, ' ', Last) REGEXP ? OR
      CAST(SID AS CHAR) REGEXP ?
    )
    AND EXISTS (
      SELECT 1
      FROM JSON_TABLE(Jobs, '$[*]' COLUMNS (Id INT PATH '$.Id')) AS job
      ...
      local params = { job }
      if term then
        table.insert(params, 1, term)
        table.insert(params, 2, term)
      end

      local fetchPeople = MySQL.query.await(qry, params)
      return fetchPeople
    end
  },
  ...
  Update = function(self, requester, id, key, value)
    ...
    if updateCharacter > 0 then
      local target = exports['hrrp-base']:FetchSID(id)
      if target then
...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("MDT:InputSearch:people", function(source, data, cb)
    cb(MDT.People.Search:People(data.term))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("MDT:InputSearch:job", function(source, data, cb)
    cb(_MDT.People.Search:Job(data.job, data.term))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("MDT:InputSearchSID", function(source, data, cb)
    cb(MDT.People.Search:People(data.term))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("MDT:Search:people", function(source, data, cb)
    cb(MDT.People.Search:People(data.term))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("MDT:View:person", function(source, data, cb)
    cb(MDT.People:View(data, true))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("MDT:Update:person", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:CheckCallsign", function(source, data, cb)
