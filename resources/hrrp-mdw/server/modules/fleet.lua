AddEventHandler("MDT:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("MDT:ViewVehicleFleet", function(source, data, cb)
      ...
      MySQL.Async.fetchAll(query, params, function(results)
        if results then
          for k, v in ipairs(results) do
            ...
            v.GovAssigned = json.decode(v.GovAssigned)
            if v.Storage then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:SetAssignedDrivers", function(source, data, cb)
    local hasPerms, loggedInJob = CheckMDTPermissions(source, {
      'FLEET_MANAGEMENT',
    })
    ...
    if hasPerms and loggedInJob and data.vehicle and data.assigned then
      ...
      local params = { json.encode(ass), data.vehicle }
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:TrackFleetVehicle", function(source, data, cb)
    local hasPerms, loggedInJob = CheckMDTPermissions(source, {
      'FLEET_MANAGEMENT',
    })
