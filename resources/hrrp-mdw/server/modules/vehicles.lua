...
_MDT.Vehicles = {
  Search = function(self, term, page, perPage)
    ...
    local query = [[
      SELECT
        ...
        VIN, RegisteredPlate, Make, Model, Type, GovAssigned, Owner -- Add other required fields
      FROM vehicles
      WHERE
    ]]
    ...
    MySQL.Async.fetchAll(query, params, function(results)
      ...
      for k, v in pairs(results) do
        ...
        results[k].GovAssigned = json.decode(v.GovAssigned)
      end
  ...
  View = function(self, VIN)
    ...
    if fetchVehicle.RegisteredPlate then
      local flagged = exports['hrrp-radar']:CheckPlate(fetchVehicle.RegisteredPlate)
    ...
    fetchVehicle.GovAssigned = json.decode(fetchVehicle.GovAssigned)
  ...
  Flags = {
    Add = function(self, VIN, data, plate)
      ...
      if updateFlags > 0 and data.Type and data.Description and plate then
        exports['hrrp-radar']:AddFlaggedPlate(plate, 'Vehicle Flagged in MDT')
      end
      return true
    end,
    Remove = function(self, VIN, flag, plate, removeRadarFlag)
      ...
      if updateFlags > 0 and plate and removeRadarFlag then
        local isFlagged = exports['hrrp-radar']:CheckPlate(plate)
...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Search:vehicle", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:View:vehicle", function(source, data, cb)
    if CheckMDTPermissions(source, false) then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Create:vehicle-flag", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Delete:vehicle-flag", function(source, data, cb)
    if CheckMDTPermissions(source, false, 'police') then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("MDT:Update:vehicle-strikes", function(source, data, cb)
    if CheckMDTPermissions(source, false, 'police') then
