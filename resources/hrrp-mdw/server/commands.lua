function RegisterChatCommands()
  Chat:RegisterAdminCommand("setcallsign", function(source, args, rawCommand)
    ...
    local tPly = exports['hrrp-base']:FetchSID(tonumber(args[1]))
    if tPly ~= nil then
      local target = tPly:GetData("Character")
      if
          exports['hrrp-jobs']:JobsPermissionsHasJob(tPly:GetData("Source"), "police")
          or exports['hrrp-jobs']:JobsPermissionsHasJob(tPly:GetData("Source"), "ems")
      then
  ...
  Chat:RegisterAdminCommand("reclaimcallsign", function(source, args, rawCommand)
    ...
    local plyr = exports['hrrp-base']:FetchSID(fetchCallsignUser.SID)
    if plyr then
