...
function RegisterEACallbacks()
  exports['hrrp-base']:CallbacksRegisterServer("EmergencyAlerts:DisablePDTracker", function(source, target, cb)
    local char = exports['hrrp-base']:FetchSource(source)
    if char then
      local tState = Player(target).state
      local targetChar = exports['hrrp-base']:FetchSource(target)
      if targetChar and tState and trackerJobs[tState.onDuty] and not tState.trackerDisabled then
        ...
        exports["hrrp-base"]:CallbacksClient(target, "EmergencyAlerts:GetStreetName", coords, function(location)
        ...
        exports['hrrp-base']:ExecuteClient(target, "Notification", "Info", "Your Tracker Has Been Disabled")
        cb(true)
        return
      end
    end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("EmergencyAlerts:EnablePDTracker", function(source, target, cb)
    local char = exports['hrrp-base']:FetchSource(source)
    ...
    if char and trackerJobs[pState.onDuty] and pState.trackerDisabled then
      ...
      exports['hrrp-jobs']:JobsDutyOff(source, false, true)
      Citizen.Wait(250)
      exports['hrrp-jobs']:JobsDutyOn(source, job, true)
