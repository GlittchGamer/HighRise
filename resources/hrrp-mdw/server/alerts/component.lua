...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("EmergencyAlerts", _pdAlerts)
end)
...
_pdAlerts = {
  ...
  OnDuty = function(self, dutyData, source, stateId, callsign)
    ...
    then
      ...
      local allJobPermissions = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, dutyData.Id)
      ...
      local token = exports["hrrp-ws"]:generateSocketToken("mdt-alerts", {
        source = source,
        job = dutyData.Id,
  ...
  GetUnitData = function(self, source, job)
    ...
    if char then
      local alertPermissions = {}
      local allJobPermissions = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, job)
  ...
  OffDuty = function(self, dutyData, source, stateId)
    ...
    if emergencyMember then
      ...
      local player = exports['hrrp-base']:FetchSource(source)
      local c = player:GetData("Character")
