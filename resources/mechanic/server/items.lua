function RegisterMechanicItems()
    for k, v in pairs(_mechanicItemsToParts) do
        Inventory.Items:RegisterUse(k, 'Mechanic', function(source, itemData)
            exports['core']:CallbacksClient(source, 'Mechanic:StartInstall', itemData.Name, function(success)
                if success then
    ...
    for k, v in pairs(_mechanicItemsToUpgrades) do
        Inventory.Items:RegisterUse(k, 'Mechanic', function(source, itemData)
            ...
            if partData then
                exports['core']:CallbacksClient(source, 'Mechanic:StartUpgradeInstall', partData, function(success, veh)
