...
function RegisterBankingCallbacks()
  exports['core']:CallbacksRegisterServer("Finance:Paycheck", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Banking:RegisterAccount", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Banking:RenameAccount", function(source, data, cb)
    cb(false)
  end)

  exports['core']:CallbacksRegisterServer("Banking:AddJoint", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char and data?.target > 0 then
      ...
      if findChar.User == char:GetData("User") then
        exports['core']:LoggerInfo("Billing",
          string.format(
  ...
  exports['core']:CallbacksRegisterServer("Banking:RemoveJoint", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char then
  ...
  exports['core']:CallbacksRegisterServer("Banking:GetAccounts", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char then
        ...
        elseif account.Type == "organization" then
          ...
          if jPData then
            for _, job in ipairs(jPData) do
              ...
              local jobPermissions = exports['jobs']:JobsPermissionsGetPermissionsFromJob(
                source,
                job.job,
                fuckingWorkplace
              )
  ...
  exports['core']:CallbacksRegisterServer("Banking:GetAccountsTransactions", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Banking:DoAccountAction", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local SID = char:GetData("SID")
    ...
    if accountData then
      if _actionCooldowns[source] and _actionCooldowns[source] > GetGameTimer() then
        exports['core']:LoggerWarn("Pwnzor",
