...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies('PedInteraction', {
    'Callbacks',
    'Notification',
    'Game',
    'Utils',
    ...
    -- exports['pedinteraction']:Add('fuck', `a_m_y_soucent_04`, vector3(-810.171, -1311.092, 4.000), 332.419, 50.0, {
...
--   exports['polyzonehandler']:PolyZoneCreateCircle('PedInteraction_' .. id, coords, range, {}, { npc_id = id })
-- end
  ...
  Add = function(self, id, model, coords, heading, range, menu, icon, scenario, enabled, anim, component, customPedSkin)
    if id and model and type(coords) == 'vector3' and type(heading) == 'number' then
      ...
      if not IsModelValid(model) or not IsModelAPed(model) then
        exports['core']:LoggerError('PedInteraction', 'Failed to Add Ped ID: ' .. id .. ' - It\'s Model is Invalid')
        return
      end
...
AddEventHandler('Proxy:Shared:RegisterReady', function() --!Completed
  exports['core']:RegisterComponent('PedInteraction', _pedShit)
end)
...
function CreateDumbAssPed(model, coords, heading, menu, icon, scenario, anim, component, showInteract)
  ...
  exports['flags']:PedsSetFlag(ped, 'isNPC', true)
...
function DoesCharacterPassJobPermissions(jobPermissions)
  ...
  for k, v in ipairs(jobPermissions) do
    if v.job then
      if not v.reqOffDuty or (v.reqOffDuty and (not exports['jobs']:JobsDutyGet(v.job))) then
        if exports['jobs']:JobsPermissionsHasJob(v.job, v.workplace, v.grade, v.gradeLevel, v.reqDuty, v.permissionKey) then
          return true
        end
      end
    elseif v.permissionKey then
      if exports['jobs']:JobsPermissionsHasPermission(v.permissionKey) then
        return true
      end
    end
  end
