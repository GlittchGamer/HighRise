...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Vendor", {
    "Callbacks",
    "Logger",
    "PedInteraction",
    "ListMenu",
...
RegisterNetEvent("Vendor:Client:Set", function(vendors)
  for k, v in pairs(vendors) do
    ...
    if v.type == "ped" then
      exports['pedinteraction']:Add(v.id, v.model, v.position.coords, v.position.heading, 50.0, {
        {
...
RegisterNetEvent(
  ...
  function(id, name, type, model, position, iconOverride, labelOverride, isUnique, isGlobalUnique)
    if LocalPlayer.state.loggedIn then
      ...
      if type == "ped" then
        exports['pedinteraction']:Add(id, model, position.coords, position.heading, 50.0, {
          {
...
RegisterNetEvent("Vendor:Client:Remove", function(id)
  if LocalPlayer.state.loggedIn then
    if _created[id].type == "ped" then
      exports['pedinteraction']:Remove(id)
...
AddEventHandler("Vendor:Client:GetItems", function(entity, data)
  exports['core']:CallbacksServer("Vendor:GetItems", data.id, function(items)
    local itemList = {}
...
AddEventHandler("Vendor:Client:BuyItem", function(data)
  exports['core']:CallbacksServer("Vendor:BuyItem", data)
end)
