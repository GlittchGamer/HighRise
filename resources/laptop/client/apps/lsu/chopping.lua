      ...
      if _validBone?.type == "door" then
        if not IsVehicleDoorDamaged(_validVeh, _validBone?.index) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:ChopPart", {
                vNet = VehToNet(_validVeh),
                index = _validBone?.index,
              }, function(c) end)
            else
              exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:CancelChop")
      ...
      elseif _validBone?.type == "tire" then
        if not IsVehicleTyreBurst(_validVeh, _validBone?.index) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:ChopTire", {
                vNet = VehToNet(_validVeh),
                index = _validBone?.index,
              }, function(r)
                if r then
      ...
      elseif _validBone?.type == "body" then
        if DoesEntityExist(_validVeh) then
          ...
          Progress:ProgressWithTickEvent({
            ...
            if not status then
              exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:ChopVehicle", {
                vNet = VehToNet(_validVeh),
              }, function(r)
                -- if r then
...
function DoChoppingThings(veh)
  ...
  Citizen.CreateThread(function()
    local keyBind = exports['keybinds']:GetKey("primary_action")
...
AddEventHandler("Laptop:Client:LSUnderground:Chopping:Pickup", function()
  exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:Pickup")
end)

AddEventHandler("Laptop:Client:LSUnderground:Chopping:GetPublicList", function()
  exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:GetPublicList")
end)
...
AddEventHandler("Laptop:Client:LSUnderground:Chopping:StartChop", function(entity, data)
  ...
  then
    ...
    exports['core']:CallbacksServer("Laptop:LSUnderground:Chopping:CheckVehicle", { vNet = vNet }, function(res)
      if res then
