...
AddEventHandler("Characters:Client:Spawn", function()
  Citizen.Wait(1000)
  exports['hud']:RegisterStatus("boosting-timer", 0, 100, "timer", "#892020", false, false, {
    hideZero = true,
  })
end)
...
RegisterNetEvent("Laptop:Client:LSUnderground:Boosting:Start", function(data)
  ...
  if _boosting then
    ...
    exports['polyzonehandler']:PolyZoneCreateCircle("boosting-dropoff", _boosting.dropOff, 30.0, {})
  end
end)
...
RegisterNetEvent("Laptop:Client:LSUnderground:Boosting:UpdateState", function(state, data)
  ...
  if state == 1 then
    exports['blips']:Remove("boosting-contract")
  elseif state == 2 then
    if data then
...
AddEventHandler("Laptop:Client:LSUnderground:Boosting:AttemptExterior", function(veh)
  if _boosting and NetworkDoesEntityExistWithNetworkId(_boosting.vehicleNet) then
    if veh == NetToVeh(_boosting.vehicleNet) then
      exports['core']:CallbacksServer("Laptop:LSUnderground:Boosting:Exterior", {}, function(numPeds, makeDifficult, noAlert)
        if numPeds then
          --numPeds = 0
...
AddEventHandler("Laptop:Client:LSUnderground:Boosting:SuccessIgnition", function(veh)
  if _boosting and NetworkDoesEntityExistWithNetworkId(_boosting.vehicleNet) then
    if veh == NetToVeh(_boosting.vehicleNet) then
      ...
      exports['core']:CallbacksServer("Laptop:LSUnderground:Boosting:Ignition", {
...
function RegisterBoostingCallbacks()
  exports['core']:CallbacksRegisterClient("Laptop:LSUnderground:Boosting:TrackerHacker", function(data, cb)
...
AddEventHandler("Polyzone:Exit", function(id)
  if id == "boosting-dropoff" and inZone then
    ...
    if _boosting and _boosting.state == 4 then
      exports['core']:CallbacksServer("Laptop:LSUnderground:Boosting:LeftArea", {})
    end
  end
end)
...
AddEventHandler("Vehicles:Client:ExitVehicle", function(veh)
  ...
  then
    exports['core']:CallbacksServer("Laptop:LSUnderground:Boosting:DropOff", {})
  end
end)
...
function CleanUpBoosting()
  if _boosting then
    ...
    exports['polyzonehandler']:PolyZoneRemove("boosting-dropoff")
    exports['blips']:Remove("boosting-contract")
