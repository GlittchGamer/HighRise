...
AddEventHandler("Laptop:Server:RegisterCallbacks", function()
  Citizen.CreateThread(function()
    ...
    while true do
      ...
      exports['core']:LoggerInfo("Laptop - LSU", "Market Place Items Restocked")
    end
  end)

  GlobalState.LSUPickupLocation = locations[math.random(#locations)]

  exports['core']:CallbacksRegisterServer("Laptop:LSUnderground:GetDetails", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Laptop:LSUnderground:Market:Checkout", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Laptop:LSUnderground:Market:Collect", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char ~= nil then
      ...
      if pendingPickup then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "fack off, not got nufink' for u m8")
      end
    ...
    if char ~= nil then
      if not pState.onDuty or not _blacklistedJobs[pState.onDuty] then
        if not hasValue(char:GetData("States") or {}, "ACCESS_LSUNDERGROUND") then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "Already A Member Of LS Underground")
        end
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "You Can't Use This Item")
      end
    end
  end)
end)
