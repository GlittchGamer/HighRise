...
function Init()
  exports['pedinteraction']:Add("hospital-check-in", `u_f_m_miranda_02`, vector3(-437.484, -323.269, 33.911), 162.630, 25.0, hospitalCheckin,
    "notes-medical",
...
AddEventHandler("Hospital:Client:LaydownAnimation", function(hitting, coords)
  ...
  exports["animations"]:EmotesPlay("passout3", true)
  LocalPlayer.state:set("isHospitalized", true, true)

  while exports["animations"]:EmotesGet() == "passout3" do
    Citizen.Wait(250)
  end
...
AddEventHandler("Hospital:Client:RetreiveItems", function()
  exports['core']:CallbacksServer("Hospital:RetreiveItems")
end)
...
AddEventHandler("Hospital:Client:HiddenRevive", function(entity, data)
  Progress:Progress({
    ...
    if not status then
      exports['core']:CallbacksServer("Hospital:HiddenRevive", {}, function(s)
        if s then
          Escort:StopEscort()
        end
      end)
...
RegisterNetEvent("Hospital:Client:GetOut", function()
  exports['core']:CallbacksServer('Hospital:LeaveBed', {}, function()
    _doing = false
    LeaveBed()
  end)
end)
