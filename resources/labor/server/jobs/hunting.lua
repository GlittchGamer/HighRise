...
AddEventHandler("Labor:Server:Startup", function()
  ...
  exports['core']:CallbacksRegisterServer("Hunting:StartJob", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Hunting:FinishJob", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Hunting:HarvestAnimal", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Hunting:GenerateAnimal", function(source, data, cb)
    exports['core']:CallbacksClient(source, "exports['polyzonehandler']:PolyZoneGetZonePlayerIn", {}, function(zone)
      if zone and string.sub(zone.id, 1, string.len("hunting")) == "hunting" then
        ...
        if HuntingConfig.Zones[zoneId] ~= nil then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "Not Allowed To Hunt Here")
        end
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "Not Allowed To Hunt Here")
      end
  ...
  exports['core']:CallbacksRegisterServer("Hunting:Sell", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    ...
    if _saleData[data] ~= nil then
      if repLvl >= _saleData[data].rep then
        ...
        if (count) > 0 then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "You Have No Hides Of That Tier")
        end
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "You Must Prove Yourself First")
      end
    end
  end)
end)
      ...
      then
        ...
        exports['core']:CallbacksClient(source, "exports['polyzonehandler']:PolyZoneGetZonePlayerIn", {}, function(zone)
          if zone and string.sub(zone.id, 1, string.len("hunting")) == "hunting" then
            local zoneId = tonumber(string.sub(zone.id, string.len("hunting") + 1))
            if HuntingConfig.Zones[zoneId] ~= nil then
              exports['core']:CallbacksClient(source, "Hunting:PlaceTrap", item.Name, function(successful)
                if successful then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "You Cannot Use Bait Yet")
      end
    end)
  end
end
