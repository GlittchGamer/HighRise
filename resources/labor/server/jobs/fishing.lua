...
AddEventHandler("Labor:Server:Startup", function()
  ...
  exports['core']:CallbacksRegisterServer("Fishing:Catch", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Fishing:Sell", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    ...
    if char and itemData then
      ...
      if (count) > 0 then
        ...
        exports['core']:ExecuteClient(source, "Notification", "Error", "You Have No " .. itemData.label)
      end
    end
  end)
end)
...
function RegisterFishingItems()
  ...
  Inventory.Items:RegisterUse("fishing_net", "Labor", function(source, itemData)
    ...
    if repLvl < 3 then
      exports['core']:ExecuteClient(source, "Notification", "Error", "Your Net is Tangled and You Don't Know What to Do...")
      return
    end
