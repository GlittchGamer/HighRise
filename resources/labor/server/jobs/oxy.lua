...
AddEventHandler("Labor:Server:Startup", function()
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:Enable", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:Disable", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    local states = char:GetData("States") or {}
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:EnteredCar", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
            ...
            while _sellers[_joiners[source]] ~= nil do
              if not ending then
                if ent ~= nil and not DoesEntityExist(ent) then
                  exports['core']:LoggerTrace("OxyRun", "Vehicle No Longer Exists")
                  ending = true
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:EnteredPickup", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:CancelPickup", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:CheckPickup", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:PickupProduct", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:EnteredArea", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
      ...
      Citizen.CreateThread(function()
        ...
        do
          if not _sellers[_joiners[source]].pending then
            ...
            exports['core']:CallbacksClient(_joiners[source], "OxyRun:GetSpawn", {
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:DeleteShit", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:SellProduct", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:DestroyVehicle", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char:GetData("TempJob") == _JOB and _joiners[source] ~= nil and _sellers[_joiners[source]] ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:VehiclePoofed", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char:GetData("TempJob") == _JOB and _joiners[source] ~= nil and _sellers[_joiners[source]] ~= nil then
      if _sellers[_joiners[source]].state == 6 then
        _sellers[_joiners[source]].state = 7
  ...
  exports['core']:CallbacksRegisterServer("OxyRun:LeftZone", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if
        char:GetData("TempJob") == _JOB
        and _joiners[source] ~= nil
...
AddEventHandler('entityRemoved', function(entity)
  if GetEntityType(entity) == 2 then
    ...
    if ent?.state?.oxyBuying and _sellers[ent?.state?.oxyBuying]?.pending then
      exports['core']:LoggerWarn("Vehicles", string.format("Oxy Vehicle For %s Deleted Unexpectedly, Clearing To Spawn New Vehicle", ent?.state?.oxyBuying))
...
AddEventHandler("OxyRun:Server:OnDuty", function(joiner, members, isWorkgroup)
  ...
  local plyr = exports['core']:FetchSource(joiner)
  local char = plyr:GetData("Character")
  if char == nil then
    Labor.Offers:Cancel(joiner, _JOB)
    Labor.Duty:Off(_JOB, joiner, false, true)
...
function Cleanup(src)
  ...
  if _sellers[joiner] ~= nil then
    if _sellers[joiner].isWorkgroup then
      if #_sellers[joiner].members > 0 then
        for k, v in ipairs(_sellers[joiner].members) do
          local mPlyr = exports['core']:FetchSource(v.ID)
          if mPlyr ~= nil then
    ...
    local jPlyr = exports['core']:FetchSource(joiner)
    if jPlyr ~= nil then
