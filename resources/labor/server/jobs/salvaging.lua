...
AddEventHandler("Labor:Server:Startup", function()
  exports['core']:CallbacksRegisterServer("Salvaging:StartJob", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Salvaging:SalvageCar", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Salvaging:TriggerDelivery", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if _salvaging[_joiners[source]].state == 2 then
      ...
      exports['core']:ExecuteClient(source, "Notification", "Error", "Not On That Step")
    end
  end)

  exports['core']:CallbacksRegisterServer("Salvaging:EndDelivery", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char:GetData("TempJob") == _JOB and _joiners[source] ~= nil and _salvaging[_joiners[source]] ~= nil and _salvaging[_joiners[source]].state == 3 then
      ...
      if (count or 0) > 0 then
        if Inventory.Items:Remove(char:GetData("SID"), 1, "packaged_parts", count) then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "Unable To Remove Packaged Parts")
          cb(false)
        end
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "You Have No Packaged Parts")
        cb(false)
      end
    else
      exports['core']:ExecuteClient(source, "Notification", "Error", "Unable To Turn In Packaged Parts")
      cb(false)
    end
  end)
end)
