...
AddEventHandler("Labor:Client:Setup", function()
  exports['pedinteraction']:Add("FarmingJob", `a_m_m_farmer_01`, vector3(2016.165, 4987.541, 41.098), 225.995, 25.0, {
    {
      icon = "wheat",
      text = "Start Work",
...
function DoAction(id)
  Progress:ProgressWithTickEvent({
    ...
    if not cancelled then
      exports['core']:CallbacksServer("Farming:CompleteNode", id)
    end
  end)
end
...
RegisterNetEvent("Farming:Client:OnDuty", function(joiner, time)
  ...
  _blip = exports['blips']:Add("FarmingStart", "Farm Supervisor", { x = 2016.165, y = 4987.541, z = 0 }, 480, 2, 1.4)
  ...
  eventHandlers["startup"] = RegisterNetEvent(string.format("Farming:Client:%s:Startup", joiner), function(nodes, actionLabel, baseDur, anim)
    exports['blips']:Remove("FarmingStart")
    ...
    for k, v in ipairs(_nodes) do
      exports['blips']:Add(string.format("FarmingNode-%s", v.id), "Farming Action", v.coords, 594, 0, 0.8)
    end
  ...
  eventHandlers["actions"] = RegisterNetEvent(string.format("Farming:Client:%s:Action", joiner), function(data)
    for k, v in ipairs(_nodes) do
      if v.id == data then
        exports['blips']:Remove(string.format("FarmingNode-%s", v.id))
...
AddEventHandler("Farming:Client:TurnIn", function()
  exports['core']:CallbacksServer('Farming:TurnIn', _joiner)
end)

AddEventHandler("Farming:Client:StartJob", function()
  exports['core']:CallbacksServer('Farming:StartJob', _joiner, function(state)
    if not state then
