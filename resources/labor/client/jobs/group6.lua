...
G6 = {
  ...
  CleanUp = function(self)
    ...
    if exports['pedinteraction']:GetPed('Group6_Task') then
      exports['pedinteraction']:Remove('Group6_Task')
    end
...
AddEventHandler('Labor:Client:Setup', function()
  exports['pedinteraction']:Add('Group6', boss.ped, boss.position, boss.heading, 25.0, {
    {
      icon = 'clock',
      text = 'Check In',
...
AddEventHandler('Group6:Client:StartJob', function()
  exports['core']:CallbacksServer('Group6:StartJob', _joiner, function(state)
    if not state then
      exports['hud']:NotificationError('Unable To Start Job')
    end
  end)

  exports['core']:CallbacksRegisterClient('Group6:Client:CrackSafe', function(netID, cb)
    if not netID then
      exports['hud']:NotificationError('Unable to crack safe')
      return
    end
    ...
    if not DoesEntityExist(entity) then
      exports['hud']:NotificationError('Invalid entity')
      return
    end
    ...
    if not hackData.Device or not hackData.Minigame then
      exports['hud']:NotificationError('Invalid hack data')
      return
    end
...
RegisterNetEvent('Group6:Client:OnDuty', function(joiner, time)
  ...
  G6.Data.Blips[blipID] = exports['blips']:Add(blipID, 'Gruppe Sechs', { x = boss.position.x, y = boss.position.y, z = boss.position.z },
    67, 52, 1.4)
  ...
  G6.Data.EventHandlers['startup'] = RegisterNetEvent(string.format('Group6:Client:%s:Startup', joiner), function()
    ...
    ClearAllBlipRoutes()
    exports['core']:CallbacksServer('Group6:VehicleSpawn', {}, function(netId, errorMessage)
      if not netId then
        G6:CleanUp()
        exports['hud']:NotificationError(errorMessage)
        return
      end
    end)
  end)
  ...
  G6.Data.EventHandlers['new-task'] = RegisterNetEvent(string.format('Group6:Client:%s:NewTask', joiner),
    function(currentTask, taskLevel)
      ...
      if G6.Data.Blips[blipID_task] then
        ClearAllBlipRoutes()
        exports['blips']:Remove(blipID_task)
      end

      G6.Data.Blips[blipID_task] = exports['blips']:Add(
        blipID_task,
        'Pickup point',
  ...
  G6.Data.EventHandlers['grab-bag'] = RegisterNetEvent(string.format('Group6:Client:%s:GrabBag', joiner), function(data)
    exports['core']:CallbacksServer('Group6:GrabBag', data, function(success)
      if not success then
        exports['hud']:NotificationError('Unable to grab bag')
      end
    end)
  end)

  G6.Data.EventHandlers['deposit-bag'] = RegisterNetEvent(string.format('Group6:Client:%s:DepositBag', joiner), function(_, data)
    exports['core']:CallbacksServer('Group6:DepositBag', data, function(success, errorMessage)
      if not success then
        exports['hud']:NotificationError(errorMessage)
      end
    end)
  ...
  G6.Data.EventHandlers['turn-in'] = AddEventHandler('Group6:Client:ReturnTruck', function()
    ...
    exports['core']:CallbacksServer('Group6:DespawnVehicle', data, function(success, errorMessage)
...
AddEventHandler('Group6:DepositBag', function(data)
  exports['core']:CallbacksServer('Group6:DepositBag', nil, function(success, errorMessage)
    if not success then
      exports['hud']:NotificationError(errorMessage)
    end
  end)
...
AddEventHandler('Group6:CrackSafe', function(entityData)
  ...
  if not VIN then
    exports['hud']:NotificationError('Unable to open safe')
    return
  end

  if not vehState.G6TRUCK then
    exports['hud']:NotificationError('This is not a Gruppe Sechs truck')
    --#TODO: Anticheat?
    return
  end

  if vehState.G6_GETTINGHACKED then
    exports['hud']:NotificationError('The safe is already being hacked')
    return
  end

  if vehState.G6_HACKED then
    exports['hud']:NotificationError('The safe is already hacked')
    return
  end

  if not vehState.HackData then
    exports['hud']:NotificationError('No hack data found')
    return
  end
...
AddEventHandler('Group6:OpenSafe', function(entityData)
  ...
  if vehState.Access and not vehState.Access[serverId] then
    ...
    if not vehState.G6_HACKED then
      exports['hud']:NotificationError('The safe is locked')
      return
    end
  ...
  exports['core']:CallbacksServer("Group6:OpenSafe", {
    vin = VIN,
