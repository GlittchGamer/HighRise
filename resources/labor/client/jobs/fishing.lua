...
AddEventHandler("Labor:Client:Setup", function()
  exports['polyzonehandler']:PolyZoneCreatePoly("fishing_river_zancudo", {
  ...
  exports['polyzonehandler']:PolyZoneCreatePoly("fishing_river_raton", {
  ...
  exports['polyzonehandler']:PolyZoneCreatePoly("fishing_ocean_shitty1", {
  ...
  exports['polyzonehandler']:PolyZoneCreateCircle("fishing_ocean_shitty2", vector3(-2216.9, 2559.83, 66.16), 519.25, {}, {
    fishing_shitty_ocean_water = true,
  })

  exports['polyzonehandler']:PolyZoneCreatePoly("fishing_deep_ocean_water", {
  ...
  for k, v in ipairs(fishingStores) do
    exports['pedinteraction']:Add(string.format("FishingJob%s", k), `a_m_m_hillbilly_01`, v.coords, v.heading, 25.0, shopData, "fishing-rod")
  end
end)
...
function CanFishHere()
  ...
  if result == 1 then
    ...
    if result2 then
      if height <= 0.5 then
        if exports['polyzonehandler']:PolyZoneIsCoordsInZone(offsetCoords, false, "fishing_shitty_ocean_water") then
          return 4
        elseif exports['polyzonehandler']:PolyZoneIsCoordsInZone(offsetCoords, "fishing_deep_ocean_water") then
          return 5
        else
          return 6
        end
      ...
      local inRiverZone = exports['polyzonehandler']:PolyZoneIsCoordsInZone(offsetCoords, false, "fishing_river")
...
RegisterNetEvent("Fishing:Client:StartFishing", function(toolUsed)
  ...
  if _isFishing then
    exports['hud']:NotificationError("Already Fishing")
    return
  end

  if not fishingZone then
    exports['hud']:NotificationError("Cannot Fish Here")
    return
  end

  if LocalPlayer.state.doingAction then
    exports['hud']:NotificationError("Already Doing Something")
    return
  end

  if toolUsed == "net" and not IsNearBoat() then
    exports['hud']:NotificationError("Have to Use a Fishing Net on a Boat")
    return
  end
  ...
  Notification.Persistent:Info("fishing-info-notif", string.format("Fishing - Press %s to Stop", exports['keybinds']:GetKey("cancel_action")), "fishing-rod")
...
function DoFishBite(zone, toolUsed)
  ...
  exports['core']:CallbacksServer("Fishing:Catch", {
    zone = zone,
    toolUsed = toolUsed,
    difficulty = maxDiff,
...
RegisterNetEvent("Fishing:Client:OnDuty", function(joiner, time)
  ...
  _blip = exports['blips']:Add("FishingStart", "Shop Owner", { x = fishingStores[1].coords.x, y = fishingStores[1].coords.y, z = fishingStores[1].coords.z },
    480, 2, 1.4)
  ...
  eventHandlers["startup"] = RegisterNetEvent(string.format("Fishing:Client:%s:Startup", joiner), function()
    ...
    if _blip ~= nil then
      exports['blips']:Remove("FishingStart")
      RemoveBlip(_blip)
    end
  end)
  ...
  eventHandlers["finish"] = RegisterNetEvent(string.format("Fishing:Client:%s:Finish", joiner), function()
    ...
    _blip = exports['blips']:Add("FishingStart", "Shop Owner",
...
AddEventHandler("Fishing:Client:Sell", function(entity, data)
  exports['core']:CallbacksServer("Fishing:Sell", data.fish)
end)
