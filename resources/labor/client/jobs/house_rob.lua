...
AddEventHandler("Labor:Client:Setup", function()
  ...
  exports['pedinteraction']:Add("HouseRobbery", `csb_grove_str_dlr`, _queueLoc.coords, _queueLoc.heading, 25.0, {
    {
  ...
  exports['core']:CallbacksRegisterClient("HouseRobbery:Lockpick", function(data, cb)
    _lpStage = 0
    DoLockpick({ timer = 1.0, base = 5 }, data, cb)
    EmergencyAlerts:CreateIfReported(40.0, "bane", true)
  end)

  exports['core']:CallbacksRegisterClient("HouseRobbery:AdvLockpick", function(data, cb)
    _lpStage = 0
...
function ExitHouse()
  exports['core']:CallbacksServer("HouseRobbery:Exit", {}, function(propId, intr)
    LocalPlayer.state.inRobbedHouse = false
    ...
    exports['polyzonehandler']:PolyZoneRemove("property-house-rob-zone")
  end)
end
...
function EnterHouse()
  if _p ~= nil and _working then
    exports['core']:CallbacksServer("HouseRobbery:BreakIn", _p, function(r, f, exit)
      if r then
...
RegisterNetEvent("HouseRobbery:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["receive"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Receive", joiner), function(propertyId, data)
    ...
    _blip = exports['blips']:Add("HouseRobbery", "Target House", { x = data.coords.x, y = data.coords.y, z = data.coords.z }, 40, 23, 0.9)
    ...
    Citizen.CreateThread(function()
      ...
      exports['core']:CallbacksServer("HouseRobbery:ArrivedNear", {})
    end)
  end)
  ...
  eventHandlers["lockpicked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Lockpicked", joiner), function(data)
    _nodes = data
    _state = 4
    exports['hud']:NotificationSuccess("House Has Been Lockpicked")
  end)
    ...
    if data.id and not _nodes.searched[data.id] then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled then
          ...
          exports['core']:CallbacksServer("HouseRobbery:Search", data.id)
        end
      end)
    end
  end)
  ...
  eventHandlers["inner"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:InnerStuff", joiner), function(intr)
    ...
    if intr.zone then
      exports['polyzonehandler']:PolyZoneCreateBox("property-house-rob-zone", intr.zone.center, intr.zone.length, intr.zone.width, intr.zone.options, {})
    end
...
AddEventHandler("HouseRobbery:Client:Enable", function()
  exports['core']:CallbacksServer('HouseRobbery:Enable', {})
end)

AddEventHandler("HouseRobbery:Client:StartJob", function()
  exports['core']:CallbacksServer('HouseRobbery:StartJob', _joiner, function(state)
    if not state then
...
AddEventHandler("HouseRobbery:Client:HackSuccess", function(data)
  ...
  if _scPass > 3 then
    _scPass = 1
    exports['core']:CallbacksServer("HouseRobbery:HackAlarm", {
      propertyId = data.pId,
      state = true
    }, function()
      exports['hud']:NotificationSuccess("Alarm Bypassed")
    end)
  else
    Citizen.Wait(1500)
    DoHack(data.pId)
