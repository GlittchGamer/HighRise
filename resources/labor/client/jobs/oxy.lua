...
AddEventHandler("Labor:Client:Setup", function()
  ...
  exports['pedinteraction']:Add("OxyRunner", `s_m_m_movspace_01`, _queueLoc.coords, _queueLoc.heading, 25.0, {
    {
      icon = "tablets",
  ...
  exports['core']:CallbacksRegisterClient("OxyRun:GetSpawn", function(data, cb)
        ...
        Citizen.CreateThread(function()
          ...
          if forceEnd or _psychoShit ~= nil and _psychoShit.veh == VehToNet(vehicle) and not _fuckedOff then
            ...
            Citizen.SetTimeout(30000, function()
              exports['core']:CallbacksServer("OxyRun:DeleteShit", _psychoShit)
            end)
          end
        end)
...
--         exports['core']:CallbacksServer("OxyRun:DestroyVehicle")
--       end
-- 	end
-- end)
...
RegisterNetEvent("OxyRun:Client:OnDuty", function(joiner, time)
  ...
  eventHandlers["primary_action"] = AddEventHandler("Keybinds:Client:KeyUp:primary_action", function()
    if _working and _state == 3 and LocalPlayer.state.inOxyPickup and not LocalPlayer.state.doingAction then
      ...
      if veh == _v.ent and GetPedInVehicleSeat(_v.ent, -1) == LocalPlayer.state.ped then
        ...
        exports['core']:CallbacksServer("OxyRun:CheckPickup", {}, function(s)
          if s then
            Progress:ProgressWithTickEvent({
              ...
              if _state ~= 3 or veh ~= _v.ent or GetPedInVehicleSeat(_v.ent, -1) ~= LocalPlayer.state.ped then
                Progress:Cancel()
                exports['core']:CallbacksServer("OxyRun:CancelPickup")
                return
              end

              exports['core']:CallbacksServer("OxyRun:PickupProduct")
            end, function(cancelled)

            end)
          else
            exports['hud']:NotificationError("Not Enough Room In Your Trunk")
          end
        end)
      end
    end
  ...
  eventHandlers["poly-enter"] = AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
    if _working and id == "OxyPickup" then
      ...
      if _state == 2 then
        exports['core']:CallbacksServer("OxyRun:EnteredPickup")
      else
        local veh = GetVehiclePedIsIn(LocalPlayer.state.ped)
        if _state == 3 and veh == _v.ent and GetPedInVehicleSeat(_v.ent, -1) == LocalPlayer.state.ped then
          exports['hud']:ActionShow("{keybind}primary_action{/keybind} Load Product")
        end
      end
  ...
  eventHandlers["poly-exit"] = AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZones, data)
    ...
    elseif _working and id == "OxySale" and LocalPlayer.state.loggedIn then
      ...
      exports['core']:CallbacksServer("OxyRun:LeftZone")
    end
  end)

  eventHandlers["entered-car"] = RegisterNetEvent('Vehicles:Client:EnterVehicle', function(veh)
    exports['core']:CallbacksServer("OxyRun:EnteredCar", {
  ...
  eventHandlers["start-pickup"] = RegisterNetEvent(string.format("OxyRun:Client:%s:StartPickup", joiner), function(pu, veh)
    ...
    _blip = exports['blips']:Add("OxyRun", "Oxy Pickup", { x = pu.coords.x, y = pu.coords.y, z = pu.coords.z }, 51, 64, 0.9)
    exports['polyzonehandler']:PolyZoneCreateBox("OxyPickup", pu.coords, pu.length, pu.width, pu.options)
        ...
        if not ending then
          if _v?.ent ~= nil and DoesEntityExist(_v.ent) then
            if GetEntityHealth(_v.ent) == 0 or not IsVehicleDriveable(_v.ent) then
              exports['core']:LoggerTrace("OxyRun", "Vehicle Health 0 or Not Drivable")
              ending = true
              exports['core']:CallbacksServer("OxyRun:DestroyVehicle")
            end
          end
  ...
  eventHandlers["start-sale"] = RegisterNetEvent(string.format("OxyRun:Client:%s:StartSale", joiner), function(location)
    ...
    if _blip then
      exports['blips']:Remove("OxyRun")
    end
    _blip = exports['blips']:Add("OxyRun", "Oxy Sale", { x = _l.coords.x, y = _l.coords.y, z = _l.coords.z }, 51, 64, 0.9)
    ...
    exports['polyzonehandler']:PolyZoneCreateCircle("OxySale", _l.coords, _l.radius, _l.options)
    ...
    Citizen.CreateThread(function()
      while _working and _state == 4 do
        ...
        if dist <= 10.0 then
          exports['core']:CallbacksServer("OxyRun:EnteredArea")
        end
  ...
  eventHandlers["target"] = RegisterNetEvent("OxyRun:Client:MakeSale", function(data)
    ...
    exports['core']:CallbacksServer("OxyRun:SellProduct", _psychoShit.veh, function(s)
      if (s) then
  ...
  eventHandlers["veh-poofed"] = RegisterNetEvent(string.format("OxyRun:Client:%s:VehiclePoofed", joiner), function()
    exports['core']:CallbacksServer("OxyRun:VehiclePoofed")
  end)
end)

AddEventHandler("OxyRun:Client:Enable", function()
  exports['core']:CallbacksServer('OxyRun:Enable', {})
end)

AddEventHandler("OxyRun:Client:TurnIn", function()
  exports['core']:CallbacksServer('OxyRun:TurnIn', _joiner)
end)

AddEventHandler("OxyRun:Client:StartJob", function()
  exports['core']:CallbacksServer('OxyRun:StartJob', _joiner, function(state)
    if not state then
...
RegisterNetEvent("OxyRun:Client:OffDuty", function(time)
  ...
  exports['polyzonehandler']:PolyZoneRemove("OxyPickup")
  exports['polyzonehandler']:PolyZoneRemove("OxySale")
  if _blip then
    exports['blips']:Remove("OxyRun")
  end
