function CHAT.RegisterCommand(self, command, callback, suggestion, arguments, job)
  ...
  RegisterCommand(command, function(source, args, rawCommand)
    local pData = exports['hrrp-base']:FetchComponent('Fetch'):Source(source)
    if pData ~= nil then
      ...
      if commands[command].job ~= nil then
        for k, v in pairs(commands[command].job) do
          if myDuty and myDuty == v.Id then
            if exports['hrrp-jobs']:JobsPermissionsHasJob(source, v.Id, v.Workplace, v.Grade, v.Level) then
              if ((#args <= commands[command].args and #args == commands[command].args) or commands[command].args == -1) then
                ...
                exports['hrrp-base']:LoggerInfo(
                  ...
                  string.format(
                    ...
                    char and
                    string.format('%s %s (SID %s)', char:GetData('First'), char:GetData('Last'), char:GetData('SID')) or
                    'No Character',
                    pData:GetData("Name"),
        ...
        if ((#args <= commands[command].args and #args == commands[command].args) or commands[command].args == -1) then
          ...
          exports['hrrp-base']:LoggerInfo(
            ...
            string.format(
              ...
              char and string.format('%s %s (SID %s)', char:GetData('First'), char:GetData('Last'), char:GetData('SID')) or
              'No Character',
              pData:GetData("Name"),
              pData:GetData("AccountID"),
              command,
  ...
  RegisterCommand(command, function(source, args, rawCommand)
    ...
    if player ~= nil then
      ...
      if player.Permissions:IsAdmin() then
        if ((#args <= commands[command].args and #args == commands[command].args) or commands[command].args == -1) then
          exports['hrrp-base']:LoggerInfo("Pwnzor",
            string.format("%s (%s) Used An Admin Command: %s.%s", player:GetData("Name"), player:GetData("AccountID"),
              command, argsStr), {
              console = false,
              file = false,
              database = true,
        ...
        exports['hrrp-base']:LoggerInfo("Pwnzor",
          string.format("%s (%s) Attempted To Use An Admin Command: %s.%s", player:GetData("Name"),
            player:GetData("AccountID"), command, argsStr), {
            console = false,
            file = true,
            database = true,
            discord = {
  ...
  RegisterCommand(command, function(source, args, rawCommand)
    ...
    if player ~= nil then
      ...
      if player.Permissions:IsStaff() then
        if ((#args <= commands[command].args and #args == commands[command].args) or commands[command].args == -1) then
          exports['hrrp-base']:LoggerInfo("Pwnzor",
            string.format("%s (%s) Used A Staff Command: %s.%s", player:GetData("Name"), player:GetData("AccountID"),
              command, argsStr), {
              console = false,
              file = true,
              database = true,
        ...
        exports['hrrp-base']:LoggerInfo("Pwnzor",
          string.format("%s (%s) Attempted To Use A Staff Command: %s.%s", player:GetData("Name"),
            player:GetData("AccountID"), command, argsStr), {
            console = false,
            file = true,
            database = true,
