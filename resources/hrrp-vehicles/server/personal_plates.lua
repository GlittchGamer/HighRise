...
function RegisterPersonalPlateCallbacks()
  Inventory.Items:RegisterUse("personal_plates", "Vehicles", function(source, itemData)
    ...
    exports["hrrp-base"]:CallbacksClient(source, "Vehicles:GetPersonalPlate", {}, function(veh, plate)
      ...
      if veh and DoesEntityExist(veh) then
        ...
        if not vehicle then
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Can't Do It on This Vehicle")
          return
        end
        ...
        if not newPlate then
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Plate Formatting")
          return
        end

        if IsPersonalPlateTaken(newPlate) then
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "That Plate is Taken")
          return
        end
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Personal Plate Setup")
        exports['hrrp-base']:LoggerInfo('Vehicles', string.format("Personal Plate Change For Vehicle: %s. %s -> %s", vehState.VIN, originalPlate, newPlate))
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Error")
      end
    end)
