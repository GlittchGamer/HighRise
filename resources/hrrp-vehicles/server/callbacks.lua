...
function RegisterCallbacks()
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:GetKeys', function(source, VIN, cb)
        Vehicles.Keys:Add(source, VIN)
        cb(true)
    end)

    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:ToggleLocks', function(source, data, cb)
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:BreakOpenLock', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data.netId)
        local vehState = Entity(veh).state
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:GetVehiclesInStorage', function(source, storageId, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
            ...
            if type(storageData.fleet) == 'table' and myDuty then
                for k, v in ipairs(storageData.fleet) do
                    if v.JobId == myDuty then
                        local jobData = exports['hrrp-jobs']:JobsPermissionsHasJob(source, v.JobId, v.WorkplaceId)
                        if jobData then
                            local jobPermissions = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, jobData.Id)
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:GetVehiclesInPropertyStorage', function(source, storageId, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:GetVehiclesInStorageSelect', function(source, data, cb)
        if data.VIN then
            Vehicles.Owned:GetVIN(data.VIN, cb)
        end
    end)

    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:RetrieveVehicleFromStorage', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
            ...
            if vehicle and vehicle.VIN then
                ...
                elseif vehicle.Owner.Type == 1 then
                    ...
                    if onDuty and onDuty == vehicle.Owner.Id then
                        local jobPermissions = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, vehicle.Owner.Id, vehicle.Owner.Workplace)
                        if jobPermissions then
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:PutVehicleInStorage', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
        ...
        if storageData.retrievalOnly then
            exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Store Vehicles Here')
            cb(false)
            return
        end
        ...
        elseif vehicleOwner.Type == 1 then
            if storageData.fleet and DoesVehiclePassFleetRestrictions(vehicleOwner, storageData.fleet) then
                ...
                if onDuty and onDuty == vehicleOwner.Id then
                    local jobPermissions = exports['hrrp-jobs']:JobsPermissionsGetPermissionsFromJob(source, vehicleOwner.Id, vehicleOwner.Workplace)
                    if jobPermissions then
                        local allowedLevel = GetAllowedFleetVehicleLevelFromJobPermissions(jobPermissions)
                ...
                exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Store This Vehicle Here')
            end
        end
    ...
    exports['hrrp-base']:CallbacksRegisterServer("Vehicles:Impound:TagVehicle", function(source, data, cb)
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:PutVehicleInPropertyStorage', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:Impound', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
        ...
        if DoesEntityExist(veh) and myDuty and exports['hrrp-jobs']:JobsPermissionsHasPermission(source, _impoundConfig.RequiredPermission) or exports['hrrp-jobs']:JobsPermissionsHasPermission(source, _impoundConfig.Police.RequiredPermission) then
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:GetVehiclesInImpound', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
        if not character then
            cb(false)
            return
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:RetrieveVehicleFromImpound', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:CompleteCustoms', function(source, data, cb)
      local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:WheelFitment', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:CompleteRepair', function(source, data, cb)
        local character = exports['hrrp-base']:FetchSource(source):GetData('Character')
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:CleanVehicle', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data.vNet)
        local vehState = Entity(veh)
        if DoesEntityExist(veh) and vehState and vehState.state.VIN then
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:RemoveFakePlate', function(source, data, cb)
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:RemoveHarness', function(source, data, cb)
        local veh = NetworkGetEntityFromNetworkId(data)
        if veh and DoesEntityExist(veh) then
            local vehState = Entity(veh).state
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:Tranfers:CompleteTransfer', function(source, data, cb)
        local SID, VIN in data
        local player = exports['hrrp-base']:FetchSource(source)

        if SID and VIN and player then
            local char = player:GetData('Character')
            local target = exports['hrrp-base']:FetchSID(SID)
            ...
            if char and target and vehicle and vehicle:GetData('Owner')?.Type == 0 and vehicle:GetData('Owner')?.Id == char:GetData('SID') then
                ...
                if targetChar and source ~= targetChar:GetData('Source') then
                    ...
                    if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) <= 10.0 then
                        ...
                        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Cannot Transfer to Someone That Isn\'t Nearby')
                    end
                end
            end
        end
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Vehicles:RemoveNitrous', function(source, data, cb)
        local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
        local veh = NetworkGetEntityFromNetworkId(data)
