...
AddEventHandler('Vehicles:Client:StartUp', function()
  exports['hrrp-pedinteraction']:Add('veh_impound', `ig_floyd`, vector3(-193.282, -1162.433, 22.7), 270.4, 10.0, {
    {
      icon = "truck-tow",
    ...
    },
    {
      ...
      isEnabled = function(data, entityData)
        if not exports['hrrp-jobs']:JobsPermissionsHasJob('tow') then
          return true
        end
        return false
      end,
...
AddEventHandler('Vehicles:Client:CharacterLogin', function()
  if _vehicleImpound then
    exports['hrrp-blips']:Add('veh_impound', _vehicleImpound.name, _vehicleImpound.coords, 317, 16, 0.6)
  end
end)
...
AddEventHandler('Vehicles:Client:RequestTow', function(entityData)
  if DoesEntityExist(entityData.entity) and IsVehicleEmpty(entityData.entity) then
    ...
    if vState and vState.VIN then
      ...
      local canRegularImpound = exports['hrrp-jobs']:JobsPermissionsHasPermission(_impoundConfig.RequiredPermission)
      local canPoliceImpound = exports['hrrp-jobs']:JobsPermissionsHasPermission(_impoundConfig.Police.RequiredPermission)
        ...
        if canPoliceImpound then
          ...
          for k, v in ipairs(_impoundConfig.Police.Levels) do
            ...
            table.insert(menu.police.items, {
              ...
              description = string.format('%s$%s Fine %s', (v.Holding > 0 and (v.Holding .. ' Hour Holding Time, ') or ''), fine,
                v.Fine.Percent and string.format('(%s%% of Est. Vehicle Value OR Min. Fine of $%s)', v.Fine.Percent, v.Fine.Min) or ''),
    ...
    if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
          exports["hrrp-base"]:CallbacksServer("Vehicles:Impound:TagVehicle", {
            ...
            exports['hrrp-hud']:NotificationSuccess('Tow Requested')
          end)
        else
          exports['hrrp-hud']:NotificationError('Tow Request Failed')
        end
      end)
    else
      exports['hrrp-hud']:NotificationError('Cannot Request Tow On That Vehicle Anymore...')
    end
  end
end)
...
AddEventHandler('Vehicles:Client:Impound', function(data)
  if attemptingImpoundOnVehicle and DoesEntityExist(attemptingImpoundOnVehicle) then
    if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
      Progress:ProgressWithTickEvent({
        ...
        if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
          exports["hrrp-base"]:CallbacksServer('Vehicles:Impound', {
            ...
            if success then
              exports['hrrp-hud']:NotificationSuccess('Vehicle Impounded')
            else
      ...
      exports['hrrp-hud']:NotificationError('Cannot Impound That Vehicle Anymore...')
    end
  end
end)

AddEventHandler('Vehicles:Client:TowReleaseMenu', function()
  exports["hrrp-base"]:CallbacksServer('Vehicles:GetVehiclesInImpound', {}, function(vehicleData, time)
    if vehicleData and #vehicleData > 0 then
      ...
      exports['hrrp-hud']:NotificationInfo('None of Your Vehicles Are Impounded')
    end
  end)
end)
...
AddEventHandler('Vehicles:Client:TowRequestRelease', function(data)
  if data and data.VIN then
    ...
    if freeImpoundSpace and #(pedCoords - freeImpoundSpace.xyz) <= 50.0 then
      exports["hrrp-base"]:CallbacksServer('Vehicles:RetrieveVehicleFromImpound', {
        VIN = data.VIN,
        coords = freeImpoundSpace.xyz,
        heading = freeImpoundSpace.w,
      }, function(success)
        if success then
          exports['hrrp-hud']:NotificationSuccess('Vehicle Retrieved From Impound. It is Parked in the Lot Outside', 10000)
        else
          exports['hrrp-hud']:NotificationError('Error Retrieving From Impound')
        end
      end)
    else
      exports['hrrp-hud']:NotificationError('Impound Parking Lot Full')
    end
  end
end)
