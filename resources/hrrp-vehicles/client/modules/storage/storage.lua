  ...
  if _vehicleStorage then
    for k, v in pairs(_vehicleStorage) do
      ...
      if v.zone and v.zone.type == 'poly' and v.zone.points then
        exports['hrrp-polyzone']:PolyZoneCreatePoly('veh_storage_' .. k, v.zone.points, {
          minZ = v.zone.minZ,
          maxZ = v.zone.maxZ,
        }, data)
      elseif v.zone and v.zone.type == 'box' and v.zone.center and v.zone.length and v.zone.width then
        exports['hrrp-polyzone']:PolyZoneCreateBox('veh_storage_' .. k, v.zone.center, v.zone.length, v.zone.width, {
  ...
  if _vehicleStorage then
    for k, v in pairs(_vehicleStorage) do
      if not v.restricted and not v.hideBlip then
        exports['hrrp-blips']:Add('veh_storage_' .. k, v.name, v.coords, blipsForVehType[v.vehType], 12, 0.6, false, 10)
      end
    end
    ...
    --         local charJobs = exports['hrrp-jobs']:JobsPermissionsGetJobs()

    --         if #charJobs > 0 then
    --             if DoesCharacterPassStorageRestrictions(-1, charJobs, v.restricted) then
    --                 exports['hrrp-blips']:Add('veh_storage_'.. k, v.name .. ' [Restricted]', v.coords, blipsForVehType[v.vehType], 6, 0.45, false, 10)
    --             end
    --         end
    --     end
    -- end
...
function GetVehicleStorageAtCoords(coords)
  local insideZone = exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(coords, false, 'veh_storage')
  ...
  if entityData and DoesEntityExist(entityData.entity) then
    ...
    if vehState and vehState.VIN and vehState.Owned and GetPedInVehicleSeat(entityData.entity) == 0 and GetEntitySpeed(entityData.entity) <= 1 then
      if inVehicleStorageZone and vehicleStorageZoneId then
        exports["hrrp-base"]:CallbacksServer('Vehicles:PutVehicleInStorage', {
          VIN = vehState.VIN,
          storageId = vehicleStorageZoneId,
        }, function(success)
          if success then
            exports['hrrp-hud']:NotificationSuccess('Stored Vehicle')
          else
            exports['hrrp-hud']:NotificationError('Error Storing Vehicle')
          end
        end)
      else
        ...
        if propGarage and propGarage.propertyId then
          ...
          if prop and prop.keys ~= nil and prop.keys[LocalPlayer.state.Character:GetData("ID")] ~= nil then
            exports["hrrp-base"]:CallbacksServer('Vehicles:PutVehicleInPropertyStorage', {
              VIN = vehState.VIN,
              ...
              if success then
                ...
                if tooFull then
                  exports['hrrp-hud']:NotificationError('Error Storing Vehicle - It\'s Full')
                else
                  exports['hrrp-hud']:NotificationError('Error Storing Vehicle')
                end
              end
            ...
            exports['hrrp-hud']:NotificationError('Don\'t Have Keys to Garage')
          end
        else
          exports['hrrp-hud']:NotificationError('Error Storing Vehicle')
        end
...
function OpenVehicleStorage()
  ...
  if inVehicleStorageZone and vehicleStorageZoneId then
    ...
    if not vehStorageData or (vehStorageData.restricted and not DoesCharacterPassStorageRestrictions(-1, exports['hrrp-jobs']:JobsPermissionsGetJobs(), vehStorageData.restricted)) then
      return exports['hrrp-hud']:NotificationError('Invalid Permission To Access This Vehicle Storage')
    end
    ...
    if vehStorageData and vehStorageData.spaces then
      ...
      if parkingSpace then
        exports["hrrp-base"]:CallbacksServer('Vehicles:GetVehiclesInStorage', vehicleStorageZoneId, function(storedVehicles)
          if not storedVehicles then
            exports['hrrp-hud']:NotificationError('Error Fetching Vehicle Storage')
            return
          end
          ...
          if #storedVehicles > 0 then
            ...
            exports['hrrp-hud']:NotificationError('Vehicle Storage Is Empty')
          end
        end)
      else
        exports['hrrp-hud']:NotificationError('Could Not Find Parking Space')
      end
    end
  else
    ...
    if propertyGarage and propertyGarage.propertyId then
      ...
      if IsParkingSpaceFree(coords) then
        exports["hrrp-base"]:CallbacksServer('Vehicles:GetVehiclesInPropertyStorage', propertyGarage.propertyId,
  ...
  local function createVehicleSubmenu(vehicle)
    ...
    if vehicle.Owner.Type == 1 and vehicle.Type ~= 2 then
      table.insert(vehItems, {
        ...
        description = string.format('Fuel: %s%%<br>Engine State: %s<br>Average Vehicle Parts State: %s', vehicle.Fuel,
          EstimateEngineHealth(vehicle.Damage?.Engine), EstimateDegenState(vehicle.DamagedParts)),
        event = false,
      })
    else
      table.insert(vehItems, {
...
AddEventHandler('Vehicles:Client:Storage:Retrieve', function(data)
  if loadingVehicleStorageVehicle then
    exports['hrrp-hud']:NotificationError('Awaiting Vehicle Load')
  ...
  if data.VIN and tempVehAppearanceData[data.VIN] and tempParkingSpace and tempCurrentStorageId then
    ...
    exports["hrrp-base"]:CallbacksServer('Vehicles:RetrieveVehicleFromStorage', {
      ...
      CleanupTempVehicle()
      if success then
        exports['hrrp-hud']:NotificationInfo('Spawned Vehicle & Received Keys')
      else
        exports['hrrp-hud']:NotificationError('Error') -- Very descriptive error message
      end
    end)
  end
end)
