...
AddEventHandler("Characters:Client:Spawn", function()
  Citizen.Wait(500)
  exports['hrrp-hud']:RegisterStatus("nos", 0, 100, "gauge-circle-plus", "#892020", true, false, {
    hideZero = true,
  })
end)

AddEventHandler("Vehicles:Client:StartUp", function()
  exports['hrrp-keybinds']:Add("vehicle_nos", "", "keyboard", "NOS", function()
  ...
  exports['hrrp-keybinds']:Add("vehicle_nos_purge", "", "keyboard", "NOS - Purge", function()
    if VEHICLE_INSIDE and _hasNOS then
      StartVehiclePurge()
    end
  end, function()
  ...
  exports['hrrp-keybinds']:Add("vehicle_nos_flow_up", "", "keyboard", "NOS - Increase Flow Rate", function()
    if VEHICLE_INSIDE and _hasNOS then
  ...
  exports['hrrp-keybinds']:Add("vehicle_nos_flow_down", "", "keyboard", "NOS - Decrease Flow Rate", function()
    if VEHICLE_INSIDE and _hasNOS then
      if _flowRate > 1 then
        _flowRate = _flowRate - 1
        Notification:Standard("Changed Flow Rate to " .. _flowRate)
  ...
  exports["hrrp-base"]:CallbacksRegisterClient('Vehicles:InstallNitrous', function(data, cb)
    if VEHICLE_INSIDE and DoesEntityExist(VEHICLE_INSIDE) and IsEntityAVehicle(VEHICLE_INSIDE) and GetPedInVehicleSeat(VEHICLE_INSIDE, -1) == LocalPlayer.state.ped then
      if not IsToggleModOn(VEHICLE_INSIDE, 18) then
        exports['hrrp-hud']:NotificationError("Need a Turbo For This Mate")
        cb(false)
        return
      end

      if Police:IsPdCar(VEHICLE_INSIDE) then
        exports['hrrp-hud']:NotificationError("How About No")
        cb(false)
        return
      end
  ...
  if VEHICLE_INSIDE and DoesEntityExist(VEHICLE_INSIDE) then
    Progress:Progress({
      ...
      if not cancelled and Vehicles:HasAccess(VEHICLE_INSIDE, true) then
        exports["hrrp-base"]:CallbacksServer('Vehicles:RemoveNitrous', VehToNet(VEHICLE_INSIDE), function(success)
          if success then
            --exports['hrrp-hud']:NotificationSuccess('Removed Nitrous Successfully')
            ...
            exports['hrrp-hud']:NotificationError('Could not Remove Nitrous')
          end
        end)
      else
        exports['hrrp-hud']:NotificationError('Could not Remove Nitrous')
