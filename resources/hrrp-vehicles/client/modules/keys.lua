...
AddEventHandler('Vehicles:Client:StartUp', function()
  exports["hrrp-base"]:CallbacksRegisterClient('Vehicles:Keys:GetVehicleToShare', function(data, cb)
  ...
  exports['hrrp-keybinds']:Add('vehicle_lock', 'u', 'keyboard', 'Vehicle - Toggle Door Locks', function()
    if not _lockdelay then
      local veh = VEHICLE_INSIDE and VEHICLE_INSIDE or GetClosestVehicleWithinRadius(GetEntityCoords(GLOBAL_PED), 2.0)
      if DoesEntityExist(veh) then
        _lockdelay = true
...
_vehicleKeysExtension = {
  ...
  SetLocks = function(self, veh, state)
    exports["hrrp-base"]:CallbacksServer('Vehicles:ToggleLocks', {
      ...
      if success then
        UnlockAnim()
        if newState then
          exports['hrrp-hud']:NotificationError('Vehicle Locked')
          Sounds.Do.Play:One('central-locking.ogg', 0.2)
          DoVehicleLockShit(veh)
        else
          exports['hrrp-hud']:NotificationSuccess('Vehicle Unlocked')
          Sounds.Do.Play:One('central-locking.ogg', 0.2)
...
AddEventHandler('Proxy:Shared:ExtendReady', function(component)
  if component == 'Vehicles' then
    exports['hrrp-base']:ExtendComponent(component, _vehicleKeysExtension)
  end
end)
