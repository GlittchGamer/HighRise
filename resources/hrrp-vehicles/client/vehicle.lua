...
VEHICLE = {
  ...
  Engine = {
    Force = function(self, veh, state)
      ...
      if state and vehState.Fuel and vehState.Fuel <= 0.0 then
        state = false
        exports['hrrp-hud']:NotificationError('Vehicle Out of Fuel', 2500, 'gas-pump')
      end

      if state and GetVehicleEngineHealth(veh) <= -2000.0 then
        state = false
        exports['hrrp-hud']:NotificationError('Vehicle Engine Damaged', 2500, 'engine-warning')
      end
    ...
    Off = function(self, customMessage)
      ...
      exports['hrrp-hud']:NotificationInfo(customMessage and customMessage or 'Engine Turned Off', 1500, 'engine')

      if Vehicles.Keys:Has(vehEnt.state.VIN, vehEnt.state.GroupKeys) then
        exports['hrrp-hud']:ActionShow('{keybind}toggle_engine{/keybind} Turn Engine On')
        _actionShowing = true
      end
    end,
    On = function(self)
      ...
      if Vehicles.Keys:Has(vehEnt.state.VIN, vehEnt.state.GroupKeys) then
        ...
        exports['hrrp-hud']:NotificationInfo('Engine Turned On', 1500, 'engine')
      else
        exports['hrrp-hud']:NotificationError('You Don\'t Have Keys For This Vehicle', 3000, 'key')
      end
    end,
    ...
    if not vehEnt.state.towObjective then
      if vehEnt and vehEnt.state and vehEnt.state.VIN and not Vehicles:HasAccess(vehicle) then
        ...
        if SLIM_JIM_ATTEMPTS[vVIN] and SLIM_JIM_ATTEMPTS[vVIN] > 3 then
          exports['hrrp-hud']:NotificationError("You Have Tried This Vehicle Enough Already")
          return
        end
        ...
        Minigame.Play:RoundSkillbar(12, 3, {
          onSuccess = function()
            ...
            if setToFail then
              exports['hrrp-hud']:NotificationError("It Was too Difficult and It Didn't Work")
            else
              if #(startCoords - GetEntityCoords(GLOBAL_PED)) <= 2.0 then
                SetVehicleHasBeenOwnedByPlayer(vehicle, true)
                SetEntityAsMissionEntity(vehicle, true, true)
                exports["hrrp-base"]:CallbacksServer("Vehicles:BreakOpenLock", {
                  netId = VehToNet(vehicle),
                }, function(success)
                  exports['hrrp-hud']:NotificationSuccess("You Managed to Unlock the Vehicle", 3000, "key")
                end)
              else
                exports['hrrp-hud']:NotificationError("Too Far")
              end
            end
          end,
          onFail = function()
      ...
      exports['hrrp-hud']:NotificationError("You Cannot Slimjim This Vehicle", 3000, 'key')
    end
  end,
  LockpickExterior = function(self, config, canUnlockOwned, vehicle, cb)
    ...
    if not vehEnt.state.towObjective and not Police:IsPdCar(vehicle) and not vehEnt.state.noLockpick then
        ...
        if not wasFailed then
          if #(startCoords - GetEntityCoords(GLOBAL_PED)) <= 2.0 then
            ...
            if vehEnt.state.Owned and not canUnlockOwned then
              exports['hrrp-hud']:NotificationError("It Was Too Hard", 3000, 'key')
            ...
            exports["hrrp-base"]:CallbacksServer("Vehicles:BreakOpenLock", {
              ...
              if success then
                exports['hrrp-hud']:NotificationSuccess("Vehicle Unlocked", 3000, "key")
              end
            end)
      ...
      exports['hrrp-hud']:NotificationError("You Cannot Lockpick This Vehicle", 3000, 'key')
    end
  end,
      ...
      if vehEnt and vehEnt.state and vehEnt.state.VIN then
        ...
        if not wasFailed then
          if VEHICLE_INSIDE and VEHICLE_SEAT == -1 then
            ...
            exports["hrrp-base"]:CallbacksServer("Vehicles:GetKeys", vehEnt.state.VIN, function(success)
              exports['hrrp-hud']:NotificationSuccess("Lockpicked Vehicle Ignition", 3000, 'key')
              exports['hrrp-hud']:ActionShow('{keybind}toggle_engine{/keybind} Turn Engine On')
              _actionShowing = true
            end)
          ...
          hackData.charSet or false, {
            onSuccess = function()
              if VEHICLE_INSIDE and VEHICLE_SEAT == -1 then
                ...
                exports["hrrp-base"]:CallbacksServer("Vehicles:GetKeys", vehEnt.state.VIN, function(success)
                  exports['hrrp-hud']:NotificationSuccess("Vehicle Ignition Bypassed", 3000, 'key')
                  exports['hrrp-hud']:ActionShow('{keybind}toggle_engine{/keybind} Turn Engine On')
                  _actionShowing = true
                end)
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Vehicles", VEHICLE)
end)
...
AddEventHandler('Vehicles:Client:BecameDriver', function(veh, seat)
  ...
  if vehEnt.state.VEH_IGNITION then
    if not vehEnt.state.PlayerDriven then -- It was stolen directly with a ped in it, get keys
      Vehicles.Engine:Force(VEHICLE_INSIDE, true)
      exports["hrrp-base"]:CallbacksServer('Vehicles:GetKeys', vehEnt.state.VIN, function()
        exports['hrrp-hud']:NotificationSuccess('You found the keys in the vehicle', 3000, 'key')
      end)
    end
  else
    if Vehicles.Keys:Has(vehEnt.state.VIN, vehEnt.state.GroupKeys) then
...
AddEventHandler("Vehicles:Client:StartUp", function()
  exports["hrrp-base"]:CallbacksRegisterClient("Vehicles:Slimjim", function(data, cb)
      ...
      if not VEHICLE_INSIDE then
        ...
        then
          ...
          if vehClass?.advLockpick then
            ...
            exports['hrrp-hud']:NotificationError("Cannot Slimjim This Vehicle")
          end
        else
          print('nope2')
          cb(false, false)
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Vehicles:Lockpick", function(data, cb)
      ...
      if VEHICLE_INSIDE then
        if VEHICLE_SEAT == -1 then
          ...
          if vehClass?.lockpick then
            ...
            exports['hrrp-hud']:NotificationError("Cannot Lockpick This Vehicle")
          end
        else
          cb(false, false)
        end
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Vehicles:AdvLockpick", function(data, cb)
    if _characterLoaded then
      if VEHICLE_INSIDE then
        if VEHICLE_SEAT == -1 then
          local vehClass = _vehicleClasses[Vehicles.Class:Get(VEHICLE_INSIDE)]
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Vehicles:Hack", function(data, cb)
    if _characterLoaded then
      if VEHICLE_INSIDE then
        if VEHICLE_SEAT == -1 then
          local vehClass = _vehicleClasses[Vehicles.Class:Get(VEHICLE_INSIDE)]
          if vehClass?.hack then
            ...
            exports['hrrp-hud']:NotificationError("Cannot Hack This Vehicle")
          end
        else
          cb(false, false)
        end
  ...
  exports["hrrp-base"]:CallbacksRegisterClient("Vehicles:AdvHack", function(data, cb)
    if _characterLoaded then
      if VEHICLE_INSIDE then
        if VEHICLE_SEAT == -1 then
          local vehClass = _vehicleClasses[Vehicles.Class:Get(VEHICLE_INSIDE)]
