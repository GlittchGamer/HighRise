...
AddEventHandler("Vehicles:Client:BecameDriver", function(veh, seat, class)
  Citizen.CreateThread(function()
    ...
    exports['hrrp-base']:LoggerTrace("Vehicles", "Execute Vehicle Pre Thread Tasks")

    for k, v in pairs(PRE_THREAD_TASKS) do
      v.func(veh, class)
    end

    exports['hrrp-base']:LoggerTrace("Vehicles", "Start Vehicle Thread on " .. veh)
    THREAD_VEHICLE = veh
              ...
              if state and state.VIN then
                ...
                if data then
                  ...
                  if data.needInit then
                    exports['hrrp-base']:LoggerInfo(
                      "Vehicles",
                      string.format("Saving vehicle properties to Databse with Vehicle VIN: %s", state.VIN)
                    )
