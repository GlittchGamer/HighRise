...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Hud", {
    ...
    exports['hrrp-base']:CallbacksRegisterServer("HUD:RemoveBlindfold", function(source, data, cb)
      ...
      if plyr ~= nil then
        ...
        if char ~= nil then
          ...
          if tarState.isBlindfolded then
            exports["hrrp-base"]:CallbacksClient(source, "HUD:PutOnBlindfold", "Removing Blindfold", function(isSuccess)
              if isSuccess then
                if Inventory:AddItem(char:GetData("SID"), "blindfold", 1, {}, 1) then
                  tarState.isBlindfolded = false
                else
                  exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Failed Adding Item")
                  cb(false)
                end
              end
            end)
          else
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Not Blindfolded")
            cb(false)
          end
        else
          cb(false)
              ...
              function(isSuccess)
                if isSuccess then
                  if tarState.isCuffed then
                    ...
                    exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Not Cuffed")
                  end
                end
              end
            )
          else
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Already Blindfolded")
          end
        else
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Nobody Near To Blindfold")
        end
      end)
    end)
