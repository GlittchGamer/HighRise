...
function DoCuff(source, target, isHardCuffed, isForced)
  ...
  exports["hrrp-base"]:CallbacksClient(target, "Handcuffs:DoCuff", {
    cuffer = source,
    isHardCuffed = isHardCuffed,
    forced = isForced,
  }, function(result)
    if result == -1 then
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Unable To Cuff Player")
    else
      ...
      if result then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Suspect Broke Out Of The Cuffs")
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "You Cuffed A Player")
  ...
  if #(vector3(mPos.x, mPos.y, mPos.z) - vector3(tPos.x, tPos.y, tPos.z)) <= 1.5 then
    if Inventory.Items:HasAnyItems(src, Config.CuffItems) then
      ...
      then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Already Hard Cuffed")
      end
    end
  else
    exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Too Far")
  end
end)
...
_HANDCUFFS = {
  ToggleCuffs = function(self, source)
    exports["hrrp-base"]:CallbacksClient(source, "HUD:GetTargetInfront", {}, function(target)
      if target ~= nil then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Nobody To Cuff")
      end
    end)
  end,
    ...
    exports["hrrp-base"]:CallbacksClient(source, "HUD:GetTargetInfront", {}, function(target)
      if target ~= nil then
        if not Player(target).state.isCuffed then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Player Already Cuffed")
        end
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Nobody To Cuff")
      end
  ...
  SoftCuffTarget = function(self, source, target, forced)
    ...
    exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Nobody Around To Cuff")
  end,
    ...
    exports["hrrp-base"]:CallbacksClient(source, "HUD:GetTargetInfront", {}, function(target)
      if target ~= nil then
        if Player(target).state.isCuffed then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Player Is Not Cuffed")
        end
      else
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Nobody To Cuff")
      end
  ...
  UncuffTarget = function(self, source, target)
    exports["hrrp-base"]:CallbacksClient(target, "Handcuffs:VehCheck", {}, function(inVeh)
      if not inVeh then
        if source ~= -1 then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Unable To Uncuff Player")
      end
    end)
  end,
}
