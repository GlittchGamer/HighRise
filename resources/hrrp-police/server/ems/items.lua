function EMSItems()
  ...
  -- 		end
  -- 	else
  -- 		exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You're Not Bleeding")
  -- 	end
  -- end)
  ...
  Inventory.Items:RegisterUse("bandage", "MedicalItems", function(source, item)
    ...
    if Inventory.Items:RemoveSlot(item.Owner, item.Name, 1, item.Slot, 1) then
      ...
      if curr < (max * 0.75) then
        local p = promise.new()
        exports["hrrp-base"]:CallbacksClient(source, "EMS:Heal", heal, function(s)
          p:resolve(s)
        end)
        Citizen.Await(p)
      end
  ...
  -- 		else
  -- 			exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You continue bleeding through the gauze")
  -- 		end
  -- 	end
  -- end)
  ...
  Inventory.Items:RegisterUse("medicalkit", "MedicalItems", function(source, item)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if exports['hrrp-jobs']:JobsPermissionsHasJob(source, "ems", false, false, 2) then
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Near Any Hospitalized Patients")
    else
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You're not trained to use this")
    end
  end)
end
