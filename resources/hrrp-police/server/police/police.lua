...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Police", {
    ...
    for k, v in pairs(Config.Armories) do
      exports['hrrp-base']:LoggerTrace("Police", string.format("Registering Poly Inventory ^2%s^7 For ^3%s^7", v.id, v.name))
    ...
    Inventory.Items:RegisterUse("spikes", "Police", function(source, slot, itemData)
      if GetVehiclePedIsIn(GetPlayerPed(source)) == 0 then
        exports["hrrp-base"]:CallbacksClient(source, "Police:DeploySpikes", {}, function(data)
          if data ~= nil then
            ...
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "You Deployed Spikes (Despawn In 20s)")
          end
        end)
      end
    end)

    exports['hrrp-base']:CallbacksRegisterServer("Police:GSRTest", function(source, data, cb)
      ...
      if pState.onDuty == "police" or pState.onDuty == "ems" then
        ...
        if target ~= nil then
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Target")
        end
      end
    end)

    exports['hrrp-base']:CallbacksRegisterServer("Police:BACTest", function(source, data, cb)
      local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    exports['hrrp-base']:CallbacksRegisterServer("Police:DNASwab", function(source, data, cb)
      local char = exports['hrrp-base']:FetchSource(source):GetData("Character")

      local pState = Player(source).state
      if char and pState.onDuty == "police" or pState.onDuty == "ems" then
        local tPlayer = exports['hrrp-base']:FetchSource(data)
        if tPlayer ~= nil then
    ...
    exports['hrrp-base']:CallbacksRegisterServer("Police:Breach", function(source, data, cb)
      local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
      ...
      if Player(source).state.onDuty == "police" then
        ...
        if data.type == "property" then
          if (_breached[data.type][data.property] or 0) > os.time() then
            cb(true)
            exports['hrrp-base']:ExecuteClient(source, "Properties", "Enter", data.property)
          else
            exports["hrrp-base"]:CallbacksClient(source, "Police:Breach", {}, function(s)
              if s then
        ...
        elseif data.type == "apartment" then
          ...
          if aptTier ~= nil then
            ...
            if id == aptTier then
              if (_breached[data.type][data.property] or 0) > os.time() then
                exports['hrrp-base']:ExecuteClient(source, "Apartment", "Enter", aptTier, data.property)
              ...
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Does Not Reside Here")
              return cb(false)
            end
          else
            exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Target Not Online")
            return cb(false)
          end
        end
      else
    ...
    exports['hrrp-base']:CallbacksRegisterServer("Police:AccessRifleRack", function(source, data, cb)
            ...
            if veh ~= 0 then
              if Config.PoliceCars[GetEntityModel(veh)] then
                ...
                if Vehicles.Keys:Has(source, entState.VIN, 'police') then
                  ...
                  exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Can't Access The Locked Compartment")
                end
              else
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Vehicle Not Outfitted With A Secured Compartment")
              end
            else
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not In A Vehicle")
            end
          end
        end
      end
    ...
    exports['hrrp-base']:CallbacksRegisterServer("Police:RemoveMask", function(source, data, cb)
      local plyr = exports['hrrp-base']:FetchSource(source)
      if plyr ~= nil then
        local char = plyr:GetData("Character")
        if char ~= nil then
          local myDuty = Player(source).state.onDuty
          if myDuty == 'police' then
            local tPlyr = exports['hrrp-base']:FetchSource(source)
            if tPlyr ~= nil then
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Police", POLICE)
end)
...
POLICE = {
  IsInBreach = function(self, source, type, id, extraCheck)
    if Player(source)?.state?.onDuty == "police" and (not extraCheck or exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, 'police', 'PD_RAID')) then
  ...
  RunPlate = function(self, source, plate, wasEntity)
    ...
    if #result == 0 then
      local stolen = exports['hrrp-radar']:CheckPlate(plate)
      if stolen then
...
RegisterNetEvent("Police:Server:Slimjim", function()
  ...
  if Player(src).state.onDuty == "police" then
    exports["hrrp-base"]:CallbacksClient(src, "Vehicles:Slimjim", true, function()
