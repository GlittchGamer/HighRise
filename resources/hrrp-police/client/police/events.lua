      ...
      if not cancelled then
        if not vehEnt.state.VIN then
          ...
          repeat
            timeout = timeout - 100
            Wait(100)
          until vehEnt.state.VIN or timeout <= 0
        end

        if timeout <= 0 then
          exports['hrrp-hud']:NotificationError("Unable To Retrieve Vehicle Information")
          return
        end
...
AddEventHandler("Police:Client:GSR", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Police:GSRTest", entity.serverId, function() end)
    end
  end)
end)
...
AddEventHandler("Police:Client:BAC", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Police:BACTest", entity.serverId, function() end)
    end
  end)
end)
...
AddEventHandler("Police:Client:DNASwab", function(entity, data)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Police:DNASwab", entity.serverId, function() end)
    end
  end)
end)
  ...
  if #(GetEntityCoords(LocalPlayer.state.ped) - positions[1]) <= 800.0 then
    ...
    if not timeout and not forceDelete[owner] then
      ...
      for k, v in ipairs(positions) do
        ...
        TriggerEvent("Police:Client:SpikeyBois", v.x, v.y, v.z, spikeObj, (spikeTime - (GetGameTimer() - start)) / 10,
          owner)
...
AddEventHandler("Police:Client:OpenLocker", function()
  exports["hrrp-base"]:CallbacksServer("MDT:OpenPersonalLocker", {}, function(success)
    if not success then
      exports['hrrp-hud']:NotificationError("Callsign Not Set, Unable To Open Locker")
    end
  end)
end)
...
AddEventHandler('Police:Client:RemoveMask', function(entity, data)
  exports["hrrp-base"]:CallbacksServer("Police:RemoveMask", entity.serverId)
end)
