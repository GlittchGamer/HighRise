...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("PickPocket", {
    "Progress"
  }, function(error)
    if #error > 0 then
      return
...
local function InteractWithPed(ped)
  ...
  if lootedPeds[ped] then
    exports['hrrp-hud']:NotificationError("Go away punk, you have taken everything I had!", 2500)
    return
  end

  -- Check if the ped is standing still
  if not IsPedStill(ped) then
    exports['hrrp-hud']:NotificationError('This person is moving too fast to pickpocket.', 2500)
    return
  end
  ...
  Progress:Progress({
    ...
    if not cancelled then
      ...
      exports['hrrp-hud']:NotificationError('Pickpocket attempt cancelled.', 2500)
    end
  end)
end
      ...
      if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(ped)) < 2.0 then
        ...
        if not lootedPeds[ped] then
          if not cooldown then
            ...
            exports['hrrp-hud']:NotificationError("Maybe you should take a break", 2500)
          end
        end
      end
    end
