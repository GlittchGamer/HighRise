  ...
  for k, v in ipairs(_mechanicShops) do
    if v.zone then
      ...
      if v.zone.type == 'poly' and v.zone.points then
        exports['hrrp-polyzone']:PolyZoneCreatePoly('mech_zone_' .. k, v.zone.points, v.zone.options, data)
      elseif v.zone.type == 'box' and v.zone.center and v.zone.length and v.zone.width then
        exports['hrrp-polyzone']:PolyZoneCreateBox('mech_zone_' .. k, v.zone.center, v.zone.length, v.zone.width, v.zone.options, data)
      end
    end
  end
end
...
function GetMechanicZoneAtCoords(coords)
  local insideZone = exports['hrrp-polyzone']:PolyZoneIsCoordsInZone(coords, false, 'mechanic_zone')
