...
function IsDrinkingAlcohol()
  local doingAnim = exports["hrrp-animations"]:EmotesGet()
...
function RegisterDrunkCallbacks()
  exports["hrrp-base"]:CallbacksRegisterClient('Status:DrinkAlcohol', function(data, cb)
    if IsDrinkingAlcohol() then
      exports['hrrp-hud']:NotificationError('Already Drinking!')
      return cb(false)
    end

    local alcohol = _alcoholConfig[data]
    if alcohol then
      exports["hrrp-animations"]:EmotesPlay(alcohol.anim, false, alcohol.initialTime, true)
      Progress:Progress({
        name = data,
        ...
        if not cancelled then
          ...
          exports["hrrp-animations"]:EmotesPlay(alcohol.anim, false, false, false)
          ...
          Citizen.CreateThread(function()
            local tick = 3
            while LocalPlayer.state.loggedIn and exports["hrrp-animations"]:EmotesGet() == alcohol.anim do
              if tick >= 5 then
