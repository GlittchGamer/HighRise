...
function CreateFurnitureCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer("Properties:PlaceFurniture", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:MoveFurniture", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local insideProperty = GlobalState[string.format("%s:Property", source)]
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:DeleteFurniture", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local insideProperty = GlobalState[string.format("%s:Property", source)]
...
function SetPropertyFurniture(pId, newFurniture, updater)
  ...
  'UPDATE properties_furniture SET furniture = @furniture, updatedTime = @updatedTime, updatedBy = @updatedBy WHERE property = @property', {
  ...
  if success == 0 then   -- if not fucking rows updated insert a new shit record FUCK SAKE
    success = MySQL.query.await(
    'INSERT INTO properties_furniture (property, furniture, updatedTime, updatedBy) VALUES (@property, @furniture, @updatedTime, @updatedBy)', {
      ['@property'] = pId,
      ['@furniture'] = json.encode(newFurniture),
      ['@updatedTime'] = os.time(),
      ['@updatedBy'] = updater
