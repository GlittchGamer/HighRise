...
function RegisterCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer("Properties:RingDoorbell", function(source, data, cb)
    TriggerClientEvent("Properties:Client:Doorbell", -1, data)
    cb()
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Properties:RequestAgent", function(source, data, cb)
    ...
    if property ~= nil then
      for k, v in pairs(Fetch:All()) do
        ...
        if c ~= nil then
          if exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(v:GetData("Source"), "realestate", "JOB_SELL") then
            Phone.Email:Send(
              v:GetData("Source"),
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:EditProperty", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:SpawnInside", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if property ~= nil and char then
      ...
      exports['hrrp-base']:MiddlewareTriggerEvent("Properties:Enter", source, data.id)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:EnterProperty", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local property = _properties[data]

    if
        (property.keys ~= nil and property.keys[char:GetData("ID")])
        or (not property.sold and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, "realestate", "JOB_DOORS"))
    ...
    then
      ...
      exports['hrrp-base']:MiddlewareTriggerEvent("Properties:Enter", source, data)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:ExitProperty", function(source, data, cb)
    local property = GlobalState[string.format("%s:Property", source)]

    exports['hrrp-pwnzor']:PwnzorPlayersTempPosIgnore(source)
    exports['hrrp-base']:MiddlewareTriggerEvent("Properties:Exit", source, property)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:ChangeLock", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Validate", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
    ...
    elseif data.type == "stash" then
      if property.keys and property.keys[char:GetData("ID")] ~= nil and property.id or Police:IsInBreach(source, "property", property.id, true) then
        ...
        exports["hrrp-base"]:CallbacksClient(source, "Inventory:Compartment:Open", {
          invType = invType,
          owner = invId,
        }, function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Upgrade", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
        ...
        if thisUpgrade then
          ...
          if nextLevel and nextLevel.price and p and p.Account then
            local success = Banking.Balance:Charge(p.Account, nextLevel.price, {
              ...
              description = string.format("Upgrade %s to Level %s on %s", thisUpgrade.name, currentLevel + 1,
                property.label),
              data = {
            ...
            if success then
              ...
              if not upgraded then
                exports['hrrp-base']:LoggerError("Properties",
                  string.format("SID %s Failed to Upgrade Property %s After Payment (%s - Level %s)", char:GetData("SID"),
                    property.id, thisUpgrade.name, currentLevel + 1))
              end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:ChangeInterior", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local property = _properties[data.id]
    ...
    if char and data.int and property.keys and property.keys[char:GetData("ID")] ~= nil and (property.keys[char:GetData("ID")].Permissions?.upgrade or property.keys[char:GetData("ID")].Owner) then
      ...
      if p and p.Account and oldInterior and newInterior and newInterior.type == property.type then
        ...
        if success then
          ...
          if not upgraded then
            exports['hrrp-base']:LoggerError("Properties",
              string.format("SID %s Failed to Upgrade Property %s After Payment (Interior - %s)", char:GetData("SID"),
                property.id, data.int))
          else
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Dyn8:Search", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    if char then
      local query = 'SELECT * FROM properties WHERE label LIKE @label'
      local params = { ['@label'] = '%' .. data .. '%' }

      if Player(source).state.onDuty ~= 'realestate' then
        query = query .. ' AND sold = @sold'
        params['@sold'] = false
      end
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Dyn8:Sell", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData('Character')
              ...
              if data.loan and data.time and data.deposit then
                ...
                if #hasLoans <= 1 then
                        ...
                        SendPendingLoanEmail({
                          ...
                          function()
                            Billing:Create(targetChar:GetData('Source'), 'Dynasty 8', downPayment,
                              string.format('Property Downpayment for %s', prop.label), function(wasPayed, withAccount)
                                if wasPayed then
                                  local loanSuccess = Loans:CreatePropertyLoan(targetChar:GetData('Source'), prop.id,
                                    prop.price, downPayment, loanWeeks)
                                  if loanSuccess then
                                    ...
                                    Phone.Notification:Add(source, "Property Sale Successful",
                                      string.format("(Loan Sale) %s was sold to %s %s.", prop.label,
                                        targetChar:GetData('First'), targetChar:GetData('Last')), os.time() * 1000, 7000,
                                      _phoneApp, {})

                                    SendPropertyProfits('Loan Sale', prop.price, prop.label, char:GetData('BankAccount'),
                                      withAccount, {
                                        SID = targetChar:GetData("SID"),
                                        First = targetChar:GetData("First"),
                                        Last = targetChar:GetData("Last"),
                                  ...
                                  Phone.Notification:Add(source, "Property Sale Failed",
                                    string.format("(Loan Sale) The downpayment failed when trying to sell %s to %s %s.",
                                      prop.label, targetChar:GetData('First'), targetChar:GetData('Last')),
                                    os.time() * 1000, 7000, _phoneApp, {})
                                end
                  ...
                  cb({
                    success = false,
                    message =
                    'Person Doesn\'t Qualify for Loan, too much active credit and/or loans.'
                  })
                end
              else
                cb({ success = true, message = 'Sale Offer Sent' })

                Billing:Create(targetChar:GetData('Source'), 'Dynasty 8', prop.price, 'Purchase of ' .. prop.label,
                  function(wasPayed, withAccount)
                    if wasPayed then
                      ...
                      Phone.Notification:Add(source, "Property Sale Successful",
                        string.format("(Cash Sale) %s was sold to %s %s.", prop.label, targetChar:GetData('First'),
                          targetChar:GetData('Last')), os.time() * 1000, 7000, _phoneApp, {})
                      ...
                      Phone.Notification:Add(source, "Property Sale Failed",
                        string.format("(Cash Sale) The bank transfer failed when trying to sell %s to %s %s.", prop
                          .label, targetChar:GetData('First'), targetChar:GetData('Last')), os.time() * 1000, 7000,
                        _phoneApp, {})
                    end
                    _selling[data.property] = nil
                  end)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Dyn8:CheckCredit", function(source, data, cb)
    local target = exports['hrrp-base']:FetchSID(tonumber(data?.target))
    if target ~= nil then
      local targetChar = target:GetData('Character')
      if targetChar then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Properties:Dyn8:Transfer", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData('Character')
    local prop = _properties[data.property]
    if Player(source).state.onDuty == 'realestate' then
      if prop ~= nil and prop.sold and char then
        local owner = exports['hrrp-base']:FetchSID(prop.owner.SID)
        local newOwner = exports['hrrp-base']:FetchSID(tonumber(data.target))
                    ...
                    if accepted and stateId == newOwnerChar:GetData("SID") then
                      if Properties.Commerce:Buy(prop.id, {
                        ...
                        Phone.Notification:Add(source, "Property Transfer Successful",
                          "The property transfer was successful.", os.time() * 1000, 7000, _phoneApp, {})

                        exports['hrrp-base']:LoggerWarn(
                          "Properties",
                          string.format(
                        ...
                        Phone.Notification:Add(source, "Property Transfer Failed", "The property transfer failed.",
                          os.time() * 1000, 7000, _phoneApp, {})
                      end
                    else
                      Phone.Notification:Add(source, "Property Transfer Failed", "The new owner declined the transfer.",
                        os.time() * 1000, 7000, _phoneApp, {})
                    end
                  end)
                else
                  Phone.Notification:Add(source, "Property Transfer Failed", "The owner declined the transfer.",
                    os.time() * 1000, 7000, _phoneApp, {})
                end
              end)
...
function SendPropertyProfits(type, propPrice, propLabel, playerBankAccount, payedAccount, buyerData)
  ...
  if dynastyAccount then
    Banking.Balance:Deposit(dynastyAccount.Account, math.floor(propPrice * (companyCut / 100)), {
      ...
      description = string.format('Property %s - %s to %s %s (SID %s)', type, propLabel, buyerData.First, buyerData.Last,
        buyerData.SID),
      data = {
        property = propLabel,
        buyer = buyerData,
