...
function CycleFurniture(direction)
    ...
    exports['hrrp-objects']:Start(GetHashKey(fKey), "Furniture:Client:Place", {}, true, "Furniture:Client:Cancel", true, fData.placeGround)
end

AddEventHandler("Furniture:Client:Place", function(data, placement)
    if _placingFurniture then

        exports["hrrp-base"]:CallbacksServer("Properties:PlaceFurniture", {
            ...
            if success then
                exports['hrrp-hud']:NotificationSuccess("Placed Item")
            else
...
AddEventHandler("Furniture:Client:Move", function(data, placement)
    if _placingFurniture and data.id then
        ...
        exports["hrrp-base"]:CallbacksServer("Properties:MoveFurniture", {
            id = data.id,
            coords = {
                x = placement.coords.x,
                y = placement.coords.y,
            ...
            if success then
                exports['hrrp-hud']:NotificationSuccess("Moved Item")
            else
                exports['hrrp-hud']:NotificationError("Error")
            end
...
AddEventHandler("Furniture:Client:CancelMove", function(data)
    if _placingFurniture and data.id then
        ...
        exports['hrrp-hud']:NotificationError("Move Cancelled")
        _placingFurniture = false
        LocalPlayer.state.placingFurniture = false
        if not _skipPhone then
            Phone:Open()
