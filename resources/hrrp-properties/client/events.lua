...
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-logout", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-closet", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-stash", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-exit", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-exit-back", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-warehouse", property.id))
-- 				exports['ox_target']:TargetingZonesRemoveZone(string.format("property-%s-office", property.id))
-- 			end
-- 		end
-- 	end
-- end)
...
RegisterNetEvent("Properties:Client:InnerStuff", function(propertyData, int, furniture)
  ...
  -- 		exports["hrrp-animations"]:EmotesWakeUp(intr.locations.wakeup)
  -- 	end)
  -- end
...
AddEventHandler("Properties:Client:Duty", function(t, data)
  ...
  if property?.data?.jobDuty then
    if LocalPlayer.state.onDuty == property?.data?.jobDuty then
      exports['hrrp-jobs']:JobsDutyOff(property?.data?.jobDuty)
    else
      exports['hrrp-jobs']:JobsDutyOn(property?.data?.jobDuty)
    end
  end
end)
  ...
  if ownedProps then
    for k, v in ipairs(ownedProps) do
      if v.type == 'house' then
        exports['hrrp-blips']:Add('property-' .. v.id, 'House: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 40, 53, 0.6, 2)
      elseif v.type == 'office' then
        exports['hrrp-blips']:Add('property-' .. v.id, 'Office: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 475, 53, 0.6,
          2)
      elseif v.type == 'warehouse' then
        exports['hrrp-blips']:Add('property-' .. v.id, 'Warehouse: ' .. v.label, vector3(v.location.front.x, v.location.front.y, v.location.front.z), 473, 53,
          0.6, 2)
      end
    end
  end
