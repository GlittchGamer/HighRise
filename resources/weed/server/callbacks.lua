...
function RegisterCallbacks()
  exports['core']:CallbacksRegisterServer("Weed:CheckPlant", function(source, pid, cb)
  ...
  exports['core']:CallbacksRegisterServer("Weed:WaterPlant", function(source, pid, cb)
    if pid ~= nil and _plants[pid] then
      if checkNearPlant(source, pid) then
          ...
          if _plants[pid] ~= nil then
            if _plants[pid].plant.water < 100 then
              if Inventory.Items:Has(char:GetData("SID"), 1, "water", 1) then
                ...
                exports['core']:ExecuteClient(source, "Notification", "Error", "You Don't Have Water")
              end
            else
              exports['core']:ExecuteClient(source, "Notification", "Error", "Plant Is Already Watered")
            end
          else
            exports['core']:ExecuteClient(source, "Notification", "Error", "Invalid Plant")
          end
        end
      else
        cb(nil)
  ...
  exports['core']:CallbacksRegisterServer("Weed:FertilizePlant", function(source, data, cb)
          ...
          if _plants[data.id] ~= nil then
            if _plants[data.id].plant.fertilizer == nil then
              ...
              then
                ...
                exports['core']:ExecuteClient(source, "Notification", "Error", "You Don't Have Fertilizer")
              end
            else
              exports['core']:ExecuteClient(source, "Notification", "Error", "Plant Is Already Fertilized")
            end
          else
            exports['core']:ExecuteClient(source, "Notification", "Error", "Invalid Plant")
          end
  ...
  exports['core']:CallbacksRegisterServer("Weed:HarvestPlant", function(source, pid, cb)
    if pid then
      if checkNearPlant(source, pid) then
        if _plants[pid] ~= nil then
          local char = exports['core']:FetchSource(source):GetData("Character")
  ...
  exports['core']:CallbacksRegisterServer("Weed:DestroyPlant", function(source, pid, cb)
      ...
      if char ~= nil then
        if pid and _plants[pid] then
          if checkNearPlant(source, pid) then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Success", "Plant Has Been Destroyed")
          else
            cb(nil)
          end
        else
  ...
  exports['core']:CallbacksRegisterServer("Weed:PDDestroyPlant", function(source, pid, cb)
    if pid and _plants[pid] then
      if checkNearPlant(source, pid) then
        local char = exports['core']:FetchSource(source):GetData("Character")
        if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Weed:BuyPackage", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    ...
    if char ~= nil then
      if _packagesAvailable > 0 and not _weedBuyers[char:GetData("ID")] then
        if Wallet:Modify(source, -Config.PackagePrice) then
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "Dont Have Enough Cash")
          cb(false)
        end
      else
        exports['core']:LoggerInfo(
        ...
        if _packagesAvailable > 0 then
          exports['core']:ExecuteClient(source, "Notification", "Error", "You've Already Bought A Package")
        else
          exports['core']:ExecuteClient(source, "Notification", "Error", "No Packages Available")
        end
        cb(false)
      end
    else
