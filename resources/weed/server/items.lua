function RegisterItems()
  Inventory.Items:RegisterUse("weedseed_male", "Weed", function(source, item)
    if GlobalState[string.format("%s:House", source)] == nil then
      ...
      exports['core']:CallbacksClient(source, "Weed:PlantingAnim", {}, function(data)
        if data.error == nil then
          ...
          if data.error == 2 then
            exports['core']:ExecuteClient(source, "Notification", "Error", "Need Better Soil")
          end
        end
      end)
    else
      exports['core']:ExecuteClient(source, "Notification", "Error", "Plant Needs Natural Light")
    end
  end)
  ...
  Inventory.Items:RegisterUse("rolling_paper", "Weed", function(source, item)
    ...
    if Inventory.Items:Has(char:GetData("SID"), 1, "weed_bud", 1) then
      exports['core']:CallbacksClient(source, "Weed:RollingAnim", {}, function(success)
        if success then
      ...
      exports['core']:ExecuteClient(source, "Notification", "Error", "You need bud you fucking idiot")
    end
  end)

  Inventory.Items:RegisterUse("weed_joint", "Weed", function(source, item)
    local char = exports['core']:FetchSource(source):GetData("Character")
    exports['core']:CallbacksClient(source, "Weed:SmokingAnim", {}, function(success, count)
  ...
  Inventory.Items:RegisterUse("weed_brick", "Weed", function(source, item)
    ...
    if Inventory.Items:Has(char:GetData("SID"), 1, "weed_brick", 1) then
      exports['core']:CallbacksClient(source, "Weed:MakingBrick", {
      ...
      exports['core']:ExecuteClient(source, "Notification", "Error", "You need 200 bud you fucking idiot")
    end
  end)
