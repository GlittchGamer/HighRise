...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['hrrp-base']:RequestDependencies('Evidence', {
    ...
    if #error > 0 then
      exports['hrrp-base']:FetchComponent('Logger'):Critical('Evidence', 'Failed To Load All Dependencies')
      return
    end
    RetrieveComponents()
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Evidence:Fetch', function(source, data, cb)
      cb(EVIDENCE_CACHE)
    end)
...
RegisterNetEvent('Evidence:Server:PickupEvidence', function(evidenceId)
  ...
  if char and exports['hrrp-jobs']:JobsPermissionsHasJob(_src, 'police') then
    for k, v in ipairs(EVIDENCE_CACHE) do
