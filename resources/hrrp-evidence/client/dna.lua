...
RegisterNetEvent("Evidence:Client:RanDNA", function(tooDegraded, success, evidenceId)
  ...
  Progress:Progress({
    ...
    if not status then
      if tooDegraded then
        return exports['hrrp-hud']:NotificationError("DNA too Degraded to Run")
      end
      if success then
        exports['hrrp-hud']:NotificationSuccess("DNA Match Found")
        ...
        exports['hrrp-hud']:NotificationError("Could Not Match DNA")
      end
    end
  end)
end)
...
AddEventHandler("Polyzone:Enter", function(id, point, insideZone, data)
  if data and data.dna and (LocalPlayer.state.onDuty == "police" or LocalPlayer.state.onDuty == "ems") then
    _withinDNATesting = true
    exports['hrrp-hud']:ActionShow("{key}Use DNA Evidence{/key} Run DNA Sample")
  end
end)
