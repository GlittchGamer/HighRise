...
AddEventHandler("Billboards:Client:SetLink", function(e, data)
  ...
  exports['core']:CallbacksServer("Billboards:UpdateURL", {
    id = data.id,
    link = tvLink
  }, function(success, invalidUrl)
    if success then
      exports['hud']:NotificationSuccess("Updated Link!", 5000)
    else
      if invalidUrl then
        exports['hud']:NotificationError("Invalid URL - Imgur Links Only", 5000)
      else
        exports['hud']:NotificationError("Error", 5000)
      end
    end
  end)
end)
