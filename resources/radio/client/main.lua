...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Radio", {
    "Callbacks",
    "Notification",
    "Action",
    "Progress",
    ...
    exports['keybinds']:Add('voip_radio_power', '', 'keyboard', 'Voice - Radio - Toggle Power On/Off', function()
    ...
    exports['keybinds']:Add('voip_radio_open', '', 'keyboard', 'Voice - Radio - Open Radio', function()
      if LocalPlayer.state.loggedIn and HAS_RADIO and not LocalPlayer.state.doingAction then
        OpenRadio()
      end
    end)

    exports['keybinds']:Add('voip_radio_vol_down', '', 'keyboard', 'Voice - Radio - Volume Down', function()
      if LocalPlayer.state.loggedIn and HAS_RADIO and RADIO_POWER then
        RadioVolumeDown(true)
      end
    end)

    exports['keybinds']:Add('voip_radio_vol_up', '', 'keyboard', 'Voice - Radio - Volume Up', function()
      if LocalPlayer.state.loggedIn and HAS_RADIO and RADIO_POWER then
        RadioVolumeUp(true)
      end
    end)

    exports['keybinds']:Add('voip_radio_next', '', 'keyboard', 'Voice - Radio - Channel Next', function()
      if LocalPlayer.state.loggedIn and HAS_RADIO and RADIO_POWER then
        CycleRadioChannel(true)
      end
    end)

    exports['keybinds']:Add('voip_radio_prev', '', 'keyboard', 'Voice - Radio - Channel Prev.', function()
      if LocalPlayer.state.loggedIn and HAS_RADIO and RADIO_POWER then
...
function ToggleRadioPower(fromUI)
  ...
  if RADIO_POWER then
    ...
    if not fromUI then
      exports['hud']:NotificationError('Radio Turned Off', 2500)
    end
  else
    if not fromUI then
      exports['hud']:NotificationSuccess('Radio Turned On', 2500)
    end
    RADIO_POWER = true
    ...
    if freq > 0 and RADIO_POWER then
      if CanRadioAccessChannel(HAS_RADIO, freq) then
        ...
        if access then
          ...
          exports['hud']:NotificationError('Encrypted Radio Channel')
        end
      else
        exports['hud']:NotificationError('Out of Range Frequency With This Radio')
        canUseFrequency = false
      end
    end
    ...
    if canUseFrequency then
      ...
      if notifyChange then
        ...
        if frequencyName then
          exports['hud']:NotificationInfo('Changed Radio Channel to ' .. frequencyName)
        else
          exports['hud']:NotificationInfo('Changed Radio Channel to #' .. RADIO_FREQUENCY)
        end
      end
    else
      SendUpdates()
...
function RadioVolumeUp(notify)
  ...
  if notify then
    exports['hud']:NotificationInfo('Radio Volume: ' .. math.floor(RADIO_VOLUME) .. '%', 1500)
  end
