...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Weapons", {
    ...
    exports['core']:CallbacksRegisterServer("Weapons:UseThrowable", function(source, data, cb)
      local plyr = exports['core']:FetchSource(source)
      if plyr ~= nil then
...
WEAPONS = {
  ...
  EquipAttachment = function(self, source, item)
    ...
    if plyr ~= nil then
      ...
      if char ~= nil then
        local p = promise.new()
        exports['core']:CallbacksClient(source, "Weapons:Check", {}, function(data)
          if not data then
            exports['core']:ExecuteClient(source, "Notification", "Error", "No Weapon Equipped")
            p:resolve(false)
          else
            ...
            if itemData ~= nil and itemData.component ~= nil then
              if itemData.component.strings[weaponData.weapon or weaponData.name] ~= nil then
                ...
                exports['core']:ExecuteClient(source, "Notification", "Error", "This Does Not Fit On This Weapon")
                return p:resolve(false)
              end
            else
              exports['core']:ExecuteClient(source, "Notification", "Error", "Something Was Not Defined")
              return p:resolve(false)
            end
          end
        end)
