...
function RegisterRandomItems()
  Inventory.Items:RegisterUse("cigarette", "RandomItems", function(source, item)
    if GetVehiclePedIsIn(GetPlayerPed(source)) == 0 then
      ...
      exports['core']:ExecuteClient(source, "Notification", "Error", "Cannot Be Used In A Vehicle")
    end
  end)
  ...
  Inventory.Items:RegisterUse("cigarette_pack", "RandomItems", function(source, item)
    if (item.MetaData.Count and tonumber(item.MetaData.Count) or 0) > 0 then
      ...
      Inventory.Items:RemoveSlot(item.Owner, item.Name, 1, item.Slot, item.invType)
      exports['core']:ExecuteClient(source, "Notification", "Error", "Pack Has No More Cigarettes In It")
    end
  end)
  ...
  Inventory.Items:RegisterUse("parachute", "RandomItems", function(source, item)
    exports['core']:CallbacksClient(source, "Weapons:CanEquipParachute", {}, function(canEquip)
      if canEquip then
        ...
        if char then
          ...
          if not hasValue(states, "SCRIPT_PARACHUTE") then
            ...
            exports['core']:ExecuteClient(source, "Notification", "Error", "Already Have Parachute Equipped")
          end
        end
      else
        exports['core']:ExecuteClient(source, "Notification", "Error", "Cannot Equip Parachute")
      end
    end)
  end)

  exports['core']:CallbacksRegisterServer("Inventory:UsedParachute", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    if char then
      local states = char:GetData("States") or {}
