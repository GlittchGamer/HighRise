...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports['core']:RegisterComponent("Crafting", CRAFTING)
end)
    ...
    if plyr.Permissions:IsAdmin() then
      ...
      if char ~= nil then
        if _cooldowns[bench] ~= nil then
          exports['core']:LoggerInfo("Crafting",
    ...
    while _cdThreading do
      ...
      MySQL.query('DELETE FROM crafting_cooldowns WHERE expires < ?', { os.time() }, function(d)
        if d.affectedRows > 0 then
          exports['core']:LoggerInfo("Inventory", string.format("Remove ^2%s^7 Expired Crafting Cooldowns", d.affectedRows))
        end
      end)
      Citizen.Wait(60000)
    end
...
function RegisterCraftingCallbacks()
  exports['core']:CallbacksRegisterServer("Crafting:GetBenchDetails", function(source, data, cb)
    ...
    if
        ...
        and (
          ...
          or (bench.restrictions.job ~= nil and exports['jobs']:JobsPermissionsHasJob(
            source,
  ...
  exports['core']:CallbacksRegisterServer("Crafting:Craft", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:Start(char:GetData("SID"), data.bench, data.result, data.qty))
  end)

  exports['core']:CallbacksRegisterServer("Crafting:End", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:End(char:GetData("SID")))
  end)

  exports['core']:CallbacksRegisterServer("Crafting:Cancel", function(source, data, cb)
    local char = exports['core']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:Cancel(char:GetData("SID")))
  end)

  exports['core']:CallbacksRegisterServer("Crafting:GetSchematics", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Crafting:UseSchematic", function(source, data, cb)
    local plyr = exports['core']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
