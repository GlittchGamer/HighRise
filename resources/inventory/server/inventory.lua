...
function RetrieveComponents()
  ...
  EntityTypes = exports['core']:FetchComponent("EntityTypes")
...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['core']:RequestDependencies("Inventory", {
    ...
    exports['core']:MiddlewareAdd("Characters:Logout", function(source)
      ...
      exports['core']:CallbacksClient(source, "Weapons:Logout", {}, function(data)
        if data ~= nil then
    ...
    exports['core']:MiddlewareAdd("Characters:Created", function(source, cData)
      local player = exports['core']:FetchSource(source)
      local docs = {}
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports['core']:RegisterComponent("Inventory", INVENTORY)
end)
...
function refreshShit(sid, adding)
  local plyr = exports['core']:FetchSID(tonumber(sid))
  if plyr ~= nil then
...
function entityPermCheck(source, invType)
  ...
  if shittyInvData then
    return (
      ...
      or (shittyInvData.restriction.job ~= nil and exports['jobs']:JobsPermissionsHasJob(
        source,
    ...
    if entityFrom.shop then
      ...
      if data.ownerFrom ~= data.ownerTo then
        if data.invTypeFrom == 1 then
          local plyr = exports['core']:FetchSID(data.ownerFrom)
        ...
        if data.invTypeTo == 1 then
          local plyr = exports['core']:FetchSID(data.ownerTo)
          if item.isThrowable then
            TriggerClientEvent(
              "Weapons:Client:UpdateCount",
              plyr:GetData("Source"),
...
function RegisterCallbacks()
  exports['core']:CallbacksRegisterServer("Inventory:MergeItem", DoMerge)
  exports['core']:CallbacksRegisterServer("Inventory:SwapItem", DoSwap)
  exports['core']:CallbacksRegisterServer("Inventory:MoveItem", DoMove)

  exports['core']:CallbacksRegisterServer("Inventory:OpenTrunk", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Inventory:UseItem", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Inventory:UseSlot", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Inventory:CheckIfNearDropZone", function(source, data, cb)
  ...
  exports['core']:CallbacksRegisterServer("Inventory:Server:retreiveStores", function(source, data, cb)
    cb(shopLocations)
  end)

  exports['core']:CallbacksRegisterServer("Inventory:Search", function(source, data, cb)
    local plyr = exports['core']:FetchSource(data.serverId)
    if plyr ~= nil then
  ...
  exports['core']:CallbacksRegisterServer("Inventory:Raid", function(source, data, cb)
    local dest = exports['core']:FetchSource(source):GetData("Character")
    local pState = Player(source).state

    if pState.onDuty ~= nil and pState.onDuty == "police" and exports['jobs']:JobsPermissionsHasPermissionInJob(source, 'police', 'PD_RAID') then
      Inventory:OpenSecondary(source, data.invType, data.owner, data.class or false, data.model or false, true)
    end
  end)

  exports['core']:CallbacksRegisterServer("Inventory:CloseSecondary", function(source, inventory, cb)
...
INVENTORY = {
  ...
  GetFreeSlotNumbers = function(self, Owner, invType, vehClass, vehModel)
    ...
    if LoadedEntitys[invType] ~= nil then
      ...
      exports['core']:LoggerError("Inventory", string.format("Entity Type ^2%s^7 Was Attempted To Be Loaded", invType))
    end
  ...
  CreateItem = function(self, Owner, Name, Count, Slot, md, invType, isRecurse, forceCreateDate, quality)
    ...
    if itemExist then
      ...
      elseif itemExist.name == "govid" and invType == 1 then
        local plyr = exports['core']:FetchSID(Owner)
        local char = plyr:GetData("Character")
        local genStr = "Male"
  ...
  AddSlot = function(self, Owner, Name, Count, MetaData, Slot, Type, forceCreateDate, quality)
    if Count <= 0 then
      exports['core']:LoggerError("Inventory", "[AddSlot] Cannot Add " .. Count .. " of an Item (" .. Owner .. ":" .. Type .. ")")
      return false
    end
    ...
    if Slot == nil then
      ...
      if #freeSlots == 0 then
        exports['core']:LoggerError("Inventory",
  ...
  Items = {
    ...
    Use = function(self, source, item, cb)
      ...
      then
        ...
        if
            ...
            or (itemData.useRestrict.job ~= nil and exports['jobs']:JobsPermissionsHasJob(
              source,
        ...
        then
          ...
          if itemData.animConfig ~= nil then
            used = false
            local p = promise.new()
            exports['core']:CallbacksClient(source, "Inventory:ItemUse", itemData.animConfig, function(state)
              p:resolve(state)
            end)
          ...
          exports['core']:ExecuteClient(source, "Notification", "Error", "You Can't Use That")
          cb(false)
        end
      else
        cb(false)
    ...
    RemoveId = function(self, owner, invType, item)
      ...
      if invType == 1 then
        local plyr = exports['core']:FetchSID(tonumber(owner))
        if plyr ~= nil then
          local source = plyr:GetData("Source")
          ...
          if char ~= nil then
            ...
            if #inv > 0 then
              ...
              if #inv <= #freeSlots then
                ...
                exports['core']:ExecuteClient(source, "Notification", "Success", "Retreived Items")
              else
                exports['core']:ExecuteClient(source, "Notification", "Error", "Not Enough Slots Available")
              end
            else
              exports['core']:ExecuteClient(source, "Notification", "Error", "No Items To Retreive")
            end
          end
  ...
  Container = {
    Open = function(self, src, item, identifier)
      exports['core']:CallbacksClient(src, "Inventory:Container:Open", {
        item = item,
  ...
  Search = {
    Character = function(self, src, tSrc, id)
      exports['core']:CallbacksClient(tSrc, "Inventory:ForceClose", {}, function(state)
        exports['core']:ExecuteClient(tSrc, "Notification", "Info", "You Were Searched")
