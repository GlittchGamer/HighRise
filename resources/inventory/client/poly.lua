  ...
  for k, v in ipairs(locs) do
    ...
    if data ~= nil then
      ...
      if data.type == "box" then
        exports['polyzonehandler']:PolyZoneCreateBox(data.id, data.coords, data.length, data.width, data.options, data.data)
      elseif data.type == "poly" then
        exports['polyzonehandler']:PolyZoneCreatePoly(data.id, data.points, data.options, data.data)
      else
        exports['polyzonehandler']:PolyZoneCreateCircle(data.id, data.coords, data.radius, data.options, data.data)
      end
    end
  end
end)
