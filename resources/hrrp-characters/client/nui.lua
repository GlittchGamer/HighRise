...
RegisterNUICallback('GetData', function(data, cb)
  ...
  exports["hrrp-base"]:CallbacksServer('Characters:GetServerData', {}, function(serverData)
    SendNUIMessage({
      type = 'LOADING_SHOW',
      data = { message = 'Getting Character Data' }
    })

    exports["hrrp-base"]:CallbacksServer('Characters:GetCharacters', {}, function(characters)
      SetEntityCoords(PlayerPedId(), 685.865, 576.222, 132.841, 0.0, 0.0, 0.0, false)

      local cam2 = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", 685.865, 576.222, 132.841, 338.730, 0.00, 0.00, 75.00,
        false, 0)
      ...
      for k, v in ipairs(characters) do
        if v.Preview then
          ...
          Ped:Preview(ped, tonumber(v.Gender), v.Preview, false, v.GangChain or nil)
...
RegisterNUICallback('CreateCharacter', function(data, cb)
  cb("ok")
  exports["hrrp-base"]:CallbacksServer('Characters:CreateCharacter', data, function(character)
...
RegisterNUICallback('DeleteCharacter', function(data, cb)
  cb("ok")
  exports["hrrp-base"]:CallbacksServer('Characters:DeleteCharacter', data.id, function(status)
    if status then
      SendNUIMessage({
...
RegisterNUICallback('SelectCharacter', function(data, cb)
  cb("ok")
  exports["hrrp-base"]:CallbacksServer('Characters:GetSpawnPoints', data.id, function(spawns)
    if spawns then
      SendNUIMessage({
        type = 'SET_SPAWNS',
        data = { spawns = spawns }
...
RegisterNUICallback('PlayCharacter', function(data, cb)
  cb("ok")
  exports["hrrp-base"]:CallbacksServer('Characters:GetCharacterData', data.character.ID, function(cData)
    cData.spawn = data.spawn
    deletePeds()
    TriggerEvent('Characters:Client:SetData', -1, cData, function()
      local pedSpawn = exports['hrrp-ped']:SpawnToWorld(cData)
      exports['hrrp-characters']:SpawnToWorld(cData, function()
        LocalPlayer.state.canUsePhone = true
        if data.spawn.event ~= nil then
          exports["hrrp-base"]:CallbacksServer(data.spawn.event, data.spawn, function()
