...
AddEventHandler("Casino:Client:Startup", function()
  ...
  for k, v in pairs(_rouletteTables) do
    ...
    exports['ox_target']:TargetingZonesAddBox("casino-roulette-" .. k, "cards", v.polyzone.center, v.polyzone.length, v.polyzone.width, v.polyzone.options, {
      {
        icon = "cards",
  ...
  exports['hrrp-keybinds']:Add("casino_camera", "C", "keyboard", "Casino - Change Roulette Camera", function()
...
AddEventHandler("Casino:Client:JoinRoulette", function(_, data)
  ...
  if chairData then
    exports["hrrp-base"]:CallbacksServer("Casino:JoinRoulette", { table = data.table, chair = chairData.chairId }, function(success, table, chair)
      if success then
        ...
        _rouletteStatebagHandler = AddStateBagChangeHandler(string.format("Casino:Blackjack:%s", tableId), nil,
    ...
    exports['hrrp-hud']:NotificationError("Too Far From Chair")
  end
end)

function LeaveRoulette(skipAnim)
  exports["hrrp-base"]:CallbacksServer("Casino:LeaveRoulette", {}, function(success)
    if success then
...
function RoulettePlaceChips(betType)
  ...
  if betAmount then
    exports["hrrp-base"]:CallbacksServer("Casino:BetRoulette", {
      amount = betAmount,
      betId = betType,
    }, function(success)
      if success then
...
function ShowRouletteGameStateUI(state)
  if state.Started then
    ...
    InfoOverlay:Show(string.format("Roulette%s", showStartTime),
      string.format("My Balance: $%s<br><br>Placed Bets: $%s<br>Table Limit: $%s", myBalance, formatNumberToCurrency(math.floor(myBets)), maxBet))
  end
end
