...
AddEventHandler("Casino:Client:Startup", function()
  for k, v in ipairs(_slotMachineObjects) do
    exports['ox_target']:TargetingAddObject(v, "slot-machine", {
      {
        icon = "slot-machine",
        text = "Use Machine",
...
AddEventHandler("Casino:Client:UseSlotMachine", function()
  ...
  if tableId then
    exports["hrrp-base"]:CallbacksServer("Casino:SlotMachineSit", tableId, function(success)
      if success then
        ...
        NetworkAddPedToSynchronisedScene(LocalPlayer.state.ped, SITTING_SCENE, "anim_casino_b@amb@casino@games@shared@player@", randomSit, 2.0, -2.0, 13, 16, 2.0,
          0)
        ...
        exports['hrrp-hud']:NotificationError("Seat Taken")
      end
    end)
  else
    exports['hrrp-hud']:NotificationError("Not Close Enough to Machine")
  end
end)
...
AddEventHandler("Casino:Client:PlaySlotMachine", function(_, data)
  if _satInChair then
    ...
    exports["hrrp-base"]:CallbacksServer("Casino:SlotMachinePlay", data, function(success, reelRotations, timeOut, sound, wonAmount)
      if success then
...
AddEventHandler("Casino:Client:LeaveSlotMachine", function()
  if _satInChair then
    exports["hrrp-base"]:CallbacksServer("Casino:SlotMachineLeave", {}, function(success)
      if success then
        InfoOverlay:Close()
...
function ShowSlotStateUI()
  if _satInChair then
    ...
    local overlay = string.format("Chip Balance: $%s<br>Session Spent: $%s<br>Session Winnings: $%s", formatNumberToCurrency(myBalance),
      formatNumberToCurrency(math.floor(_sessionSpent)), formatNumberToCurrency(math.floor(_sessionWinnings)))
