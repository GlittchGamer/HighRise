...
AddEventHandler("Casino:Client:Startup", function()
  ...
  for k, v in pairs(_blackjackTables) do
    ...
    exports['ox_target']:TargetingZonesAddBox("casino-blackjack-" .. k, "cards", v.polyzone.center, v.polyzone.length, v.polyzone.width, v.polyzone.options, {
      {
        icon = "cards",
      ...
      },
      {
        ...
        isEnabled = function()
          return _BJsatAtTable and not _inSittingDownAnimation and not _blackjackAwaitingResponse and
              not GlobalState[string.format("Casino:Blackjack:%s", k)]?.Started and GlobalState["CasinoOpen"]
        end,
      },
    }, 1.5, true)
  end

  exports["hrrp-base"]:CallbacksRegisterClient("Casino:Client:RequestHitStand", function(data, cb)
    local itemList = {
      {
        label = "Stand",
    ...
    elseif res?.timeout then
      cb(false)
      exports['hrrp-hud']:NotificationError("Ran Out of Time...")
    else
      cb(false)
    end
  end)
...
AddEventHandler("Casino:Client:Enter", function()
  ...
  for k, v in pairs(_blackjackTables) do
    ...
    local dealer = CreatePed(26, serverData.dealerVariation >= 8 and `s_f_y_casino_01` or `s_m_y_casino_01`, v.dealer.coords.x, v.dealer.coords.y,
      v.dealer.coords.z, v.dealer.heading, false, true)
...
AddEventHandler("Casino:Client:JoinBlackjack", function(_, data)
  ...
  if tableId == data.table then
    exports["hrrp-base"]:CallbacksServer("Casino:JoinBlackjack", _BJclosestChair, function(success, table, chair)
      if success then
        ...
        if table == "vip" then
          exports['hrrp-hud']:NotificationError("You're Not a VIP Loser")
        else
          exports['hrrp-hud']:NotificationError("Someone Is Sat There")
        end
      end
    end)
  end
...
AddEventHandler("Casino:Client:StartBlackjack", function(_, data)
  if _BJsatAtTable and data?.table == _BJsatAtTable then
    exports["hrrp-base"]:CallbacksServer("Casino:StartBlackjack", {}, function(success)
      if success then
...
AddEventHandler("Casino:Client:LeaveBlackjack", function(_, data)
  if _BJsatAtTable and data?.table == _BJsatAtTable then
    exports["hrrp-base"]:CallbacksServer("Casino:LeaveBlackjack", {}, function(success)
      if success then
        _BJsatAtTable = false
        _BJsatAtLocalChair = false
        leaveBlackjackSeat()
...
RegisterNetEvent("Casino:Client:BlackjackConfirmBet", function(betAmounts, tableId)
  ...
  if res?.success and res?.data?.confirmBet and res.data.confirmBet >= 100 then
    exports["hrrp-base"]:CallbacksServer("Casino:BetBlackjack", res.data.confirmBet, function(success, gameData)
      if success then
...
RegisterNetEvent("Casino:Client:BlackjackFinishStandOrHitPhase", function(tableId, chairId)
  if _insideCasino then
    ...
    TaskPlayAnim(dealerPed, "anim_casino_b@amb@casino@games@blackjack@dealer", genderAnimString .. "dealer_focus_player_0" .. chairAnimId .. "_idle_outro", 3.0,
      1.0, -1, 2, 0, 0, 0, 0)
