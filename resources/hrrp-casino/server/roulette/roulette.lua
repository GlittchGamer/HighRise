...
function CheckRouletteWinners(tableId, bets, winningIndex)
  ...
  for k, v in pairs(totalsWon) do
    if v > 0 then
      local plyr = exports['hrrp-base']:FetchSource(k)
      if plyr then
...
AddEventHandler("Casino:Server:Startup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Casino:JoinRoulette", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Casino:LeaveRoulette", function(source, data, cb)
    --local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  -- Bets
  exports['hrrp-base']:CallbacksRegisterServer("Casino:BetRoulette", function(source, data, cb)
    --local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    ...
    if (amount >= 100) and _roulette[roulettePlayer.Table] and not _roulette[roulettePlayer.Table].Started and _roulette[roulettePlayer.Table].Seats[roulettePlayer.LocalChair] then
      if (_roulette[roulettePlayer.Table].Seats[roulettePlayer.LocalChair].TotalBet + amount) <= _rouletteTables[roulettePlayer.Table].maxBet then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Over Table Bet Limit")
        cb(false)
      end
    else
      cb(false)
