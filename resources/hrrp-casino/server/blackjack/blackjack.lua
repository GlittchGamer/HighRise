...
AddEventHandler("Casino:Server:Startup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Casino:JoinBlackjack", function(source, chairId, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Casino:LeaveBlackjack", function(source, data, cb)
    --local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Casino:StartBlackjack", function(source, data, cb)
    local blackjackPlayer = _blackjackPlayers[source]
    if not blackjackPlayer then
      return cb(false)
    end
  ...
  -- Bet Confirmation
  exports['hrrp-base']:CallbacksRegisterServer("Casino:BetBlackjack", function(source, data, cb)
    --local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local blackjackPlayer = _blackjackPlayers[source]
    if not blackjackPlayer then
      return cb(false)
    ...
    if _blackjack[blackjackPlayer.Table] and _blackjack[blackjackPlayer.Table].Seats[blackjackPlayer.LocalChair] then
      if Casino.Chips:Modify(source, -data) then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Enough Chips")
        cb(false)
      end
    else
      cb(false)
...
function GetHitStandResponse(src, currentHand, canDouble, currentBet)
  local p = promise.new()
  exports["hrrp-base"]:CallbacksClient(src, "Casino:Client:RequestHitStand", { currentHand = currentHand, canDouble = canDouble, currentBet = currentBet },
