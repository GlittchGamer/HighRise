...
function RetrieveComponents()
  ...
  Scenes = exports['hrrp-base']:FetchComponent('Scenes')
...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['hrrp-base']:RequestDependencies('Scenes', {
    'Logger',
    'Fetch',
    'Callbacks',
    'Game',
    ...
    exports['hrrp-keybinds']:Add('scene_create', '', 'keyboard', 'Scenes - Create Scene', function()
...
_SCENES = {
  BeginCreation = function(self, text, staff)
    ...
    if not hitting then
      return exports['hrrp-hud']:NotificationError('Cannot Place Here')
    end

    if #(GetEntityCoords(LocalPlayer.state.ped) - endCoords) > 5.0 then
      return exports['hrrp-hud']:NotificationError('Cannot Place That Far Away')
    end

    if IsEntityAVehicle(entity) or IsEntityAPed(entity) then
      return exports['hrrp-hud']:NotificationError('Cannot Place On a Vehicle or Person')
    end
  ...
  Deletion = function(self)
    if _nearbyScenes and #_nearbyScenes > 0 then
      ...
      if hitting and endCoords then
        ...
        exports["hrrp-base"]:CallbacksServer('Scenes:Delete', lastId, function(success, invalidPermissions)
          if success then
            exports['hrrp-hud']:NotificationSuccess('Scene Deleted')
          else
            if invalidPermissions then
              exports['hrrp-hud']:NotificationError('Invalid Permissions to Delete This Scene')
            else
              exports['hrrp-hud']:NotificationError('Failed to Delete Scene')
            end
          end
        end)
      end
  ...
  Edit = function(self)
    if _nearbyScenes and #_nearbyScenes > 0 then
      ...
      if hitting and endCoords then
        ...
        exports["hrrp-base"]:CallbacksServer('Scenes:CanEdit', lastId, function(success, isStaff)
          if success then
            EditScene(lastId, _loadedScenes[lastId], { staff = isStaff })
          else
            exports['hrrp-hud']:NotificationError('Invalid Permissions to Edit This Scene')
          end
        end)
      end
    end
...
AddEventHandler('Scenes:Client:OpenOptionsMenu', function(values, data)
  ...
  _creationMenu = exports['hrrp-menu']:Create('scenes', 'Scene Creation', function()
    _creationOpen = true
    Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
      while _creationOpen do
        if (#(GetEntityCoords(LocalPlayer.state.ped) - creatingSceneData.coords) > 10.0) then
          exports['hrrp-hud']:NotificationError('Scene Creation Cancelled - Too Far Away')
  ...
  _creationMenu.Add:Button('Create Scene', { success = true }, function()
    ...
    exports["hrrp-base"]:CallbacksServer('Scenes:Create', {
      scene = creatingSceneData,
      data = data,
    }, function(success)
      if success then
        exports['hrrp-hud']:NotificationSuccess('Scene Placed')
      else
        exports['hrrp-hud']:NotificationError('Failed to Place Scene')
      end
    end)
...
function EditScene(id, fuckface, data)
  ...
  _creationMenu = exports['hrrp-menu']:Create('scenes', 'Edit Scene', function()
    _creationOpen = true
    ...
    Citizen.CreateThread(function()
      while _creationOpen do
        if (#(GetEntityCoords(LocalPlayer.state.ped) - creatingSceneData.coords) > 10.0) then
          exports['hrrp-hud']:NotificationError('Scene Edit Cancelled - Too Far Away')
  ...
  _creationMenu.Add:Button('Edit Scene', { success = true }, function()
    exports["hrrp-base"]:CallbacksServer('Scenes:Edit', {
      id = id,
      scene = creatingSceneData,
      data = data,
    }, function(success)
      if success then
        exports['hrrp-hud']:NotificationSuccess('Scene Edited')
      else
        exports['hrrp-hud']:NotificationError('Failed to Edit Scene')
      end
    end)
...
AddEventHandler('Proxy:Shared:RegisterReady', function()
  exports["hrrp-base"]:RegisterComponent('Scenes', _SCENES)
end)
