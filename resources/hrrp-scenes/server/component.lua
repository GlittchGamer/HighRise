...
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports['hrrp-base']:RequestDependencies('Scenes', {
    ...
    if #error > 0 then
      exports['hrrp-base']:FetchComponent('Logger'):Critical('Scenes', 'Failed To Load All Dependencies')
      return
    end
    RetrieveComponents()
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Scenes:Create', function(source, data, cb)
      ...
      if player and data.scene and data.data then
        local wasCreated = _SCENES:Create(data.scene, data.data.staff and player.Permissions:IsStaff())
        if wasCreated then
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Scenes:Delete', function(source, sceneId, cb)
      local player = exports['hrrp-base']:FetchSource(source)
      ...
      if scene and player then
        ...
        cb(_SCENES:Delete(sceneId))
      else
        cb(false)
      end
    end)

    exports['hrrp-base']:CallbacksRegisterServer('Scenes:CanEdit', function(source, sceneId, cb)
      local player = exports['hrrp-base']:FetchSource(source)
      local scene = _loadedScenes[sceneId]
      local timeStamp = GetGameTimer()
    ...
    exports['hrrp-base']:CallbacksRegisterServer('Scenes:Edit', function(source, data, cb)
      local player = exports['hrrp-base']:FetchSource(source)
      ...
      if scene and player then
        ...
        cb(_SCENES:Edit(data.id, data.scene, player.Permissions:IsStaff()))
      else
        cb(false)
      end
    end)
