CRAFTING = {
  Benches = {
    Open = function(self, bench)
      exports["hrrp-base"]:CallbacksServer("Crafting:GetBenchDetails", bench, function(results)
        if results == nil then
          return
        else
        ...
        if v.targeting and not v.targeting.manual then
          ...
          then
            ...
            if
                ...
                or (v.restrictions.job ~= nil and exports['hrrp-jobs']:JobsPermissionsHasJob(
            ...
            then
              ...
              if obj ~= nil then
                -- exports['ox_target']:TargetingAddEntity(obj, v.targeting.icon, menu)
                print("This is broke", obj, v.targeting.icon, menu)
...
AddEventHandler("Crafting:Client:AddSchematic", function(ent, data)
  exports["hrrp-base"]:CallbacksServer("Crafting:GetSchematics", data, function(schematics)
    if #schematics > 0 then
      ...
      exports['hrrp-hud']:NotificationError("You Have No Schematics")
    end
  end)
end)
...
AddEventHandler("Crafting:Client:UseSchematic", function(data)
  Progress:Progress({
    ...
    if not cancelled then
      exports["hrrp-base"]:CallbacksServer("Crafting:UseSchematic", data, function(s)
...
RegisterNUICallback("Crafting:Craft", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Crafting:Craft", data, function(state)
    if not state.error then
      if state.data.time > 0 then
        ...
        Progress:Progress({
          ...
          if cancelled then
            exports["hrrp-base"]:CallbacksServer("Crafting:Cancel", {}, function(s)
              if s then
                SendNUIMessage({
                  type = "END_CRAFTING",
                })
        ...
        if state.data ~= nil then
          exports["hrrp-animations"]:EmotesPlay(state.data.animation, true, state.data.time, true)
        end
      end
      cb(true)
    else
      exports['hrrp-hud']:NotificationError(string.format("Error - %s", state.message or "Something Is Broken, Report This"))
      cb(false)
    end
  end)
end)
...
RegisterNUICallback("Crafting:End", function(data, cb)
  cb("OK")
  exports["hrrp-base"]:CallbacksServer("Crafting:End", {}, function(state)
    SendNUIMessage({
      type = "END_CRAFTING",
    })
...
RegisterNUICallback("Crafting:Cancel", function(data, cb)
  exports["hrrp-base"]:CallbacksServer("Crafting:Cancel", {}, function(state)
    cb(state)
    if state then
