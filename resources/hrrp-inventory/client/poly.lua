  ...
  for k, v in ipairs(locs) do
    ...
    if data ~= nil then
      ...
      if data.type == "box" then
        exports['hrrp-polyzone']:PolyZoneCreateBox(data.id, data.coords, data.length, data.width, data.options, data.data)
      elseif data.type == "poly" then
        exports['hrrp-polyzone']:PolyZoneCreatePoly(data.id, data.points, data.options, data.data)
      else
        exports['hrrp-polyzone']:PolyZoneCreateCircle(data.id, data.coords, data.radius, data.options, data.data)
      end
    end
  end
end)
