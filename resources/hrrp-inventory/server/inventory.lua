...
function RetrieveComponents()
  ...
  EntityTypes = exports["hrrp-base"]:FetchComponent("EntityTypes")
  ...
  Drugs = exports["hrrp-base"]:FetchComponent("Drugs")
  Robbery = exports["hrrp-base"]:FetchComponent("Robbery")
end

local setupEvent = nil
setupEvent = AddEventHandler("Core:Shared:Ready", function()
  exports["hrrp-base"]:RequestDependencies("Inventory", {
    ...
    exports['hrrp-base']:MiddlewareAdd("Characters:Logout", function(source)
      ...
      exports["hrrp-base"]:CallbacksClient(source, "Weapons:Logout", {}, function(data)
        if data ~= nil then
    ...
    exports['hrrp-base']:MiddlewareAdd("Characters:Created", function(source, cData)
      local player = exports['hrrp-base']:FetchSource(source)
      local docs = {}
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Inventory", INVENTORY)
end)
...
function refreshShit(sid, adding)
  local plyr = exports['hrrp-base']:FetchSID(tonumber(sid))
  if plyr ~= nil then
...
function entityPermCheck(source, invType)
  ...
  if shittyInvData then
    return (
      ...
      or (shittyInvData.restriction.job ~= nil and exports['hrrp-jobs']:JobsPermissionsHasJob(
        source,
    ...
    if entityFrom.shop then
      ...
      if entityFrom.free or paymentType ~= nil then
        ...
        then
          ...
          if paid then
            ...
            CreateStoreLog(data.ownerFrom, data.name, data.countTo or 1, char:GetData("SID"), insData.metadata,
              insData.id)
          end
      ...
      if data.ownerFrom ~= data.ownerTo then
        if data.invTypeFrom == 1 then
          local plyr = exports['hrrp-base']:FetchSID(data.ownerFrom)
        ...
        if data.invTypeTo == 1 then
          local plyr = exports['hrrp-base']:FetchSID(data.ownerTo)
          if item.isThrowable then
            TriggerClientEvent(
              "Weapons:Client:UpdateCount",
              plyr:GetData("Source"),
...
function RegisterCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:MergeItem", DoMerge)
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:SwapItem", DoSwap)
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:MoveItem", DoMove)

  exports['hrrp-base']:CallbacksRegisterServer("Inventory:OpenTrunk", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:UseItem", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:UseSlot", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:CheckIfNearDropZone", function(source, data, cb)
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:Server:retreiveStores", function(source, data, cb)
    cb(shopLocations)
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Inventory:Search", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(data.serverId)
    if plyr ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Inventory:Raid", function(source, data, cb)
    local dest = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local pState = Player(source).state

    if pState.onDuty ~= nil and pState.onDuty == "police" and exports['hrrp-jobs']:JobsPermissionsHasPermissionInJob(source, 'police', 'PD_RAID') then
      Inventory:OpenSecondary(source, data.invType, data.owner, data.class or false, data.model or false, true)
    end
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Inventory:CloseSecondary", function(source, inventory, cb)
...
RegisterNetEvent("Inventory:Server:UpdateSettings", function(data)
  local src = source
  local player = exports['hrrp-base']:FetchSource(src)
  if player ~= nil then
    local char = player:GetData("Character")
    if char ~= nil then
...
RegisterNetEvent("Inventory:Server:Request", function(secondary)
  ...
  TriggerClientEvent("Inventory:Client:Load", src, plyrInvData,
    secondary and
    Inventory:GetSecondary(src, secondary.invType, secondary.owner, secondary.class or false, secondary.model or false) or
    nil)
end)
...
INVENTORY = {
  ...
  OpenSecondary = function(self, _src, invType, Owner, vehClass, vehModel, isRaid, nameOverride, slotOverride,
                           capacityOverride)
    if _src and invType and Owner then
      local player = exports['hrrp-base']:FetchSource(_src)
      local char = player:GetData("Character")
  ...
  GetFreeSlotNumbers = function(self, Owner, invType, vehClass, vehModel)
    ...
    if LoadedEntitys[invType] ~= nil then
      ...
      exports['hrrp-base']:LoggerError("Inventory", string.format("Entity Type ^2%s^7 Was Attempted To Be Loaded", invType))
    end
  ...
  CreateItem = function(self, Owner, Name, Count, Slot, md, invType, isRecurse, forceCreateDate, quality)
    ...
    if itemExist then
      ...
      elseif itemExist.name == "govid" and invType == 1 then
        local plyr = exports['hrrp-base']:FetchSID(Owner)
        local char = plyr:GetData("Character")
        local genStr = "Male"
  ...
  AddItem = function(self, Owner, Name, Count, md, invType, vehClass, vehModel, entity, isRecurse, Slot, forceCreateDate,
                     quality)
    local MetaData = table.copy(md or {})
  ...
  AddSlot = function(self, Owner, Name, Count, MetaData, Slot, Type, forceCreateDate, quality)
    if Count <= 0 then
      exports['hrrp-base']:LoggerError("Inventory", "[AddSlot] Cannot Add " .. Count .. " of an Item (" .. Owner .. ":" .. Type .. ")")
      return false
    end
    ...
    if Slot == nil then
      ...
      if #freeSlots == 0 then
        exports['hrrp-base']:LoggerError("Inventory",
  ...
  Items = {
    ...
    Use = function(self, source, item, cb)
      ...
      then
        ...
        if
            ...
            or (itemData.useRestrict.job ~= nil and exports['hrrp-jobs']:JobsPermissionsHasJob(
              source,
        ...
        then
          ...
          TriggerClientEvent("Inventory:Client:Changed", source, itemData.type == 2 and "holster" or "used", item.Name, 0,
            item.Slot)
          ...
          if itemData.animConfig ~= nil then
            used = false
            local p = promise.new()
            exports["hrrp-base"]:CallbacksClient(source, "Inventory:ItemUse", itemData.animConfig, function(state)
              p:resolve(state)
            end)
          ...
          exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "You Can't Use That")
          cb(false)
        end
      else
        cb(false)
    ...
    RemoveId = function(self, owner, invType, item)
      ...
      if invType == 1 then
        local plyr = exports['hrrp-base']:FetchSID(tonumber(owner))
        if plyr ~= nil then
          local source = plyr:GetData("Source")
          ...
          if char ~= nil then
            ...
            if #inv > 0 then
              ...
              if #inv <= #freeSlots then
                ...
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Success", "Retreived Items")
              else
                exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Not Enough Slots Available")
              end
            else
              exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "No Items To Retreive")
            end
          end
  ...
  Container = {
    Open = function(self, src, item, identifier)
      exports["hrrp-base"]:CallbacksClient(src, "Inventory:Container:Open", {
        item = item,
  ...
  Search = {
    Character = function(self, src, tSrc, id)
      exports["hrrp-base"]:CallbacksClient(tSrc, "Inventory:ForceClose", {}, function(state)
        exports['hrrp-base']:ExecuteClient(tSrc, "Notification", "Info", "You Were Searched")
