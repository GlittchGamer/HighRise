...
-- 	exports['hrrp-base']:LoggerInfo("Inventory", string.format("Cleaned Up ^2%s^7 Items In Dropzones", f.affectedRows))
...
-- 	exports['hrrp-base']:LoggerInfo("Inventory", string.format("Cleaned Up ^2%s^7 Items In Trash Inventories", trash))
-- 	local delTmp =
-- 		MySQL.query.await(string.format("DELETE FROM inventory WHERE item_id IN (%s)", table.concat(tmpItems, ",")))
-- 	exports['hrrp-base']:LoggerInfo("Inventory", string.format("Cleaned Up ^2%s^7 Temporary Items", delTmp.affectedRows))
-- end
...
function ClearBrokenItems()
  ...
  -- local count = 0
  -- exports['hrrp-base']:LoggerWarn("Inventory", "^1DELETING EXPIRED ITEMS, THIS WILL LIKELY TAKE A MINUTE^7")
  ...
  -- 			-- 				exports['hrrp-base']:LoggerInfo("Inventory", string.format("^1Cleaned Up ^2%s^1 Degraded ^2%s^7", d.affectedRows, v.name))
  -- 			-- 			end
  -- 			-- 			checked += 1
  -- 			-- 		end)
  -- 			-- 	else
  ...
  -- exports['hrrp-base']:LoggerWarn("Inventory", "^1FINISHED DELETING EXPIRED ITEMS^7")
  ...
  CreateThread(function()
    while _started do
      ...
      if tmpItems and #tmpItems > 0 then
        ...
        exports['hrrp-base']:LoggerInfo("Inventory", "No Temporary Items to Clean Up")
      end
  ...
  CreateThread(function()
    while _started do
      MySQL.query("DELETE FROM inventory WHERE expiryDate < ? AND expiryDate != -1", { os.time() }, function(d)
        exports['hrrp-base']:LoggerInfo("Inventory", string.format("Cleaned Up ^2%s^7 Degraded Items", d.affectedRows))
      end)
      Wait((1000 * 60) * 30)
    end
  end)
...
function LoadItems()
    ...
    for k, v in ipairs(its) do
      ...
      if v.state ~= nil then
        ...
        Inventory.Items:RegisterUse(v.name, "CharacterState", function(source, item)
          refreshShit(exports['hrrp-base']:FetchSource(source):GetData("Character"):GetData("SID"))
        end)
      end
      ...
      elseif v.type == 9 then
        Inventory.Items:RegisterUse(v.name, "Ammo", function(source, item)
          exports["hrrp-base"]:CallbacksClient(source, "Weapons:AddAmmo", itemsDatabase[item.Name], function(state)
            if state then
              Inventory.Items:RemoveSlot(item.Owner, item.Name, 1, item.Slot, 1)
            end
          end)
  ...
  RegisterRandomItems()
  exports['hrrp-base']:LoggerTrace("Inventory", string.format("Loaded ^2%s^7 Items", c))
end
...
function LoadEntityTypes()
  ...
  exports['hrrp-base']:LoggerTrace("Inventory", string.format("Loaded ^2%s^7 Inventory Entity Types", #_entityTypes))
end
...
function LoadShops()
  Citizen.CreateThread(function()
    ...
    exports['hrrp-base']:LoggerTrace("Inventory", string.format("Loaded ^2%s^7 Shop Locations", #_shops))
  end)
end
...
function RegisterCommands()
  ...
  Chat:RegisterAdminCommand("closeinv", function(source, args, rawCommand)
    exports['hrrp-base']:LoggerInfo("Inventory", "Closing all inventories")
    _openInvs = {}
  end, {
  ...
  Chat:RegisterAdminCommand("clearinventory", function(source, args, rawCommand)
    local player = exports["hrrp-base"]:FetchComponent("Fetch"):SID(tonumber(args[1]))
    if player == nil then
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "This player is not online")
      return
    end
    ...
    exports['hrrp-base']:ExecuteClient(
      ...
      "Notification",
      "Error",
      "Your inventory was cleared by " .. tostring(exports['hrrp-base']:FetchSource(source):GetData("Character"):GetData("SID"))
    )
    exports['hrrp-base']:ExecuteClient(
      source,
      "Notification",
  ...
  Chat:RegisterAdminCommand("giveitem", function(source, args, rawCommand)
    ...
    if tostring(args[1]) ~= nil and tonumber(args[2]) ~= nil then
      ...
      if itemExist then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Item not located")
      end
    else
  ...
  Chat:RegisterAdminCommand("giveweapon", function(source, args, rawCommand)
    ...
    if tostring(args[1]) ~= nil then
      ...
      if itemExist then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Weapon not located")
      end
    end
  end, {
    help = "Give Weapon",
  ...
  Chat:RegisterAdminCommand("vanityitem", function(source, args, rawCommand)
    ...
    if player and player.Permissions:GetLevel() >= 100 then
      ...
      if char and label and image and amount and amount > 0 then
        ...
        exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Wrong")
      end
    end
  end, {
