...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Crafting", CRAFTING)
end)
    ...
    if plyr.Permissions:IsAdmin() then
      ...
      if char ~= nil then
        if _cooldowns[bench] ~= nil then
          exports['hrrp-base']:LoggerInfo("Crafting",
            string.format("%s %s (%s) Reset Cooldown %s on bench %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), id, bench))
          ...
          exports['hrrp-base']:LoggerInfo("Crafting",
            string.format("%s %s (%s) Attempted To Remove Cooldown %s From Non-Existent Bench %s", char:GetData("First"), char:GetData("Last"),
              char:GetData("SID"), id, bench))
          Chat.Send.Server:Single(source, "Not A Valid Bench")
        end
      end
    else
      exports['hrrp-base']:LoggerInfo("Crafting",
        string.format("%s %s (%s) Attempted To Remove Cooldown %s From Bench %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), id, bench))
    end
  end
end
    ...
    while _cdThreading do
      ...
      MySQL.query('DELETE FROM crafting_cooldowns WHERE expires < ?', { os.time() }, function(d)
        if d.affectedRows > 0 then
          exports['hrrp-base']:LoggerInfo("Inventory", string.format("Remove ^2%s^7 Expired Crafting Cooldowns", d.affectedRows))
        end
      end)
      Citizen.Wait(60000)
    end
...
function RegisterCraftingCallbacks()
  exports['hrrp-base']:CallbacksRegisterServer("Crafting:GetBenchDetails", function(source, data, cb)
    ...
    if
        ...
        and (
          ...
          or (bench.restrictions.job ~= nil and exports['hrrp-jobs']:JobsPermissionsHasJob(
            source,
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Crafting:Craft", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:Start(char:GetData("SID"), data.bench, data.result, data.qty))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Crafting:End", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:End(char:GetData("SID")))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Crafting:Cancel", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    cb(Crafting.Craft:Cancel(char:GetData("SID")))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Crafting:GetSchematics", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Crafting:UseSchematic", function(source, data, cb)
    local plyr = exports['hrrp-base']:FetchSource(source)
    if plyr ~= nil then
      local char = plyr:GetData("Character")
      if char ~= nil then
