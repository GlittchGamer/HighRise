...
function AddBankingATMs()
  for k, v in ipairs(atmObjects) do
    exports['ox_target']:TargetingAddObject(v, "money-from-bracket", {
      {
        text = "Use ATM",
        icon = "dollar-sign",
  ...
  for k, v in ipairs(atmPolys) do
    exports['ox_target']:TargetingZonesAddBox("atm-" .. k, "money-from-bracket", v.center, v.length, v.width, v.options, {
      {
        text = "Use ATM",
        icon = "dollar-sign",
        event = "Banking:Client:StartOpenATM",
