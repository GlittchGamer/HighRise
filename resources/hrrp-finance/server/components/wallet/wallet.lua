AddEventHandler("Finance:Server:Startup", function()
  exports['hrrp-base']:CallbacksRegisterServer("Wallet:GetCash", function(source, data, cb)
    cb(Wallet:Get(source))
  end)

  exports['hrrp-base']:CallbacksRegisterServer("Wallet:GiveCash", function(source, data, cb)
    local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
    local targetChar = exports['hrrp-base']:FetchSID(data.target)
  ...
  Chat:RegisterCommand("givecash", function(source, args, rawCommand)
    ...
    if target and target > 0 then
      local char = exports['hrrp-base']:FetchSource(source):GetData("Character")
      local targetChar = exports['hrrp-base']:FetchSID(target)
