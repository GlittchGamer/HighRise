AddEventHandler("Finance:Server:Startup", function()
  ...
  exports['hrrp-base']:CallbacksRegisterServer("Crypto:GetAll", function(source, data, cb)
    cb(_cryptoCoins)
  end)
  ...
  Inventory.Items:RegisterUse("crypto_voucher", "RandomItems", function(source, item)
    ...
    if item.MetaData.CryptoCoin and ((item.MetaData.Quantity and tonumber(item.MetaData.Quantity) or 0) > 0) then
      ...
      exports['hrrp-base']:ExecuteClient(source, "Notification", "Error", "Invalid Voucher")
    end
  end)
...
function DoesCryptoWalletExist(wallet)
  ...
  local doesWalletExist = MySQL.single.await("SELECT * FROM characters WHERE CryptoWallet = @wallet", {
    ["@wallet"] = wallet,
  })

  return doesWalletExist ~= nil
end
