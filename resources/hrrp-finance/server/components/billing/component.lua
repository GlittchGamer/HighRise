...
_BILLING = {
  ...
  PlayerCreateOrganizationBill = function(self, billingSource, stateId, account, amount, description)
    ...
    local targetChar = exports['hrrp-base']:FetchSID(tonumber(stateId))
        ...
        PENDING_BILLS_CB[newBillingId] = function(wasPayed, withAccount)
          if wasPayed then
            local success = Banking.Balance:Deposit(billData.Account, billData.Amount, {
              ...
              title = 'Bill Payment',
              description = string.format('Bill Payment From State ID: %s With Account: %s. Bill Description: %s',
                targetStateId, withAccount, billData.Description),
              data = {
                biller = billerStateId,
            ...
            if success then
              Phone.Notification:Add(billingSource, "Bill Payment Received",
                string.format("Payment for a bill you sent to State ID: %s was just received.", targetStateId), os.time(),
                5000, "bank", {})
            end
          end
        end
  ...
  Fine = function(self, finingSource, targetSource, amount)
    ...
    if amount and amount > 0 then
      ...
      if finingChar and targetChar and finingChar:GetData('SID') ~= targetChar:GetData('SID') then
        ...
        local policeJob = exports['hrrp-jobs']:JobsPermissionsHasJob(finingSource, 'police')
          ...
          if success then
            ...
            if policeAccount then
              Banking.Balance:Deposit(policeAccount.Account, policeCutAmount, {
                type = 'fine_profit',
                title = 'Fine Profit',
                description = string.format('Fine Profit (Fine to State ID: %s By State ID: %s)', targetCharSID,
                  finingCharSID),
                data = {
                  finer = finingCharSID,
                  fined = targetCharSID,
            ...
            Phone.Notification:Add(targetSource, "Received Fine",
              string.format("You received a fine of $%d from the State of San Andreas", amount), os.time(), 7500, "bank",
              {})

            exports['hrrp-base']:LoggerInfo("Billing",
              string.format("%s %s (%s) Fined $%s By %s %s (%s).", targetChar:GetData("First"),
                targetChar:GetData("Last"), targetChar:GetData("SID"), amount, finingChar:GetData("First"),
                finingChar:GetData("Last"), finingChar:GetData("SID")), {
                console = true,
                file = true,
                database = true,
                discord = {
  ...
  Charge = function(self, source, amount, title, description)
    local amount = math.tointeger(amount)
    if amount and amount > 0 then
      local targetChar = exports['hrrp-base']:FetchSource(source):GetData("Character")
      if targetChar then
        local targetCharSID = targetChar:GetData('SID')
        local targetCharAccount = Banking.Accounts:GetPersonal(targetCharSID)
        ...
        if targetCharAccount then
          ...
          if success then
            Phone.Notification:Add(source, "New Bank Charge", string.format("Received Charge of $%d - %s", amount, title),
              os.time(), 7500, "bank", {})
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
  exports["hrrp-base"]:RegisterComponent("Billing", _BILLING)
end)
