AddEventHandler('Finance:Server:Startup', function()
  ...
  Chat:RegisterAdminCommand('testbilling', function(source, args, rawCommand)
    exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Info', 'Bill Created')
    Billing:Create(source, 'Some Random Fucking Business', 1500, 'This is a shitty description of a test bill.', function(wasPayed)
      if wasPayed then
        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Success', 'Bill Accepted')
      else
        exports['hrrp-base']:ExecuteClient(source, 'Notification', 'Error', 'Bill Declined')
      end
    end)
  end, {
    help = 'Test Billing'
